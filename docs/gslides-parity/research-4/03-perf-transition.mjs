...
async function editorRun(label, { hover }) {
  ...
  page.on('response', (r) => {
    ...
    if (u.includes('/api/render'))
      renders.push({
        ...
        x: r.headers()['x-vercel-cache'],
        t: Date.now(),
      });
    if (u.includes('_serverFn'))
      fns.push({ u: u.split('?')[0].split('/').pop(), s: r.status(), t: Date.now() });
  });
  const t0 = Date.now();
  await page.goto(BASE + '/edit/gt-brand', { waitUntil: 'domcontentloaded', timeout: 90_000 });
  const tDcl = Date.now() - t0;
  await page
    .waitForFunction(() => typeof window.turboslide !== 'undefined', null, { timeout: 60_000 })
    .catch(() => {});
  ...
  const thumbStates = await page.evaluate(() => {
    ...
    for (const el of document.querySelectorAll('.ts-filmstrip .ts-thumb'))
      s[el.getAttribute('data-thumb')] = (s[el.getAttribute('data-thumb')] || 0) + 1;
    return s;
  });
  const domNodes = await page.evaluate(() => document.getElementsByTagName('*').length);
  const filmstripNodes = await page.evaluate(
    () => document.querySelector('.ts-filmstrip')?.getElementsByTagName('*').length ?? null,
  );
  ...
  if (hasLink) {
    ...
    await page
      .waitForFunction(
        () =>
          ...
          document.querySelectorAll('a[href^="/edit/"], a[href^="/deck/"]').length > 0,
        null,
        { timeout: 60_000 },
      )
      .catch(() => {});
    transition = {
      ms: Date.now() - tClick,
      hover,
      serverFnCallsDuring: fns.length - fnBefore,
      fnsAfterHover: fns.slice(fnBefore),
    };
  }
  await context.close();
  return {
    ...
    renderSample: renders.slice(0, 3),
    fns: fns.map((f) => f.u),
    transition,
  };
}
...
// the viewer to editor transition (a rep opens a deck from the list)
{
  ...
  await page
    .waitForSelector('a[href^="/edit/"], a[href^="/deck/"]', { timeout: 60_000 })
    .catch(() => {});
  const links = await page.evaluate(() =>
    Array.from(document.querySelectorAll('a[href^="/edit/"], a[href^="/deck/"]'))
      .slice(0, 6)
  ...
  if ((await target.count()) > 0) {
    ...
    await target.click();
    await page
      .waitForFunction((h) => location.pathname.startsWith(h.split('?')[0]), href, {
        timeout: 60_000,
      })
      .catch(() => {});
    await page
      .waitForSelector('.ts-sheet, .sheet, .ts-stagewrap', { timeout: 60_000 })
      .catch(() => {});
...
await browser.close();
writeFileSync(
  '/private/tmp/claude-501/-Users-kevinliu-gt-gt-cloud/293a64b7-8ef6-4b00-b382-682288c84431/scratchpad/perf-transition.json',
