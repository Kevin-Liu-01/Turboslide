...
const listAll = async (prefix, mode) => {
  ...
  do {
    ...
    else out.push(...page.blobs.map((b) => ({ pathname: b.pathname, size: b.size, uploadedAt: b.uploadedAt })));
...
let withManifest = 0;
const kinds = { versions: 0, snapshots: 0, slides: 0, state: 0, comments: 0, assets: 0, thumbs: 0, manifest: 0, leases: 0, access: 0, other: 0 };
let totalFiles = 0;
let totalBytes = 0;
const perDeck = [];
for (const folder of sample) {
  const files = await listAll(folder, 'expanded');
  const row = { versions: 0, snapshots: 0, slides: 0, state: 0, comments: 0, assets: 0, thumbs: 0, manifest: 0, leases: 0, access: 0, other: 0, bytes: 0 };
  for (const f of files) {
    const rel = f.pathname.slice(folder.length);
    totalFiles += 1;
    totalBytes += f.size ?? 0;
    row.bytes += f.size ?? 0;
    const kind = rel === 'deck.json' ? 'manifest'
      : rel === 'leases.json' ? 'leases'
      : rel === 'access.json' ? 'access'
      : rel.startsWith('versions/') ? 'versions'
      : rel.startsWith('snapshots/') ? 'snapshots'
      : rel.startsWith('slides/') ? 'slides'
      : rel.startsWith('.turboslide/') ? 'state'
      : rel.startsWith('comments/') ? 'comments'
      : rel.startsWith('assets/') ? 'assets'
      : rel.startsWith('.thumbs/') ? 'thumbs'
      : 'other';
    row[kind] += 1;
    kinds[kind] += 1;
  }
...
console.log(`folders with a manifest: ${withManifest} of ${sample.length}; files under decks/: ${totalFiles}; bytes: ${totalBytes}`);
...
console.log(`folders without a manifest (leftovers): ${phantoms.length}; their files by kind: versions ${sum(phantoms, 'versions')} snapshots ${sum(phantoms, 'snapshots')} slides ${sum(phantoms, 'slides')} state ${sum(phantoms, 'state')} thumbs ${sum(phantoms, 'thumbs')} assets ${sum(phantoms, 'assets')} other ${sum(phantoms, 'other')}; bytes ${sum(phantoms, 'bytes')}`);
const jobs = await listAll('exports/.jobs/', 'expanded');
console.log(`export job records: ${jobs.length}`);
const top = await listAll('', 'folded');
console.log(`top level folders: ${top.length} (${top.map((f) => f.replace(/\/$/, '')).join(', ')})`);
