...
| `formatting.superscript.chord` | formatting | works | failed | double click beta, Cmd+.: error: page.evaluate: RangeError: No slide "blank-2" | 200 |
| `formatting.subscript.chord` | formatting | works | failed | double click beta, Cmd+,: error: no run fmt-a/text on the stage | 201 |
| `formatting.superscript.menu-word` | formatting | broken | failed | double click beta, Format > Text > Superscript: error: no run fmt-a/text on the stage | 202 |
| `formatting.subscript.menu-word` | formatting | broken | failed | double click beta, Format > Text > Subscript: error: no run fmt-a/text on the stage | 203 |
| `formatting.italic.menu-word` | formatting | broken | failed | double click beta, Format > Text > Italic: error: no run fmt-a/text on the stage | 204 |
| `formatting.context.selection-rows` | formatting | not driven | failed | double click gamma, right click it, read the rows, Superscript from the menu: error: no run fmt-a/text on the stage | 205 |
| `formatting.capitalization.upper` | formatting | works | failed | select the box, Format > Text > Capitalization > UPPERCASE; Cmd+Z: error: locator.waitFor: Timeout 6000ms exceeded. | 206 |
| `formatting.capitalization.lower` | formatting | works | failed | select the box, Format > Text > Capitalization > lowercase; Cmd+Z: error: locator.waitFor: Timeout 6000ms exceeded. | 207 |
| `formatting.capitalization.title` | formatting | works | failed | select the box, Format > Text > Capitalization > Title Case; Cmd+Z: error: locator.waitFor: Timeout 6000ms exceeded. | 208 |
| `formatting.align.justified-menu` | formatting | works | failed | select the box, Format > Align & indent > Justified: align null -> null (drawn null); after Cmd+Z null | 209 |
| `formatting.align.justified-chord` | formatting | works | failed | select the box, Cmd+Shift+J: align null -> null (drawn null); after Cmd+Z null | 210 |
| `formatting.align.toolbar-justify` | formatting | not driven | failed | select the box, the toolbar Align list, Justify: error: locator.boundingBox: Timeout 30000ms exceeded. | 211 |
| `formatting.spacing.add-before-remove` | formatting | works | failed | select the box, Format > Line & paragraph spacing > Add space before paragraph, read the row, click it again: error: locator.waitFor: Timeout 8000ms exceeded. | 212 |
| `formatting.spacing.add-after` | formatting | works | failed | select the box, Format > Line & paragraph spacing > Add space after paragraph; Cmd+Z: spaceAfter null; after Cmd+Z null | 213 |
| `formatting.spacing.custom-dialog` | formatting | works | failed | select the box, Format > Line & paragraph spacing > Custom spacing; 1.3, 12, 6; Apply; Cmd+Z: error: locator.waitFor: Timeout 8000ms exceeded. | 214 |
| `formatting.spacing.1-15-value` | formatting | broken | failed | select the box, Format > Line & paragraph spacing > 1.15: the row labelled 1.15 wrote leading null | 215 |
| `formatting.highlight.word` | formatting | works | failed | double click delta, Highlight color, Amber; Cmd+Z: error: no run fmt-a/text on the stage | 216 |
| `formatting.paint-format.button` | formatting | works | failed | select box A, Paint format, click box B; Cmd+Z: error: Cannot read properties of null (reading 'free') | 218 |
| `formatting.paint-format.chords` | formatting | not driven | failed | select box A, Cmd+Option+C; select box B, Cmd+Option+V: A {"weight":700,"size":32,"color":"red"}; B {"weight":null,"size":null,"color":null} -> {"weight":null,"size":null,"color":null} | 219 |
| `formatting.clear.inline-marks` | formatting | broken | failed | an italic word and a bold word in box B; select the box; Clear formatting; Cmd+Z: error: no run fmt-b/text on the stage | 220 |
...
| `formatting.persistence` | formatting | works | failed | a superscript word, UPPERCASE, a highlighted word and a grouped pair; reload; the show; the viewer: error: no run fmt-a/text on the stage | 224 |
...
| `chrome.split.hover-no-inversion` | chrome | broken | failed | hover the Slideshow half, then the chevron half: token --pt-plate-on-ink "#07070729"; wrapper rgb(242, 242, 240); label half rgba(0, 0, 0, 0)/rgb(7, 7, 7) -> hovered rgba(0, 0, 0, 0)/rgb(7, 7, 7) (chevron then rgba(0, 0, 0, 0)); chevron half rgba(0, 0, 0, 0) -> hovered rgba(7, 7, 7, 0.16)/rgb(7, 7, 7) (label then rgba(0, 0, 0, 0)) | 605 |
...
| `versions.show-changes-marks` | versions | not driven | failed | a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.842.pick (named row versionHistory.842.pick); marks with Show changes on 0 (); off 0 | 488 |
...
| `inbox.settings-persist` | inbox | broken | failed | Tools > Notification settings, None, Save; reopen; reload: level forYou -> picked none; Save closed true; reopened forYou; after a reload forYou | 602 |
...
| `brand.logo.use-on-every-slide` | brand | not driven | passed |  | 529 |
| `brand.logo.remove` | brand | not driven | passed |  | 530 |
| `brand.colors.primary-live` | brand | not driven | failed | Colors > Primary: type #0b3d91, read the sheet before Enter, Enter, read another slide, Cmd+Z: --blue #2f5ce0 -> #0b3d91 while typing (revision 856 -> 856); after Enter revision 858, record {"dark":{"primary":"#0b3d91"}}; the title slide reads #0b3d91; after Cmd+Z #2f5ce0 | 531 |
| `brand.colors.palette-row` | brand | broken | passed |  | 532 |
| `brand.colors.control-ids-unique` | brand | broken | passed |  | 533 |
| `brand.colors.version-history-entry` | brand | not driven | passed |  | 534 |
| `brand.colors.role-tooltips` | brand | not driven | passed |  | 535 |
| `brand.background.enter-keeps-open` | brand | broken | passed |  | 536 |
| `brand.fonts.roles` | brand | not driven | passed |  | 537 |
| `brand.counter.format` | brand | not driven | passed |  | 538 |
| `brand.reset.default-kit` | brand | not driven | passed |  | 539 |
| `brand.layout.tiles-in-kit` | brand | broken | passed |  | 540 |
| `brand.agent.set-get` | brand | not driven | passed |  | 541 |
...
| `assist.outside-write.snackbar` | assist | broken | failed | the seller types a word; POST /api/actions/text.replaceAll as an agent while the editor is open; Undo from the snackbar: seller's edit stored true; HTTP 200 {"replacements":2,"slideIds":["blank-17"],"revision":953}; snackbar "none" after 10023 ms with Undo false; the agent's word landed true; no Undo to press | 597 |
...
| `brand.objects.kit-colours-first` | brand | not driven | not driven | not on this build: formatOptions.chart.swatches.primary (docs/PRODUCT.md 7.1, B3); the chart's series swatches list formatOptions.chart.swatches.ink, formatOptions.chart.swatches.paper, formatOptions.chart.swatches.ink-2, formatOptions.chart.swatches.titanium, formatOptions.chart.swatches.hair, formatOptions.chart.swatches.hair-soft first and no kit role (P1, FEATURES.md 2.3 item 4); table fill pl | 542 |
...
| `shaders.panel.preset-tiles` | shaders | broken | failed | read the Preset row's tiles; click a tile that is not pressed: 10 tiles ("Paper on ink", "Ink on paper", "Primary", "Accent", "Captions", "Hints", "Diamond" pressed, "Sphere", "Chrome", "Noir") against 10 presets of paper:liquid-metal; labels not in sentence case: none; click on formatOptions.shader.preset.chrome: preset diamond -> chrome (revision 934), canvas mean luminance moved 153.3, 94 perce | 581 |
| `shaders.panel.control-sentences` | shaders | not driven | passed |  | 582 |
| `shaders.panel.kit-colours` | shaders | not driven | failed | read the Colors row's swatches; brand.set /colors/<appearance>/primary '#0b3d91'; read the shader and its frame: the Primary swatch clicked (revision 935 -> 936); swatches text, background, caption, hint, primary, accent, custom (6 of the six roles); the sheet renders dark (deck.info and the chrome read dark); brand.set /colors/dark/primary ok; frame frame-f91c12dd0d05c000 (key sha256:f91c1) -> fr | 583 |
| `shaders.panel.one-home` | shaders | broken | failed | Tools > Advanced tools; Tools > Pictures and materials with the shader selected; the Background dialog's options link: Pictures and materials sections Pictures and materials, Pictures1 (with the switch on); the Background dialog's link absent (ground not placed by the place row) | 585 |
| `shaders.background.place-answers` | shaders | broken | not driven | not on this build: dialog.background.shader (docs/PRODUCT.md 7.1, B1 (dialogs/Background.tsx)); no Shader row and no Material row in the Background dialog (FEATURES.md 5.4) | 584 |
| `shaders.perf.one-context` | shaders | not driven | failed | a second shader block placed through the window API; select each in turn and count the canvases: with nothing selected 1 canvas on the stage (2 on the page); first selected: 1 on the stage, 1 on the page; second selected: 1 on the stage after 3399 ms, the second's root 1 canvas, the first's root 0 canvas, frame img decoded | 586 |
