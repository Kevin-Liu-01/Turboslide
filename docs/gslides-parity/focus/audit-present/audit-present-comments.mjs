...
const attempt = async (feature, interaction, fn, { tries = 3 } = {}) => {
  ...
  for (let t = 1; t <= tries; t += 1) {
    try {
      ...
      if (r.ok) return record(feature, interaction, t === 1 ? 'works' : 'flaky', t === 1 ? r.evidence : seen.join(' | '));
    } catch (error) {
...
const runInfo = (page, run) =>
  page.evaluate((r) => {
    ...
    return { text: el.textContent ?? '', rect: { x: rect.x, y: rect.y, w: rect.width, h: rect.height } };
  }, run);
...
const objectsOf = async (page, slideId) => {
  ...
  const walk = (node) => {
    ...
    if (node && typeof node === 'object') {
      if (typeof node.id === 'string' && typeof node.type === 'string' && node.pos && typeof node.pos === 'object') out.push({ id: node.id, type: node.type });
...
const shot = (name) => page.screenshot({ path: path.join(SHOTS, `${name}.png`) }).catch(() => undefined);
...
const submitComment = async (label) => {
  ...
  if (!f.newField) return { ok: false, evidence: `${label}; no comment card; snackbar ${JSON.stringify(await snackbar(page))}` };
  ...
  return { ok: after.markers >= 1 && panel.threads >= 1, evidence: `${label}; snackbar within 6 s ${JSON.stringify(toast)}; markers after 20 s ${after.markers}; panel threads ${panel.threads}, list "${panel.text.slice(0, 100)}"` };
};
...
try {
  ...
  const slideId = (await state(page)).slideId;
  {
    ...
    record('Scratch deck', 'Open /new, type a title', 'works', `${deckId}; revision ${(await state(page)).revision}`);
  }
  await attempt('Comments', 'Nothing selected, Cmd+Option+M, type, Comment (the slide as the anchor)', async () => {
    await clearAll(page);
  ...
  await attempt('Comments', 'Insert a text box, select it, Cmd+Option+M, type, Comment', async () => {
    await clearAll(page);
    ...
    const obj = await pollUntil(async () => (await objectsOf(page, slideId)).find((o) => !before.includes(o.id)) ?? null, (o) => o !== null, 15_000);
    ...
    const box = await rectOf(page, `.ts-stagewrap.ts-editor .pt-slide [data-block="${obj.id}"], .ts-stagewrap.ts-editor .pt-slide [data-run^="${obj.id}"]`);
    if (box) await clickAt(page, box.x + box.w / 2, box.y + box.h / 2);
    if (await editing(page)) await press(page, 'Escape');
    await sleep(200);
    const chip = await textOf(page, '.ts-overlay .ts-select-chip');
    await press(page, 'Meta+Alt+m');
    const r = await submitComment(`text box ${obj.id} (${obj.type}); chip ${JSON.stringify(chip)}`);
  ...
  await attempt('Comments', 'Open the thread from its marker and reply', async () => {
    ...
    return { ok: f.replies >= 1, evidence: `first "${f.first}"; replies ${f.replies}; snackbar ${JSON.stringify(await snackbar(page))}` };
  });
  await attempt('Comments', 'Resolve the thread', async () => {
    ...
    await closePanel();
    return { ok: label !== null && after.markers < markersBefore, evidence: `resolve button "${label}"; markers ${markersBefore} -> ${after.markers}; panel threads ${panel.threads}, reopen buttons ${panel.reopen}, list "${panel.text.slice(0, 120)}"` };
  });
} catch (error) {
  record('The comments probe ran to completion', 'no exception outside a step', 'broken', error instanceof Error ? (error.stack ?? error.message) : String(error));
} finally {
  if (deckId) {
    let trashed = false;
    try {
