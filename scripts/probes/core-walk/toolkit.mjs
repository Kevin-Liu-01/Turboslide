...
export function createToolkit({ page, context, browser, BASE, headers, lib, report, options }) {
  ...
  /**
   * The revision once it has stopped moving (b3 C2-R4, then C3-R4 for the memory tier): the
   * revision read after the last op has settled, then a read every second until `quietReads`
   * more reads agree with it, a quiet window of 3 s by default, within `ms`. A row that judges
   * "no write" is then not counting the previous row's write landing over the channel. Two reads
   * a second apart were not enough: on the memory and redis tiers the tab's revision follows the
   * checkpoint, which the room's checkpointer writes 2 s after the last op
   * (`apps/studio/src/server/checkpoint.ts` `CHECKPOINT_IDLE_MS`), so the two reads agreed before
   * the previous row's redo had landed as a revision and `images.crop.menu-escape` read that
   * checkpoint as the crop's write (VERIFICATION.md C2-F21, "revision 141 -> 142"); the blob tier
   * moves the revision on the ack and loses nothing by the longer wait. A read that moves starts
   * the window again.
   */
  t.stableRevision = async (ms = 15_000, quietReads = 3) => {
    ...
    while (Date.now() < until) {
      ...
      if (now === last) {
        agreed += 1;
        if (agreed >= quietReads) return now;
      } else {
        last = now;
        agreed = 0;
      }
