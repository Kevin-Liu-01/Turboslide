...
try {
  ...
  await t.step('replace image > upload swaps the asset', 'asset changes, pos kept', async () => {
    ...
    return {
      ok: after && after.block.asset !== before.block.asset && JSON.stringify(after.pos) === JSON.stringify(before.pos),
  ...
  await t.step('transparency is a slider; border draws', 'a range input; a ring', async () => {
    ...
    const slider = await page.evaluate(() => {
      const el = document.querySelector('[data-control="formatOptions.adjustments.transparency.slider"]');
      const field = document.querySelector('[data-control="formatOptions.adjustments.transparency"]');
      const r = el?.getBoundingClientRect();
      const f = field?.getBoundingClientRect();
      return { slider: el ? { type: el.type, w: r.width, h: r.height, display: getComputedStyle(el).display } : null, field: f ? { w: f.width, h: f.height } : null };
    });
    await t.shot('format-options-picture');
    return { ok: Boolean(slider.slider) && slider.slider.w > 40, observed: `sections ${sections.join(',')}; ${JSON.stringify(slider)}` };
  });
  await t.step('border color writes and draws', 'a visible ring', async () => {
    ...
    const read = await page.evaluate((id) => {
      ...
      const cs = (n) => (n ? { border: getComputedStyle(n).border, outline: getComputedStyle(n).outline, cls: n.className } : null);
      return { fig: cs(el), img: cs(img), crop: cs(crop), html: el.outerHTML.slice(0, 600) };
    }, pic.id);
    await t.shot('border');
    return { ok: Boolean(read?.img?.border && !/0px/.test(read.img.border)) || Boolean(read?.crop?.border && !/0px/.test(read.crop.border)), observed: JSON.stringify(read) };
  });

  // ---- text box grow
  await t.step('a long paragraph grows the text box', 'pos.h follows the text', async () => {
    await t.clearAll();
    const box = await t.placeBlock(S, { id: 'tb-grow', type: 'text', text: '', autofit: 'grow', pos: { x: 900, y: 500, w: 340, h: 48, z: 5 } });
    ...
    const para = 'The renewal terms apply to every seat in the workspace from the first day of the next quarter, with the discount held for two years and the support tier unchanged for the whole period of the agreement.';
    ...
    const live = await page.evaluate((id) => {
      ...
      return { free: r ? { w: r.width, h: r.height, bottom: r.bottom } : null, textBottom: bottom, overflow: r ? bottom - r.bottom : null, k: document.querySelector('.ts-stagewrap.ts-editor .pt-slide').getBoundingClientRect().width / 1600 };
    }, box.id);
    ...
    await t.shot('grow');
    return { ok: after.pos.h > 100 && live.overflow < 2, observed: `editing ${editing}; pos.h 48 -> ${after.pos.h}; autofit ${after.block.autofit}; live ${JSON.stringify(live)}; console ${consoleErrors.slice(-3).join(' | ')}` };
  });

  // ---- shift home on the third line
  await t.step('Shift+Home selects the third line alone', 'one line', async () => {
    await t.clearAll();
    const box = await t.placeBlock(S, { id: 'tb-lines', type: 'text', text: '', autofit: 'grow', pos: { x: 200, y: 600, w: 600, h: 48, z: 6 } });
    const run = (await t.runsOfBlock(box.id))[0];
    const info = await t.runInfo(run);
    await t.dblclickAt(info.rect.x + info.rect.w / 2, info.rect.y + info.rect.h / 2);
    ...
    await t.press('Escape');
    await sleep(500);
    return { ok: sel.trim() === 'generaltranslation.com', observed: `selection "${sel}"; html ${html.slice(0, 300)}` };
  });

  // ---- double click on a dotted address
  await t.step('double click on generaltranslation.com selects the whole address', 'the address', async () => {
  ...
  await t.step('Insert > Image > Upload lands where and how fast', 'centred in the body, no snackbar', async () => {
    await t.clearAll();
    ...
    const t0 = await t.uploadThrough(() => t.clickRow('insert.image.upload'), 'product.png', await t.pngBuffer(400, 300));
    const obj = await t.newObjectAfter(S, before, 15_000);
    const ms = Date.now() - t0;
    const snack = await t.snackbar();
    const chip = await t.chip();
    return { ok: Boolean(obj), observed: `${obj ? `${obj.type} at ${JSON.stringify(obj.pos)}` : 'nothing'} in ${ms} ms; snackbar ${snack}; chip ${chip}` };
  });
  t.finish();
} finally {
  await browser.close();
