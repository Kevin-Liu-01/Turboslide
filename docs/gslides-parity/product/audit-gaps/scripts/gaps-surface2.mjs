...
try {
  ...
  let boxId = null;
  await t.step(
    'Text box: Insert > Text box, click at 900,500, type a long paragraph',
    'the box grows or the text shrinks; the text stays inside the box',
    async () => {
      ...
      const facts = await page.evaluate((id) => {
        ...
        return {
          ...
          run: `${Math.round(r.width)}x${Math.round(r.height)}`,
          lines: new Set(rects.map((x) => Math.round(x.top))).size,
          textBottomPastBox: Math.round(bottom - f.bottom),
          pastSheet: Math.round(bottom - sheet.bottom),
          overflow: getComputedStyle(free).overflow,
          font: getComputedStyle(run).fontSize,
        };
      }, obj.id);
      ...
      return {
        ok: facts !== null && facts.textBottomPastBox <= 2,
        observed: `${JSON.stringify(facts)}; autofit "${block?.autofit}"; pos ${JSON.stringify(block?.pos)}; ${shot}`,
      };
    },
  );
  await t.step(
    ...
    async () => {
      ...
      const facts = await page.evaluate(() => {
        ...
        return {
          panel: panel !== null,
          sections: panel
            ? [...panel.querySelectorAll('.ts-panel-section-head')].map((h) =>
                h.textContent?.trim(),
              )
            : null,
          ...
          checked: seg
            ? [
                ...seg.querySelectorAll(
                  '[aria-pressed="true"], [aria-checked="true"], .is-on, [data-on]',
                ),
  ...
  await t.step(
    'Alt text on the text box (Format options > Alt text)',
  ...
  let second = null;
  await t.step(
    'Link the text box to slide 2: New slide, back to slide 1, select the box, Insert > Link, pick the slide, Apply',
    'the block carries link { slide }',
    async () => {
      ...
      if (!boxId || !second)
        return { ok: false, observed: `box ${boxId}; slides ${ids.join(',')}` };
  ...
  await t.step(
    'The linked box in the show: Slideshow, click it',
    'the show jumps to slide 2',
    async () => {
      ...
      const target = await page.evaluate((id) => {
        const els = [...document.querySelectorAll(`[data-block="${id}"]`)].filter(
          (el) => el.getClientRects().length > 0 && el.closest('.ts-stagewrap.ts-editor') === null,
        );
        const el = els[0];
        if (!el) return null;
      ...
      await sleep(600);
      return {
        ok: show && Boolean(target) && after.slideId === second,
        observed: `show ${show}; target ${JSON.stringify(target)}; slide ${before.slideId} -> ${after.slideId} (linked ${second}); ${shot}`,
      };
    },
  );

  await t.step(
    'File > Print settings and preview: the layout rows, paper and orientation',
    ...
    async () => {
      ...
      const facts = await p.evaluate(() => {
        ...
        return {
          ...
          sameTab: true,
          options: sel
            ? [...(sel.querSelectorAllSafe?.('option') ?? sel.querySelectorAll('option'))].map(
          ...
          controls: [...document.querySelectorAll('[data-control^="print."]')]
            ...
            .filter((c, i, a) => a.indexOf(c) === i && !/print\.notes\./.test(c)),
          paperWord: /Paper|Letter|A4/.test(document.body.textContent ?? ''),
          orientationWord: /Orientation|Landscape|Portrait/.test(document.body.textContent ?? ''),
  ...
  await t.step(
    ...
    async () => {
      ...
      return {
        ...
        observed: `tabs ${tabs.join('/')}; accept "${accept}"; before "${sentenceBefore?.slice(0, 260)}"; after "${text?.slice(0, 420)}"; ${shot}`,
      };
    },
  );
  await t.step('File > Import slides > Upload with a .pptx', 'the same refusal', async () => {
    ...
    if ((await input.count()) === 0) {
      const txt = await page.evaluate(() =>
        document.querySelector('[role="dialog"]')?.textContent?.replace(/\s+/g, ' ').slice(0, 300),
      );
      await t.closeDialogs();
      return { ok: false, observed: `no upload input; dialog "${txt}"` };
    }
    ...
    return {
      ok: /PowerPoint import is not available/.test(text ?? ''),
      observed: `dialog "${text?.slice(0, 420)}"; ${shot}`,
    };
  });

  await t.step(
    'File > Share > Publish to web: Publish, the link, fresh fetches, Embed, Done, reopen',
    'a published link; Stop publishing offered on reopen; the player refuses without its token',
    async () => {
      await backToEditor();
      ...
      if (link && /^https?:/.test(link)) {
        ...
        fetched.withoutToken = (
          await fresh.get(link.replace(/[?&]p=[^&]+/, '').replace(/\?$/, ''), { maxRedirects: 0 })
        ).status();
        fetched.plainDeck = (
          await fresh.get(`${BASE}/deck/${deck.id}`, { maxRedirects: 0 })
        ).status();
        fetched.embedNoToken = (
          await fresh.get(`${BASE}/embed/${deck.id}`, { maxRedirects: 0 })
        ).status();
        await fresh.dispose();
      }
      await t.menuPath('file', 'file.share', 'file.share.publish');
      ...
      return {
        ...
        observed: `state before "${before}"; after "${after}"; link ${link}; fresh fetch ${JSON.stringify(fetched)}; embed "${embed?.slice(0, 140)}"; reopened "${reopened}"; Stop publishing offered ${stop}; ${shot}; ${shot2}; ${shot3}`,
      };
    },
  );

  await t.step(
    'Comment on the text box: Cmd+Option+M, type with an @, the mention list, Comment, the marker, Resolve',
    'a composer with a mention picker of people; the thread posts within 20 s; Resolve works',
    async () => {
      await backToEditor();
      if (!boxId) return { ok: false, observed: 'no text box' };
      const card = await t.rectOf(`[data-control="filmstrip.slide.${X}"]`);
      ...
      const mentions = await page.evaluate(() =>
        [...document.querySelectorAll('[data-control*=".mention."]')]
      ...
      return {
        ok: markers > 0,
        observed: `mention list ${JSON.stringify(mentions)}; markers ${markers} after ${ms} ms; snackbar "${snack}"; markers after Resolve ${resolved}; ${shot}; ${shot2}; ${shot3}`,
      };
    },
  );
  await t.closeDialogs();
  ...
  await t.step(
    'Paste plain text with Cmd+V, then with Cmd+Shift+V',
    'both land the clipboard text in the body',
    async () => {
      await backToEditor();
      const card = await t.rectOf(`[data-control="filmstrip.slide.${X}"]`);
      if (card) await t.clickAt(card.x + card.w / 2, card.y + card.h / 2);
      ...
      await t.settled();
      return {
        ok: on && /PASTED/.test(afterV ?? '') && (afterShiftV ?? '').split('PASTED').length === 3,
        observed: `session ${on}; after Cmd+V "${afterV?.slice(0, 120)}"; after Cmd+Shift+V "${afterShiftV?.slice(0, 120)}"; snackbar "${snack}"`,
      };
    },
  );
  ...
  writeFileSync(
    path.join(EVIDENCE, 'surface2-console.json'),
