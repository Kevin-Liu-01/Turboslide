...
for (let i = 0; i < 5; i++) {
  ...
  console.log(
    `try ${i} deck.info ${g.status} ${(g.json?.error?.message ?? g.json?.revision ?? g.text).toString().slice(0, 90)}`,
  );
  if (g.status === 404) {
    console.log('gone');
    process.exit(0);
  ...
  if (g.status === 200) {
    ...
    if (!g.json.trashedAt) {
      ...
      console.log('trash', tr.status);
      const a = await act('deck.info');
      rev = a.json?.revision ?? rev + 1;
    }
    let rm = await act('deck.remove', { id: DECK, confirm: true, baseRevision: rev });
    if (rm.status === 409) {
      const m = /at revision (\d+)/.exec(rm.json?.error?.message ?? '');
      if (m) {
        rm = await act('deck.remove', { id: DECK, confirm: true, baseRevision: Number(m[1]) });
      }
    }
    console.log(
      'remove',
      rm.status,
      (rm.json?.removed ? 'removed' : (rm.json?.error?.message ?? rm.text)).toString().slice(0, 90),
    );
    const gone = await act('deck.info');
    console.log('final deck.info', gone.status);
    process.exit(0);
  }
  if (g.status === 409) {
    const m = /at revision (\d+)/.exec(g.json?.error?.message ?? '');
  }
  await sleep(1500);
}
