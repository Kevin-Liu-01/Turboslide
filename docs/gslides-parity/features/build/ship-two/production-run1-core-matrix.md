...
Base https://turboslide.vercel.app, started 2026-09-26T07:08:03.939Z, 9894 s. 856 rows judged: 798 passed, 32 failed, 26 not driven (1 of them manual, the checklist's: text.clipboard.paste-without-formatting), 0 no step. Measurement rows (PRODUCT.md 8.2, recorded and never holding the ship; the cost rows of SYNC.md 6.1 among them, which hold it over their ceiling on the preview): export.download.large-deck-pdf passed (PDF: GT brand deck.pdf, 85 slides in 12.5 s, 0.15 s per slide; PDF: GT brand deck.pdf, 85 slides in 12.5 s, 0.15 s per slide); export.download.large-deck-pptx passed (Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 178.3 s, 2.10 s per slide; Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 178.3 s, 2.10 s per slide); cost.editor-idle.calls passed (function requests 9.66 a minute (ceiling 12; 29 in 3 min); the store settled 2.58 s after the state was ready (1 sync.status read(s) before the window); store simple 28 a minute (ceiling 40; 3 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced 9 a minute (ceiling 11); instances answering sync.status 1); cost.editor-hidden.calls not driven (function requests 9.65 a minute and 9 session poll(s) were read from the tab that stayed visible); cost.editor-editing.calls failed (function requests 72.94 a minute (ceiling 75; 219 in 3 min); the store settled 2.84 s after the state was ready (1 sync.status read(s) before the window); store simple 156 a minute (ceiling 140; 3 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced 71 a minute (ceiling 85); instances answering sync.status 2); cost.two-tabs-idle.calls failed (the store settled 5.26 s after the state was ready (1 sync.status read(s) before the window); store advanced 19 a minute (ceiling 30); store list 1 (ceiling 0); instances answering sync.status 2); cost.show.calls passed (function requests 0 in the window (ceiling 0); the store settled 73.68 s after the state was ready (8 sync.status read(s) before the window; the first read: head 27, put 20, list 1, del 0); store calls at the first sample 1 (head 1, get 0, put 0, list 0, del 0), of which 1 the probe's own sync.status read(s) (one deck.json head each, b3.md R7 b); the page's 0 (ceiling 0); function requests between the load and the window 0 (the settle, 73.68 s); instances answering sync.status 1); shaders.perf.editor-frame passed (longest animation frame 22.9 ms over 5 s with one shader on the stage (619 animation frames; rAF gaps; 1 canvas; renderer ANGLE (Google, Vulkan 1.3.0 (SwiftShader Device (LLVM 10.0.0) (0x0000C0DE)), SwiftShader driver)); longest animation frame 22.9 ms over 5 s with one shader on the stage (619 animation frames; rAF gaps; 1 canvas; renderer ANGLE (Google, Vulkan 1.3.0 (SwiftShader Device (LLVM 10.0.0) (0x0000C0DE)), SwiftShader driver))). Cost rows over their ceiling in this run: sync.pull.no-listing, cost.editor-editing.calls, cost.two-tabs-idle.calls. Verdict failed with the committed parked list inbox, templates and the parked rows arrange.group.tail-text-controls, logos.kit.find-a-logo, logos.picker.variants, shaders.background.add-to-theme, shaders.background.place-answers, shaders.frame.scrubber-capture, shaders.library.glyph-engines-render, tables.bar.row-column-buttons, tables.edge.add-row-column, tables.heads.select-row-column, tables.seam.row-drag, versions.show-changes-marks, view.live-pointers.second-browser, wordart.tail.fill-outline; retries 0 configured, 0 test(s) retried; exit 1. A not driven row is never counted as passed. Features a ship on this run would park (rule 4 of section 1; RETURN.md rule 2): inbox, brand, fonts, templates, assist; rows whose own controls a ship would keep parked: view.live-pointers.second-browser (view.livePointers.mine, view.livePointers.collaborators); decks.file.open-list-search (file.open); versions.show-changes-marks (file.versionHistory.showChanges); templates.gallery.page (home.gallery, file.new.templateGallery); templates.save.as-template (file.saveAsTemplate); templates.save.same-name-replaces (file.saveAsTemplate); templates.card.rename-and-delete (templates.card.menu); templates.default.use-for-new (panel.brand.template.useForNew); assist.rewrite.card-accept-undo (panel.assist.starter.shorter); assist.notes.draft (panel.assist.starter.notes); assist.free-ask.fallback-sentence (panel.assist.prompt); tables.seam.row-drag (handle.table.row); tables.edge.add-row-column (handle.table.add.column, handle.table.add.row); tables.heads.select-row-column (handle.table.head.column, handle.table.head.row); tables.bar.row-column-buttons (bar.table); arrange.group.tail-text-controls (toolbar.group.text); wordart.tail.fill-outline (toolbar.wordart.outline); logos.index.refresh-fixture (insert.logo); logos.index.cached-offline (insert.logo); logos.picker.variants (dialog.logo.kind.wordmark, dialog.logo.tone.mono); logos.kit.find-a-logo (panel.brand.logo.find); shaders.panel.preset-tiles (formatOptions.shader.preset); shaders.panel.kit-colours (formatOptions.shader.color); shaders.frame.reuse-and-prune (insert.shader); shaders.background.place-answers (dialog.background.shader); shaders.perf.one-context (insert.shader); shaders.background.add-to-theme (dialog.background.shader.addToTheme); shaders.library.glyph-engines-render (dialog.shader.engine.glyph); shaders.frame.scrubber-capture (formatOptions.shader.frame.scrubber, formatOptions.shader.frame.capture); shaders.show.plays-when-on (view.playShaders, formatOptions.shader.play); shaders.show.frame-when-off (view.playShaders); svg.render.vector-at-zoom (intake.svg.upload, intake.svg.paste, intake.svg.drop, intake.svg.url); svg.copy.markup (picture.svg.copy); rows of an unparkable feature blocking the ship: images.insert.upload, text.link.detect-url, menus.icons.insert-rows, sync.title.concurrent-both-kept, sync.block.offline-replay-converges, sync.viewer.live-updates, sync.reload.same-document, sync.undo.after-remote, sync.resend.idempotent, sync.pull.no-listing.
...
| `images.insert.upload` | images | core/images.spec.ts | flaky | failed | Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoBeGreaterThan[2m([22m[32mexpected[39m[2m)[22m |
...
| `decks.file.open-list-search` | decks | probe --core | works | failed | File > Open; read the list; type in the search; clear it: 88 decks listed after 1236 ms, first untitled-20260926-78f3; filtered by nonsense 0; cleared 88 |
...
| `versions.show-changes-marks` | versions | probe --core | not driven | failed | a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.964.pick (named row versionHistory.964.pick); marks with Show changes on 0 (); off 0 |
...
| `inbox.settings-persist` | inbox | probe --core | broken | failed | Tools > Notification settings, None, Save; reopen; reload: level forYou -> picked none; Save closed true; reopened none; after a reload forYou |
...
| `templates.save.as-template` | templates | core/brand.spec.ts | not driven | not driven | not driven on production: the row writes a deployment wide record every seller would see (the default template or a saved template); its reading is the preview run's |
| `templates.save.same-name-replaces` | templates | core/brand.spec.ts | not driven | not driven | not driven on production: the row writes a deployment wide record every seller would see (the default template or a saved template); its reading is the preview run's |
| `templates.card.rename-and-delete` | templates | core/brand.spec.ts | not driven | not driven | not driven on production: the row writes a deployment wide record every seller would see (the default template or a saved template); its reading is the preview run's |
| `templates.default.use-for-new` | templates | core/brand.spec.ts | not driven | not driven | not driven on production: the row writes a deployment wide record every seller would see (the default template or a saved template); its reading is the preview run's |
| `templates.deck.read-only` | templates | core/brand.spec.ts | not driven | not driven | not driven on production: the row writes a deployment wide record every seller would see (the default template or a saved template); its reading is the preview run's |
...
| `text.link.detect-url` | text | probe --core | broken | failed | type generaltranslation.com then a space at the end of the box; Cmd+Z: text "Visit the site for the terms generaltranslation.com"; link "generaltranslation.com" -> https://generaltranslation.com; after Cmd+Z link gone, text "Visit the site for the terms g" |
...
| `menus.icons.insert-rows` | chrome | probe --core | broken | failed | open the Insert menu with the switch on and read every row of docs/VECTOR.md 3.2, the submenus hovered: 43 rows read (switch on true); 38 of 39 named rows draw a glyph; missing: insert.material not drawn; drawn optional rows insert.audio, insert.video, insert.shader missing none |
...
| `brand.background.enter-keeps-open` | brand | probe --core | broken | failed | Slide > Change background, type #0b3d91, Enter, Done; then Add to theme: the deck back: no control dialog.background.close |
...
| `assist.agent.propose-accept` | assist | probe --core | not driven | failed | assist.propose over HTTP, assist.accept with the card, then the card with one byte changed: assist.propose answered 500 {"error":{"name":"AssistUnavailableError","status":500,"message":"The assistant is not set up on this Turboslide yet","action":"assist.propose"}} |
...
| `sync.viewer.live-updates` | sync | core/sync.spec.ts | broken | failed | TimeoutError: page.waitForFunction: Timeout 90000ms exceeded. |
| `sync.reload.same-document` | sync | core/sync.spec.ts | works | failed | Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoBe[2m([22m[32mexpected[39m[2m) // Object.is equality[22m |
| `sync.undo.after-remote` | sync | core/sync.spec.ts | not driven | failed | Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoBe[2m([22m[32mexpected[39m[2m) // Object.is equality[22m |
| `sync.resend.idempotent` | sync | core/sync.spec.ts | not driven | failed | TimeoutError: page.waitForFunction: Timeout 90000ms exceeded. |
| `sync.pull.no-listing` | sync | cost-probe | broken | failed | A never read every word of B's ten commits |
...
| `cost.editor-editing.calls` | cost | cost-probe | broken | failed | store simple 156 a minute over 140 |
...
| `logos.index.refresh-fixture` | logos | core/logos.spec.ts | not driven | not driven | not driven: the index reads network, not the ten mark fixture (TURBOSLIDE_LOGO_UPSTREAM=fixture on a preview); the unit tests apps/studio/src/server/logos.test.ts cover the 404 marking and the takedown (docs/FEATURES.md 7.3) |
| `logos.index.cached-offline` | logos | core/logos.spec.ts | not driven | not driven | not driven: the index reads network, not the outage (TURBOSLIDE_LOGO_UPSTREAM=down on a preview); the unit test apps/studio/src/server/logos.test.ts covers the cache during an outage (docs/FEATURES.md 7.3) |
...
| `shaders.frame.box-aspect` | shaders | core/shaders.spec.ts | not driven | passed |  |
...
| `shaders.background.place-answers` | shaders | probe --core | broken | failed | Slide > Change background on the body slide; Shader; a tile; Place; read the ground, the button and the dialog: chose Liquid metal; Place dialog.background.shader.place; the ground changed after 14193 ms (covering picture background, asset liquid-metal); the button read "Placing, 12 s" with the seco |
...
- `templates.save.as-template`: not driven on production: the row writes a deployment wide record every seller would see (the default template or a saved template); its reading is the preview run's
- `templates.save.same-name-replaces`: not driven on production: the row writes a deployment wide record every seller would see (the default template or a saved template); its reading is the preview run's
- `templates.card.rename-and-delete`: not driven on production: the row writes a deployment wide record every seller would see (the default template or a saved template); its reading is the preview run's
- `templates.default.use-for-new`: not driven on production: the row writes a deployment wide record every seller would see (the default template or a saved template); its reading is the preview run's
- `templates.deck.read-only`: not driven on production: the row writes a deployment wide record every seller would see (the default template or a saved template); its reading is the preview run's
...
- `logos.index.refresh-fixture`: not driven: the index reads network, not the ten mark fixture (TURBOSLIDE_LOGO_UPSTREAM=fixture on a preview); the unit tests apps/studio/src/server/logos.test.ts cover the 404 marking and the takedown (docs/FEATURES.md 7.3)
- `logos.index.cached-offline`: not driven: the index reads network, not the outage (TURBOSLIDE_LOGO_UPSTREAM=down on a preview); the unit test apps/studio/src/server/logos.test.ts covers the cache during an outage (docs/FEATURES.md 7.3)
...
- `images.insert.upload`: Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoBeGreaterThan[2m([22m[32mexpected[39m[2m)[22m
- `view.live-pointers.second-browser`: Error: the second browser's pointer is drawn on the first within 10 s
- `decks.file.open-list-search`: File > Open; read the list; type in the search; clear it: 88 decks listed after 1236 ms, first untitled-20260926-78f3; filtered by nonsense 0; cleared 88
- `versions.show-changes-marks`: a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.964.pick (named row versionHistory.964.pick); marks with Show changes on 0 (); off 0
...
- `inbox.settings-persist`: Tools > Notification settings, None, Save; reopen; reload: level forYou -> picked none; Save closed true; reopened none; after a reload forYou
...
- `text.link.detect-url`: type generaltranslation.com then a space at the end of the box; Cmd+Z: text "Visit the site for the terms generaltranslation.com"; link "generaltranslation.com" -> https://generaltranslation.com; after Cmd+Z link gone, text "Visit the site for the terms g"
- `menus.icons.insert-rows`: open the Insert menu with the switch on and read every row of docs/VECTOR.md 3.2, the submenus hovered: 43 rows read (switch on true); 38 of 39 named rows draw a glyph; missing: insert.material not drawn; drawn optional rows insert.audio, insert.video, insert.shader missing none
- `brand.background.enter-keeps-open`: Slide > Change background, type #0b3d91, Enter, Done; then Add to theme: the deck back: no control dialog.background.close
...
- `assist.agent.propose-accept`: assist.propose over HTTP, assist.accept with the card, then the card with one byte changed: assist.propose answered 500 {"error":{"name":"AssistUnavailableError","status":500,"message":"The assistant is not set up on this Turboslide yet","action":"assist.propose"}}
...
- `sync.viewer.live-updates`: TimeoutError: page.waitForFunction: Timeout 90000ms exceeded.
- `sync.reload.same-document`: Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoBe[2m([22m[32mexpected[39m[2m) // Object.is equality[22m
- `sync.undo.after-remote`: Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoBe[2m([22m[32mexpected[39m[2m) // Object.is equality[22m
- `sync.resend.idempotent`: TimeoutError: page.waitForFunction: Timeout 90000ms exceeded.
- `sync.pull.no-listing`: A never read every word of B's ten commits
- `cost.editor-editing.calls`: store simple 156 a minute over 140
...
- `export.download.large-deck-pdf`: passed; recorded PDF: GT brand deck.pdf, 85 slides in 12.5 s, 0.15 s per slide; PDF: GT brand deck.pdf, 85 slides in 12.5 s, 0.15 s per slide
- `export.download.large-deck-pptx`: passed; recorded Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 178.3 s, 2.10 s per slide; Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 178.3 s, 2.10 s per slide
- `cost.editor-idle.calls`: passed; recorded function requests 9.66 a minute (ceiling 12; 29 in 3 min); the store settled 2.58 s after the state was ready (1 sync.status read(s) before the window); store simple 28 a minute (ceiling 40; 3 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced 9 a  (a cost row: over its ceiling on the preview it holds the ship)
...
- `cost.editor-editing.calls`: failed (store simple 156 a minute over 140); recorded function requests 72.94 a minute (ceiling 75; 219 in 3 min); the store settled 2.84 s after the state was ready (1 sync.status read(s) before the window); store simple 156 a minute (ceiling 140; 3 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced  (a cost row: over its ceiling on the preview it holds the ship)
- `cost.two-tabs-idle.calls`: failed (store list 1, ceiling none); recorded the store settled 5.26 s after the state was ready (1 sync.status read(s) before the window); store advanced 19 a minute (ceiling 30); store list 1 (ceiling 0); instances answering sync.status 2 (a cost row: over its ceiling on the preview it holds the ship)
- `cost.show.calls`: passed; recorded function requests 0 in the window (ceiling 0); the store settled 73.68 s after the state was ready (8 sync.status read(s) before the window; the first read: head 27, put 20, list 1, del 0); store calls at the first sample 1 (head 1, get 0, put 0, list 0, del 0), of which 1 the probe's own sync.statu (a cost row: over its ceiling on the preview it holds the ship)
- `shaders.perf.editor-frame`: passed; recorded longest animation frame 22.9 ms over 5 s with one shader on the stage (619 animation frames; rAF gaps; 1 canvas; renderer ANGLE (Google, Vulkan 1.3.0 (SwiftShader Device (LLVM 10.0.0) (0x0000C0DE)), SwiftShader driver)); longest animation frame 22.9 ms over 5 s with one shader on the stage (619 anim
