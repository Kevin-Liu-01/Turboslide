...
Base https://turboslide-ffc7yeovv-kl01s-projects.vercel.app, started 2026-09-26T01:20:32.775Z, 9604 s. 814 rows judged: 744 passed, 48 failed, 22 not driven (1 of them manual, the checklist's: text.clipboard.paste-without-formatting), 0 no step. Measurement rows (PRODUCT.md 8.2, recorded and never holding the ship; the cost rows of SYNC.md 6.1 among them, which hold it over their ceiling on the preview): export.download.large-deck-pdf passed (PDF: GT brand deck.pdf, 85 slides in 12.0 s, 0.14 s per slide; PDF: GT brand deck.pdf, 85 slides in 12.0 s, 0.14 s per slide); export.download.large-deck-pptx passed (Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 210.9 s, 2.48 s per slide; Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 210.9 s, 2.48 s per slide); cost.editor-idle.calls passed (function requests 9.99 a minute (ceiling 12; 30 in 3 min); the store settled 2.30 s after the state was ready (1 sync.status read(s) before the window); store simple 39 a minute (ceiling 40; 3 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced 10 a minute (ceiling 11); instances answering sync.status 1); cost.editor-hidden.calls not driven (function requests 9.65 a minute and 9 session poll(s) were read from the tab that stayed visible); cost.editor-editing.calls passed (function requests 72.60 a minute (ceiling 75; 218 in 3 min); the store settled 2.26 s after the state was ready (1 sync.status read(s) before the window); store simple 123 a minute (ceiling 140; 2 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced 70 a minute (ceiling 85); instances answering sync.status 1); cost.two-tabs-idle.calls passed (the store settled 2.21 s after the state was ready (1 sync.status read(s) before the window); store advanced 18 a minute (ceiling 30); store list 0 (ceiling 0); instances answering sync.status 1); cost.show.calls passed (function requests 0 in the window (ceiling 0); the store settled 53.61 s after the state was ready (6 sync.status read(s) before the window; the first read: head 16, put 0, list 0, del 0); store calls at the first sample 1 (head 1, get 0, put 0, list 0, del 0), of which 1 the probe's own sync.status read(s) (one deck.json head each, b3.md R7 b); the page's 0 (ceiling 0); function requests between the load and the window 0 (the settle, 53.61 s); instances answering sync.status 1); shaders.perf.editor-frame failed (longest animation frame 163.1 ms over 5 s with one shader on the stage (305 animation frames; rAF gaps; 1 canvas; renderer ANGLE (Google, Vulkan 1.3.0 (SwiftShader Device (LLVM 10.0.0) (0x0000C0DE)), SwiftShader driver)); longest animation frame 163.1 ms over 5 s with one shader on the stage (305 animation frames; rAF gaps; 1 canvas; renderer ANGLE (Google, Vulkan 1.3.0 (SwiftShader Device (LLVM 10.0.0) (0x0000C0DE)), SwiftShader driver))). Cost rows over their ceiling in this run: none. Verdict failed with the committed parked list inbox, templates and the parked rows arrange.group.tail-text-controls, logos.kit.find-a-logo, logos.picker.variants, shaders.background.add-to-theme, shaders.background.place-answers, shaders.frame.scrubber-capture, shaders.library.glyph-engines-render, tables.bar.row-column-buttons, tables.edge.add-row-column, tables.heads.select-row-column, tables.seam.row-drag, versions.show-changes-marks, view.live-pointers.second-browser, wordart.tail.fill-outline; retries 0 configured, 0 test(s) retried; exit 1. A not driven row is never counted as passed. Features a ship on this run would park (rule 4 of section 1; RETURN.md rule 2): formatting, inbox, brand, fonts, templates, assist; rows whose own controls a ship would keep parked: view.live-pointers.second-browser (view.livePointers.mine, view.livePointers.collaborators); decks.file.open-upload-bundle (file.open); versions.show-changes-marks (file.versionHistory.showChanges); templates.gallery.page (home.gallery, file.new.templateGallery); templates.card.rename-and-delete (templates.card.menu); templates.default.use-for-new (panel.brand.template.useForNew); tables.seam.row-drag (handle.table.row); tables.edge.add-row-column (handle.table.add.column, handle.table.add.row); tables.heads.select-row-column (handle.table.head.column, handle.table.head.row); tables.bar.row-column-buttons (bar.table); arrange.group.tail-text-controls (toolbar.group.text); wordart.tail.fill-outline (toolbar.wordart.outline); logos.insert.mono-tint (insert.logo); logos.index.refresh-fixture (insert.logo); logos.index.cached-offline (insert.logo); logos.picker.variants (dialog.logo.kind.wordmark, dialog.logo.tone.mono); logos.kit.find-a-logo (panel.brand.logo.find); shaders.panel.preset-tiles (formatOptions.shader.preset); shaders.panel.kit-colours (formatOptions.shader.color); shaders.panel.one-home (formatOptions.shader); shaders.frame.box-aspect (insert.shader); shaders.background.place-answers (dialog.background.shader); shaders.perf.one-context (insert.shader); shaders.background.add-to-theme (dialog.background.shader.addToTheme); shaders.library.glyph-engines-render (dialog.shader.engine.glyph); shaders.frame.scrubber-capture (formatOptions.shader.frame.scrubber, formatOptions.shader.frame.capture); shaders.show.plays-when-on (view.playShaders, formatOptions.shader.play); shaders.show.frame-when-off (view.playShaders); rows of an unparkable feature blocking the ship: decks.card.download, images.insert.first-on-new-deck, versions.restore, chrome.split.hover-no-inversion, sync.serial.order-and-latency, sync.title.concurrent-both-kept, sync.block.concurrent-same-offset-order, sync.resend.idempotent.
...
| `decks.card.download` | decks | core/decks.spec.ts | works | failed | Error: the card menu mints the bundle ticket and starts the download (the server functions answered: no server function answered) |
...
| `images.insert.first-on-new-deck` | images | core/images.spec.ts | broken | failed | Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoBeGreaterThan[2m([22m[32mexpected[39m[2m)[22m |
...
| `versions.restore` | versions | core/share.spec.ts | flaky | failed | Error: the restore lands within 35 s: the text did not land within 35 s (panel notice Restored the version of Sep 25, 08:23 PM) |
...
| `formatting.superscript.chord` | formatting | probe --core | works | failed | double click beta, Cmd+.: error: page.evaluate: RangeError: No slide "blank-2" |
| `formatting.subscript.chord` | formatting | probe --core | works | failed | double click beta, Cmd+,: error: no run fmt-a/text on the stage |
| `formatting.superscript.menu-word` | formatting | probe --core | broken | failed | double click beta, Format > Text > Superscript: error: no run fmt-a/text on the stage |
| `formatting.subscript.menu-word` | formatting | probe --core | broken | failed | double click beta, Format > Text > Subscript: error: no run fmt-a/text on the stage |
| `formatting.italic.menu-word` | formatting | probe --core | broken | failed | double click beta, Format > Text > Italic: error: no run fmt-a/text on the stage |
| `formatting.context.selection-rows` | formatting | probe --core | not driven | failed | double click gamma, right click it, read the rows, Superscript from the menu: error: no run fmt-a/text on the stage |
| `formatting.capitalization.upper` | formatting | probe --core | works | failed | select the box, Format > Text > Capitalization > UPPERCASE; Cmd+Z: error: locator.waitFor: Timeout 6000ms exceeded. |
| `formatting.capitalization.lower` | formatting | probe --core | works | failed | select the box, Format > Text > Capitalization > lowercase; Cmd+Z: error: locator.waitFor: Timeout 6000ms exceeded. |
| `formatting.capitalization.title` | formatting | probe --core | works | failed | select the box, Format > Text > Capitalization > Title Case; Cmd+Z: error: locator.waitFor: Timeout 6000ms exceeded. |
| `formatting.align.justified-menu` | formatting | probe --core | works | failed | select the box, Format > Align & indent > Justified: align null -> null (drawn null); after Cmd+Z null |
| `formatting.align.justified-chord` | formatting | probe --core | works | failed | select the box, Cmd+Shift+J: align null -> null (drawn null); after Cmd+Z null |
| `formatting.align.toolbar-justify` | formatting | probe --core | not driven | failed | select the box, the toolbar Align list, Justify: error: locator.boundingBox: Timeout 30000ms exceeded. |
| `formatting.spacing.add-before-remove` | formatting | probe --core | works | failed | select the box, Format > Line & paragraph spacing > Add space before paragraph, read the row, click it again: error: locator.waitFor: Timeout 8000ms exceeded. |
| `formatting.spacing.add-after` | formatting | probe --core | works | failed | select the box, Format > Line & paragraph spacing > Add space after paragraph; Cmd+Z: spaceAfter null; after Cmd+Z null |
| `formatting.spacing.custom-dialog` | formatting | probe --core | works | failed | select the box, Format > Line & paragraph spacing > Custom spacing; 1.3, 12, 6; Apply; Cmd+Z: error: locator.waitFor: Timeout 8000ms exceeded. |
| `formatting.spacing.1-15-value` | formatting | probe --core | broken | failed | select the box, Format > Line & paragraph spacing > 1.15: the row labelled 1.15 wrote leading null |
| `formatting.highlight.word` | formatting | probe --core | works | failed | double click delta, Highlight color, Amber; Cmd+Z: error: no run fmt-a/text on the stage |
| `formatting.paint-format.button` | formatting | probe --core | works | failed | select box A, Paint format, click box B; Cmd+Z: error: Cannot read properties of null (reading 'free') |
| `formatting.paint-format.chords` | formatting | probe --core | not driven | failed | select box A, Cmd+Option+C; select box B, Cmd+Option+V: A {"weight":700,"size":32,"color":"red"}; B {"weight":null,"size":null,"color":null} -> {"weight":null,"size":null,"color":null} |
| `formatting.clear.inline-marks` | formatting | probe --core | broken | failed | an italic word and a bold word in box B; select the box; Clear formatting; Cmd+Z: error: no run fmt-b/text on the stage |
...
| `formatting.persistence` | formatting | probe --core | works | failed | a superscript word, UPPERCASE, a highlighted word and a grouped pair; reload; the show; the viewer: error: no run fmt-a/text on the stage |
...
| `chrome.split.hover-no-inversion` | chrome | probe --core | broken | failed | hover the Slideshow half, then the chevron half: token --pt-plate-on-ink "#07070729"; wrapper rgb(242, 242, 240); label half rgba(0, 0, 0, 0)/rgb(7, 7, 7) -> hovered rgba(0, 0, 0, 0)/rgb(7, 7, 7) (chevron then rgba(0, 0, 0, 0)); chevron half rgba(0, 0, 0, 0) -> hovered rgba(7, 7, 7, 0.16)/rgb(7, 7,  |
...
| `decks.file.open-upload-bundle` | decks | core/decks.spec.ts | works | failed | Error: the bundle ticket (the server functions answered: 200 {"t":10,"i":0,"p":{"k":["result","error","context"],"v":[{"t":1,"s":"{\\\"deckId\\\":\\\"untitled-20260926-rtsf\\\",\\\"document\\\":{\\\"deck\\\":{\\\"schemaVersion\\\":1,\\\"id\\\":\\\"untitled-20260926-rtsf\\\",\\\"title\\\":\\\"Northwi |
...
| `versions.show-changes-marks` | versions | probe --core | not driven | failed | a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.842.pick (named row versionHistory.842.pick); marks with Show changes on 0 (); off 0 |
...
| `brand.colors.primary-live` | brand | probe --core | not driven | failed | Colors > Primary: type #0b3d91, read the sheet before Enter, Enter, read another slide, Cmd+Z: --blue #2f5ce0 -> #0b3d91 while typing (revision 856 -> 856); after Enter revision 858, record {"dark":{"primary":"#0b3d91"}}; the title slide reads #0b3d91; after Cmd+Z #2f5ce0 |
...
| `assist.outside-write.snackbar` | assist | probe --core | broken | failed | the seller types a word; POST /api/actions/text.replaceAll as an agent while the editor is open; Undo from the snackbar: seller's edit stored true; HTTP 200 {"replacements":2,"slideIds":["blank-17"],"revision":953}; snackbar "none" after 10023 ms with Undo false; the agent's word landed true; no Und |
...
| `sync.serial.order-and-latency` | sync | core/sync.spec.ts | works | failed | Error: every word within 5000 ms of the keystroke ( a29 5408 ms) |
...
| `sync.resend.idempotent` | sync | core/sync.spec.ts | not driven | failed | Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoContain[2m([22m[32mexpected[39m[2m) // indexOf[22m |
...
| `shaders.frame.auto-capture` | shaders | core/shaders.spec.ts | broken | passed |  |
...
| `shaders.background.place-answers` | shaders | probe --core | broken | not driven | not on this build: dialog.background.shader (docs/PRODUCT.md 7.1, B1 (dialogs/Background.tsx)); no Shader row and no Material row in the Background dialog (FEATURES.md 5.4) |
...
| `shaders.perf.one-context` | shaders | probe --core | not driven | failed | a second shader block placed through the window API; select each in turn and count the canvases: with nothing selected 1 canvas on the stage (2 on the page); first selected: 1 on the stage, 1 on the page; second selected: 1 on the stage after 3399 ms, the second's root 1 canvas, the first's root 0 c |
...
- `shaders.background.place-answers`: not on this build: dialog.background.shader (docs/PRODUCT.md 7.1, B1 (dialogs/Background.tsx)); no Shader row and no Material row in the Background dialog (FEATURES.md 5.4)
...
- `decks.card.download`: Error: the card menu mints the bundle ticket and starts the download (the server functions answered: no server function answered)
- `images.insert.first-on-new-deck`: Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoBeGreaterThan[2m([22m[32mexpected[39m[2m)[22m
- `versions.restore`: Error: the restore lands within 35 s: the text did not land within 35 s (panel notice Restored the version of Sep 25, 08:23 PM)
- `formatting.superscript.chord`: double click beta, Cmd+.: error: page.evaluate: RangeError: No slide "blank-2"
- `formatting.subscript.chord`: double click beta, Cmd+,: error: no run fmt-a/text on the stage
- `formatting.superscript.menu-word`: double click beta, Format > Text > Superscript: error: no run fmt-a/text on the stage
- `formatting.subscript.menu-word`: double click beta, Format > Text > Subscript: error: no run fmt-a/text on the stage
- `formatting.italic.menu-word`: double click beta, Format > Text > Italic: error: no run fmt-a/text on the stage
- `formatting.context.selection-rows`: double click gamma, right click it, read the rows, Superscript from the menu: error: no run fmt-a/text on the stage
- `formatting.capitalization.upper`: select the box, Format > Text > Capitalization > UPPERCASE; Cmd+Z: error: locator.waitFor: Timeout 6000ms exceeded.
- `formatting.capitalization.lower`: select the box, Format > Text > Capitalization > lowercase; Cmd+Z: error: locator.waitFor: Timeout 6000ms exceeded.
- `formatting.capitalization.title`: select the box, Format > Text > Capitalization > Title Case; Cmd+Z: error: locator.waitFor: Timeout 6000ms exceeded.
- `formatting.align.justified-menu`: select the box, Format > Align & indent > Justified: align null -> null (drawn null); after Cmd+Z null
- `formatting.align.justified-chord`: select the box, Cmd+Shift+J: align null -> null (drawn null); after Cmd+Z null
- `formatting.align.toolbar-justify`: select the box, the toolbar Align list, Justify: error: locator.boundingBox: Timeout 30000ms exceeded.
- `formatting.spacing.add-before-remove`: select the box, Format > Line & paragraph spacing > Add space before paragraph, read the row, click it again: error: locator.waitFor: Timeout 8000ms exceeded.
- `formatting.spacing.add-after`: select the box, Format > Line & paragraph spacing > Add space after paragraph; Cmd+Z: spaceAfter null; after Cmd+Z null
- `formatting.spacing.custom-dialog`: select the box, Format > Line & paragraph spacing > Custom spacing; 1.3, 12, 6; Apply; Cmd+Z: error: locator.waitFor: Timeout 8000ms exceeded.
- `formatting.spacing.1-15-value`: select the box, Format > Line & paragraph spacing > 1.15: the row labelled 1.15 wrote leading null
- `formatting.highlight.word`: double click delta, Highlight color, Amber; Cmd+Z: error: no run fmt-a/text on the stage
- `formatting.paint-format.button`: select box A, Paint format, click box B; Cmd+Z: error: Cannot read properties of null (reading 'free')
- `formatting.paint-format.chords`: select box A, Cmd+Option+C; select box B, Cmd+Option+V: A {"weight":700,"size":32,"color":"red"}; B {"weight":null,"size":null,"color":null} -> {"weight":null,"size":null,"color":null}
- `formatting.clear.inline-marks`: an italic word and a bold word in box B; select the box; Clear formatting; Cmd+Z: error: no run fmt-b/text on the stage
- `formatting.persistence`: a superscript word, UPPERCASE, a highlighted word and a grouped pair; reload; the show; the viewer: error: no run fmt-a/text on the stage
- `chrome.split.hover-no-inversion`: hover the Slideshow half, then the chevron half: token --pt-plate-on-ink "#07070729"; wrapper rgb(242, 242, 240); label half rgba(0, 0, 0, 0)/rgb(7, 7, 7) -> hovered rgba(0, 0, 0, 0)/rgb(7, 7, 7) (chevron then rgba(0, 0, 0, 0)); chevron half rgba(0, 0, 0, 0) -> hovered rgba(7, 7, 7, 0.16)/rgb(7, 7, 
- `view.live-pointers.second-browser`: Error: the second browser's pointer is drawn on the first within 10 s
- `decks.file.open-upload-bundle`: Error: the bundle ticket (the server functions answered: 200 {"t":10,"i":0,"p":{"k":["result","error","context"],"v":[{"t":1,"s":"{\\\"deckId\\\":\\\"untitled-20260926-rtsf\\\",\\\"document\\\":{\\\"deck\\\":{\\\"schemaVersion\\\":1,\\\"id\\\":\\\"untitled-20260926-rtsf\\\",\\\"title\\\":\\\"Northwi
- `versions.show-changes-marks`: a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.842.pick (named row versionHistory.842.pick); marks with Show changes on 0 (); off 0
...
- `brand.colors.primary-live`: Colors > Primary: type #0b3d91, read the sheet before Enter, Enter, read another slide, Cmd+Z: --blue #2f5ce0 -> #0b3d91 while typing (revision 856 -> 856); after Enter revision 858, record {"dark":{"primary":"#0b3d91"}}; the title slide reads #0b3d91; after Cmd+Z #2f5ce0
- `assist.outside-write.snackbar`: the seller types a word; POST /api/actions/text.replaceAll as an agent while the editor is open; Undo from the snackbar: seller's edit stored true; HTTP 200 {"replacements":2,"slideIds":["blank-17"],"revision":953}; snackbar "none" after 10023 ms with Undo false; the agent's word landed true; no Und
- `sync.serial.order-and-latency`: Error: every word within 5000 ms of the keystroke ( a29 5408 ms)
...
- `sync.resend.idempotent`: Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoContain[2m([22m[32mexpected[39m[2m) // indexOf[22m
...
- `shaders.panel.kit-colours`: read the Colors row's swatches; brand.set /colors/<appearance>/primary '#0b3d91'; read the shader and its frame: the Primary swatch clicked (revision 935 -> 936); swatches text, background, caption, hint, primary, accent, custom (6 of the six roles); the sheet renders dark (deck.info and the chrome 
...
- `export.download.large-deck-pdf`: passed; recorded PDF: GT brand deck.pdf, 85 slides in 12.0 s, 0.14 s per slide; PDF: GT brand deck.pdf, 85 slides in 12.0 s, 0.14 s per slide
- `export.download.large-deck-pptx`: passed; recorded Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 210.9 s, 2.48 s per slide; Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 210.9 s, 2.48 s per slide
- `cost.editor-idle.calls`: passed; recorded function requests 9.99 a minute (ceiling 12; 30 in 3 min); the store settled 2.30 s after the state was ready (1 sync.status read(s) before the window); store simple 39 a minute (ceiling 40; 3 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced 10 a (a cost row: over its ceiling on the preview it holds the ship)
- `cost.editor-hidden.calls`: not driven (document.visibilityState read "visible" after a second page was brought to the front (headless Chromium reports no hidden page), so the hidden state was not reached); recorded function requests 9.65 a minute and 9 session poll(s) were read from the tab that stayed visible (a cost row: over its ceiling on the preview it holds the ship)
- `cost.editor-editing.calls`: passed; recorded function requests 72.60 a minute (ceiling 75; 218 in 3 min); the store settled 2.26 s after the state was ready (1 sync.status read(s) before the window); store simple 123 a minute (ceiling 140; 2 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced  (a cost row: over its ceiling on the preview it holds the ship)
- `cost.two-tabs-idle.calls`: passed; recorded the store settled 2.21 s after the state was ready (1 sync.status read(s) before the window); store advanced 18 a minute (ceiling 30); store list 0 (ceiling 0); instances answering sync.status 1 (a cost row: over its ceiling on the preview it holds the ship)
- `cost.show.calls`: passed; recorded function requests 0 in the window (ceiling 0); the store settled 53.61 s after the state was ready (6 sync.status read(s) before the window; the first read: head 16, put 0, list 0, del 0); store calls at the first sample 1 (head 1, get 0, put 0, list 0, del 0), of which 1 the probe's own sync.status (a cost row: over its ceiling on the preview it holds the ship)
- `shaders.perf.editor-frame`: failed (Error: under 150 ms); recorded longest animation frame 163.1 ms over 5 s with one shader on the stage (305 animation frames; rAF gaps; 1 canvas; renderer ANGLE (Google, Vulkan 1.3.0 (SwiftShader Device (LLVM 10.0.0) (0x0000C0DE)), SwiftShader driver)); longest animation frame 163.1 ms over 5 s with one shader on the stage (305 an
