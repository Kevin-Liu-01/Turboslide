...
async function twins(frame, name, params, plate) {
  ...
  record.assets.push({ name, frame: `${frame}.png`, params: r.params, backend: r.backend, ms, litFraction: r.metrics.litFraction, plateClear: r.metrics.plateClear ?? null, warnings: r.metrics.warnings, bytes: { dark: r.dark.png.length, light: r.light.png.length } });
  return r;
}

// the hero: liquid metal field, frame 5500 ms, white point 120 so the field is ink and the contour bands paper
const hero = await twins('lm-field-5500', 'hero', { autocontrast: 0.5, white: 120, gamma: 1, polarity: 'light-ground', cell: 2 }, PLATE_BOXES.opener);
// the 800 by 450 screen at one cell per pixel, for viewports under 900 px and the pipeline figure
writeFileSync(`${OUT}/hero-screen-dark.png`, encodePng1(hero.positive.width ? { ...hero.positive, bits: hero.dark.bits.length === 1600 * 900 ? downsampleBits(hero.dark.bits) : hero.positive.bits } : hero.positive));
function downsampleBits(bits) { // the dark twin at cell 2 back to one bit per cell
  const out = new Uint8Array(800 * 450);
  for (let y = 0; y < 450; y += 1) for (let x = 0; x < 800; x += 1) out[y * 800 + x] = bits[(y * 2) * 1600 + x * 2];
  return out;
}
...
{
  const raw = await sharp('frames/lm-field-rep2-7000.png').resize(70, 79, { fit: 'cover', position: 'right', kernel: 'lanczos3' }).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
  let gray = toGray({ width: 70, height: 79, data: new Uint8Array(raw.data.buffer, raw.data.byteOffset, raw.data.byteLength) }, 'gray');
  ...
  record.assets.push({ name: 'og-band', frame: 'lm-field-rep2-7000.png', cells: '70 by 79 at 8 px', stages: 'lanczos3 cover crop (sharp), toGray, autocontrast 0.5, tone white 90, ditherGray, inverted', litFraction: +lit.toFixed(4) });
}
// the figure: gem smoke in ink and paper, the empty state, the 404 and the curtain
await twins('gs-ink-metaballs-6000', 'figure', { autocontrast: 0.5, black: 10, gamma: 1, polarity: 'dark-ground', cell: 2 });
...
await sharp('frames/lm-field-backblue-tintlight-5500.png').jpeg({ quality: 82 }).toFile(`${OUT}/hero-live.jpg`);
writeFileSync(`${OUT}/dither-record.json`, JSON.stringify(record, null, 2) + '\n');
console.log(JSON.stringify(record.assets.map((a) => ({ name: a.name, backend: a.backend, ms: a.ms, lit: a.litFraction, bytes: a.bytes })), null, 0));
