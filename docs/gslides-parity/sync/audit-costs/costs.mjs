...
const STORE = {
  ...
  streamLife: {
    pulseHeadsPerMin: 30,
    recheckPerMin: 1 /* authorize every 60 s: S+G */,
    closeS: 1,
    closeA: 3,
  },
...
const BYTES = {
  G: 2_000,
  staticPerEditorLoad: 1_450_000,
  staticPerDecksLoad: 740_000,
  staticPerViewerLoad: 735_000,
  pdf: 600_000,
};
...
function perEditorHour(share) {
  ...
  add(
    'cpuS',
    im *
      (RATE.idle.presence * CPU_S.presence +
        RATE.idle.poll * CPU_S.poll +
        RATE.idle.streamOpens * CPU_S.streamOpen),
  );
  ...
  add(
    'S',
    im *
      (RATE.idle.presence * STORE.presence.S +
        STORE.streamLife.pulseHeadsPerMin +
        STORE.streamLife.recheckPerMin +
        (60 / STORE.presencePush.everyS) * STORE.presencePush.S),
  );
  ...
  add(
    'invocations',
    em *
      (RATE.editing.presence +
        RATE.editing.poll +
        RATE.editing.streamOpens +
        RATE.editing.ops +
        RATE.editing.listVersions),
  );
  add(
    'cpuS',
    em *
      (RATE.editing.presence * CPU_S.presence +
        RATE.editing.poll * CPU_S.poll +
        RATE.editing.streamOpens * CPU_S.streamOpen +
        RATE.editing.ops * CPU_S.ops +
        RATE.editing.listVersions * CPU_S.listVersions +
        (60 / STORE.cardThumb.everyS) * CPU_S.render),
  );
  ...
  add(
    'S',
    em *
      (RATE.editing.presence * STORE.presence.S +
        STORE.streamLife.pulseHeadsPerMin +
        STORE.streamLife.recheckPerMin +
        (60 / STORE.presencePush.everyS) * STORE.presencePush.S +
        RATE.editing.ops * STORE.ops.S +
        RATE.editing.listVersions * STORE.listVersions.S +
        (60 / STORE.cardThumb.everyS) * STORE.cardThumb.S),
  );
  add(
    'A',
    em *
      ((60 / STORE.presencePush.everyS) * STORE.presencePush.A +
        RATE.editing.ops * STORE.ops.A +
        (60 / STORE.cardThumb.everyS) * STORE.cardThumb.A),
  );
  add(
    'G',
    em *
      (RATE.editing.presence * STORE.presence.G +
        STORE.streamLife.recheckPerMin +
        RATE.editing.ops * STORE.ops.G +
        RATE.editing.listVersions * STORE.listVersions.G),
  );
  // the loads
  add(
    'invocations',
    HOUR.editorLoads *
      (1 +
        ...
        1 +
        1 +
        4) /* document, readEditorDeck, attach, exportCapabilities, connectFacts, four warms */,
  );
  ...
  add(
    ...
    HOUR.decksLoads *
      (CPU_S.decksDocument +
        STORE.decksPage.preloads * CPU_S.readEditorDeck +
        STORE.decksPage.renders * 0.03),
  );
  add(
    'S',
    HOUR.decksLoads *
      (STORE.listingWarm.S +
        STORE.decksPage.preloads * STORE.readEditorDeck.S +
        STORE.decksPage.renders),
  );
  ...
  add(
    ...
    HOUR.decksLoads *
      (STORE.listingWarm.G +
        STORE.decksPage.preloads * STORE.readEditorDeck.G +
        STORE.decksPage.renders),
  );
...
function perShowView() {
  ...
  e.invocations +=
    ...
    1 /* attach */ +
    4 /* warms */ +
    SHOW_VIEW_MINUTES * RATE.show.poll;
  e.cpuS +=
    CPU_S.viewerDocument + 0.1 + 4 * CPU_S.warm + SHOW_VIEW_MINUTES * RATE.show.poll * CPU_S.poll;
...
console.log(
  `| one editor hour (${HOUR.editingMinutes} min editing, ${HOUR.idleMinutes} idle, ${HOUR.decksLoads} /decks loads, ${HOUR.homeLoads} /home, ${HOUR.editorLoads} editor load, ${HOUR.pdfExports} PDF) | ${f(ha.invocations)} | ${f(ha.cpuS)} | ${ha.gbH.toFixed(2)} / ${hs.gbH.toFixed(2)} | ${f(ha.S)} | ${f(ha.A)} | ${f(ha.G)} |`,
);
const v = perShowView();
console.log(
  `| one show view (${SHOW_VIEW_MINUTES} min) | ${f(v.invocations)} | ${f(v.cpuS, 1)} | ${v.gbH.toFixed(3)} | ${f(v.S)} | ${f(v.A)} | ${f(v.G)} |`,
);
...
for (const share of ['alone', 'shared']) {
  ...
  for (const hours of [5, 50, 500]) {
    ...
    if (t.invocations > inc.invocations)
      hobbyStops.push(`invocations day ${Math.ceil((inc.invocations / t.invocations) * 30)}`);
    if (t.cpuS / 3600 > inc.cpuHours)
      hobbyStops.push(`CPU day ${Math.ceil((inc.cpuHours / (t.cpuS / 3600)) * 30)}`);
    if (t.gbH > inc.gbHours) hobbyStops.push(`memory day ${Math.ceil((inc.gbHours / t.gbH) * 30)}`);
    if (t.S > inc.simple)
      hobbyStops.push(`Blob simple day ${Math.max(1, Math.ceil((inc.simple / t.S) * 30))}`);
    if (t.A > inc.advanced)
      hobbyStops.push(`Blob advanced day ${Math.max(1, Math.ceil((inc.advanced / t.A) * 30))}`);
    if (t.storageGb > inc.storageGb) hobbyStops.push('storage over 1 GB today');
    console.log(
      `| ${hours} editor h/day + 1,000 show views/day | ${f(t.invocations)} | ${f(t.cpuS / 3600, 1)} | ${f(t.gbH)} | ${f(t.S)} | ${f(t.A)} | ${t.bdtGb.toFixed(1)} | ${t.fotGb.toFixed(1)} | ${hobbyStops.length ? `paused: ${hobbyStops.join(', ')}` : 'within the free tier'} | ${money(dp.total)} (inv ${money(dp.invocations)}, CPU ${money(dp.cpu)}, mem ${money(dp.memory)}, FOT ${money(dp.fot)}, Blob ${money(dp.blobSimple + dp.blobAdvanced + dp.blobStorage + dp.blobTransfer)}) | ${money(Math.max(0, dp.total - CREDIT_PRO))} |`,
    );
  }
}
...
savings.push([
  'the session long poll: none for viewers and shows, none while hidden, the stream carries the command frame',
  perMonthMemory(pollGbHPerHour * (tabHoursPerMonth + showHoursPerMonth)) +
    ((pollInvPerHour * (tabHoursPerMonth + showHoursPerMonth)) / 1e6) * PRICE.invocationPerM,
  `${f(pollGbHPerHour * (tabHoursPerMonth + showHoursPerMonth))} GB-h and ${f(pollInvPerHour * (tabHoursPerMonth + showHoursPerMonth))} invocations a month`,
]);
// 2 the show on a published snapshot
const showInv = perShowView().invocations - SHOW_VIEW_MINUTES * RATE.show.poll;
savings.push([
  'a show reading a published snapshot from the public host: no server function after the HTML',
  (((showInv - 1) * 1000 * 30) / 1e6) * PRICE.invocationPerM +
    ((STORE.viewerLoad.S * 1000 * 30) / 1e6) * PRICE.blobSimplePerM +
    ((1000 * 30 * (CPU_S.viewerDocument + 0.1 + 4 * CPU_S.warm)) / 3600) * PRICE.cpuPerHour,
  `${f((showInv - 1) * 1000 * 30)} invocations, ${f(STORE.viewerLoad.S * 1000 * 30)} heads and ${f((1000 * 30 * (CPU_S.viewerDocument + 0.1 + 4 * CPU_S.warm)) / 3600, 1)} CPU h a month`,
]);
...
savings.push([
  'close an idle stream after 5 minutes without input and reopen on focus or input (the pulse poll stops with it)',
  perMonthMemory(
    streamGbHPerIdleHour * STREAM_BILLED_SHARE.shared * idleShare * tabHoursPerMonth * 0.8,
  ) +
    ((pulseSPerHour * idleShare * tabHoursPerMonth * 0.8) / 1e6) * PRICE.blobSimplePerM,
  `about ${f(streamGbHPerIdleHour * STREAM_BILLED_SHARE.shared * idleShare * tabHoursPerMonth * 0.8)} GB-h shared (${f(streamGbHPerIdleHour * idleShare * tabHoursPerMonth * 0.8)} alone) and ${f(pulseSPerHour * idleShare * tabHoursPerMonth * 0.8)} heads a month`,
]);
...
savings.push([
  'the presence heartbeat at 15 s when nothing moved (5 s while active) and the access and index records trusted for 60 s on the blob tier',
  ((((idlePresencePerHour * 2) / 3) * idleShare * tabHoursPerMonth) / 1e6) * PRICE.invocationPerM +
    ((((idlePresencePerHour * 2) / 3) * idleShare * tabHoursPerMonth * CPU_S.presence) / 3600) *
      PRICE.cpuPerHour +
    ((idlePresencePerHour * STORE.presence.S * 0.9 * idleShare * tabHoursPerMonth) / 1e6) *
      PRICE.blobSimplePerM,
  `${f(((idlePresencePerHour * 2) / 3) * idleShare * tabHoursPerMonth)} invocations and ${f(idlePresencePerHour * STORE.presence.S * 0.9 * idleShare * tabHoursPerMonth)} heads a month`,
]);
...
savings.push([
  'the card thumbnail rendered once when the deck rests 30 s or the tab hides, not every 8 s of typing',
  ((rendersPerEditingHour * editingHoursPerMonth * CPU_S.render) / 3600) * PRICE.cpuPerHour * 0.95 +
    ((rendersPerEditingHour * editingHoursPerMonth * STORE.cardThumb.A * 0.95) / 1e6) *
      PRICE.blobAdvancedPerM,
  `${f(rendersPerEditingHour * editingHoursPerMonth * 0.95)} Chromium renders, ${f((rendersPerEditingHour * editingHoursPerMonth * CPU_S.render * 0.95) / 3600, 1)} CPU h and ${f(rendersPerEditingHour * editingHoursPerMonth * STORE.cardThumb.A * 0.95)} advanced operations a month`,
]);
// 6 listVersions after every edit
const lvPerMonth = RATE.editing.listVersions * 60 * editingHoursPerMonth;
savings.push([
  'no listVersions round trip after every write (the checkpoint frame carries the record; the panel reads when opened)',
  (lvPerMonth / 1e6) * PRICE.invocationPerM +
    ((lvPerMonth * CPU_S.listVersions) / 3600) * PRICE.cpuPerHour +
    ((lvPerMonth * STORE.listVersions.S) / 1e6) * PRICE.blobSimplePerM,
  `${f(lvPerMonth)} invocations and ${f(lvPerMonth * STORE.listVersions.S)} heads a month`,
]);
// 7 the /decks preloads
const decksLoadsPerMonth = HOUR.decksLoads * tabHoursPerMonth;
savings.push([
  'the /decks cards preload the editor loader on intent only, and the listing reads one index record instead of one head per deck',
  ((decksLoadsPerMonth * STORE.decksPage.preloads) / 1e6) * PRICE.invocationPerM +
    ((decksLoadsPerMonth * STORE.decksPage.preloads * CPU_S.readEditorDeck) / 3600) *
      PRICE.cpuPerHour +
    ((decksLoadsPerMonth *
      (STORE.decksPage.preloads * STORE.readEditorDeck.S + STORE.listingWarm.S)) /
      1e6) *
      PRICE.blobSimplePerM,
  `${f(decksLoadsPerMonth * STORE.decksPage.preloads)} invocations and ${f(decksLoadsPerMonth * (STORE.decksPage.preloads * STORE.readEditorDeck.S + STORE.listingWarm.S))} heads a month`,
]);
// 8 the pulse tick backing off when idle
savings.push([
  'the pulse tick at 2 s while an op landed in the last 30 s, 10 s otherwise (head() stays: it is the only fresh read)',
  ((pulseSPerHour * 0.8 * idleShare * tabHoursPerMonth) / 1e6) * PRICE.blobSimplePerM,
  `${f(pulseSPerHour * 0.8 * idleShare * tabHoursPerMonth)} heads a month`,
]);
...
savings.push([
  'one put fewer per edit (the snapshot only at a named version or every tenth revision; the record proves the document)',
  ((editsPerMonth * 1) / 1e6) * PRICE.blobAdvancedPerM,
  `${f(editsPerMonth)} advanced operations a month`,
]);
// 10 pictures served by their store URL
savings.push([
  'the viewer payload names the picture twin by its store URL: no 302 through the asset route per picture per load',
  ((decksLoadsPerMonth * 3 + 1000 * 30 * 2) / 1e6) * PRICE.invocationPerM,
  `${f(decksLoadsPerMonth * 3 + 1000 * 30 * 2)} invocations a month at three pictures a deck`,
]);
