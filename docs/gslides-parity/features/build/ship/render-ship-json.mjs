...
if (out) {
  ...
  const list = {
    ...
    rule: `docs/FEATURES.md 7.2 and docs/RETURN.md section 1 rule 2: parkedFeaturesOf over the last readings of the two gate runs of record on the enforce preview ${one('preview') ?? ''} built from ${one('built-from') ?? one('commit') ?? 'the ship commit'}; a row carrying parks red in both runs parks the controls it names, a parkable feature red through a row without parks in both runs is parked whole, a row of a feature parked whole is not listed a second time, and a row or feature red in one run and green in the other is flaky and is not listed; the previous ship's list (${carryName}) is carried forward where its rows did not read green in both runs, since the menu model's advanced flags follow this list, and a carried row green in both runs leaves it (leaves); the exceptions the ship rule makes over the gate's mechanical wouldPark are in notParked, each with its reason`,
    parkedFeatures,
    parkedRows,
    notParked,
    leaves,
