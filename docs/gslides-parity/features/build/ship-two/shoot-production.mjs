// The ship step's pictures of the features round, ship two, on a deployment (docs/FEATURES.md
// 7.2: the new surfaces on production at 1440 in both appearances): Insert > Shader's gallery,
// the selected shader with its Shader section (the live canvas over the frame), the block at rest
// with its frame, Change background's Shader row with the picker open, View > Play shaders and
// the show's Options menu mirroring it. Adapted from the integrator's
// build/integrator/pictures-ship-two.mjs (the preview's pictures at 1440 and 1280 as PNG): one
// width, JPEG files named for the production table, the Vercel Authentication header from
// VERCEL_OIDC_TOKEN when the base is a preview. One context per appearance makes its own /new deck
// through the window API and trashes and removes it before it closes (the 404 read back), so
// nothing of this run stays on the shared Blob store.
//
//   node docs/gslides-parity/features/build/ship-two/shoot-production.mjs --base <origin>
//     [--out docs/gslides-parity/focus/verification] [--prefix production-features-two] [--only <names>]
...
const BASE = argOf('--base', 'https://turboslide.vercel.app').replace(/\/$/, '');
const OUT = argOf('--out', 'docs/gslides-parity/focus/verification');
const PREFIX = argOf('--prefix', 'production-features-two');
  ...
  for (const appearance of APPEARANCES) {
    ...
    try {
      ...
      try {
        ...
        await closeAll();
        // the block at rest: unselected, the frame drawn by the sheet (5.5), no canvas
        await page.keyboard.press('Escape');
        await page
          .waitForFunction(
            (id) => {
              const root = document.querySelector(`.ts-stagewrap.ts-editor [data-block="${id}"]`);
...
writeFileSync(
  join(OUT, `${PREFIX}-views.json`),
  `${JSON.stringify({ base: BASE, shotAt: new Date().toISOString(), results }, null, 2)}\n`,
);
