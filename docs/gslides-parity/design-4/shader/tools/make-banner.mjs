...
const { data, info } = await sharp(Buffer.from(markSvg(16, { plate: '#ffffff', ink: '#000000' })))
  .raw()
  .toBuffer({ resolveWithObject: true });
const ink = (x, y) => data[(y * info.width + x) * info.channels] < 128;
const rows = [];
for (let y = 4; y < 12; y += 2) {
  // the frame occupies rows 4 to 11
  let line = '';
...
writeFileSync(
  '/Users/kevinliu/repos/Turboslide/docs/gslides-parity/design-4/shader/previews/cli-banner.txt',
  banner,
);
