...
const record = (feature, interaction, result, evidence, extra = {}) => {
  const row = { n: rows.length + 1, feature, interaction, result, evidence: String(evidence), ...extra };
  rows.push(row);
  const tag = { works: 'ok  ', broken: 'FAIL', flaky: 'FLKY', 'not driven': 'n/d ' }[result] ?? '????';
  console.log(`${tag} ${String(row.n).padStart(2)} ${feature} :: ${interaction}\n       ${row.evidence}`);
  return row;
};
/**
...
const attempt = async (feature, interaction, fn, { tries = 3 } = {}) => {
  ...
  for (let t = 1; t <= tries; t += 1) {
    try {
      ...
      if (r.ok) return record(feature, interaction, t === 1 ? 'works' : 'flaky', t === 1 ? r.evidence : seen.join(' | '), { tries: t });
    } catch (error) {
      seen.push(`try ${t}: error ${error instanceof Error ? error.message.split('\n')[0] : String(error)}`);
    }
  }
...
const textOf = (page, selector) => page.evaluate((sel) => document.querySelector(sel)?.textContent?.trim() ?? null, selector);
...
const runs = (page) =>
  page.evaluate(() =>
    [...document.querySelectorAll('.ts-stagewrap.ts-editor .pt-slide:not(.is-leaving) [data-run]')].map((el) => el.getAttribute('data-run')),
  );
const runInfo = (page, run) =>
  page.evaluate((r) => {
    const el = document.querySelector(`.ts-stagewrap.ts-editor .pt-slide:not(.is-leaving) [data-run="${r}"]`);
    if (!el) return null;
    const clone = el.cloneNode(true);
    clone.querySelectorAll('[data-prompt]').forEach((p) => p.remove());
    const rect = el.getBoundingClientRect();
    return { text: clone.textContent ?? '', rect: { x: rect.x, y: rect.y, w: rect.width, h: rect.height } };
  }, run);
...
const answerNamePrompt = async (page, name) => {
  ...
  await sleep(500);
  if (await has(page, '[data-control="dialog.namePrompt"]')) await clickControl(page, 'dialog.namePrompt.continue').catch(() => undefined);
  await sleep(300);
  return true;
};
...
const showFacts = (page) =>
  page.evaluate(() => {
    ...
    const counter = document.querySelector('[data-control="present.counter"]')?.textContent?.trim() ?? '';
    return {
...
const waitIndex = (page, index, timeout = 5000) => pollUntil(() => showFacts(page), (f) => f.index === index, timeout);
const editModeOf = (page) => page.evaluate(() => document.querySelector('[data-edit-mode]')?.getAttribute('data-edit-mode') ?? null);
const roleOf = (page) => page.evaluate(() => window.turboslide?.studio?.describe?.().state?.access?.role ?? null).catch(() => null);
...
const watch = (page, tag) => {
  ...
  page.on('response', async (res) => {
    ...
    apiResponses.push({ tag, at: Date.now(), url: url.replace(BASE, ''), status: res.status(), body });
  });
};
...
const apiSince = (n, pattern) => apiResponses.slice(n).filter((r) => pattern.test(r.url)).map((r) => `${r.url} ${r.status}${r.body && r.body.error ? ` ${JSON.stringify(r.body).slice(0, 120)}` : ''}`);
...
try {
  ...
  headRun = await headRunOf(page);
  {
    ...
    await press(page, 'Escape');
    await pollUntil(() => state(page), (s) => s.revision >= 1, 30_000);
    await settled(page);
  ...
  const addSlide = async () => {
    ...
    let after = await pollUntil(async () => (await slideOrder(page)).length, (n) => n > before, 4000);
    let via = 'Ctrl+M';
    if (after <= before) {
      ...
      after = await pollUntil(async () => (await slideOrder(page)).length, (n) => n > before, 6000);
    }
    await clearAll(page);
    await settled(page);
  ...
  };
  {
    ...
    record('Scratch deck', 'Add two slides (Ctrl+M)', a.ok && b.ok ? 'works' : 'broken', `slides ${a.before} -> ${a.after} (${a.via}) -> ${b.after} (${b.via})`);
  }
  const order = await slideOrder(page);
  {
    ...
    if (r) await clickAt(page, r.x + r.width / 2, r.y + r.height / 2);
    await pollUntil(() => state(page), (s) => s.slideId === order[0], 6000);
    ...
    if (notesShown) {
      ...
      await settled(page);
      await sleep(800);
      const slide = await invoke(page, 'slide.get', { slideId: order[0] }).then((g) => g.slide ?? g);
    ...
    record(
      ...
      notesShown ? `notes pane shown; stored in the slide ${stored}` : 'no notes pane on the page and View > Show speaker notes did not show one',
    );
  }
  ...
  const startShow = async () => {
    ...
    return pollUntil(() => showFacts(page), (f) => f.present, 10_000);
  };
  const stopShow = async () => {
    await press(page, 'Escape');
    await pollUntil(() => showFacts(page), (f) => !f.present, 8000);
    await sleep(300);
  };
  await attempt('Slideshow', 'Click Slideshow in the title row', async () => {
    ...
    return { ok: on.present && f.index === 0 && f.total === 3, evidence: `present ${f.present}; index ${f.index} of ${f.total}; counter "${f.counter}"; fullscreen ${f.fullscreen}` };
  });
  const keyStep = (feature, interaction, key, from, to) =>
    attempt(feature, interaction, async () => {
      ...
      return { ok: before.index === from && after.index === to, evidence: `index ${before.index} -> ${after.index}; counter "${after.counter}"` };
    });
  ...
  await attempt('Slideshow keys', 'Type 3 then Enter jumps to slide 3', async () => {
    ...
    return { ok: f.index === 2, evidence: `after "3" snackbar ${JSON.stringify(toast)}; index ${f.index}; counter "${f.counter}"` };
  });
  ...
  await attempt('Slideshow', 'A click on the slide advances', async () => {
    ...
    return { ok: f.index === 1, evidence: `click at ${Math.round(x)},${Math.round(y)}; index 0 -> ${f.index}; counter "${f.counter}"` };
  });
  await attempt('Slideshow', 'The slide counter reads n of total', async () => {
    ...
    return { ok: /1\s*(of|\/)\s*3/.test(c1.counter) && /2\s*(of|\/)\s*3/.test(c2.counter), evidence: `counter "${c1.counter}" then "${c2.counter}"; toolbar visible ${visible}` };
  });
  await attempt('Slideshow', 'L toggles the laser pointer', async () => {
    ...
    return { ok: on.laser && on.dot && !off.laser, evidence: `after L laser ${on.laser}, dot ${on.dot}${dot ? ` at ${Math.round(dot.x)},${Math.round(dot.y)}` : ''}; after L again laser ${off.laser}` };
  });
  {
    if (!(await showFacts(page)).present) await startShow();
    await moveHuman(page, { x: 400, y: 600 }, { x: 120, y: 860 }, 10);
    ...
    if (await has(page, '[data-control="present.options"]')) {
      ...
      penState = await page.evaluate(() => {
        ...
        if (!row) return null;
        return { text: row.textContent?.trim() ?? '', disabled: row.getAttribute('aria-disabled') ?? row.getAttribute('disabled') ?? 'none' };
      });
      const rowsShown = await page.evaluate(() => [...document.querySelectorAll('[data-control^="menu.present.options"]')].map((el) => el.getAttribute('data-control')));
      evidence = `Options rows ${rowsShown.join(', ')}; pen row ${JSON.stringify(penState)}`;
      await shot(page, '02b-slideshow-options');
      await press(page, 'Escape');
      await sleep(300);
    }
    if (penState && penState.disabled !== 'none' && penState.disabled !== 'false') skip('Slideshow', 'Draw with the pen', `the Options menu lists Pen as a disabled row (${evidence})`);
    else if (penState) {
      await attempt('Slideshow', 'Draw with the pen', async () => {
        ...
        await sleep(400);
        const armed = await page.evaluate(() => document.querySelector('[data-control="present.show"]')?.className ?? '');
        return { ok: /pen|ink|draw/i.test(armed), evidence: `show classes "${armed}"; ${evidence}` };
      });
    } else skip('Slideshow', 'Draw with the pen', `no pen row in the show's Options menu (${evidence})`);
  }
  await attempt('Slideshow', 'Escape leaves the show', async () => {
    if (!(await showFacts(page)).present) await startShow();
    await press(page, 'Escape');
    const f = await pollUntil(() => showFacts(page), (x) => !x.present, 8000);
    const s = await state(page);
    return { ok: !f.present, evidence: `present ${f.present}; editor slide ${order.indexOf(s.slideId) + 1}; fullscreen ${f.fullscreen}` };
  });
  ...
  // ---- 3. Presenter view
  await attempt('Presenter view', 'Slideshow arrow > Presenter view opens a second window with notes, next slide and timer', async () => {
    await clearAll(page);
    await invoke(page, 'view.goto', { slideId: order[0] });
    await sleep(400);
    await clickControl(page, 'present.arrow');
    await page.locator('[data-control="menu.title.slideshow.presenterView"]').waitFor({ timeout: 6000 });
    ...
    await sleep(1500);
    const show = await pollUntil(() => showFacts(page), (f) => f.present, 8000);
    const facts = async () =>
      popup.evaluate(() => {
        ...
        return {
          ...
          notes: document.querySelector('[data-control="presenter.notesText"]')?.textContent?.trim() ?? '',
    ...
    const ok = show.present && f1.url === `/present/${deckId}` && f1.notes.includes(NOTES) && f1.elapsed !== f2.elapsed && aud.index === 1 && f3.index === 1 && f4.index === 2 && pages === 2;
  ...
  // ---- 4. a stranger and the new deck, before anything is shared
  {
    ...
    if (!accessPage) {
      ...
      const seen = await pollUntil(() => headingText(page), (t) => (t ?? '').includes('STRANGER'), 30_000);
    ...
    record('Restricted deck', 'A second browser with no link opens /edit/<id> of the new deck', accessPage ? 'works' : 'broken', evidence);
  ...
  const shareFacts = () =>
    page.evaluate(() => {
      ...
      const opts = (el) => (el ? [...el.options].map((o) => `${o.value}:${o.textContent?.trim()}`) : []);
      const rowText = (id) => q(`[data-control="dialog.share.${id}"]`)?.textContent?.trim().replace(/\s+/g, ' ') ?? null;
      return {
        ...
        footer: q('.ts-share-footer-sentence, [data-control="dialog.share.footer"]')?.textContent?.trim() ?? '',
        buttons: [...document.querySelectorAll('[data-control="dialog.share"] button')].map((b) => b.textContent?.trim() || b.getAttribute('aria-label') || '').filter(Boolean),
      };
    });
  let share = null;
  ...
  if (share && !share.inviteRole && share.linkRole === null) {
    skip('Share dialog', 'Set a role (Viewer, Commenter, Editor) on a person or on the link', `the dialog on production has no role control: it lists ${share.legacyRows.join('; ')}`);
    skip('Share dialog', 'General access: Restricted or Anyone with the link', 'the dialog on production has no General access select; the three addresses are always open (see the stranger row)');
  }
  ...
  const copyRow = async (id, expected) => {
    ...
    const toast = await snackbar(page);
    return { ok: clip === expected, evidence: `clipboard "${clip}"; snackbar ${JSON.stringify(toast)}` };
  };
  let viewLink = `${BASE}/deck/${deckId}`;
  let editLink = `${BASE}/edit/${deckId}`;
  let presentLink = `${BASE}/deck/${deckId}?present=1`;
  if (share?.legacy) {
    await attempt('Share dialog', 'Copy link on the View link row', () => copyRow('view', viewLink));
    await attempt('Share dialog', 'Copy link on the Present link row', () => copyRow('present', presentLink));
    await attempt('Share dialog', 'Copy link on the Edit link row', () => copyRow('edit', editLink));
    ...
    await attempt('Share dialog', 'Copy link (footer)', async () => {
      ...
      await sleep(700);
      const clip = await clipboard();
      return { ok: clip === viewLink, evidence: `clipboard "${clip}"; snackbar ${JSON.stringify(await snackbar(page))}` };
    });
  }
  ...
  await attempt('View link', 'A second browser opens the View link', async () => {
    ...
    return { ok: landed === `/deck/${deckId}` && f.viewer && !f.editor && !f.toolbar && f.text, evidence: `landed ${landed}; viewer ${f.viewer}, editor chrome ${f.editor}, toolbar ${f.toolbar}, menubar ${f.menubar}; contenteditable fields ${f.editable}; title "${f.title}"; deck text shown ${f.text}` };
  });
  await attempt('View link', 'The Present link opens as a show', async () => {
    await B.page.goto(presentLink, { waitUntil: 'domcontentloaded' });
    await B.page.waitForSelector('.pt-viewer', { timeout: 30_000 }).catch(() => undefined);
    const f = await pollUntil(() => showFacts(B.page), (x) => x.present, 15_000);
    await shot(B.page, '07-present-link');
    await B.page.keyboard.press('ArrowRight');
    const f2 = await waitIndex(B.page, 1, 5000);
    return { ok: f.present && f.index === 0 && f2.index === 1, evidence: `show ${f.present}; index ${f.index} of ${f.total}; ArrowRight -> ${f2.index}` };
  });
  await attempt('View link', 'The viewer changes the address to /edit/<id>: read only?', async (t) => {
    await B.page.goto(`${BASE}/edit/${deckId}`, { waitUntil: 'domcontentloaded' });
    await sleep(2000);
    if (await has(B.page, '[data-control="access.page"]')) return { ok: true, evidence: 'the editor address answers the You need access page' };
    ...
    const seen = await pollUntil(() => headingText(page), (x) => (x ?? '').includes(marker.trim()), 20_000);
    const rev1 = (await state(page)).revision;
    const toolbar = await B.page.evaluate(() => Boolean(document.querySelector('[data-control="toolbar"]')));
    ...
    return { ok: !leaked && !ed && rev1 === rev0, evidence: `data-edit-mode ${mode}; role ${role}; toolbar ${toolbar}; double click opened a text session ${ed}; owner title after the viewer typed "${marker}": "${seen}"; owner revision ${rev0} -> ${rev1}` };
  });
  ...
  await attempt('Edit link', 'A third browser opens the Edit link', async () => {
    ...
    return { ok: landed === `/edit/${deckId}` && mode === 'editing' && cHead !== null, evidence: `landed ${landed}; data-edit-mode ${mode}; role ${role}; heading run ${cHead}` };
  });
  await attempt('Collaboration', 'An edit in the third browser reaches the owner within a few seconds', async (t) => {
    const word = ` +C${t}`;
    ...
    const seen = await pollUntil(() => headingText(page), (x) => (x ?? '').includes(word.trim()), 30_000);
    const ms = Date.now() - t0;
    await shot(C.page, '09-editor-third-browser');
    return { ok: (seen ?? '').includes(word.trim()), evidence: `typed "${word}" in C; owner title "${seen}" after ${ms} ms` };
  });
  await attempt('Collaboration', 'An edit by the owner reaches the third browser within a few seconds', async (t) => {
    const word = ` +A${t}`;
    const r = (await runInfo(page, headRun))?.rect;
    ...
    await press(page, 'Escape');
    const t0 = Date.now();
    const seen = await pollUntil(() => headingText(C.page), (x) => (x ?? '').includes(word.trim()), 30_000);
    const ms = Date.now() - t0;
    return { ok: (seen ?? '').includes(word.trim()), evidence: `typed "${word}" in A; third browser title "${seen}" after ${ms} ms` };
  });
  await attempt('Collaboration', 'Presence chips show the other person in both tabs', async () => {
    ...
    const a = await pollUntil(() => chips(page), (x) => x.length >= 1, 15_000);
    const c = await pollUntil(() => chips(C.page), (x) => x.length >= 1, 15_000);
    ...
    return { ok: a.length >= 1 && c.length >= 1, evidence: `owner tab chips ${JSON.stringify(a)} count "${countA}" slot text "${slotA}"; third browser chips ${JSON.stringify(c)}` };
  });
  await attempt('Collaboration', 'A slide added in the third browser appears in the owner filmstrip', async () => {
    const before = (await slideOrder(page)).length;
    await clearAll(C.page);
    await press(C.page, 'Control+M');
    const t0 = Date.now();
    const after = await pollUntil(async () => (await slideOrder(page)).length, (n) => n > before, 30_000);
    const ms = Date.now() - t0;
    const cards = await page.evaluate(() => document.querySelectorAll('[data-control^="filmstrip.slide."]').length);
    return { ok: after > before, evidence: `owner slides ${before} -> ${after} after ${ms} ms; filmstrip cards ${cards}` };
  });
  ...
  const cardFacts = (p) =>
    p.evaluate(() => {
      ...
      return {
        ...
        first: document.querySelector('[data-control="comment.card.first"]')?.textContent?.trim().slice(0, 80) ?? '',
        replies: document.querySelectorAll('[data-control^="comment.card.reply."]').length,
        resolveLabel: document.querySelector('[data-control="comment.card.resolve"]')?.getAttribute('aria-label') ?? null,
  ...
  const panelFacts = () =>
    page.evaluate(() => ({
      tabs: [...document.querySelectorAll('[data-control^="panel.comments.tab."]')].map((el) => `${el.getAttribute('data-control').split('.').pop()}:${el.textContent?.trim()}`),
      ...
      text: document.querySelector('[data-control="panel.comments.list"]')?.textContent?.trim().slice(0, 200) ?? '',
    }));
  ...
  await attempt('Comments', 'Select the title block and add a comment (Cmd+Option+M)', async () => {
    ...
    let f = await pollUntil(() => cardFacts(page), (x) => x.newField, 3000);
    if (!f.newField) {
      via = 'toolbar Insert comment';
      await clickControl(page, 'toolbar.insertComment');
      f = await pollUntil(() => cardFacts(page), (x) => x.newField, 5000);
    }
    if (!f.newField) return { ok: false, evidence: `chip "${chip}"; no comment card after ${via}; snackbar ${JSON.stringify(await snackbar(page))}` };
    ...
    const after = await pollUntil(() => cardFacts(page), (x) => x.markers >= 1, 20_000);
    const toast = await snackbar(page);
    await settled(page);
  ...
  await attempt('Comments', 'Open the thread from its marker and reply', async () => {
    ...
    let f = await pollUntil(() => cardFacts(page), (x) => x.card && x.first !== '', 6000);
    if (!(await has(page, '[data-control="comment.card.reply"]'))) return { ok: false, evidence: `card ${f.card}; first "${f.first}"; no Reply control` };
    ...
    f = await pollUntil(() => cardFacts(page), (x) => x.replies >= 1, 15_000);
    await settled(page);
    await shot(page, '13-comment-reply');
    return { ok: f.replies >= 1, evidence: `first "${f.first}"; replies ${f.replies}; card text "${f.text.slice(0, 120)}"` };
  });
  await attempt('Comments', 'The comment reaches the third browser', async () => {
    if (!commented) return { skip: 'no comment was created' };
    const n = await pollUntil(() => C.page.evaluate(() => document.querySelectorAll('[data-control="comment.marker"]').length), (x) => x >= 1, 20_000);
  ...
  await attempt('Comments', 'Resolve the thread', async () => {
    ...
    if (!f.card) {
      ...
      await clickAt(page, m.x + m.w / 2, m.y + m.h / 2);
      f = await pollUntil(() => cardFacts(page), (x) => x.card, 6000);
    }
    ...
    await closePanel();
    return { ok: label !== null && after.markers === 0 && (panel.reopen >= 1 || /resolved/i.test(panel.text)), evidence: `resolve button "${label}"; markers after ${after.markers}; panel tabs ${panel.tabs.join(', ')}; threads ${panel.threads}, reopen buttons ${panel.reopen}; list "${panel.text.slice(0, 120)}"` };
  });
  ...
  const versionsOpen = async () => {
    ...
    if (await has(page, '[data-control="deck.lastEdit"]')) {
      ...
      if (await page.locator('[data-control="panel.versionHistory"]').waitFor({ timeout: 6000 }).then(() => true).catch(() => false)) return 'Last edit';
    }
    await openMenu(page, 'file');
  ...
  const versionFacts = () =>
    page.evaluate(() => ({
      ...
      picks: document.querySelectorAll('[data-control^="versionHistory."][data-control$=".pick"]').length,
      restores: [...document.querySelectorAll('[data-control^="versionHistory."][data-control$=".restore"]')].map((el) => el.getAttribute('data-control')),
      named: [...document.querySelectorAll('[data-control^="versionHistory."][data-control$=".name"]')].map((el) => el.textContent?.trim()).filter(Boolean),
      changes: document.querySelectorAll('[data-control="versions.change"], [data-control="versions.changeRun"]').length,
      text: document.querySelector('[data-control="panel.versionHistory"]')?.textContent?.trim().slice(0, 200) ?? '',
    }));
  ...
  await attempt('Version history', 'Open Version history from Last edit', async () => {
    ...
    return { ok: f.windows >= 1 || f.picks >= 1, evidence: `opened via ${via}; windows ${f.windows}; picks ${f.picks}; text "${f.text.slice(0, 140)}"` };
  });
  await attempt('Version history', 'Expand a window and pick a version', async () => {
    ...
    const picked = await page.evaluate(() => {
      const el = [...document.querySelectorAll('[aria-selected="true"]')].find((x) => x.closest('[data-control="panel.versionHistory"]'));
      return el ? (el.getAttribute('data-control') ?? el.closest('[data-control^="versionHistory."]')?.getAttribute('data-control') ?? 'selected') : null;
    });
    const showChanges = await page.evaluate(() => document.querySelector('[data-control="versionHistory.showChanges"]')?.checked ?? null);
    const f2 = await versionFacts();
    await shot(page, '16-version-picked');
    return { ok: picked !== null, evidence: `picks ${f1.picks}, restores ${f1.restores.length}; picked ${picked}; Show changes ${showChanges}; change marks ${f2.changes}` };
  });
  await attempt('Version history', 'Name current version', async () => {
    await versionsOpen();
    if (!(await has(page, '[data-control="versionHistory.nameCurrent"]'))) return { ok: false, evidence: 'no Name current version control' };
    await clickControl(page, 'versionHistory.nameCurrent');
    await page.locator('[data-control="versionHistory.nameCurrent.field"]').waitFor({ timeout: 5000 });
    await typeHuman(page, 'Before the call');
    await press(page, 'Enter');
    await sleep(1000);
    await expandAll();
    const f = await pollUntil(versionFacts, (x) => x.named.some((n) => /Before the call/.test(n)), 10_000);
    const toast = await snackbar(page);
    return { ok: f.named.some((n) => /Before the call/.test(n)), evidence: `named rows ${JSON.stringify(f.named)}; snackbar ${JSON.stringify(toast)}` };
  });
  const snapshot = async () => ({ title: await headingText(page), slides: (await slideOrder(page)).length, rev: (await state(page)).revision });
  const restoreRow = (interaction, pickIndex) =>
    attempt('Version history', interaction, async () => {
      ...
      const control = pickIndex === 'oldest' ? f.restores[f.restores.length - 1] : f.restores[Math.min(1, f.restores.length - 1)];
      ...
      const toast = await snackbar(page);
      await shot(page, pickIndex === 'oldest' ? '17b-version-restore-oldest' : '17-version-restore-previous');
      const changed = after.title !== before.title || after.slides !== before.slides;
      return { ok: after.rev > before.rev && changed, evidence: `restore control ${control} of ${f.restores.length}; title "${before.title}" -> "${after.title}"; slides ${before.slides} -> ${after.slides}; revision ${before.rev} -> ${after.rev}; snackbar ${JSON.stringify(toast)}` };
    });
  ...
  if (restoreA.result === 'works' || restoreA.result === 'flaky' || restoreB.result === 'works' || restoreB.result === 'flaky') {
    await attempt('Version history', 'Cmd+Z undoes the restore', async () => {
      ...
      return { ok: after.rev > before.rev && (after.title !== before.title || after.slides !== before.slides), evidence: `title "${before.title}" -> "${after.title}"; slides ${before.slides} -> ${after.slides}; revision ${before.rev} -> ${after.rev}` };
    });
  } else skip('Version history', 'Cmd+Z undoes the restore', 'no restore succeeded, so there was nothing to undo');
  ...
  await attempt('Rename', 'Click the deck name, type a new name, Enter; the third browser shows it', async (t) => {
    ...
    await press(page, 'Enter');
    const t0 = Date.now();
    const own = await pollUntil(() => textOf(page, 'button[data-control="deck.name"]'), (x) => x === NAME, 10_000);
    const theirs = await pollUntil(() => textOf(C.page, 'button[data-control="deck.name"]'), (x) => x === NAME, 30_000);
    const ms = Date.now() - t0;
    const theirTitle = await C.page.title();
    await shot(C.page, '18-rename-third-browser');
    return { ok: own === NAME && theirs === NAME, evidence: `owner name "${own}"; third browser name "${theirs}" after ${ms} ms; its document.title "${theirTitle}"` };
  });

  // ---- 11. You need access
  await attempt('You need access', 'Open the editor address of a deck that does not exist', async () => {
    const res = await B.page.goto(`${BASE}/edit/untitled-20260915-zzzz`, { waitUntil: 'domcontentloaded' });
    await sleep(2000);
    const f = await B.page.evaluate(() => ({
      ...
      sentence: document.querySelector('[data-control="access.sentence"]')?.textContent?.trim().slice(0, 160) ?? '',
      form: Boolean(document.querySelector('[data-control="access.form"]')),
      role: [...(document.querySelector('[data-control="access.role"]')?.options ?? [])].map((o) => o.textContent?.trim()),
      request: document.querySelector('[data-control="access.request"]')?.textContent?.trim() ?? '',
      signIn: Boolean(document.querySelector('[data-control="access.signIn"]')),
    }));
    await shot(B.page, '19-you-need-access');
    return { ok: (res?.status() ?? 0) === 404 && f.page && /You need access/.test(f.title), evidence: `status ${res?.status()}; page ${f.page}; title "${f.title}"; sentence "${f.sentence}"; request form ${f.form} with roles ${JSON.stringify(f.role)}, button "${f.request}"; sign in section ${f.signIn}` };
  });
  await attempt('You need access', 'Open the viewer address of a deck that does not exist', async () => {
    const res = await B.page.goto(`${BASE}/deck/untitled-20260915-zzzz`, { waitUntil: 'domcontentloaded' });
    await sleep(1500);
    const f = await B.page.evaluate(() => ({ page: Boolean(document.querySelector('[data-control="access.page"]')), title: document.querySelector('.ts-access-title, h1')?.textContent?.trim() ?? '' }));
    return { ok: (res?.status() ?? 0) === 404 && f.page, evidence: `status ${res?.status()}; page ${f.page}; title "${f.title}"` };
  });
  await attempt('You need access', 'Request access on the page', async () => {
    ...
    await sleep(1500);
    if (!(await has(B.page, '[data-control="access.request"]'))) return { ok: false, evidence: 'no Request access button' };
    ...
    const answer = await pollUntil(() => textOf(B.page, '[data-control="access.answer"]'), (x) => Boolean(x), 10_000);
    await shot(B.page, '20-request-access');
    return { ok: Boolean(answer), evidence: `answer "${answer}"` };
  });
  if (share && !share.stop) skip('Share dialog', 'Stop sharing revokes the link', 'the dialog on production has no Stop sharing control; the three addresses cannot be revoked short of the trash');
} catch (error) {
  ...
  if (deckId) {
    ...
    try {
      ...
      record('Teardown', 'File > Move to trash, Delete forever', 'broken', `failed: ${error instanceof Error ? error.message : String(error)}; falling back to the actions API`);
    }
    if (!trashed) {
      try {
    ...
    for (const route of ['edit', 'deck']) {
      ...
      record('Teardown', `GET /${route}/${deckId} after the delete`, status === 404 ? 'works' : 'broken', `status ${status}`);
    }
  }
  ...
  const summary = {
    ...
    rows,
    consoleErrors,
    apiResponses: apiResponses.map((r) => ({ tag: r.tag, url: r.url, status: r.status, keys: r.body && typeof r.body === 'object' ? Object.keys(r.body) : typeof r.body, error: r.body?.error ?? undefined })),
  };
