...
apps/studio/
  ...
  src/routes/index.tsx          redirects to /new (gslides-parity SPEC 6.1 and 7.9 item 5: the root opens a fresh presentation)
  src/routes/new.tsx            the fresh presentation: the editor over a draft the first write creates
  src/routes/decks.index.tsx    the home page (SSR): Recent, the deck cards, Move to trash
  src/routes/decks.trash.tsx    trash: Restore and Delete forever
  ...
  src/routes/present.$deckId.tsx Presenter view (notes, timer, next slide) driving the slideshow in the editor window, ssr: false
  src/routes/print.$deckId.tsx  the print preview, one page per slide
...
Amended by the Google Slides parity round (gslides-parity SPEC 1 and 7.9 item 6): the editor's frame is Google's. A title row (the deck name, Last edit, Share, Slideshow) sits above a menu bar (File, Edit, View, Insert, Format, Slide, Arrange, Tools, Extensions, Help) and a toolbar in Google's order whose tail changes with the selection and collapses into More at or below 1100 px; the filmstrip of thumbnails is on the left, the canvas in the middle with the speaker notes pane under it, the Format options, Themes, Version history and Check slides panels on the right, and a bottom bar with the filmstrip and grid view buttons and the side panel toggle. The status chip, the toolbar's revision, the lint badge and the filter row leave the default view; the save state reads "Saving", "All changes saved" or "Not saved yet" in the title row and the revision through `describe().state`. The diagram above is the layout of the view route's shell and of the editor before that round.

Search params carry view state so links reproduce a view: `?mode=slide|grid|book`, `?edit=1`, `?theme=light|dark`, `?twin=1`, `?lint=1`, `?src=1`; the slide is the hash (`#12` for existing links, `#s/<slideId>` as the stable form the studio writes).
...
Amended by the Google Slides parity round (gslides-parity SPEC 7.9 item 4, 10.1 and 10.2): the view route `/deck/:deckId` keeps the shell keys below; the editor at `/edit/:deckId` binds no bare letters and takes Google's shortcuts instead (Ctrl+M new slide, Cmd+D duplicate, Cmd+Shift+H find and replace, Cmd+Option+Shift+F and +C for the filmstrip and the canvas), so E, `⌘/`, `⌘L` and `⇧D` are retired there and their toggles live in the View and Tools menus.
