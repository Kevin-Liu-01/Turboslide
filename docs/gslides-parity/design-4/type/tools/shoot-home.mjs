...
async function shoot(out, { width = 1440, height = 900, theme = 'dark', dpr = 1 } = {}) {
  ...
  await page.addInitScript((t) => { try { localStorage.setItem('gt-theme', t); } catch {} }, theme);
  ...
  for (let y = 0; y < total; y += 700) { await page.evaluate((yy) => window.scrollTo(0, yy), y); await page.waitForTimeout(80); }
  await page.evaluate(() => window.scrollTo(0, 0));
  const pending = await page.evaluate(async () => {
    const imgs = [...document.images];
    const results = await Promise.all(imgs.map((i) => Promise.race([i.decode().then(() => 'ok').catch(() => 'fail'), new Promise((r) => setTimeout(() => r('timeout'), 20000))])));
    return results.map((r, k) => [imgs[k].getAttribute('src').split('/').pop(), r, imgs[k].naturalWidth]).filter(([, r, w]) => r !== 'ok' || w === 0);
  });
  ...
  const size = await page.evaluate(() => [document.documentElement.scrollWidth, document.documentElement.scrollHeight, document.fonts.check('500 20px "Inter Local"')]);
  await ctx.close();
