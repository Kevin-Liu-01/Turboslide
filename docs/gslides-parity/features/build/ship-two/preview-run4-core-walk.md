...
| `surface.cleanup` | surface | works | failed | Delete forever on /decks/trash, then GET /edit/<id> and /deck/<id>: the trash page; /edit 200, /deck 404 | 639 |
...
| `versions.show-changes-marks` | versions | not driven | failed | a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.912.pick (named row versionHistory.912.pick); marks with Show changes on 0 (); off 0 | 488 |
...
| `text.link.detect-url` | text | broken | failed | type generaltranslation.com then a space at the end of the box; Cmd+Z: text "Visit the site for the terms generaltranslation.com"; link "generaltranslation.com" -> https://generaltranslation.com; after Cmd+Z link gone, text "Visit the site for the terms generaltran" | 188 |
...
| `shaders.perf.one-context` | shaders | not driven | failed | a second shader block placed through the window API; select each in turn and count the canvases: with nothing selected 1 canvas on the stage (2 on the page); first selected: 1 on the stage, 1 on the page; second selected: 1 on the stage after 613 ms, the second's root 1 canvas, the first's root 0 canvas, frame img decoded | 586 |
