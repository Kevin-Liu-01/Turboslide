 ...
 * action count, the MCP tool count, the HTTP path count, the layouts, the shape presets, the
 * materials, the check step count and the parity audit row count are written into
 * `packages/theme/brand/facts.json` by `scripts/build-brand.ts --facts` from the tree (B1's file
 * and script) and `brand.test.ts` asserts them against the tree, so a literal count in the page's
 * copy is a defect and every count flows through this module. The page does not import the facts
 * file itself: it carries 141 measured rows the page never reads (about 120 KB of JavaScript,
 * measured on the dev server), so `scripts/build-home-assets.ts` copies the ten values the page
 * needs into `facts-data.ts` with the file's sha256 and `--check` fails when facts.json has moved
 * on. The measured numbers of the speed rows and the closing sentence do not come from here: they
 * are quoted with their document and date in `copy.ts` (`MEASURED`) and cross checked against the
 * verifier's JSON by `copy.test.ts`.
 */
...
export const HOME_FACTS: HomeFacts = {
  ...
  shapePresets: FACTS_DATA.shapePresets,
