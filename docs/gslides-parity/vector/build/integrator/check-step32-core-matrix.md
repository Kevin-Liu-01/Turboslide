...
Base http://localhost:4321, started 2026-09-25T05:09:49.552Z, 7733 s. 827 rows judged: 768 passed, 36 failed, 23 not driven (1 of them manual, the checklist's: text.clipboard.paste-without-formatting), 0 no step. Measurement rows (PRODUCT.md 8.2, recorded and never holding the ship; the cost rows of SYNC.md 6.1 among them, which hold it over their ceiling on the preview): export.download.large-deck-pdf passed (PDF: GT brand deck.pdf, 85 slides in 7.0 s, 0.08 s per slide; PDF: GT brand deck.pdf, 85 slides in 7.0 s, 0.08 s per slide); export.download.large-deck-pptx passed (Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 55.5 s, 0.65 s per slide; Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 55.5 s, 0.65 s per slide); cost.editor-idle.calls passed (function requests 10 a minute (ceiling 12; 30 in 3 min); the store half reads zero on this base (the file store); the function requests alone are asserted); cost.editor-hidden.calls not driven (function requests 9.67 a minute and 9 session poll(s) were read from the tab that stayed visible); cost.editor-editing.calls passed (function requests 71.33 a minute (ceiling 75; 214 in 3 min); the store half reads zero on this base (the file store); the function requests alone are asserted); cost.two-tabs-idle.calls passed (the store half reads zero on this base (the file store); the function requests alone are asserted); cost.show.calls passed (function requests 0 in the window (ceiling 0); the store half reads zero on this base (the file store); the function requests alone are asserted). Cost rows over their ceiling in this run: none. Verdict failed; retries 0 configured, 0 test(s) retried; exit 1. A not driven row is never counted as passed. Features a ship on this run would park (rule 4 of section 1; RETURN.md rule 2): lines, inbox, brand, fonts, templates, assist, logos; rows whose own controls a ship would keep parked: view.live-pointers.second-browser (view.livePointers.mine, view.livePointers.collaborators); versions.show-changes-marks (file.versionHistory.showChanges); templates.gallery.page (home.gallery, file.new.templateGallery); assist.rewrite.card-accept-undo (panel.assist.starter.shorter); assist.notes.draft (panel.assist.starter.notes); assist.free-ask.fallback-sentence (panel.assist.prompt); tables.seam.row-drag (handle.table.row); tables.edge.add-row-column (handle.table.add.column, handle.table.add.row); tables.heads.select-row-column (handle.table.head.column, handle.table.head.row); tables.bar.row-column-buttons (bar.table); arrange.group.tail-text-controls (toolbar.group.text); wordart.tail.fill-outline (toolbar.wordart.outline); logos.picker.search (insert.logo); logos.picker.paper-and-ink (insert.logo); logos.picker.recents (dialog.logo.group.recent); logos.picker.licence-words (dialog.logo.source); logos.picker.chrome-1280 (insert.logo); logos.insert.one-click-asset (insert.logo); logos.insert.logo-size (insert.logo); logos.insert.mono-tint (insert.logo); logos.insert.every-slide (dialog.logo.everySlide); logos.tailor.find-customer-logo (dialog.tailor.logo.find); logos.replace-image.row (format.image.replaceImage.logo); logos.route.mark-headers (insert.logo); logos.index.refresh-fixture (insert.logo); logos.index.cached-offline (insert.logo); logos.cache.open-licence-only (insert.logo); logos.picker.variants (dialog.logo.kind.wordmark, dialog.logo.tone.mono); logos.kit.find-a-logo (panel.brand.logo.find); logos.export.svgblip (export.svg.vector); svg.import.upload (intake.svg.upload); svg.import.url (intake.svg.url); svg.render.picture-gestures (intake.svg.upload, intake.svg.paste, intake.svg.drop, intake.svg.url); svg.copy.markup (picture.svg.copy); svg.export.pptx-svgblip (export.svg.vector); svg.export.pptx-fallback (export.svg.vector); svg.sanitize.script-and-handlers (intake.svg.upload); svg.sanitize.cap (intake.svg.upload); svg.sanitize.broken (intake.svg.upload); rows of an unparkable feature blocking the ship: sync.resend.idempotent, logos.export.pdf-pptx-crisp.
...
| `lines.tail.colour-weight-dash-ends` | lines | probe --core | not driven | failed | the line toolbar's Line color, Line weight, Line dash, Line start and Line end; Undo: error: page.evaluate: Execution context was destroyed, most likely because of a navigation |
| `lines.context.line` | lines | probe --core | not driven | failed | right click the line, read the rows, Line end > Arrow from it: error: Cannot read properties of null (reading 'free') |
...
| `lines.insert.arrow-head` | lines | probe --core | works | failed | Insert > Line > Arrow by a drag; read the svg: error: page.evaluate: TypeError: Cannot read properties of undefined (reading 'studio') |
| `lines.tail.line-start-end-menu` | lines | probe --core | not driven | failed | select the line; Format > Borders & lines > Line start > Arrow, Cmd+Z; Line end > None, Cmd+Z: Line start > Arrow: start field true (drawn parts 1 -> 2), Cmd+Z restored false; Line end > Arrow then None: arrow true -> false (drawn parts 1); two Cmd+Z restored false |
...
| `versions.show-changes-marks` | versions | probe --core | not driven | failed | a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.481.pick (named row versionHistory.481.pick); marks with Show changes on 0 (); off 0 |
...
| `logos.picker.search` | logos | probe --core | not driven | failed | Insert > Logo; type figma at human speed; Enter: 0 result tiles 7 ms after the last key (first none; foot "Logos from thesvg.org. Brand marks belong to their owners; use them to name the brand, not to imply endorsement"); Enter inserted nothing; dialog closed false |
...
| `logos.export.pdf-pptx-crisp` | export | core/export.spec.ts | not driven | not driven | not driven: the logo index answered "page.evaluate: Error: No logo named figma on thesvg.org     at Object.deserialize (http://localhost:4321/@fs/Users/kevin" on this base |
...
| `logos.export.svgblip` | logos | core/logos.spec.ts | not driven | not driven | not driven: the logo index answered "page.evaluate: Error: No logo named figma on thesvg.org     at Object.deserialize (http://localhost:4321/@fs/Users/kevin" on this base |
...
| `svg.import.url` | svg | core/svg.spec.ts | not driven | not driven | not driven: /api/logo/mark/figma/default.svg answered 404 on this base (the fixture upstream on the preview, the live index on production; docs/VECTOR.md 4.3) |
...
- `logos.export.pdf-pptx-crisp`: not driven: the logo index answered "page.evaluate: Error: No logo named figma on thesvg.org     at Object.deserialize (http://localhost:4321/@fs/Users/kevin" on this base
...
- `logos.export.svgblip`: not driven: the logo index answered "page.evaluate: Error: No logo named figma on thesvg.org     at Object.deserialize (http://localhost:4321/@fs/Users/kevin" on this base
- `svg.import.url`: not driven: /api/logo/mark/figma/default.svg answered 404 on this base (the fixture upstream on the preview, the live index on production; docs/VECTOR.md 4.3)

## Failed rows, by id and reason

- `lines.tail.colour-weight-dash-ends`: the line toolbar's Line color, Line weight, Line dash, Line start and Line end; Undo: error: page.evaluate: Execution context was destroyed, most likely because of a navigation
- `lines.context.line`: right click the line, read the rows, Line end > Arrow from it: error: Cannot read properties of null (reading 'free')
...
- `lines.insert.arrow-head`: Insert > Line > Arrow by a drag; read the svg: error: page.evaluate: TypeError: Cannot read properties of undefined (reading 'studio')
- `lines.tail.line-start-end-menu`: select the line; Format > Borders & lines > Line start > Arrow, Cmd+Z; Line end > None, Cmd+Z: Line start > Arrow: start field true (drawn parts 1 -> 2), Cmd+Z restored false; Line end > Arrow then None: arrow true -> false (drawn parts 1); two Cmd+Z restored false
- `view.live-pointers.second-browser`: Error: the second browser's pointer is drawn on the first within 10 s
- `versions.show-changes-marks`: a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.481.pick (named row versionHistory.481.pick); marks with Show changes on 0 (); off 0
...
- `logos.picker.search`: Insert > Logo; type figma at human speed; Enter: 0 result tiles 7 ms after the last key (first none; foot "Logos from thesvg.org. Brand marks belong to their owners; use them to name the brand, not to imply endorsement"); Enter inserted nothing; dialog closed false
...
- `export.download.large-deck-pdf`: passed; recorded PDF: GT brand deck.pdf, 85 slides in 7.0 s, 0.08 s per slide; PDF: GT brand deck.pdf, 85 slides in 7.0 s, 0.08 s per slide
- `export.download.large-deck-pptx`: passed; recorded Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 55.5 s, 0.65 s per slide; Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 55.5 s, 0.65 s per slide
...
- `cost.editor-editing.calls`: passed; recorded function requests 71.33 a minute (ceiling 75; 214 in 3 min); the store half reads zero on this base (the file store); the function requests alone are asserted (a cost row: over its ceiling on the preview it holds the ship)
