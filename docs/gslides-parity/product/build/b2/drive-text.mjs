...
try {
  ...
  await t.placeBlock(S, {
    ...
    type: 'text',
    text: '',
    pos: { x: 200, y: 200, w: 700, h: 60, z: 5 },
  });
  await t.step(
    'text.link.detect-url: typing generaltranslation.com then a space wraps a link; one Cmd+Z removes it and keeps the text',
    'a link, then plain text',
    async () => {
      ...
      await t.placeBlock(S, {
        ...
        type: 'text',
        text: '',
        pos: { x: 950, y: 200, w: 500, h: 60, z: 11 },
      });
      ...
      return {
        ok:
          /<a href="https:\/\/generaltranslation\.com"[^>]*>generaltranslation\.com<\/a>/.test(
            linked,
          ) &&
          /<\/a>(&nbsp;| |\u00a0)today/.test(afterMore) &&
          /\(https:\/\/generaltranslation\.com\) today$/.test(storedBefore ?? '') &&
          /\[acme\.com\]\(https:\/\/acme\.com\)/.test(linked2 ?? '') &&
          undone === 'See acme.com',
  ...
  await t.step(
    'text.link.detect-email: an email then Enter wraps a mailto link',
    'a mailto link',
    async () => {
      await t.placeBlock(S, {
        id: 'tb-mail',
        type: 'text',
        text: '',
        pos: { x: 200, y: 300, w: 700, h: 60, z: 6 },
      });
      ...
      return {
        ok:
          /<a href="mailto:kevin@generaltranslation\.com"[^>]*>kevin@generaltranslation\.com<\/a>/.test(
            h,
          ) &&
          /any time/.test(s ?? '') &&
          !/any time\]\(/.test(s ?? ''),
        observed: `html ${h.slice(0, 260)} | stored ${s}`,
      };
    },
  );
  ...
  await t.placeBlock(S, {
    ...
    text: '',
    autofit: 'grow',
    pos: { x: 200, y: 420, w: 600, h: 48, z: 7 },
  });
  await t.step(
    'text.select.shift-home-line: Shift+Home on the third line selects that line alone',
    'one line',
    async () => {
      ...
      return {
        ok: sel.trim() === 'generaltranslation.com' && sel2.trim() === 'generaltranslation.com',
        observed: `Shift+Home "${sel}"; Home, Shift+End "${sel2}"`,
      };
    },
  );
  await t.step(
    'text.select.double-click-address: a double click on generaltranslation.com selects the whole address',
    'the address',
    async () => {
  ...
  // ---- the popover
  await t.step(
    'text.link.popover-apply-remove: Cmd+K shows the popover with Link, the text, the field, the slides select, Apply and Remove; Enter applies; the chip shows Change and Remove; Remove takes the link off',
    'popover, chip, removal',
    async () => {
      const run = await runOf('tb-lines');
      ...
      const parts = await page.evaluate(() => {
        ...
        return {
          ...
          slide: [...pop.querySelectorAll('[data-control="popover.link.slide"] option')]
            .map((o) => o.textContent)
            .slice(0, 6),
      ...
      const chip = await page.evaluate(() => {
        ...
        return c
          ? {
              href: c.querySelector('[data-control="chip.link.href"]')?.textContent,
              change: Boolean(c.querySelector('[data-control="chip.link.change"]')),
              remove: Boolean(c.querySelector('[data-control="chip.link.remove"]')),
            }
          : null;
      });
      await t.shot('chip');
      ...
      return {
        ok:
          parts !== null &&
          parts.label === 'Link' &&
          (parts.text ?? '').includes('founder') &&
          parts.url &&
          parts.apply &&
          parts.remove &&
          parts.slide.length >= 5 &&
          parts.focused === 'popover.link.url' &&
          /<a href="https:\/\/example\.com\/acme"[^>]*>founder<\/a>/.test(h1) &&
          chip !== null &&
          chip.change &&
          chip.remove &&
          /example\.com\/acme/.test(chip.href ?? '') &&
          !/<a [^>]*>founder<\/a>/.test(h2) &&
          /Kevin Liu, founder<\/span>/.test(h2) &&
          chipGone,
  ...
  await t.step(
    'text.link.slide-target: Slides in this presentation > slide 3 writes a slide link; the show jumps to it on a click',
    'a #s link and the jump',
    async () => {
      const run = await runOf('tb-lines');
      ...
      const point = await page.evaluate((r) => {
        ...
        let node;
        while ((node = walker.nextNode())) {
          const i = node.nodeValue.indexOf('Kevin');
          if (i >= 0) {
            const range = document.createRange();
      ...
      const options = await page.evaluate(() =>
        [...document.querySelectorAll('[data-control="popover.link.slide"] option')].map((o) => ({
          v: o.value,
          t: o.textContent,
        })),
      );
      ...
      // the show
      await t.press('Escape');
      await t.clickControl('present.open').catch(async () => {
        await t.invoke('view.present', { on: true });
      });
      await sleep(1200);
      ...
      if (linkInShow) {
        ...
        active = await page.evaluate(
          () =>
            document
              .querySelector('[data-control="present.show"]')
      ...
      return {
        ok:
          /href="#s\/[^"]+"/.test(h) &&
          new RegExp(`#s/${S3}`).test(s ?? '') &&
          showing &&
          linkInShow !== null &&
          linkInShow.target === null &&
          active === S3,
        observed: `third option ${JSON.stringify(third)}; html ${h.slice(0, 160)}; stored ${s}; show ${showing}; link ${JSON.stringify(linkInShow)}; active after click ${active} (want ${S3})`,
      };
    },
  );
  ...
  await t.gotoSlide(S);
  await t.step(
    'text.autofit.textbox-grow: the frame follows the text while typing and the stored height holds it',
    'no overflow during and after',
    async () => {
      await t.clearAll();
      const box = await t.placeBlock(S, {
        ...
        text: '',
        autofit: 'grow',
        pos: { x: 900, y: 500, w: 340, h: 48, z: 8 },
      });
      const run = await openBox(box.id);
      const para =
        'The renewal terms apply to every seat in the workspace from the first day of the next quarter, with the discount held for two years and the support tier unchanged.';
      ...
      const live = await page.evaluate((id) => {
        ...
        const ring = document
          .querySelector('.ts-overlay .ts-select.is-selected')
      ...
      return {
        ok: live.overflow < 2 && after.pos.h > 100 && drawn.free.h > 60,
        observed: `live ${JSON.stringify(live)}; stored h ${after.pos.h}; drawn free h ${drawn.free.h}`,
      };
    },
  );

  await t.step(
    'text.autofit.shrink-on-overflow: with Shrink text on overflow a small box steps the size down until the text fits',
    'the size steps down; no overflow',
    async () => {
      await t.clearAll();
      const box = await t.placeBlock(S, {
        id: 'tb-shrink',
        ...
        autofit: 'shrink',
        typography: { size: 22 },
        pos: { x: 900, y: 300, w: 340, h: 90, z: 9 },
      });
      const run = await openBox(box.id);
      const para =
        'Every seat in the workspace renews on the first day of the next quarter and the discount holds for two more years.';
      await t.typeHuman(para);
      await sleep(700);
      await t.press('Escape');
      await sleep(800);
      ...
      return {
        ok: (after.block.typography?.size ?? 22) < 22 && read.overflow < 4,
        observed: `stored size ${after.block.typography?.size}; drawn ${JSON.stringify(read)}`,
      };
    },
  );
  ...
  await t.step(
    'text.find-replace.count-while-typing: typing Acme reads "1 of 2"; Next moves the selection to the second',
    'the count and the step',
    async () => {
      await t.clearAll();
      await t.placeBlock(S, {
        id: 'tb-acme',
        type: 'text',
        text: 'Acme renews in Q3',
        pos: { x: 200, y: 700, w: 500, h: 48, z: 10 },
      });
      await t.gotoSlide(S3);
      await t.placeBlock(S3, {
        id: 'tb-acme-2',
        type: 'text',
        text: 'Pricing for Acme',
        pos: { x: 200, y: 200, w: 500, h: 48, z: 1 },
      });
      await t.gotoSlide(S);
      await t.clearAll();
      ...
      const count1 = await page.evaluate(
        () =>
          document.querySelector('[data-control="dialog.findReplace.count"]')?.textContent ?? null,
      );
      ...
      return {
        ok:
          ...
          count3 === '3 of 3' &&
          st3.slideId === S3 &&
          st3.blockId === 'tb-acme-2',
        observed: `count ${count1} -> ${count2} -> ${count3}; after Next: slide ${st2.slideId} block ${st2.blockId}; after Next: slide ${st3.slideId} (want ${S3}) block ${st3.blockId}`,
      };
    },
  );
  ...
  t.record(
    'console errors during the text drive',
    'none',
    errs.join(' | ') || 'none',
    errs.length === 0,
  );
