...
try {
  ...
  record(
    ...
    SLIDE && SLIDE !== firstOrder[0] ? 'works' : 'broken',
    `slides ${order.length}; working slide ${SLIDE}; active ${(await state(page)).slideId}`,
  );
  ...
  await attempt(
    'Insert picture',
    'Insert > Image > By URL with an https PNG on a host outside the capture allowlist (raw.githubusercontent.com), then Insert',
    () => insertByUrl(URL_EXTERNAL),
    { reset: () => closeMenus(page) },
  );
  await attempt(
    'Insert picture',
    'Insert > Image > By URL with an https PNG on an allowlisted host (upload.wikimedia.org), then Insert',
    async () => {
  ...
  await attempt(
    'Insert picture',
    'Insert > Image > Upload from computer, a PNG through the file chooser',
    async () => {
      const before = await ids();
      const mark = net.length;
      await openMenu(page, 'insert');
      ...
      return {
        ok: Boolean(obj),
        evidence: `file chooser opened (multiple ${chooser.isMultiple()}); ${Date.now() - t0} ms; ${await describeBlock(obj)}; snackbar ${await snackbar(page)}; network ${netSince(mark)}`,
      };
    },
    { reset: () => closeMenus(page) },
  );
  ...
  await attempt(
    'Insert picture',
    'drag a PNG file from the desktop and drop it on the slide (a DataTransfer built in the page, dispatched as dragover and drop at a point)',
    async () => {
      await clearAll(page);
      const before = await ids();
      ...
      return {
        ok: Boolean(obj),
        evidence: `dropped at sheet 1000,560; ${await describeBlock(obj)}; snackbar ${await snackbar(page)}; network ${netSince(mark)}`,
      };
    },
  );
  ...
  await attempt(
    'Insert picture',
    'copy a PNG to the clipboard and press Cmd V on the slide',
    async (i) => {
      await clearAll(page);
      const before = await ids();
      ...
      return {
        ok: Boolean(obj),
        evidence: `${how}; ${await describeBlock(obj)}; snackbar ${await snackbar(page)}; network ${netSince(mark)}`,
      };
    },
  );
  ...
  if (all.length === 0) {
    ...
    record(
      ...
      obj ? 'works' : 'broken',
      `${how}; ${await describeBlock(obj)}; network ${netSince(mark)}`,
    );
  ...
  for (const extra of all) {
    ...
    record(
      ...
      ok
        ...
        : `the block is still on the slide: ${posStr((await blockOf(page, SLIDE, extra.id))?.pos)}`,
    );
  }
  ...
  record(
    'setup',
    'the subject placed at 560,300 480x300 through the window API',
    startBox ? 'works' : 'broken',
    `${await describeBlock(await blockOf(page, SLIDE, ID))}; k ${fmt(k)}; frame past the box: ${startBox ? `${fmt(startBox.inner.h - startBox.free.h)} css px tall, ${fmt(startBox.inner.w - startBox.free.w)} wide` : '?'}`,
  );

  await attempt(
    'Select picture',
    'click the picture: the chip, the eight handles and the ring',
    async () => {
      ...
      const tail = await page.evaluate(() =>
        [...document.querySelectorAll('[data-control="toolbar.tail"] [data-control^="toolbar."]')]
          .map((el) => el.getAttribute('data-control'))
          .filter((c) => c !== 'toolbar.tail')
          .join(','),
      );
      return {
        ok: Boolean(ctrls) && dirs.length === 8 && ctrls.includes(`handle.${ID}.rotate`),
        evidence: `chip "${c}"; handles ${dirs.join(',')}${ctrls?.includes(`handle.${ID}.rotate`) ? ',rotate' : ''}; toolbar tail ${tail || 'none'}`,
      };
    },
  );
  await shot('selected');
  ...
  await attempt(
    'Move picture',
    'drag the picture by its frame edge 120 by 80 sheet px',
    async () => {
      ...
      const from = edge
        ...
        : center(await handleRect(page, `handle.${ID}.move`));
      const to = { x: from.x + 120 * k, y: from.y + 80 * k };
      const mid = await drag(page, from, to, {
        during: async () => ({ readout: await readout(page), guides: await guidesCount(page) }),
      });
      ...
      return {
        ok: near(dx, 120, 12) && near(dy, 80, 12) && ro === null,
        evidence: `moved ${fmt(dx)},${fmt(dy)}; readout during ${mid.readout ?? 'none'}, after ${ro ?? 'none'}; guides during ${mid.guides}; pos ${posStr(before.pos)} -> ${posStr(after.pos)}`,
      };
    },
    { reset: () => clearAll(page) },
  );
  ...
  await attempt(
    'Alignment guides',
    'drag the picture so its centre comes within 3 px of the slide centre: a guide shows and the centre snaps to 800',
    async () => {
      await selectObject(ID);
      const before = await blockOf(page, SLIDE, ID);
      const edge = await rectOf(page, '.ts-overlay .ts-frame-edge[data-side="n"]');
      ...
      return {
        ...
        evidence: `guides during ${mid.guides} (${mid.axes}); centre ${fmt(cx)} -> ${fmt(centreAfter)} (aimed at 803); pos ${posStr(after.pos)}`,
      };
    },
    { reset: () => clearAll(page) },
  );
  ...
  await attempt(
    'Alignment guides',
    'drag the picture so its left edge comes within 3 px of the title box left edge: a guide shows and the edge snaps',
    async () => {
      await selectObject(ID);
      const before = await blockOf(page, SLIDE, ID);
      const others = (await blocksOf(page, SLIDE)).filter(
        (b) => b.id !== ID && b.pos && b.pos.w > 40,
      );
      ...
      const mid = await drag(page, from, to, {
        steps: 20,
        during: async () => ({ guides: await guidesCount(page) }),
      });
      const after = await waitPos(ID, (p) => p.x !== before.pos.x);
      await settled(page);
      return {
        ok: mid.guides > 0 && near(after.pos.x, other.pos.x, 0.5),
        evidence: `other ${other.type} ${other.id} at x ${fmt(other.pos.x)}; guides during ${mid.guides}; x ${fmt(before.pos.x)} -> ${fmt(after.pos.x)} (aimed at ${fmt(other.pos.x + 3)})`,
      };
    },
    { reset: () => clearAll(page) },
  );
  ...
  await attempt(
    'Nudge picture',
    'ArrowRight moves 1 px, Shift ArrowRight 10 px, ArrowDown 1 px',
    async () => {
      await selectObject(ID);
      const before = await blockOf(page, SLIDE, ID);
      ...
      return {
        ok:
          near(a.pos.x - before.pos.x, 1, 0.01) &&
          near(b.pos.x - a.pos.x, 10, 0.01) &&
          near(c.pos.y - before.pos.y, 1, 0.01),
        evidence: `x ${fmt(before.pos.x)} -> ${fmt(a.pos.x)} -> ${fmt(b.pos.x)}; y ${fmt(before.pos.y)} -> ${fmt(c.pos.y)}`,
      };
    },
    { reset: () => clearAll(page) },
  );
  ...
  const undoToolbar = async (id, wantPos) => {
    ...
    const back = await waitPos(
      id,
      (p) => p.x === wantPos.x && p.y === wantPos.y && p.w === wantPos.w && p.h === wantPos.h,
      12_000,
    );
    await settled(page);
    return (
      back &&
      back.pos.x === wantPos.x &&
      back.pos.w === wantPos.w &&
      back.pos.h === wantPos.h &&
      back.pos.y === wantPos.y
    );
  };
  ...
  for (const mod of [null, 'shift']) {
    for (const dir of DIRS) {
      ...
      await attempt(
        'Resize picture',
        `drag the ${dir} handle by ${dxS},${dyS} sheet px${mod ? ' with Shift held' : ''}, then Undo from the toolbar`,
        async () => {
          await clearAll(page);
          const ctrls = await selectObject(ID);
          ...
          const mid = await drag(page, from, to, {
            mods: mod ? [mod] : [],
            during: async () => ({ readout: await readout(page), box: await boxOf(page, ID) }),
          });
          ...
          if (!mod) {
            geometryOk =
              near(after.pos.x, exp.x, 12) &&
              near(after.pos.y, exp.y, 12) &&
              near(after.pos.w, exp.w, 12) &&
              near(after.pos.h, exp.h, 12);
          ...
          const contentOk = box?.img
            ? near(box.img.w, box.free.w, 3) && near(box.img.h, box.free.h, 3)
            : false;
          const content = box?.img
            ? `img ${fmt(box.img.w)}x${fmt(box.img.h)} in box ${fmt(box.free.w)}x${fmt(box.free.h)} css (frame ${fmt(box.inner.w)}x${fmt(box.inner.h)})`
            : 'no img';
        ...
        },
        {
          reset: async () => {
            ...
            await waitPos(ID, (p) => p.x === 560 && p.w === 480);
          },
        },
      );
    }
  ...
  record(
    'Resize picture',
    'the picture content against its box across the 16 resizes',
    contentOff.length === 0 ? 'works' : 'broken',
    contentOff.length === 0
      ? 'the img filled the box within 3 css px after every resize'
      : `${contentOff.length} of 16 off: ${contentOff.slice(0, 3).join('; ')}`,
  );

  await attempt(
    'Resize picture',
    'drag the se handle with Alt held: the box grows from its centre',
    async () => {
      await clearAll(page);
      await selectObject(ID);
      const before = await blockOf(page, SLIDE, ID);
      ...
      const undone = await undoToolbar(ID, before.pos);
      return {
        ok: near(cxA, cxB, 6) && after.pos.w > before.pos.w && undone,
        evidence: `pos ${posStr(before.pos)} -> ${posStr(after.pos)}; centre x ${fmt(cxB)} -> ${fmt(cxA)}; undo restored ${undone}`,
      };
    },
    { reset: () => clearAll(page) },
  );
  ...
  await attempt(
    'Rotate picture',
    'drag the rotation ring about 35 degrees clockwise, then Undo',
    async () => {
      await clearAll(page);
      await selectObject(ID);
      const before = await blockOf(page, SLIDE, ID);
      ...
      await settled(page);
      return {
        ok: near(angle, 35, 6) && ro === null && Boolean(back),
        evidence: `rotate ${before.pos.rotate ?? 0} -> ${fmt(angle)}; readout during ${mid.readout ?? 'none'}, after ${ro ?? 'none'}; undo restored ${Boolean(back)}`,
      };
    },
    { reset: () => clearAll(page) },
  );
  ...
  const cropFacts = () =>
    page.evaluate(() => {
      ...
      return {
        ...
        handles: [...document.querySelectorAll('.ts-overlay [data-control*=".crop."]')].map((el) =>
          el.getAttribute('data-control'),
        ),
        chip: document.querySelector('.ts-overlay .ts-select-chip')?.textContent ?? null,
        stageCrop: Boolean(
          document.querySelector('.ts-stagewrap[data-crop], .ts-editor[data-crop]'),
        ),
      };
    });
  await attempt(
    'Crop picture',
    'double click the picture: crop mode with the dimmed full picture, the frame and eight crop handles',
    async () => {
      await clearAll(page);
      await selectObject(ID);
      const box = await boxOf(page, ID);
      ...
      return {
        ok: f.on && f.handles.length === 8,
        evidence: `crop frame ${f.on}; handles ${f.handles.length} (${f.handles.slice(0, 2).join(',')}...); chip "${f.chip}"; frame ${f.frame ? `${fmt(f.frame.w)}x${fmt(f.frame.h)}` : 'none'}, full ${f.full ? `${fmt(f.full.w)}x${fmt(f.full.h)}` : 'none'}`,
      };
    },
    { reset: () => clearAll(page) },
  );
  ...
  await attempt(
    'Crop picture',
    'drag the east crop edge 100 sheet px to the left: the frame narrows by 100',
    async () => {
      ...
      return {
        ok: after.on && near(dw, 100, 6),
        evidence: `frame w ${fmt(f.frame.w / k)} -> ${fmt(after.frame.w / k)} sheet px (delta ${fmt(dw)}); during ${mid.frame ? fmt(mid.frame.w / k) : 'none'}; full stays ${fmt(after.full.w / k)}x${fmt(after.full.h / k)}`,
      };
    },
  );
  await attempt(
    'Crop picture',
    'drag the south crop edge 60 sheet px up: the frame shortens by 60',
    async () => {
      ...
      return {
        ok: after.on && near(dh, 60, 6),
        evidence: `frame h ${fmt(f.frame.h / k)} -> ${fmt(after.frame.h / k)} sheet px (delta ${fmt(dh)})`,
      };
    },
  );
  ...
  await attempt(
    'Crop picture',
    'press Enter: the crop applies (trim written, the box shrinks, the picture shows the kept part)',
    async () => {
      const f = await cropFacts();
      await press(page, 'Enter');
      const after = await waitPos(
        ID,
        (p, b) => b.block.trim !== undefined || p.w !== cropBefore.pos.w,
      );
      await settled(page);
      ...
      cropApplied = after;
      return {
        ok: Boolean(after?.block.trim) && !off.on && Boolean(box?.cropFrame),
        evidence: `crop mode on before ${f.on}; trim ${JSON.stringify(after?.block.trim ?? null)}; pos ${posStr(cropBefore.pos)} -> ${posStr(after?.pos)}; crop frame on the sheet ${box?.cropFrame}; img ${box?.img ? `${fmt(box.img.w)}x${fmt(box.img.h)} in ${fmt(box.free.w)}x${fmt(box.free.h)}` : 'none'}; crop mode after ${off.on}`,
      };
    },
  );
  ...
  await attempt(
    'Crop picture',
    'Undo from the toolbar: the trim leaves and the box returns',
    async () => {
      await clickControl(page, 'toolbar.undo');
      const back = await waitPos(
        ID,
        (p, b) =>
          b.block.trim === undefined && p.w === cropBefore.pos.w && p.h === cropBefore.pos.h,
      );
      await settled(page);
      return {
        ok: Boolean(back),
        evidence: `trim ${JSON.stringify(back?.block.trim ?? null)}; pos ${posStr(back?.pos)} (before the crop ${posStr(cropBefore.pos)})`,
      };
    },
  );
  await attempt('Crop picture', 'Redo from the toolbar: the crop returns', async () => {
    ...
    await settled(page);
    return {
      ok: Boolean(again?.block.trim),
      evidence: `trim ${JSON.stringify(again?.block.trim ?? null)}; pos ${posStr(again?.pos)}`,
    };
  });
  await attempt(
    'Crop picture',
    'Format > Image > Crop image enters crop mode; Escape leaves it',
    async () => {
      await clearAll(page);
      await selectObject(ID);
      ...
      return {
        ok: st === 'enabled' && on.on && !off.on,
        evidence: `row ${st}; crop mode ${on.on} then after Escape ${off.on}; trim unchanged ${JSON.stringify(before?.block.trim) === JSON.stringify(after?.block.trim)}`,
      };
    },
    { reset: () => clearAll(page) },
  );
  await attempt(
    'Crop picture',
    'the toolbar Crop image button enters crop mode; a west edge drag then Escape: what Escape does with the change',
    async () => {
      await clearAll(page);
      await selectObject(ID);
      ...
      return {
        ok: on.on && !off.on,
        evidence: `crop mode from the toolbar ${on.on}; after Escape ${off.on}; trim ${JSON.stringify(before?.block.trim)} -> ${JSON.stringify(after?.block.trim)} (Escape ${changed ? 'committed the edge drag' : 'discarded the edge drag'}); pos ${posStr(before?.pos)} -> ${posStr(after?.pos)}`,
      };
    },
    { reset: () => clearAll(page) },
  );
  ...
  await attempt(
    'Format options',
    'the toolbar Image options button opens the panel with Picture, Adjustments and Dither sections',
    async () => {
      await clearAll(page);
      await selectObject(ID);
      ...
      const recolor = await page.evaluate(() =>
        Boolean(document.querySelector('[data-control="formatOptions.adjustments.recolor"]')),
      );
      const reflection = await page.evaluate(() =>
        Boolean(document.querySelector('[data-control="formatOptions.adjustments.reflection"]')),
      );
      return {
        ok: sections.includes('picture') && sections.includes('adjustments'),
        evidence: `sections ${sections.join(',')}; recolor control present ${recolor}; reflection control present ${reflection}`,
        extra: { recolor, reflection },
      };
    },
    { reset: () => clearAll(page) },
  );
  ...
  await attempt(
    'Format options',
    'drag the Transparency slider to about 40 percent: the picture fades on the slide and adjust.transparency is written',
    async () => {
      await selectObject(ID);
      await openFormatOptions();
      ...
      return {
        ...
        evidence: `slider value ${v}; adjust ${JSON.stringify(after?.block.adjust ?? null)}; img opacity on the sheet ${box?.img?.opacity}`,
      };
    },
    { reset: () => clearAll(page) },
  );
  ...
  await attempt(
    'Format options',
    'drag the Brightness slider to about 70 percent: adjust.brightness is written and the picture filter changes',
    async () => {
      await selectObject(ID);
      await openFormatOptions();
      await openSection('adjustments');
      ...
      return {
        ...
        evidence: `slider value ${v}; adjust ${JSON.stringify(after?.block.adjust ?? null)}; img filter ${box?.img?.filter}`,
      };
    },
    { reset: () => clearAll(page) },
  );
  if (hasRecolor) {
    await attempt('Format options', 'pick a Recolor preset', async () => {
      ...
      return {
        ok: Boolean(after?.block.adjust?.recolor),
        evidence: `adjust ${JSON.stringify(after?.block.adjust ?? null)}`,
      };
    });
  } else {
    notDriven(
      'Format options',
      'Recolor a picture',
      'production has no Recolor control; the Adjustments section carries Transparency, Brightness and Contrast',
    );
  }
  await attempt(
    'Format options',
    ...
    async () => {
      ...
      return {
        ...
        evidence: `adjust ${JSON.stringify(after?.block.adjust ?? null)}; img opacity ${box?.img?.opacity}`,
      };
    },
    { reset: () => clearAll(page) },
  );
  ...
  await attempt(
    ...
    async () => {
      ...
      return {
        ...
        evidence: `dither before ${JSON.stringify(before?.block.dither ?? null)} -> ${JSON.stringify(after?.block.dither ?? null).slice(0, 120)}; canvas ${box?.dither ? `${box.dither.w}x${box.dither.h} hidden ${box.dither.hidden}` : 'none'}; button aria-pressed ${pressed}`,
      };
    },
    { reset: () => clearAll(page) },
  );
  ...
  await attempt(
    'Dither',
    'the toolbar Dither button again turns the screen off',
    async () => {
      await clearAll(page);
      await selectObject(ID);
      await clickControl(page, 'toolbar.dither');
      ...
      return {
        ...
        evidence: `dither ${JSON.stringify(after?.block.dither ?? null)}; canvas ${box?.dither ? `hidden ${box.dither.hidden}` : 'none'}`,
      };
    },
    { reset: () => clearAll(page) },
  );
  ...
  await attempt(
    'Reset image',
    'Format > Image > Reset image removes the crop',
    async () => {
      await clearAll(page);
      await selectObject(ID);
      const before = await blockOf(page, SLIDE, ID);
      ...
      if (st !== 'enabled') {
        ...
        return {
          ok: false,
          evidence: `row ${st} with trim ${JSON.stringify(before?.block.trim ?? null)}`,
        };
      }
      ...
      const box = await boxOf(page, ID);
      return {
        ok: Boolean(after) && after.block.trim === undefined,
        evidence: `row ${st}; trim ${JSON.stringify(before?.block.trim ?? null)} -> ${JSON.stringify(after?.block.trim ?? null)}; pos ${posStr(before?.pos)} -> ${posStr(after?.pos)}; crop frame on the sheet ${box?.cropFrame}`,
      };
    },
    { reset: () => closeMenus(page) },
  );
  ...
  await attempt(
    'Replace image',
    'right click the picture > Replace image > By URL, a second https PNG, then Replace: the asset changes and the box stays',
    async () => {
      await clearAll(page);
      await selectObject(ID);
      const before = await blockOf(page, SLIDE, ID);
      ...
      const items = await page.evaluate(() =>
        [...document.querySelectorAll('.ts-context-menu [data-control^="menu."]')].map((el) =>
          el.getAttribute('data-control'),
        ),
      );
      await hoverRow(
      ...
      const label = await page.evaluate(
        () => document.querySelector('[data-control="dialog.imageByUrl.ok"]')?.textContent ?? '',
      );
      await clickControl(page, 'dialog.imageByUrl.url');
      ...
      return {
        ok:
          ...
          !dialogOpen,
        evidence: `context rows ${items.length} (${items.filter((i) => i.includes('image')).join(',')}); button "${label}"; asset ${before.block.asset} -> ${after?.block.asset}; pos ${posStr(before.pos)} -> ${posStr(after?.pos)}; img natural ${box?.img?.natural} -> ${boxAfter?.img?.natural}`,
      };
    },
    { reset: () => closeMenus(page) },
  );
  ...
  await attempt(
    'Replace image',
    'the toolbar Replace image button > Upload from computer, a PNG through the file chooser',
    async () => {
      await clearAll(page);
      await selectObject(ID);
      const before = await blockOf(page, SLIDE, ID);
      ...
      return {
        ok:
          ...
          after.block.asset !== before.block.asset &&
          posStr(after.pos) === posStr(before.pos),
        evidence: `asset ${before.block.asset} -> ${after?.block.asset}; pos ${posStr(before.pos)} -> ${posStr(after?.pos)}; snackbar ${await snackbar(page)}`,
      };
    },
    { reset: () => closeMenus(page) },
  );
  ...
  await attempt(
    'Slide background',
    'Slide > Change background, pick a colour, Done: the slide takes the colour',
    async () => {
      await clearAll(page);
      ...
      const pick =
        swatches.find(
          (s) => s.bg !== before.bg && !/rgba\(0, 0, 0, 0\)/.test(s.bg) && s.pressed !== 'true',
        ) ??
        swatches[1] ??
        swatches[0];
      if (!pick)
        return {
          ok: false,
          evidence: `no colour swatches; dialog controls ${await page.evaluate(() => [...document.querySelectorAll('[data-control^="dialog.background."]')].map((e) => e.getAttribute('data-control')).join(','))}`,
        };
      chosenSwatch = pick;
      ...
      return {
        ok: Boolean(slide.background) && after.bg !== null && after.bg === pick.bg,
        evidence: `swatches ${swatches.length}; picked ${pick.control} (${pick.bg}); preview while open ${preview.bg}; slide.background ${JSON.stringify(slide.background ?? null)}; .slide-bg after ${after.bg} (${after.bgStyle})`,
      };
    },
    { reset: () => closeMenus(page) },
  );
  ...
  await attempt(
    'Slide background',
    'the toolbar Background button opens the same dialog',
    async () => {
      await clearAll(page);
  ...
  await attempt(
    'Slide background',
    'Change background > Choose image > By URL, an https PNG: a picture object covers the slide at the bottom of the stack',
    async () => {
      await clearAll(page);
      const before = await ids();
      await openMenu(page, 'slide');
      ...
      return {
        ...
        evidence: `${await describeBlock(obj)}; on the sheet ${facts.picture}; img ${facts.pictureImg}; stack order ${order.join(' < ')}; dialogs open after ${dialogs || 'none'}`,
      };
    },
    { reset: () => closeMenus(page) },
  );
  ...
  await attempt(
    ...
    async () => {
      ...
      return {
        ...
        evidence: `dither ${JSON.stringify(before?.block.dither ?? null)} -> ${JSON.stringify(after?.block.dither ?? null).slice(0, 100)}; canvas on the sheet ${facts.dither}`,
      };
    },
    { reset: () => closeMenus(page) },
  );
  ...
  await attempt(
    ...
    async () => {
      ...
      return {
        ...
        evidence: `picture block after ${gone ? 'still there' : 'gone'}; on the sheet ${facts.picture ?? 'none'}; colour still ${facts.bg}`,
      };
    },
    { reset: () => closeMenus(page) },
  );
  ...
  await attempt(
    'Slide background',
    'Change background > Reset to theme, Done: the colour leaves and the theme ground returns',
    async () => {
      await clearAll(page);
      await openMenu(page, 'slide');
      await clickRow(page, 'slide.changeBackground');
      ...
      const facts = await bgFacts();
      return {
        ok: slide.background === undefined && facts.bg === null,
        evidence: `slide.background ${JSON.stringify(slide.background ?? null)}; .slide-bg ${facts.bg ?? 'none'}`,
      };
    },
    { reset: () => closeMenus(page) },
  );
  ...
  await attempt(
    'Slide background',
    'Change background > Choose image > Upload from computer through the file chooser, then Remove picture',
    async () => {
      await clearAll(page);
      const before = await ids();
      await openMenu(page, 'slide');
      ...
      await closeMenus(page);
      return {
        ok: Boolean(obj) && obj.type === 'picture' && removed === 'removed',
        evidence: `${await describeBlock(obj)}; on the sheet ${facts.picture}; then ${removed}`,
      };
    },
    { reset: () => closeMenus(page) },
  );
  ...
  await attempt(
    'Slide background',
    'colour again for the present and PDF checks (Change background, the same swatch, Done)',
    async () => {
      await clearAll(page);
      await openMenu(page, 'slide');
      await clickRow(page, 'slide.changeBackground');
      ...
      return {
        ...
        evidence: `slide.background ${JSON.stringify(slide.background ?? null)}`,
      };
    },
    { reset: () => closeMenus(page) },
  );
    ...
    async () => {
      ...
      const facts = await pollUntil(
        () =>
          page.evaluate((slideId) => {
            ...
            const slide =
              root.querySelector(`[data-slide="${slideId}"]`) ??
              root.querySelector('.pt-slide:not(.is-leaving)');
            const imgs = slide
              ? [...slide.querySelectorAll('img')].map(
                  (i) => `${i.naturalWidth}x${i.naturalHeight}/${i.complete}`,
                )
              : [];
      ...
      return {
        ok:
          ...
          facts.bg !== null &&
          off === false,
        evidence: `present ${facts.present}; show ${facts.show} at ${facts.url}; slide ${facts.size}; imgs ${facts.imgs.join(',') || 'none'}; background ${facts.bg}; after Escape present view ${off}; shot ${file ? path.basename(file) : 'none'}`,
      };
    },
    { reset: () => clearAll(page) },
  );
  ...
  await attempt(
    'PDF export',
    'File > Download > PDF Document, Download: a PDF with one page per slide that carries the picture',
    async () => {
      await clearAll(page);
      ...
      const ready = await page
        .locator('[data-control="dialog.download.ready"]')
        .waitFor({ timeout: 120_000 })
        .then(() => true)
        .catch(() => false);
      const readyText = await page.evaluate(
        () =>
          document.querySelector('[data-control="dialog.download.ready"]')?.textContent ??
          document.querySelector('[data-control="dialog.download.progress"]')?.textContent ??
          null,
      );
      const dl = await dlP;
      let file = null;
      ...
      if (await has(page, '[data-control="dialog.download.done"]'))
        await clickControl(page, 'dialog.download.done');
      await closeMenus(page);
      return {
        ok:
          Boolean(file) && file.head.startsWith('%PDF') && file.pages === slides && file.images > 0,
        evidence: `${Date.now() - t0} ms; ready ${ready} ("${(readyText ?? '').slice(0, 80)}"); download ${file ? `${file.name} ${file.bytes} bytes head ${file.head.trim()} pages ${file.pages} of ${slides} slides, image objects ${file.images}` : 'none'}`,
        extra: file ? { pdf: file.saved } : {},
      };
    },
    { reset: () => closeMenus(page) },
  );
