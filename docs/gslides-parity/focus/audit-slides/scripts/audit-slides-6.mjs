...
try {
  ...
  for (let trial = 1; trial <= 3; trial += 1) {
    await row(
      'Delete slide',
      `Trial ${trial}: in the grid, Shift click two tiles, press Delete, click Undo on the snackbar; then reload and compare the server's order`,
      async () => {
        ...
        if (!ra || !rb) {
          ...
          return {
            ok: false,
            evidence: `tiles missing: ${a} ${Boolean(ra)}, ${b} ${Boolean(rb)}; grid ${JSON.stringify(await gridFacts(page))}`,
          };
        }
  ...
  for (let trial = 1; trial <= 3; trial += 1) {
    await row(
      'Delete slide',
      `Trial ${trial}: in the filmstrip, Shift click two cards, press Delete, click Undo on the snackbar; then reload and compare the server's order`,
      async () => {
        const before = await topUp();
        const a = before[before.length - 2];
        const b = before[before.length - 1];
        ...
        return {
          ok:
            cs.filter((c) => c.selected).length === 2 &&
            mid.length === before.length - 2 &&
            after.join(',') === before.join(',') &&
            server.join(',') === before.join(',') &&
            ref.dialogs.length === 0,
  ...
  for (let trial = 1; trial <= 3; trial += 1) {
    await row(
      'Delete slide',
      `Trial ${trial}: two cards selected, Slide menu > Delete slide, then Cmd+Z; then reload and compare the server's order`,
      async () => {
        const before = await topUp();
        const a = before[before.length - 2];
        const b = before[before.length - 1];
        ...
        return {
          ...
          evidence: `selected ${cs
            ...
            .join(
              ',',
            )}; ${before.length} -> Delete ${mid.length} (snackbar ${JSON.stringify(sb)}; sync ${JSON.stringify(syncMid)}) -> Cmd+Z ${after.length} (restored in the tab ${after.join(',') === before.join(',')}; sync ${JSON.stringify(syncAfter)}; save "${ref.save}"; dialogs ${ref.dialogs.length ? ref.dialogs.join(' || ') : 'none'}); server after reload ${server.length}: ${server.join(',')} (matches ${server.join(',') === before.join(',')})${dismissed ? `; dismissed ${dismissed}` : ''}`,
          shot: sh,
        };
      },
      { tries: 1 },
  ...
  for (let trial = 1; trial <= 3; trial += 1) {
    await row(
      'Delete slide',
      `Trial ${trial}: in the grid, Shift click two tiles and press Delete, no undo; reload and compare the server's order`,
      async () => {
        const before = await topUp();
        const a = before[before.length - 2];
        const b = before[before.length - 1];
        ...
        if (!ra || !rb) {
          await clickControl(page, 'view.filmstripView');
          return { ok: false, evidence: `tiles missing` };
        }
        await clickAt(page, ra.x + ra.w / 2, ra.y + ra.h / 2);
        await sleep(300);
        await clickAt(page, rb.x + rb.w / 2, rb.y + rb.h / 2, { modifiers: ['Shift'] });
