...
const shot = (n) =>
  page.screenshot({ path: path.join(SCRATCH, `destroy-${n}.png`) }).catch(() => undefined);
try {
  ...
  const banner = await page.evaluate(
    () => document.querySelector('.ts-shell-banner')?.textContent ?? null,
  );
  ...
  const rows = await page.evaluate(() =>
    [...document.querySelectorAll('#ts-menu-file [data-control]')].map(
      (e) =>
        e.getAttribute('data-control') + (e.classList.contains('is-disabled') ? ' (disabled)' : ''),
    ),
  );
  ...
  if (rows.some((r) => r.startsWith('menu.file.moveToTrash'))) {
    ...
    await page
      .waitForURL(/\/decks$/, { timeout: 20_000 })
      .catch((e) => console.log('no /decks after trash:', e.message.split('\n')[0]));
    ...
    const t = await invoke(page, 'deck.trash', { id, baseRevision: info.revision }).catch((e) => ({
  ...
  const cards = await page.evaluate(() =>
    [...document.querySelectorAll('[data-control^="trash.card."]')].map((e) =>
  ...
  console.log(
    ...
    status,
    'GET /deck status',
    (await page.request.get(`${BASE}/deck/${id}`, { maxRedirects: 0 })).status(),
  );
} catch (e) {
