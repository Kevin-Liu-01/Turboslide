...
Base http://localhost:4418, started 2026-09-22T15:47:30.661Z, 5831 s. 785 rows judged: 741 passed, 28 failed, 16 not driven (1 of them manual, the checklist's: text.clipboard.paste-without-formatting), 0 no step. Measurement rows (PRODUCT.md 8.2, recorded and never holding the ship; the cost rows of SYNC.md 6.1 among them, which hold it over their ceiling on the preview): export.download.large-deck-pdf passed (PDF: GT brand deck.pdf, 85 slides in 3.1 s, 0.04 s per slide; PDF: GT brand deck.pdf, 85 slides in 3.1 s, 0.04 s per slide); export.download.large-deck-pptx passed (Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 29.3 s, 0.34 s per slide; Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 29.3 s, 0.34 s per slide); cost.editor-idle.calls passed (function requests 10 a minute (ceiling 12; 30 in 3 min); the store half reads zero on this base (the file store); the function requests alone are asserted); cost.editor-hidden.calls not driven (function requests 9.66 a minute and 9 session poll(s) were read from the tab that stayed visible); cost.editor-editing.calls passed (function requests 71.04 a minute (ceiling 75; 214 in 3.01 min); the store half reads zero on this base (the file store); the function requests alone are asserted); cost.two-tabs-idle.calls passed (the store half reads zero on this base (the file store); the function requests alone are asserted); cost.show.calls passed (function requests 0 in the window (ceiling 0); the store half reads zero on this base (the file store); the function requests alone are asserted). Cost rows over their ceiling in this run: none. Verdict failed; retries 0 configured, 0 test(s) retried; exit 1. A not driven row is never counted as passed. Features a ship on this run would park (rule 4 of section 1; RETURN.md rule 2): tables, charts, inbox, brand, fonts, templates, assist, logos; rows whose own controls a ship would keep parked: view.live-pointers.second-browser (view.livePointers.mine, view.livePointers.collaborators); versions.show-changes-marks (file.versionHistory.showChanges); templates.gallery.page (home.gallery, file.new.templateGallery); assist.tailor.dialog-one-undo (tools.tailor); tables.seam.row-drag (handle.table.row); tables.edge.add-row-column (handle.table.add.column, handle.table.add.row); tables.heads.select-row-column (handle.table.head.column, handle.table.head.row); tables.bar.row-column-buttons (bar.table); arrange.group.tail-text-controls (toolbar.group.text); wordart.tail.fill-outline (toolbar.wordart.outline); logos.picker.paper-and-ink (insert.logo); logos.picker.empty-state (dialog.logo.upload); logos.picker.recents (dialog.logo.group.recent); logos.picker.licence-words (dialog.logo.source); logos.picker.chrome-1280 (insert.logo); logos.insert.one-click-asset (insert.logo); logos.insert.logo-size (insert.logo); logos.insert.mono-tint (insert.logo); logos.insert.every-slide (dialog.logo.everySlide); logos.index.cached-offline (insert.logo); logos.picker.variants (dialog.logo.kind.wordmark, dialog.logo.tone.mono); logos.kit.find-a-logo (panel.brand.logo.find); rows of an unparkable feature blocking the ship: slides.delete.two-selected-key-undo, sync.structural.concurrent, sync.block.offline-replay-converges, sync.resend.idempotent, logos.export.pdf-pptx-crisp, logos.intake.svg-sentence, logos.intake.url-sentence.
...
| `versions.show-changes-marks` | versions | probe --core | not driven | failed | a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.442.pick (named row versionHistory.442.pick); marks with Show changes on 0 (); off 0 |
...
| `assist.tailor.dialog-one-undo` | assist | probe --core | not driven | failed | Tools > Tailor for a customer: Acme to Globex, skip the pricing slide, Apply; Cmd+Z: count "7 places on 5 slides"; skip box true; Acme 7 -> 1 -> 7; Globex 0 -> 6 -> 0; pricing skipped false -> true -> false; revision 526 -> 527 -> 527; snackbar "Tailored for GlobexUndo" with Undo true |
| `assist.tailor.agent-deck-tailor` | assist | probe --core | not driven | failed | deck.tailor over HTTP with the bearer: one replacement, one skipped slide: HTTP 200 {"replacements":0,"slideIds":[],"pictures":0,"skipped":[],"revision":527}; revision 527 -> 528; Acme 7 -> 7 -> 7 (put back through the window API, the snackbar carried no Undo); pricing skipped false; snackbar "none" |
...
| `sync.block.offline-replay-converges` | sync | core/sync.spec.ts | works | failed | Error: every scratch deck is trashed, deleted forever and answers 404 |
...
| `charts.panel.no-duplicate-controls` | charts | probe --core | broken | failed | Format options with the chart selected; count the controls; Tools > Advanced tools on and read again: switch off: chart type groups 1 (kind controls 0), Title 2, Legend 1, textareas 1; switch on (true): JSON view elements 4 (FEATURES.md 2.2 rank 11, B3) |
...
| `fonts.catalog.six-families` | fonts | probe --core | not driven | failed | read the six rows and their groups; Fraunces on the title heading: instrument-sans in sans, manrope in sans, schibsted-grotesk in sans, bricolage-grotesque in sans, fraunces in serif, newsreader in serif; Fraunces on the heading: picked true, drawn Fraunces, Georgia, "Times New Roman", serif, loaded |
...
| `logos.picker.paper-and-ink` | logos | probe --core | not driven | failed | search vercel; read the tile's paper and ink halves; Slide > Change theme light; insert; read the mark: ink half lit 15.44 percent on #070707; paper half lit 11.37 percent, white 0.00 percent on #070707; on the light deck ({"theme":"light","deck":"light","state":"light","tries":1}) the inserted mark |
| `logos.picker.your-brand` | logos | probe --core | not driven | passed |  |
| `logos.picker.empty-state` | logos | probe --core | not driven | failed | search zzqx; Upload; then General Translation: error: locator.boundingBox: Timeout 30000ms exceeded. |
| `logos.picker.recents` | logos | core/logos.spec.ts | not driven | failed | Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoBe[2m([22m[32mexpected[39m[2m) // Object.is equality[22m |
| `logos.picker.licence-words` | logos | probe --core | not driven | failed | read the foot; search figma and read the licence row and its tooltip; the dialog's text; search aws; Include cloud service icons: error: locator.boundingBox: Timeout 30000ms exceeded. |
| `logos.picker.chrome-1280` | logos | core/chrome.spec.ts | not driven | failed | Error: five tiles wide at 1280 by 800 |
| `logos.insert.one-click-asset` | logos | probe --core | not driven | failed | search figma; one click on the tile; read the asset record, the img and the source document: shot logo-7 asset figma-3: no record in the source document; img src /decks/untitled-20260922-juzq/assets/figma-3.ff9b40d2.png (same origin true); thesvg.org in the document false |
| `logos.insert.logo-size` | logos | probe --core | not driven | failed | read the inserted Figma mark against the body slot, its chip and the text runs: 934,598 72x107 (height under 160 true); body slot {"x":80,"y":218,"w":1440,"h":553}, centred false; move handle true; resize handles 8; rotate true; ring true; chip "Logo"; session false; caret false; text runs under it  |
| `logos.insert.mono-tint` | logos | probe --core | not driven | failed | on the dark deck insert the GitHub mark; read its variant and the fills of its source file; then AWS with the cloud switch on: deck appearance dark; GitHub asset no record; the source file's fills unread against the kit's text colour #f2f2f0 (tinted false); the twin lit 1.51 percent; AWS no AWS tile |
...
| `logos.export.pdf-pptx-crisp` | export | core/export.spec.ts | not driven | failed | [31mTest timeout of 300000ms exceeded.[39m |
| `logos.agent.search-insert` | logos | core/logos.spec.ts | not driven | failed | Error: the CLI lists the row |
...
| `logos.intake.svg-sentence` | images | core/images.spec.ts | broken | failed | Error: the chooser lists the four raster types |
| `logos.intake.url-sentence` | images | probe --core | broken | failed | asset.add { url: https://thesvg.org/... } on the window API; then the allowlisted Wikimedia PNG: thesvg.org by URL: "page.evaluate: Error: Pictures can be fetched from these sites only: generaltranslation.com, prototemplate.com, glyphfield.com and Wikimedia Commons. Upload the file instead" (sites n |
...
- `versions.show-changes-marks`: a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.442.pick (named row versionHistory.442.pick); marks with Show changes on 0 (); off 0
...
- `assist.tailor.dialog-one-undo`: Tools > Tailor for a customer: Acme to Globex, skip the pricing slide, Apply; Cmd+Z: count "7 places on 5 slides"; skip box true; Acme 7 -> 1 -> 7; Globex 0 -> 6 -> 0; pricing skipped false -> true -> false; revision 526 -> 527 -> 527; snackbar "Tailored for GlobexUndo" with Undo true
- `assist.tailor.agent-deck-tailor`: deck.tailor over HTTP with the bearer: one replacement, one skipped slide: HTTP 200 {"replacements":0,"slideIds":[],"pictures":0,"skipped":[],"revision":527}; revision 527 -> 528; Acme 7 -> 7 -> 7 (put back through the window API, the snackbar carried no Undo); pricing skipped false; snackbar "none"
- `sync.structural.concurrent`: Error: the loser's reject card is shown
- `sync.block.offline-replay-converges`: Error: every scratch deck is trashed, deleted forever and answers 404
- `tables.range.size-color`: a range over the header row; the size step up; a text colour from the plate; Align right from the tail: range "0,0:0,3" over 4 cells; size 20,20,20,20 -> 20,20,20,20; colour by toolbar.textColor.green: rgb(242, 242, 240) rgb(242, 242, 240) rgb(242, 242, 240) rgb(242, 242, 240) -> rgb(242, 242, 240) 
- `charts.panel.no-duplicate-controls`: Format options with the chart selected; count the controls; Tools > Advanced tools on and read again: switch off: chart type groups 1 (kind controls 0), Title 2, Legend 1, textareas 1; switch on (true): JSON view elements 4 (FEATURES.md 2.2 rank 11, B3)
- `brand.objects.kit-colours-first`: the table cell's Fill color plate, the chart's series swatches, the text colour plate; a new chart's series: table fill plate: toolbar.fillColor.kit.text, toolbar.fillColor.kit.background, toolbar.fillColor.kit.caption, toolbar.fillColor.kit.hint, toolbar.fillColor.kit.primary, toolbar.fillColor.kit
...
- `fonts.catalog.six-families`: read the six rows and their groups; Fraunces on the title heading: instrument-sans in sans, manrope in sans, schibsted-grotesk in sans, bricolage-grotesque in sans, fraunces in serif, newsreader in serif; Fraunces on the heading: picked true, drawn Fraunces, Georgia, "Times New Roman", serif, loaded
- `logos.picker.paper-and-ink`: search vercel; read the tile's paper and ink halves; Slide > Change theme light; insert; read the mark: ink half lit 15.44 percent on #070707; paper half lit 11.37 percent, white 0.00 percent on #070707; on the light deck ({"theme":"light","deck":"light","state":"light","tries":1}) the inserted mark
- `logos.picker.empty-state`: search zzqx; Upload; then General Translation: error: locator.boundingBox: Timeout 30000ms exceeded.
- `logos.picker.recents`: Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoBe[2m([22m[32mexpected[39m[2m) // Object.is equality[22m
- `logos.picker.licence-words`: read the foot; search figma and read the licence row and its tooltip; the dialog's text; search aws; Include cloud service icons: error: locator.boundingBox: Timeout 30000ms exceeded.
- `logos.picker.chrome-1280`: Error: five tiles wide at 1280 by 800
- `logos.insert.one-click-asset`: search figma; one click on the tile; read the asset record, the img and the source document: shot logo-7 asset figma-3: no record in the source document; img src /decks/untitled-20260922-juzq/assets/figma-3.ff9b40d2.png (same origin true); thesvg.org in the document false
- `logos.insert.logo-size`: read the inserted Figma mark against the body slot, its chip and the text runs: 934,598 72x107 (height under 160 true); body slot {"x":80,"y":218,"w":1440,"h":553}, centred false; move handle true; resize handles 8; rotate true; ring true; chip "Logo"; session false; caret false; text runs under it 
- `logos.insert.mono-tint`: on the dark deck insert the GitHub mark; read its variant and the fills of its source file; then AWS with the cloud switch on: deck appearance dark; GitHub asset no record; the source file's fills unread against the kit's text colour #f2f2f0 (tinted false); the twin lit 1.51 percent; AWS no AWS tile
- `logos.insert.every-slide`: read the foot check and its sentence; check it; insert Figma; read the title slide and a new slide; the snackbar; Cmd+Z; open the dialog again: check off by default true: "Use as this presentation’s logo on every slide" line "Replaces the brand kit’s logo on the title slide and in every footer"; che
- `logos.export.pdf-pptx-crisp`: [31mTest timeout of 300000ms exceeded.[39m
- `logos.agent.search-insert`: Error: the CLI lists the row
- `logos.intake.svg-sentence`: Error: the chooser lists the four raster types
- `logos.intake.url-sentence`: asset.add { url: https://thesvg.org/... } on the window API; then the allowlisted Wikimedia PNG: thesvg.org by URL: "page.evaluate: Error: Pictures can be fetched from these sites only: generaltranslation.com, prototemplate.com, glyphfield.com and Wikimedia Commons. Upload the file instead" (sites n

## Measurement rows, by id (PRODUCT.md 8.2; the cost rows of SYNC.md 6.1)

- `export.download.large-deck-pdf`: passed; recorded PDF: GT brand deck.pdf, 85 slides in 3.1 s, 0.04 s per slide; PDF: GT brand deck.pdf, 85 slides in 3.1 s, 0.04 s per slide
- `export.download.large-deck-pptx`: passed; recorded Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 29.3 s, 0.34 s per slide; Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 29.3 s, 0.34 s per slide
...
- `cost.editor-editing.calls`: passed; recorded function requests 71.04 a minute (ceiling 75; 214 in 3.01 min); the store half reads zero on this base (the file store); the function requests alone are asserted (a cost row: over its ceiling on the preview it holds the ship)
