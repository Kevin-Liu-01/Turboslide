...
| `decks.save.acknowledged` | decks | probe --core | flaky | failed | every window API write of the walk is acknowledged within 60 s: 1 unanswered call(s): slide.remove at 2026-09-25T04:18:24.285Z on untitled-20260925-3hv6 in slides (no step); 1 fresh deck(s) made for the areas after |
...
| `versions.show-changes-marks` | versions | probe --core | not driven | failed | a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.356.pick (named row versionHistory.356.pick); marks with Show changes on 0 (); off 0 |
...
| `slides.import.none-preselected` | slides | probe --core | broken | failed | File > Import slides, the General Translation brand deck; click three; Shift click a range; None, three, Import: source the General Translation brand deck; list settled after 2123 ms; 85 tiles; preselected 0; after three clicks 3, button "Import 3 slides"; after the Shift click 8 (8 expected); after |
...
| `sync.reload.same-document` | sync | core/sync.spec.ts | works | failed | Error: each reload within 5 s (15183, 15162, 15184 ms) |
...
| `cost.editor-editing.calls` | cost | cost-probe | broken | failed | function requests 84.98 a minute over 75 |
...
- `decks.save.acknowledged`: every window API write of the walk is acknowledged within 60 s: 1 unanswered call(s): slide.remove at 2026-09-25T04:18:24.285Z on untitled-20260925-3hv6 in slides (no step); 1 fresh deck(s) made for the areas after
...
- `versions.show-changes-marks`: a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.356.pick (named row versionHistory.356.pick); marks with Show changes on 0 (); off 0
...
- `slides.import.none-preselected`: File > Import slides, the General Translation brand deck; click three; Shift click a range; None, three, Import: source the General Translation brand deck; list settled after 2123 ms; 85 tiles; preselected 0; after three clicks 3, button "Import 3 slides"; after the Shift click 8 (8 expected); after
...
- `sync.reload.same-document`: Error: each reload within 5 s (15183, 15162, 15184 ms)
- `cost.editor-editing.calls`: function requests 84.98 a minute over 75
...
- `export.download.large-deck-pptx`: passed; recorded Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 32.9 s, 0.39 s per slide; Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 32.9 s, 0.39 s per slide
- `cost.editor-idle.calls`: passed; recorded function requests 9.66 a minute (ceiling 12; 29 in 3 min); the store half reads zero on this base (the file store); the function requests alone are asserted (a cost row: over its ceiling on the preview it holds the ship)
...
- `cost.editor-editing.calls`: failed (function requests 84.98 a minute over 75); recorded function requests 84.98 a minute (ceiling 75; 255 in 3 min); the store half reads zero on this base (the file store); the function requests alone are asserted (a cost row: over its ceiling on the preview it holds the ship)
