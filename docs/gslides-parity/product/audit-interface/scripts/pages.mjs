...
for (const size of SIZES) {
  for (const theme of THEMES) {
    ...
    const P = async (surface, what, sel) =>
    ...
    await step(table, 'home', `${tag} appearance group`, async () => {
      ...
      return btns.map((b) => ({
        text: b.text,
        pressed: b.pressed,
        box: `${Math.round(b.box.w)}x${Math.round(b.box.h)}`,
        border: b.borderHex,
    ...
    await step(table, 'home', `${tag} tooltip on a nav button (delay)`, async () => {
      ...
      const name = shown
        ? await page.evaluate(
            () => document.querySelector('#pt-tip .pt-tip-name')?.textContent ?? null,
          )
        : null;
      const doc = shown
        ? await page.evaluate(
            () => document.querySelector('#pt-tip .pt-tip-doc')?.textContent ?? null,
          )
        : null;
      const key = shown
        ? await page.evaluate(
            () => document.querySelector('#pt-tip .pt-tip-key')?.textContent ?? null,
          )
        : null;
      if (shown)
        await S('home-tooltip', pad({ x: r.x, y: r.y, w: r.w, h: r.h + 60 }, 40, size.w, size.h));
      return {
        shown,
        afterMs: at,
        name,
    ...
    await step(table, 'home', `${tag} keyboard focus ring`, async () => {
      ...
      return f
        ? {
            ...
            outline: f.outline,
            outlineContrast: f.outlineContrast,
          }
        : 'no active element';
    });
    await step(table, 'home', `${tag} type ladder and titanium text`, async () => {
      ...
      for (const [k, sel] of Object.entries({
        ...
        body: '.ts-product-card-copy',
        small: '.ts-product-number-line',
        credit: '.ts-product-credit',
        figure: '.ts-product-figure',
      })) {
    ...
    await step(table, 'home', `${tag} page scroll and band rhythm`, async () => {
      const facts = await page.evaluate(() => {
        const bands = [...document.querySelectorAll('.ts-product-band')].map((b) => {
        ...
        return {
          ...
          bands: [...new Set(bands)],
          nav: getComputedStyle(document.querySelector('.ts-product-nav')).height,
        };
      });
    ...
    await step(table, 'decks', `${tag} load (the frames while the list streams)`, async () => {
      ...
      const pending = await page.evaluate(() => ({
        frames: document.querySelectorAll('.ts-hm-card-frame').length,
        hydrated: document.querySelector('.ts-home-page')?.hasAttribute('data-hydrated') ?? false,
        pendingBand: document.querySelector('.ts-recent[data-pending]') !== null,
      }));
      ...
      await page
        .waitForFunction(
          () =>
            document.querySelectorAll('.ts-hm-card:not(.ts-hm-card-frame)').length > 0 ||
            document.querySelector('.ts-home-empty-figure'),
          null,
          { timeout: 30_000 },
        )
        .catch(() => undefined);
      ...
      const cards = await page.evaluate(
        () => document.querySelectorAll('.ts-hm-card:not(.ts-hm-card-frame)').length,
      );
    ...
    await step(table, 'decks', `${tag} card hover and the more button`, async () => {
      ...
      return {
        cardBorder: card?.borderHex,
        cardBorderContrast: card?.borderContrast,
        moreOpacity: more?.opacity,
        moreVisible: more ? more.box.w > 0 : null,
        cursor: card?.cursor,
      };
    });
      ...
      return {
        menu: menu
          ? {
              w: menu.box.w,
              radius: menu.radius,
              border: menu.borderHex,
              borderContrast: menu.borderContrast,
              font: menu.font,
            }
          : null,
        rows: rows.map((x) => ({
    ...
    await step(table, 'decks', `${tag} search with no match (empty state)`, async () => {
      ...
      return {
        ...
        sentenceColor: sentence?.colorHex,
        sentenceContrast: sentence?.textContrast,
        action: action?.text,
      };
    });
    await step(table, 'decks', `${tag} list view`, async () => {
      ...
      await sleep(300);
      return {
        row: row ? { h: row.box.h, font: row.font } : 'no row',
        head: head
          ? { text: head.text, font: head.font, color: head.colorHex, contrast: head.textContrast }
          : null,
      };
    });
    ...
    await step(table, 'trash', `${tag} load`, async () => {
      ...
      return await page.evaluate(() => ({
        cards: document.querySelectorAll('.ts-trash-card').length,
        empty: document.querySelector('[data-control="trash.empty-state"]') !== null,
        headText: document.querySelector('.ts-trash-head')?.textContent?.trim().slice(0, 200),
      }));
    });
    ...
    await step(table, 'trash', `${tag} the Empty trash confirm dialog`, async () => {
      ...
      const disabled = await page.evaluate(
        () => document.querySelector('[data-control="trash.empty"]')?.disabled ?? false,
      );
      ...
      const body = await page.evaluate(() =>
        document.querySelector('[role="dialog"], .ts-dialog')?.textContent?.trim().slice(0, 300),
      );
      await S('trash-confirm');
      await press(page, 'Escape');
      await sleep(300);
      const gone = !(await page.evaluate(
        () => document.querySelector('[data-control="trash.confirm.ok"]') !== null,
      ));
      if (!gone) {
        const c = await rectOf(page, '[data-control="trash.confirm.cancel"]');
        if (c) await clickAt(page, c.x + c.w / 2, c.y + c.h / 2);
      }
      return {
        dialog: dialog
          ? { w: dialog.box.w, border: dialog.borderHex, radius: dialog.radius }
          : null,
        title: title?.text,
        titleFont: title?.font,
        body,
        ok: ok
          ? { text: ok.text, solid: /is-solid/.test(ok.cls), radius: ok.radius, h: ok.height }
          : null,
        cancel: cancel ? { text: cancel.text, radius: cancel.radius } : null,
        escapeCloses: gone,
      };
    });
    ...
    await step(table, 'new', `${tag} the skeleton`, async () => {
      ...
      for (let i = 0; i < 60; i += 1) {
        skeleton = await page
          .evaluate(() => {
            const s = document.querySelector('.ts-skeleton, [data-skeleton], .ts-editor-skeleton');
            return s ? { cls: s.className, rows: s.querySelectorAll('*').length } : null;
          })
          .catch(() => null);
        if (
          skeleton ||
          (await page.evaluate(() => Boolean(window.turboslide?.studio)).catch(() => false))
        )
          break;
        await sleep(30);
      }
      ...
      return {
        skeleton,
        skeletonAtMs: atMs,
        editorReadyMs: readyMs,
        address: page.url().replace(BASE, ''),
      };
    });
    await step(table, 'new', `${tag} the placeholder prompts and the save words`, async () => {
      const prompts = await page.evaluate(() =>
        [...document.querySelectorAll('.ts-stagewrap.ts-editor .pt-slide [data-prompt]')].map(
          (p) => p.getAttribute('data-prompt') ?? p.textContent,
        ),
      );
      ...
      return {
        prompts,
        save: save
          ? { text: save.text, font: save.font, color: save.colorHex, contrast: save.textContrast }
          : null,
        name: name ? { text: name.text, font: name.font } : null,
        promptColor: prompt
          ? { color: prompt.colorHex, contrast: prompt.textContrast, font: prompt.font }
          : null,
      };
    });
    ...
    await step(table, 'notfound', `${tag} /no-such-page`, async () => {
      const res = await page.goto(`${BASE}/no-such-page-${Date.now()}`, {
        waitUntil: 'domcontentloaded',
      });
      await page
        .waitForSelector('[data-control="notfound"], .ts-access, .ts-empty', { timeout: 30_000 })
        .catch(() => undefined);
      await sleep(500);
      await S('notfound');
      ...
      return {
        ...
        fig: fig
          ? `${fig.box.w}x${fig.box.h} at ${Math.round(fig.box.x)},${Math.round(fig.box.y)}`
          : null,
        bodyBg: body?.background,
      };
    });
    await step(table, 'access', `${tag} /deck/<missing> (You need access)`, async () => {
      const res = await page.goto(`${BASE}/deck/no-such-deck-${Date.now()}`, {
        waitUntil: 'domcontentloaded',
      });
      await page
        .waitForSelector('.ts-access, [data-control="notfound"]', { timeout: 30_000 })
        .catch(() => undefined);
      await sleep(500);
      await S('access');
      ...
      const controls = await page.evaluate(() =>
        [...document.querySelectorAll('.ts-access [data-control]')].map((e) =>
      ...
      return {
        ...
        field: field
          ? {
              h: field.height,
              border: field.borderHex,
              borderContrast: field.borderContrast,
              radius: field.radius,
              font: field.font,
            }
          : null,
        label: label
          ...
          : null,
        button: btn
          ? { text: btn.text, solid: /is-solid/.test(btn.cls), h: btn.height, radius: btn.radius }
          : null,
        signin: signin?.text,
        card: card ? { w: card.box.w, top: card.box.y, pad: card.padding } : null,
        controls,
      };
    });
    await step(table, 'access', `${tag} 1280 body scroll`, async () =>
      page.evaluate(() => ({
