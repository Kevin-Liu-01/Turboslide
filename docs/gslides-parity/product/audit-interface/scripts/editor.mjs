...
const loadState = () =>
  existsSync(STATE_FILE) ? JSON.parse(readFileSync(STATE_FILE, 'utf8')) : null;
...
const summarize = (f) =>
  f
    ? {
        text: f.text,
        box: `${Math.round(f.box.w)}x${Math.round(f.box.h)} at ${Math.round(f.box.x)},${Math.round(f.box.y)}`,
        h: f.height,
        pad: f.padding,
        radius: f.radius,
        font: f.font,
        color: f.colorHex,
        ground: f.groundHex,
        textContrast: f.textContrast,
        border: f.border,
        borderHex: f.borderHex,
        borderContrast: f.borderContrast,
        outline: f.outline,
...
const toolbarFacts = (page) =>
  page.evaluate(() => {
    ...
    const ctrls = [...bar.querySelectorAll('[data-control]')]
      .filter(vis)
      .filter((el) => el.querySelector('[data-control]') === null)
      .map((el) => {
        ...
        return {
          ...
          svg: s ? Math.round(s.getBoundingClientRect().width) : null,
        };
      });
    ...
    return {
      ...
      clipped: sorted.filter((c) => c.right > r.right + 0.5).map((c) => c.id),
      more: document.querySelector('[data-control="toolbar.more"]') !== null,
      seps: [...bar.querySelectorAll('.ts-tb-sep')].map((s) => {
        const b = s.getBoundingClientRect();
        return { x: Math.round(b.x), h: Math.round(b.height) };
      }),
      gaps: [...new Set(gaps)].sort((a, b) => a - b),
      heights: [...new Set(sorted.map((c) => c.h))],
      widths: sorted.map((c) => `${c.id.replace('toolbar.', '')}:${c.w}`),
      count: sorted.length,
      ctrls: sorted,
    };
  });
...
const menuFacts = (page, id) =>
  page.evaluate((menuId) => {
    ...
    const items = [...root.querySelectorAll('.ts-menu-item')].filter(
    ...
    return {
      ...
      overflowsViewport: r.bottom > window.innerHeight,
      scrolls: root.scrollHeight > root.clientHeight + 1,
      rows: items.length,
      disabled: items.filter(
        (i) => i.classList.contains('is-disabled') || i.getAttribute('aria-disabled') === 'true',
      ).length,
      withIcon: items.filter((i) => i.querySelector('.ts-menu-ic svg')).length,
      withKey: items.filter((i) => i.querySelector('.ts-menu-key')?.textContent?.trim()).length,
      submenus: items.filter((i) => i.querySelector('.ts-menu-sub')).length,
      dividers: root.querySelectorAll('.ts-menu-divider').length,
      rowH: [...new Set(items.map((i) => Math.round(i.getBoundingClientRect().height)))],
      labels: items
        .slice(0, 40)
        .map(
          (i) =>
            i.querySelector('.ts-menu-label')?.textContent?.trim() ??
            i.textContent.trim().slice(0, 30),
        ),
    };
  }, id);
...
const tooltipOn = async (page, control) => {
  ...
  for (let i = 0; i < 50; i += 1) {
    ...
    shown = await page.evaluate(() => {
      const t = document.getElementById('pt-tip');
      return t !== null && !t.hidden && t.getClientRects().length > 0;
    });
    if (shown) {
      at = Date.now() - t0;
      break;
  ...
  const f = await page.evaluate(() => {
    ...
    return {
      name: t.querySelector('.pt-tip-name')?.textContent ?? null,
      key: t.querySelector('.pt-tip-key')?.textContent ?? null,
      doc: t.querySelector('.pt-tip-doc')?.textContent ?? null,
      box: { x: r.x, y: r.y, w: r.width, h: r.height },
      keyRadius: t.querySelector('.pt-tip-key')
        ? getComputedStyle(t.querySelector('.pt-tip-key')).borderRadius
        : null,
    };
  });
  ...
  return {
    ...
    afterMs: at,
    ...f,
    gapFromControl: Math.round(f.box.y - (r.y + r.h)),
    font: tip.font,
    border: tip.borderHex,
    borderContrast: tip.borderContrast,
    docColor: (await probe(page, '#pt-tip .pt-tip-doc'))?.colorHex,
    docContrast: (await probe(page, '#pt-tip .pt-tip-doc'))?.textContrast,
  };
};
const dialogFacts = async (page, tag, name, open, S) => {
  ...
  const actions = await probeAll(
    page,
    `${sel} .ts-dialog-actions .pt-ib, ${sel} .ts-dialog-actions button`,
    6,
  );
  const fields = await page.evaluate((s) => {
    const root = document.querySelector(s);
    return {
      inputs: root.querySelectorAll('input, select, textarea').length,
      labels: root.querySelectorAll('.ts-dialog-field-label').length,
      checks: root.querySelectorAll('.ts-dialog-check, .ts-dialog-radio').length,
      controls: [...root.querySelectorAll('[data-control]')]
        .map((e) => e.getAttribute('data-control'))
        .slice(0, 60),
      scrolls: [...root.querySelectorAll('*')].some(
        (e) =>
          e.scrollHeight > e.clientHeight + 2 && /auto|scroll/.test(getComputedStyle(e).overflowY),
      ),
      text: root.textContent.trim().replace(/\s+/g, ' ').slice(0, 400),
  ...
  const input = await probe(
    page,
    `${sel} input[type="text"], ${sel} input[type="url"], ${sel} input[type="search"], ${sel} select`,
  );
  await S(`dialog-${name}`);
  // Escape
  ...
  return {
    dialog: d
      ? {
          w: Math.round(d.box.w),
          h: Math.round(d.box.h),
          top: Math.round(d.box.y),
          border: d.borderHex,
          borderContrast: d.borderContrast,
          radius: d.radius,
          font: d.font,
          animation: d.animation,
        }
      : null,
    title: title ? { text: title.text, font: title.font } : null,
    lead: lead?.text ?? null,
    x: x
      ? {
          size: `${Math.round(x.box.w)}x${Math.round(x.box.h)}`,
          tip: x.tip,
          title: x.title,
    ...
    actions: actions.map((a) => ({
      text: a.text,
      solid: /is-solid/.test(a.cls),
      textStyle: /is-text/.test(a.cls),
      h: a.height,
      radius: a.radius,
      border: a.borderHex,
      borderContrast: a.borderContrast,
    })),
    label: label ? { font: label.font, color: label.colorHex, contrast: label.textContrast } : null,
    input: input
      ? {
          h: input.height,
          border: input.borderHex,
          borderContrast: input.borderContrast,
          radius: input.radius,
        }
      : null,
    ...fields,
...
const panelFacts = async (page) =>
  page.evaluate(() => {
    ...
    const heads = [...p.querySelectorAll('.ts-panel-section-head')].map((h) => ({
      text: h.textContent.trim().slice(0, 40),
      h: Math.round(h.getBoundingClientRect().height),
      closed: h.closest('.ts-panel-section')?.classList.contains('is-closed') ?? null,
      disabled: h.classList.contains('is-disabled'),
    }));
    return {
      w: Math.round(r.width),
      h: Math.round(r.height),
      headH: Math.round(p.querySelector('.ts-panel-head')?.getBoundingClientRect().height ?? 0),
      title: p.querySelector('.ts-panel-title')?.textContent?.trim(),
      count: p.querySelector('.ts-panel-count')?.textContent?.trim() ?? null,
      sections: heads,
      bodyScrolls: body ? body.scrollHeight > body.clientHeight + 1 : null,
      bodyScrollClass: body?.className,
      empty: p.querySelector('.ts-panel-empty')?.textContent?.trim() ?? null,
      notice: p.querySelector('.ts-fo-notice')?.textContent?.trim() ?? null,
      fields: p.querySelectorAll('input, select, textarea').length,
      rows: p.querySelectorAll('.ts-insp-row').length,
      controls: [...p.querySelectorAll('[data-control]')]
        .map((e) => e.getAttribute('data-control'))
        .slice(0, 50),
    };
  });
const contextFacts = (page) =>
  page.evaluate(() => {
    ...
    const items = [...m.querySelectorAll('.ts-menu-item')].filter(
      (el) => el.getClientRects().length > 0,
    );
    return {
      w: Math.round(r.width),
      ...
      x: Math.round(r.x),
      y: Math.round(r.y),
      fits: r.bottom <= window.innerHeight && r.right <= window.innerWidth,
      rows: items.length,
      disabled: items.filter((i) => i.classList.contains('is-disabled')).length,
      withIcon: items.filter((i) => i.querySelector('.ts-menu-ic svg')).length,
      labels: items
        .map((i) => i.querySelector('.ts-menu-label')?.textContent?.trim() ?? '')
        .slice(0, 30),
    };
  });
...
try {
  for (const run of RUNS) {
    ...
    const context = await newContext(browser, {
      ...
      height: run.h,
      theme: run.theme,
      storageState: deckId && existsSync(STORAGE_FILE) ? STORAGE_FILE : null,
    });
    ...
    try {
      ...
      const P = (surface, what, sel) =>
        step(table, surface, `${tag} ${what}`, async () => summarize(await probe(page, sel)));
      const V = { w: run.w, h: run.h };
      const clipOf = async (sel, p = 8) => {
        const r = await rectOf(page, sel);
        return r ? pad(r, p, V.w, V.h) : null;
      };
      ...
      if (!deckId) {
        ...
        await step(table, 'editor', `${tag} the editor skeleton on /edit`, async () => {
          ...
          for (let i = 0; i < 80; i += 1) {
            sk = await page
              .evaluate(() => {
                const s = document.querySelector('.ts-skeleton');
                return s ? { at: performance.now(), rows: s.querySelectorAll('*').length } : null;
              })
              .catch(() => null);
            if (sk) break;
            if (await page.evaluate(() => Boolean(window.turboslide?.studio)).catch(() => false))
              break;
            await sleep(25);
          }
      ...
      await step(table, 'editor', `${tag} the frame`, async () => {
        const tk = await page.evaluate(() => {
          ...
          return {
            ...
            appearance: localStorage.getItem('ts-chrome-appearance'),
            paper: pick('--pt-paper'),
            ink: pick('--pt-ink'),
            titanium: pick('--pt-titanium'),
            hair: pick('--pt-hair'),
            select: pick('--pt-select'),
          };
        });
        const rows = {};
        for (const [k, sel] of Object.entries({
          ...
          const r = await rectOf(page, sel);
          rows[k] = r
            ? `${Math.round(r.w)}x${Math.round(r.h)} at ${Math.round(r.x)},${Math.round(r.y)}`
            : null;
        }
        await S('editor');
      ...
      await P(
        'title',
        'presence own chip',
        '.ts-title-r [data-control^="title.presence"] , .ts-presence-slot .ts-chip, .ts-presence-slot button',
      );
      await step(table, 'title', `${tag} the right cluster geometry`, async () =>
        page.evaluate(() =>
          [...document.querySelectorAll('.ts-title-r > *')]
            .filter((e) => e.getClientRects().length > 0)
            .map((e) => {
              const r = e.getBoundingClientRect();
              return {
                cls: e.className.split(' ')[0] || e.tagName,
                ctl: e.getAttribute('data-control'),
                x: Math.round(r.x),
                w: Math.round(r.width),
                h: Math.round(r.height),
                radius: getComputedStyle(e).borderRadius,
                empty: r.width > 0 && e.textContent.trim() === '' && !e.querySelector('svg'),
              };
            }),
        ),
      );
      await step(table, 'title', `${tag} hover states of the right cluster`, async () => {
        ...
        for (const c of ['deck.name', 'share.open', 'present.open', 'present.arrow']) {
          ...
          if (c === 'share.open' || c === 'present.open')
            await S(`title-hover-${c.replace('.', '-')}`, pad(r, 20, V.w, V.h));
        }
        return out;
      });
      await step(table, 'title', `${tag} tooltip on Share and the split button`, async () => ({
        share: await tooltipOn(page, 'share.open'),
        slideshow: await tooltipOn(page, 'present.open'),
        arrow: await tooltipOn(page, 'present.arrow'),
        name: await tooltipOn(page, 'deck.name'),
        save: await tooltipOn(page, 'deck.saveState'),
      }));
      await step(table, 'title', `${tag} keyboard focus through the title row`, async () => {
        ...
        const seq = [];
        for (let i = 0; i < 8; i += 1) {
          const f = await focusNext(page);
          if (!f) break;
          seq.push({
            on: f.control ?? f.cls.split(' ')[0],
            fv: f.focusVisible,
            outline: f.outline.replace(/rgb\([^)]*\)/, (m) => m),
            inRow: f.box.y < 44,
          });
          if (f.box.y > 44) break;
        }
        ...
        if (shareR)
          await S(
            'title-focus',
            pad({ x: shareR.x - 420, y: 0, w: 420 + shareR.w + 20, h: 44 }, 4, V.w, V.h),
          );
        return seq;
      });
      await step(table, 'title', `${tag} a long deck name (truncation)`, async () => {
        ...
        await typeHuman(
          page,
          'Q3 enterprise renewal pitch for the EMEA team, revision four with pricing',
        );
        ...
        const clipped = await page.evaluate(() => {
          const el = document.querySelector('[data-control="deck.name"]');
          return el ? el.scrollWidth > el.clientWidth + 1 : null;
        });
        return {
          fieldW: field.box.w,
          fieldFont: field.font,
          nameW: Math.round(name.box.w),
          maxWidth: name
            ? await page.evaluate(
                () =>
                  getComputedStyle(document.querySelector('[data-control="deck.name"]')).maxWidth,
              )
            : null,
          ellipsis: name.textOverflow,
          clipped,
          save: save?.text,
          tabTitle: tab.slice(0, 90),
        };
      });
      ...
      await step(table, 'menubar', `${tag} the titles`, async () =>
        page.evaluate(() =>
          [...document.querySelectorAll('.ts-menubar-title')].map((t) => t.textContent.trim()),
        ),
      );
      await step(table, 'menubar', `${tag} hover a title`, async () => {
        ...
        return {
          hover: f.hover,
          ownBg: f.ownBgRgb,
          groundContrast: f.groundContrast,
          color: f.colorHex,
          cursor: f.cursor,
          gapsBetweenTitles: await page.evaluate(() => {
            const ts = [...document.querySelectorAll('.ts-menubar-title')];
            const g = [];
            for (let i = 1; i < ts.length; i += 1)
              g.push(
                Math.round(
                  ts[i].getBoundingClientRect().x - ts[i - 1].getBoundingClientRect().right,
                ),
              );
            return [...new Set(g)];
          }),
      ...
      for (const id of [
        ...
        await step(table, 'menus', `${tag} the ${id} menu`, async () => {
          ...
          if (run.full && ['file', 'format', 'view', 'insert'].includes(id))
            await S(
              `menu-${id}`,
              pad(
                { x: 0, y: 44, w: Math.min(V.w, 640), h: Math.min(V.h - 44, (f?.h ?? 400) + 80) },
                0,
                V.w,
                V.h,
              ),
          ...
          const disabledRow = await probe(
            page,
            `#ts-menu-${id} .ts-menu-item.is-disabled .ts-menu-label`,
          );
          await closeMenus(page);
          const still = await has(page, `#ts-menu-${id}`);
          return {
            ...f,
            plateFont: plate?.font,
            plateBorder: plate?.borderHex,
            plateBorderContrast: plate?.borderContrast,
            plateAnimation: plate?.animation,
            key: key ? { font: key.font, color: key.colorHex, contrast: key.textContrast } : null,
            iconPx: icon?.box.w,
            subChevronPx: sub?.box.w,
            disabledRow: disabledRow
              ? {
                  text: disabledRow.text,
                  color: disabledRow.colorHex,
                  contrast: disabledRow.textContrast,
                }
              : null,
            escapeCloses: !still,
            newConsoleErrors: consoleErrors.length - errs,
          };
        });
      }
      await step(
        table,
        'menus',
        `${tag} a row hover and a submenu (Format > Align & indent)`,
        async () => {
          ...
          const ids = await page.evaluate(() =>
            [...document.querySelectorAll('#ts-menu-format [data-control^="menu."]')].map((e) =>
          ...
          if (alignId) {
            ...
            await hoverRow(
              page,
              alignId,
              `[id^="ts-menu-"][id$="${alignId.split('.').pop()}"], .ts-menu.is-sub, [role="menu"]:not(#ts-menu-format)`,
            ).catch(() => undefined);
            sub = await page.evaluate(() => {
              const menus = [...document.querySelectorAll('.ts-menu')].filter(
                (m) => m.getClientRects().length > 0,
              );
              const s = menus[menus.length - 1];
              if (!s || menus.length < 2) return { open: false, count: menus.length };
              ...
              return {
                ...
                y: Math.round(r.y),
                w: Math.round(r.width),
                overlapWithParent: Math.round(parent.right - r.x),
                rows: s.querySelectorAll('.ts-menu-item').length,
              };
            });
            sub.openAfterMs = Date.now() - t0;
            await S(
              'menu-format-submenu',
              pad({ x: 0, y: 44, w: Math.min(V.w, 760), h: Math.min(V.h - 44, 640) }, 0, V.w, V.h),
            );
          }
          await closeMenus(page);
          return {
            hovered: hovered
              ? {
                  ownBg: hovered.ownBgRgb,
                  groundContrast: hovered.groundContrast,
                  color: hovered.colorHex,
                }
              : null,
            alignId,
            sub,
      ...
      await step(table, 'menus', `${tag} the disabled rows of Edit`, async () => {
        ...
        const rows = await page.evaluate(() =>
          [...document.querySelectorAll('#ts-menu-edit .ts-menu-item')]
            .map((i) => ({
              label: i.querySelector('.ts-menu-label')?.textContent?.trim(),
              disabled: i.classList.contains('is-disabled'),
              ariaDisabled: i.getAttribute('aria-disabled'),
              cursor: getComputedStyle(i).cursor,
              title: i.getAttribute('title'),
              tip: i.getAttribute('data-tip'),
            }))
            .slice(0, 12),
        );
      ...
      await step(table, 'menus', `${tag} View > Appearance`, async () => {
        ...
        const rows = await page.evaluate(() =>
          [...document.querySelectorAll('[data-control^="menu.view.appearance."]')].map((e) => ({
            id: e.getAttribute('data-control'),
            checked:
              e.getAttribute('aria-checked') ??
              (e.querySelector('.ts-menu-check') ? 'check' : null),
            role: e.getAttribute('role'),
          })),
        );
        await S(
          'menu-view-appearance',
          pad({ x: 0, y: 44, w: Math.min(V.w, 700), h: Math.min(V.h - 44, 620) }, 0, V.w, V.h),
        );
        ...
        await hoverRow(
          page,
          'view.appearance',
          `[data-control="menu.view.appearance.${run.theme}"]`,
        );
        ...
        return {
          ...
          switchedTo: switched,
          back,
          transitionOnRoot: await page.evaluate(
      ...
      await step(table, 'toolbar', `${tag} hover, pressed and disabled states`, async () => {
        ...
        for (const c of [
          ...
          const r =
            (await rectOfControl(page, c)) ??
            (c === 'toolbar.zoom' ? await rectOf(page, '.ts-tb-zoom') : null);
          if (!r) {
            out[c] = 'absent';
            continue;
          }
          await hoverAt(page, r.x + r.w / 2, r.y + r.h / 2, 260);
          const f =
            (await probe(page, `[data-control="${c}"]`)) ?? (await probe(page, '.ts-tb-zoom'));
          out[c] = {
            hover: f.hover,
            border: f.borderHex,
            borderContrast: f.borderContrast,
          ...
          if (c === 'toolbar.newSlide' || c === 'toolbar.undo')
            await S(`toolbar-hover-${c.split('.')[1]}`, pad(r, 16, V.w, V.h));
        }
        return out;
      });
      await step(
        table,
        'toolbar',
        `${tag} tooltips (New slide, Undo, Zoom, Select, Insert image)`,
        async () => ({
          newSlide: await tooltipOn(page, 'toolbar.newSlide'),
          undo: await tooltipOn(page, 'toolbar.undo'),
          select: await tooltipOn(page, 'toolbar.select'),
          image: await tooltipOn(page, 'toolbar.insertImage'),
          paint: await tooltipOn(page, 'toolbar.paintFormat'),
        }),
      );
      await step(table, 'toolbar', `${tag} keyboard focus ring on toolbar buttons`, async () => {
        await page.evaluate(() =>
          document.querySelector('[data-control="toolbar.search"]')?.focus(),
        );
        const seq = [];
        for (let i = 0; i < 6; i += 1) {
          const f = await focusNext(page);
      ...
      await step(table, 'toolbar', `${tag} the zoom dropdown and the New slide arrow`, async () => {
        ...
        await sleep(400);
        const zoomMenu = await page.evaluate(() => {
          const m = [...document.querySelectorAll('.ts-menu')].find(
            (x) => x.getClientRects().length > 0,
          );
          return m
            ? {
                rows: [...m.querySelectorAll('.ts-menu-item')]
        ...
        await S(
          'toolbar-zoom-menu',
          pad({ x: 0, y: 72, w: 700, h: Math.min(V.h - 72, 420) }, 0, V.w, V.h),
        );
        await closeMenus(page);
        const a = await rectOf(page, '.ts-tb-split-arrow');
        let layoutMenu = null;
        if (a) {
          await clickAt(page, a.x + a.w / 2, a.y + a.h / 2);
          await sleep(500);
          layoutMenu = await page.evaluate(() => {
            const p = document.querySelector('.ts-layout-plate');
            const m = [...document.querySelectorAll('.ts-menu')].find(
              (x) => x.getClientRects().length > 0,
            );
            return {
              plate: p ? Math.round(p.getBoundingClientRect().width) : null,
              menuRows: m ? m.querySelectorAll('.ts-menu-item').length : null,
              tiles: document.querySelectorAll('.ts-layout-tile').length,
            };
          });
          await S(
            'toolbar-newslide-arrow',
            pad({ x: 0, y: 72, w: 720, h: Math.min(V.h - 72, 720) }, 0, V.w, V.h),
          );
          await closeMenus(page);
        }
      ...
      await step(
        table,
        'filmstrip',
        `${tag} New slide from the toolbar (a second card)`,
        async () => {
          ...
          const cards = await page.evaluate(() =>
            [...document.querySelectorAll('.ts-card')].map((c) => {
              const r = c.getBoundingClientRect();
              const fr = c.querySelector('.ts-card-frame').getBoundingClientRect();
              const cs = getComputedStyle(c.querySelector('.ts-card-frame'));
              return {
                ...
                outline: cs.outlineWidth + ' ' + cs.outlineStyle + ' offset ' + cs.outlineOffset,
              };
            }),
          );
      ...
      await step(table, 'filmstrip', `${tag} hover a non current card and its number`, async () => {
        ...
        return {
          ...
          borderContrast: f.borderContrast,
          transition: f.transition,
          number: { font: n.font, color: n.colorHex, contrast: n.textContrast },
          cursor: f.cursor,
        };
      });
      await step(table, 'filmstrip', `${tag} right click a card`, async () => {
        ...
        await S(
          'context-filmstrip',
          f ? pad({ x: f.x - 8, y: f.y - 8, w: f.w + 16, h: f.h + 16 }, 8, V.w, V.h) : null,
        );
        await closeMenus(page);
        return f;
      });
      await step(
        table,
        'filmstrip',
        `${tag} drag card 2 above card 1 (the drop line)`,
        async () => {
          const cards = await page.evaluate(() =>
            [...document.querySelectorAll('.ts-card .ts-card-frame')].map((c) => {
              const r = c.getBoundingClientRect();
          ...
          const mid = await drag(page, from, to, {
            ...
            during: async () => {
              const d = await page.evaluate(() => {
                const c = document.querySelector('.ts-card[data-drop]');
                const g = document.querySelector('.ts-card.is-dragging');
                return {
                  drop: c?.getAttribute('data-drop') ?? null,
                  draggingOpacity: g ? getComputedStyle(g).opacity : null,
                  ghost: document.querySelector('.ts-card-ghost, .ts-drag-ghost') !== null,
                };
              });
              await shot(
                page,
          ...
          if (order && order[0] !== objects.slide1) {
          ...
          return {
            during: mid,
            orderAfter: order,
            restored: (await invoke(page, 'slide.list', {})).slides?.map((s) => s.id),
          };
        },
      );
      ...
      await step(
        table,
        'overlay',
        `${tag} select the title placeholder (ring, handles, chip)`,
        async () => {
          ...
          const handle = await probe(
            page,
            '.ts-overlay .ts-handle[data-kind="free-resize"]',
            '::before',
          );
          const handleBox = await probe(page, '.ts-overlay .ts-handle[data-kind="free-resize"]');
          const rot = await probe(
            page,
            '.ts-overlay .ts-handle[data-kind="free-rotate"]',
            '::before',
          );
          ...
          return {
            ...
            ring: ring
              ? { border: ring.border, color: ring.borderHex, contrast: ring.borderContrast }
              : null,
            handle: handle
              ? {
                  drawn: `${Math.round(handle.box.w)}x${Math.round(handle.box.h)}`,
                  hit: `${Math.round(handleBox.box.w)}x${Math.round(handleBox.box.h)}`,
                  border: handle.borderHex,
                  bg: handle.background,
                  cursor: handleBox.cursor,
                  ariaLabel: handleBox.ariaLabel,
                }
              : null,
            rotate: rot
              ? {
                  drawn: `${Math.round(rot.box.w)}x${Math.round(rot.box.h)}`,
                  hit: `${Math.round(rotBox.box.w)}x${Math.round(rotBox.box.h)}`,
                  radius: rot.radius,
                  ariaLabel: rotBox.ariaLabel,
                }
              : null,
            chip: c
              ? {
      ...
      await step(
        table,
        'overlay',
        `${tag} the hover outline over the body placeholder`,
        async () => {
          const all = await runs(page);
          ...
          return h
            ? {
                ...
                box: `${Math.round(h.box.w)}x${Math.round(h.box.h)}`,
              }
            : 'no hover outline';
        },
      );
      await step(
        table,
        'overlay',
        `${tag} resize by the se handle: the readout during the drag`,
        async () => {
          const ctrls = await handleControls(page);
          ...
          const facts = await drag(page, from, to, {
            during: async () => {
              const ro = await probe(page, '.ts-readout');
              const sheet = await rectOf(page, SHEET);
              await shot(page, `${tag}-overlay-resize-readout`, pad(sheet, 24, V.w, V.h));
              return ro
                ? {
                    text: ro.text,
                    h: ro.height,
                    ...
                    bg: ro.ownBgRgb,
                    color: ro.colorHex,
                    contrast: ro.textContrast,
                    x: Math.round(ro.box.x),
                    y: Math.round(ro.box.y),
                  }
                : 'no readout during the drag';
            },
          });
          await sleep(400);
      ...
      await step(table, 'overlay', `${tag} the sheet on the plate ground`, async () => {
        ...
        return {
          stageBg: stage.ownBgRgb,
          stageGroundContrast: stage.groundContrast,
          sheetBg: sheet?.ownBgRgb,
          sheetBorder: sheet?.border,
          sheetShadow: await page.evaluate(
            (s) => getComputedStyle(document.querySelector(s)).boxShadow,
            SHEET,
          ),
          sheetBox: sheet ? `${Math.round(sheet.box.w)}x${Math.round(sheet.box.h)}` : null,
          margins:
            sheet && stage
              ? {
                  left: Math.round(sheet.box.x - stage.box.x),
                  top: Math.round(sheet.box.y - stage.box.y),
                  right: Math.round(stage.box.x + stage.box.w - sheet.box.x - sheet.box.w),
                  bottom: Math.round(stage.box.y + stage.box.h - sheet.box.y - sheet.box.h),
                }
              : null,
        };
      });
      ...
      const park = async (slideId, id, pos) => {
        for (let a = 0; a < 2; a += 1) {
      ...
      const selectAndTail = async (kind, id, slideId, expectControl) => {
        ...
        if (!sel || !sel.ctrls.some((c) => c.startsWith(`handle.${id}.`))) {
          await press(page, 'Escape', 2);
          await sleep(200);
          const b = await boxOf(page, id);
          if (b) {
            await clickAt(page, b.free.x + 4, b.free.y + 4);
            await sleep(250);
            sel = {
              ctrls: await handleControls(page),
              chip: await chip(page),
              ring: await has(page, '.ts-overlay .ts-select'),
            };
          }
        }
        if (
        ...
        return {
          selected: sel?.ctrls.some((c) => c.startsWith(`handle.${id}.`)) ?? false,
          chip: sel?.chip,
          tail: expectControl ? tb.ctrls.some((c) => c.id === expectControl) : null,
          toolbar: tb,
        };
      };
      if (run.full && !objects.text) {
        await step(
          table,
          'objects',
          `${tag} insert a text box, a rectangle, a line, a picture (slide 2)`,
          async () => {
            await goto(objects.slide2);
            const text = await insertByTool(
              page,
              objects.slide2,
              ['insert.textBox'],
              { x: 1200, y: 760 },
            ...
            if (text) {
              objects.text = text.id;
              if (
                await page.evaluate(
                  () => document.querySelector('.ts-stagewrap.ts-editor[data-editing]') !== null,
                )
              ) {
                await typeHuman(page, 'Pricing note');
              }
              await press(page, 'Escape', 2);
              await settled(page);
              await park(objects.slide2, text.id, { x: 80, y: 600, w: 400, h: 100 });
            }
            const shape = await insertByTool(
              page,
              objects.slide2,
              ['insert.shape', 'insert.shape.shapes', 'insert.shape.shapes.rectangle'],
              { x: 700, y: 760 },
              null,
              'shape',
            );
            if (shape) {
              ...
              await press(page, 'Escape', 2);
              await settled(page);
              await park(objects.slide2, shape.id, { x: 560, y: 600, w: 300, h: 160 });
            }
            const line = await insertByTool(
              page,
              objects.slide2,
            ...
            if (line && line.id !== objects.shape) {
              objects.line = line.id;
              await press(page, 'Escape', 2);
              await settled(page);
            }
            ...
            const asset = await invoke(page, 'asset.add', {
              id: `shot-audit-${Date.now().toString(36)}-asset`,
              url: pngDataUrl(96, 64),
              role: 'capture',
              alt: 'audit picture',
            ...
            await invoke(page, 'block.insert', {
              slideId: objects.slide2,
              slot: 'main',
              block: {
                id: picId,
                type: 'shot',
                asset: asset.id,
                pos: { x: 1160, y: 80, w: 320, h: 200 },
              },
              baseRevision: Math.max(s1.revision, asset.revision ?? 0),
            });
            ...
            return {
              ...
              shape: objects.shape,
              line: objects.line,
              picture: objects.picture,
              objects: (await objectsOf(page, objects.slide2)).map(
                (o) =>
                  `${o.type}:${o.id}@${Math.round(o.pos.x)},${Math.round(o.pos.y)} ${Math.round(o.pos.w)}x${Math.round(o.pos.h)}`,
              ),
            };
          },
        );
        await step(
          table,
          'objects',
          `${tag} insert a table (slide 3) and a chart (slide 4)`,
          async () => {
            await clickControl(page, 'toolbar.newSlide');
            await pollUntil(
              () => state(page),
              (s) => s.slideId !== objects.slide2,
              10_000,
            );
            await settled(page);
            ...
            await hoverRow(
              page,
              'insert.table',
              '[data-control$=".pick.3x3"], [data-control$="pick.3x3"]',
            ).catch(() => undefined);
            const pick = await page.evaluate(
              () =>
                [...document.querySelectorAll('[data-control]')]
                  .map((e) => e.getAttribute('data-control'))
                  .find((c) => /pick\.3x3$/.test(c)) ?? null,
            );
            let tableId = null;
            if (pick) {
              ...
              if (!obj) {
                const p = await sheetPoint(page, 800, 500);
                await clickAt(page, p.x, p.y);
                obj = await newObjectAfter(page, objects.slide3, before, 15_000, 'table');
              }
              tableId = obj?.id ?? null;
              await press(page, 'Escape', 2);
              await settled(page);
            ...
            if (!chartObj) {
              const p = await sheetPoint(page, 800, 500);
              await clickAt(page, p.x, p.y);
              chartObj = await newObjectAfter(page, objects.slide4, before4, 15_000, 'chart');
            }
            ...
            return {
              ...
              table: objects.table,
              chart: objects.chart,
              slide3: objects.slide3,
              slide4: objects.slide4,
            };
          },
        );
      }
      for (const [kind, id, slideId, expect] of [
        ...
        if (!id) {
          table.add('tails', `${tag} the ${kind} tail`, 'not driven: the object was not inserted');
          continue;
        }
        await step(table, 'tails', `${tag} the ${kind} tail`, async () => {
          ...
          return {
            ...
            chip: f.chip,
            expected: f.tail,
            count: f.toolbar.count,
            overflow: f.toolbar.overflow,
            clipped: f.toolbar.clipped,
            more: f.toolbar.more,
            gaps: f.toolbar.gaps,
            heights: f.toolbar.heights,
            disabled,
            widths: f.toolbar.widths,
            lastRight: f.toolbar.ctrls[f.toolbar.ctrls.length - 1]?.right,
            barRight: f.toolbar.bar.right,
            seps: f.toolbar.seps.length,
          };
        });
      }
      if (objects.text) {
        await step(table, 'tails', `${tag} the text tail's controls up close`, async () => {
          ...
          await S(
            'toolbar-tail-text-bold-pressed',
            pad({ x: 0, y: 72, w: V.w, h: 40 }, 0, V.w, V.h),
          );
          await page.keyboard.press('Meta+B');
          await sleep(200);
          await press(page, 'Escape', 2);
          ...
          return {
            font: font
              ? {
                  text: font.text,
                  w: Math.round(font.box.w),
                  disabled: font.disabled,
                  color: font.colorHex,
                  contrast: font.textContrast,
                  cursor: font.cursor,
                  tip: fontTip,
                }
              : 'absent',
            size: size
              ? {
                  w: size.box.w,
                  h: size.height,
                  border: size.borderHex,
                  borderContrast: size.borderContrast,
                  font: size.font,
                  value: await page.evaluate(
                    () => document.querySelector('.ts-tb-size-field')?.value,
                  ),
                }
              : null,
            sizeStepW: step1?.box.w,
            bold: bold
              ? { pressed: bold.pressed, color: bold.colorHex, border: bold.borderHex }
              : null,
            boldOn: boldOn
              ? {
                  pressed: boldOn.pressed,
                  color: boldOn.colorHex,
                  border: boldOn.borderHex,
                  borderContrast: boldOn.borderContrast,
                  ownBg: boldOn.ownBgRgb,
                }
              : null,
            colorSwatch: color ? { w: color.box.w, svg: color.svg } : null,
            align: align ? { w: align.box.w } : null,
          };
        });
        await step(table, 'tails', `${tag} the text colour plate`, async () => {
          ...
          const facts = await page.evaluate(() => {
            const p = document.querySelector('.ts-tb-swatches, .ts-picker');
            if (!p) return null;
            return {
              swatches: p.querySelectorAll('.ts-tb-swatch, .ts-picker-tile').length,
              hasHex: p.querySelector('input') !== null,
              hasCustom: /custom|\+/i.test(p.textContent),
          ...
          return {
            plate: plate
              ? { w: plate.box.w, border: plate.borderHex, borderContrast: plate.borderContrast }
              : null,
            swatch: swatch
              ? {
                  size: `${swatch.box.w}x${swatch.box.h}`,
                  border: swatch.borderHex,
                  tip: swatch.tip,
                  title: swatch.title,
                  ariaLabel: swatch.ariaLabel,
                }
              : null,
            ...facts,
          };
      ...
      await step(table, 'panel', `${tag} Format options for the text box`, async () => {
        ...
        if (await has(page, '[data-control="toolbar.formatOptions"]'))
          await clickControl(page, 'toolbar.formatOptions');
        else await menuPath(page, 'format', 'format.formatOptions');
        ...
        const input = await probe(
          page,
          '.ts-rpanel .ts-fo-field input, .ts-rpanel .ts-insp-field input',
        );
        ...
        return {
          ...f,
          head: head ? { h: head.height, border: head.border } : null,
          title: title ? { text: title.text, font: title.font } : null,
          x: x ? { size: `${x.box.w}x${x.box.h}`, tip: x.tip, ariaLabel: x.ariaLabel } : null,
          sectionHead: secHead
            ? { text: secHead.text, h: secHead.height, font: secHead.font, chevron: secHead.svg }
            : null,
          label: label
            ? { font: label.font, color: label.colorHex, contrast: label.textContrast }
            : null,
          rowLabel: rowLabel
            ? { font: rowLabel.font, color: rowLabel.colorHex, contrast: rowLabel.textContrast }
            : null,
          input: input
            ? {
                h: input.height,
            ...
            : null,
          panelBorder: panelLeft?.borderWidths,
      ...
      await step(table, 'panel', `${tag} a section closes and the body scrolls`, async () => {
        ...
        const body = await page.evaluate(() => {
          const b = document.querySelector('.ts-rpanel .ts-panel-body');
          return b
            ? {
                scrollH: b.scrollHeight,
                clientH: b.clientHeight,
                overflowY: getComputedStyle(b).overflowY,
                scrollClass: b.className,
              }
            : null;
        });
        return {
          firstClosedAfterClick: after?.sections?.[0]?.closed,
          body,
          chevronTransition: await page.evaluate(() => {
            const c = document.querySelector('.ts-rpanel .ts-panel-chev');
            return c ? getComputedStyle(c).transition : null;
          }),
        };
      });
      for (const [kind, id, slideId] of [
        ['shape', objects.shape, objects.slide2],
        ['picture', objects.picture, objects.slide2],
        ['table', objects.table, objects.slide3],
        ['chart', objects.chart, objects.slide4],
      ]) {
        ...
        await step(table, 'panel', `${tag} Format options for the ${kind}`, async () => {
          ...
          await sleep(500);
          if (!(await has(page, '.ts-rpanel .ts-panel'))) {
            if (await has(page, '[data-control="toolbar.formatOptions"]'))
              await clickControl(page, 'toolbar.formatOptions');
            else await menuPath(page, 'format', 'format.formatOptions');
            await sleep(500);
      ...
      await step(
        table,
        'panel',
        `${tag} nothing selected: the panel's empty sentence`,
        async () => {
          ...
          return {
            ...
            empty: f?.empty,
            notice: f?.notice,
            sections: f?.sections?.map((s) => `${s.text}${s.disabled ? ' (disabled)' : ''}`),
          };
        },
      );
      await step(table, 'panel', `${tag} close the panel by its X`, async () => {
        await clickControl(page, 'panel.close').catch(async () => {
          const x = await rectOf(page, '.ts-rpanel .ts-panel-x');
          await clickAt(page, x.x + x.w / 2, x.y + x.h / 2);
        });
        await sleep(400);
        return { open: await has(page, '.ts-rpanel .ts-panel') };
      });
      ...
      await step(table, 'panel', `${tag} Themes panel`, async () => {
        ...
        const tiles = await page.evaluate(
          () => document.querySelectorAll('.ts-themes-tile').length,
        );
        const importBtn = await probe(
          page,
          '.ts-rpanel .pt-ib.is-disabled, .ts-rpanel button:disabled',
        );
        ...
        return {
          title: f?.title,
          tiles,
          tile: tile ? { w: tile.box.w, h: tile.box.h } : null,
          frame: frame
            ? {
                size: `${Math.round(frame.box.w)}x${Math.round(frame.box.h)}`,
                border: frame.borderHex,
          ...
          name: name
            ? {
                ...
                font: name.font,
                color: name.colorHex,
                contrast: name.textContrast,
              }
            : null,
          groupHead: head
            ? { text: head.text, font: head.font, contrast: head.textContrast }
            : null,
          importButton: importBtn
            ? { text: importBtn.text, color: importBtn.colorHex, contrast: importBtn.textContrast }
            : null,
          empty: f?.empty,
        };
      });
      await step(table, 'panel', `${tag} Comments panel (empty)`, async () => {
        ...
        const empty = await probe(
          page,
          '.ts-rpanel .ts-panel-empty, .ts-rpanel .ts-comments-empty',
        );
        const body = await page.evaluate(() =>
          document
            .querySelector('.ts-rpanel .ts-panel-body')
            ?.textContent?.trim()
            .replace(/\s+/g, ' ')
            .slice(0, 300),
        );
        ...
        return {
          ...
          empty: empty
            ? {
                text: empty.text,
                font: empty.font,
                color: empty.colorHex,
                contrast: empty.textContrast,
              }
            : null,
          body,
          togglePressed: toggle?.pressed,
          toggleBorder: toggle?.borderHex,
          controls: f?.controls,
          closedByToggle: !(await has(page, '.ts-rpanel .ts-panel')),
        };
      });
      await step(table, 'panel', `${tag} Version history panel`, async () => {
        ...
        return {
          title: f?.title,
          rows,
          row: row ? { h: row.height } : null,
          note: note ? { text: note.text, font: note.font, contrast: note.textContrast } : null,
          meta: meta
            ? {
                text: meta.text,
                font: meta.font,
                color: meta.colorHex,
                contrast: meta.textContrast,
              }
            : null,
          named: named?.text,
          field: field
            ? {
                ...
                placeholder: await page.evaluate(
                  () => document.querySelector('.ts-versions-note')?.placeholder,
                ),
              }
            : null,
          restore: restore ? { text: restore.text, h: restore.height } : null,
          body,
          empty: f?.empty,
        };
      });
      ...
      await step(table, 'plates', `${tag} the Layout picker`, async () => {
        ...
        const facts = await page.evaluate(() => {
          const p = document.querySelector('.ts-layout-plate');
          const tiles = [...document.querySelectorAll('.ts-layout-tile')];
          return {
            tiles: tiles.length,
            current: tiles.findIndex((t) => t.classList.contains('is-current')),
            fits: p ? p.getBoundingClientRect().bottom <= window.innerHeight : null,
            scrolls: p ? p.scrollHeight > p.clientHeight + 1 : null,
            plateBottom: p ? Math.round(p.getBoundingClientRect().bottom) : null,
            headText: document.querySelector('.ts-layout-plate-head')?.textContent?.trim(),
            missing: document.querySelectorAll('.ts-layout-missing').length,
          };
        });
        ...
        const hovered = t2
          ? await probe(page, '.ts-layout-tile:nth-child(2) .ts-layout-frame')
          : null;
        ...
        return {
          ...
          plate: plate
            ? {
                w: Math.round(plate.box.w),
                h: Math.round(plate.box.h),
                border: plate.borderHex,
                pad: plate.padding,
              }
            : null,
          gridW: grid?.box.w,
          tile: tile ? `${tile.box.w}x${Math.round(tile.box.h)}` : null,
          frame: frame
            ? {
                size: `${frame.box.w}x${frame.box.h}`,
                border: frame.borderHex,
                borderContrast: frame.borderContrast,
              }
            : null,
          name: name
            ? {
            ...
            : null,
          rule: rule ? { text: rule.text, font: rule.font, contrast: rule.textContrast } : null,
          head: head ? { font: head.font, contrast: head.textContrast } : null,
          hoveredBorder: hovered?.borderHex,
          closedByEscape: !(await has(page, '.ts-layout-plate')),
        };
      });
      ...
      if (gone()) throw new Error('the page is gone');
      const D = (name, open) =>
        step(table, 'dialogs', `${tag} ${name}`, () => dialogFacts(page, tag, name, open, S));
      ...
      await step(table, 'dialogs', `${tag} the Share dialog's body up close`, async () => {
        ...
        await sleep(700);
        const facts = await page.evaluate(() => {
          const root = document.querySelector('[data-control="dialog.share"]');
          const vis = (e) => e.getClientRects().length > 0;
          const ctrls = [...root.querySelectorAll('[data-control]')].filter(vis).map((e) => {
            const r = e.getBoundingClientRect();
            return {
              id: e.getAttribute('data-control').replace('dialog.share.', ''),
              h: Math.round(r.height),
              w: Math.round(r.width),
          ...
          const heights = [
            ...new Set(ctrls.filter((c) => /button|select|input/.test(c.tag)).map((c) => c.h)),
          ];
          return {
            controls: ctrls.slice(0, 40),
        ...
        return {
          ...
          emails: emails
            ? {
                h: emails.height,
                border: emails.borderHex,
                borderContrast: emails.borderContrast,
                placeholder: await page.evaluate(
                  () =>
                    document.querySelector('[data-control="dialog.share.emails"]')?.placeholder ??
                    null,
                ),
              }
            : null,
          role: role ? { h: role.height, tag: role.tag } : null,
          send: send
            ? {
                text: send.text,
                solid: /is-solid/.test(send.cls),
                h: send.height,
                disabled: send.disabled,
              }
            : null,
          done: done ? { text: done.text, solid: /is-solid/.test(done.cls), h: done.height } : null,
          copy: copy ? { text: copy.text, h: copy.height } : null,
          footer: footer
            ? { text: footer.text, font: footer.font, contrast: footer.textContrast }
            : null,
          general: general ? { text: general.text.slice(0, 120) } : null,
        };
      });
      ...
      await step(table, 'notes', `${tag} type a note and the handle hover`, async () => {
        ...
        const ph = await page.evaluate(
          () => document.querySelector('.ts-notes-field')?.placeholder ?? null,
        );
        ...
        return {
          placeholder: ph,
          fieldOutlineWhileFocused: focused.outline,
          fieldFocusVisible: focused.focusVisible,
          handle: {
            size: `${h.w}x${h.h}`,
            color: handle.colorHex,
            cursor: handle.cursor,
            tip: handle.tip,
            title: handle.title,
            ariaLabel: handle.ariaLabel,
          },
          paneH: (await rectOf(page, '.ts-notes'))?.h,
        };
      });
      ...
      await step(table, 'bottom', `${tag} the bottom bar's buttons`, async () => {
        ...
        return btns.map((b) => ({
          ctl: b.control,
          size: `${b.box.w}x${b.box.h}`,
          svg: b.svg,
          tip: b.tip,
          title: b.title,
          ariaLabel: b.ariaLabel,
          color: b.colorHex,
          contrast: b.textContrast,
          pressed: b.pressed,
          tooltip: b.control ? tips[b.control] : null,
        }));
      });
      ...
      await step(table, 'context', `${tag} right click the empty sheet`, async () => {
        ...
        if (f)
          await S(
            'context-sheet',
            pad({ x: f.x - 8, y: f.y - 8, w: f.w + 16, h: f.h + 16 }, 8, V.w, V.h),
          );
        await closeMenus(page);
        return f;
      });
      await step(table, 'context', `${tag} right click the text box`, async () => {
        ...
        await page.mouse.click(b.inner.x + b.inner.w / 2, b.inner.y + b.inner.h / 2, {
          button: 'right',
        });
        await page
          .locator('.ts-context-menu')
      ...
      if (run.full) {
        await step(table, 'help', `${tag} the ? key`, async () => {
          ...
          return {
            helpCard: help
              ? {
                  w: Math.round(help.box.w),
                  h: Math.round(help.box.h),
                  border: help.borderHex,
                  pad: help.padding,
                  font: help.font,
                }
              : null,
            dialog: dialog ? { w: Math.round(dialog.box.w) } : null,
            title: title?.text,
            titleFont: title?.font,
            closed: !(await has(page, '.pt-help-card')) && !(await has(page, '.ts-dialog-scrim')),
          };
        });
      }
      ...
      if (run.full) {
        await step(table, 'show', `${tag} Slideshow from the title row`, async () => {
          ...
          await sleep(1200);
          const facts = await page.evaluate(() => {
            const s = document.querySelector('.ts-slideshow');
            const root = document.querySelector('.pt-viewer');
            const vis = (e) => e.getClientRects().length > 0;
            return {
              show: s !== null,
              present: root?.classList.contains('is-present'),
              controls: s
                ? [...s.querySelectorAll('[data-control]')].filter(vis).map((e) => {
                    const r = e.getBoundingClientRect();
                    return {
                      id: e.getAttribute('data-control'),
              ...
              classes: s
                ...
                : [],
              stageBg: getComputedStyle(document.querySelector('.pt-stagewrap')).backgroundColor,
          ...
          const barShown = await page.evaluate(() => {
            const b = document.querySelector(
              '.ts-slideshow .ts-present-bar, .ts-slideshow [class*="bar"]',
            );
            return b
              ? {
                  cls: b.className,
          ...
          const barBtn = await probe(
            page,
            '.ts-slideshow .ts-present-bar .pt-ib, .ts-slideshow button',
          );
          await press(page, 'Escape');
          await sleep(800);
          return {
            ...
            barShown,
            bar: bar
              ? { bg: bar.background, border: bar.border, radius: bar.radius, h: bar.height }
              : null,
            barBtn: barBtn
              ? {
                  size: `${barBtn.box.w}x${barBtn.box.h}`,
                  color: barBtn.colorHex,
                  contrast: barBtn.textContrast,
                  tip: barBtn.tip,
                  title: barBtn.title,
                }
              : null,
            backInEditor: await has(page, '.ts-menubar'),
          };
        });
        await step(table, 'show', `${tag} the presenter view /present/<id>`, async () => {
          ...
          for (let i = 0; i < 60; i += 1) {
            sk = await page
              .evaluate(() => document.querySelector('.ts-skeleton') !== null)
              .catch(() => false);
            if (sk) break;
            if (
              await page
                .evaluate(() => document.querySelector('.ts-presenter:not(.ts-skeleton)') !== null)
                .catch(() => false)
            )
              break;
            await sleep(30);
          ...
          await page
            .waitForSelector('.ts-presenter:not(.ts-skeleton), [data-control^="presenter"]', {
              timeout: 30_000,
            })
            .catch(() => undefined);
          await sleep(1500);
          ...
          const facts = await page.evaluate(() => {
            const vis = (e) => e.getClientRects().length > 0;
            const ctrls = [...document.querySelectorAll('[data-control]')].filter(vis).map((e) => {
              const r = e.getBoundingClientRect();
            ...
            const texts = [...document.querySelectorAll('h1,h2,h3,p,span,b,time')]
              .filter(vis)
              .map((e) => ({
                tag: e.tagName.toLowerCase(),
                cls: e.className.toString().split(' ')[0],
                text: e.textContent.trim().slice(0, 40),
                font: getComputedStyle(e).fontSize + '/' + getComputedStyle(e).fontWeight,
            ...
            return {
              ...
              root:
                document.querySelector('.ts-presenter')?.className ??
          ...
          const tabs = await probeAll(
            page,
            '.ts-presenter-tabs button, .ts-presenter-tabs .pt-ib, [role="tab"]',
            6,
          );
          const notes = await probe(
            page,
            '.ts-presenter-notes, [data-control="presenter.notes"], .ts-presenter [class*="notes"]',
          );
          ...
          return {
            ...
            skeleton: sk,
            readyMs: Date.now() - t0,
            button: btn ? { size: `${btn.box.w}x${btn.box.h}`, contrast: btn.textContrast } : null,
            tabs: tabs.map((t) => ({
              text: t.text,
              pressed: t.pressed,
              selected: t.ariaLabel,
              font: t.font,
            })),
            notes: notes
              ? {
                  font: notes.font,
                  color: notes.colorHex,
                  contrast: notes.textContrast,
                  text: notes.text.slice(0, 60),
                }
              : null,
        ...
        await step(table, 'show', `${tag} the viewer /deck/<id>`, async () => {
          ...
          await page
            .waitForSelector('.pt-viewer[data-settled], .pt-toolbar', { timeout: 60_000 })
            .catch(() => undefined);
          await sleep(1200);
          await S('viewer');
          const bar = await probe(page, '.pt-toolbar');
          const ctrls = await page.evaluate(() =>
            [...document.querySelectorAll('.pt-toolbar [data-control], .pt-toolbar .pt-ib')]
              .filter((e) => e.getClientRects().length > 0)
              .map((e) => ({
                id: e.getAttribute('data-control') ?? e.className.split(' ').slice(-1)[0],
                text: e.textContent.trim().slice(0, 20),
      ...
      await step(table, 'motion', `${tag} the transitions the chrome declares`, async () =>
        page.evaluate(() => {
          const read = (sel) => {
            ...
            if (!e) return null;
            const cs = getComputedStyle(e);
            return `${cs.transitionProperty} ${cs.transitionDuration}${cs.animationName !== 'none' ? `; anim ${cs.animationName} ${cs.animationDuration}` : ''}`;
          };
          return {
            button: read('.ts-toolbar .pt-ib'),
            cardFrame: read('.ts-card-frame'),
            filmstrip: read('.pt-sb'),
            menubarTitle: read('.ts-menubar-title'),
            titleName: read('[data-control="deck.name"]'),
            stage: read('.pt-stagewrap'),
            slide: read('.pt-slide'),
            notesHandle: read('.ts-notes-handle'),
      ...
      table.add(
        'run',
        `${tag} ended early`,
        `${error instanceof Error ? error.message.split('\n')[0] : String(error)}; page closed ${page.isClosed()}, browser connected ${browser.isConnected()}`,
        { error: true },
      );
    }
    saveState({ deckId, objects });
  ...
  if (DESTROY) {
    ...
    const context = await newContext(b2, {
      width: 1440,
      height: 900,
      theme: 'light',
