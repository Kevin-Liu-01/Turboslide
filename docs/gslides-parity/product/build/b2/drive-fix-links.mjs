...
const errorsSince = (n) =>
  consoleErrors.slice(n).filter((e) => !/CSP|Content Security|Fast Refresh|hmr|\[vite\]|\[Server\]/i.test(e));
...
const anchorsOf = (run) =>
  page.evaluate((r) => {
    ...
    if (!el) return null;
    return [...el.querySelectorAll('a')].map((a) => ({ href: a.getAttribute('href'), text: a.textContent }));
  }, run);
...
async function sequence(label, run, stored, { single }) {
  const trace = [];
  const note = async (at) =>
    trace.push({ at, editing: await t.editing(), anchors: await anchorsOf(run), text: await textOf(run), stored: await stored() });
  await t.openRun(run);
  ...
  if (!point) return { ok: false, observed: `no Renewal on the sheet; trace ${JSON.stringify(trace)}` };
  await t.dblclickAt(point.x, point.y);
  await sleep(300);
  const selected = await t.selectionText();
  await t.press('Meta+k');
  await popoverField().waitFor({ timeout: 6000 }).catch(() => undefined);
  const fieldValue = await popoverField().inputValue().catch(() => null);
  await popoverField().click().catch(() => undefined);
...
try {
  ...
  await t.placeBlock(S, { id: 'hd', type: 'heading', level: 'h2', text: 'Title here', pos: { x: 160, y: 140, w: 900, h: 90, z: 5 } });
  ...
  await t.step(
    ...
    () => sequence('heading block', hdRun, async () => (await t.blockOf(S, 'hd'))?.block.text ?? null, { single: true }),
  );
  await t.shot('heading-block-after');
  await t.clearAll();

  /* a body paragraph: the control case (Enter breaks the line) */
  await t.placeBlock(S, { id: 'tb', type: 'text', text: 'Body here', pos: { x: 160, y: 320, w: 900, h: 120, z: 6 } });
  ...
  await t.step(
    ...
    async () => {
      const r = await sequence('body paragraph', tbRun, async () => (await t.blockOf(S, 'tb'))?.block.text ?? null, { single: false });
      const stored = (await t.blockOf(S, 'tb'))?.block.text ?? '';
      return {
        ok: /\[sales@acme\.com\]\(mailto:sales@acme\.com\)\n\[Renewal\]\(https:\/\/acme\.com\/renewal\)/.test(stored) && !/\)l\]\(/.test(stored),
  ...
  await t.step(
    ...
    () => sequence('title heading', headRun, async () => (await t.slideJson(deck.titleSlide))?.heading ?? null, { single: true }),
  );
