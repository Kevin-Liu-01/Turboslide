...
const dialogFacts = async (page, tag, name, open, S) => {
  ...
  const actions = await probeAll(
    page,
    `${ROOT} .ts-dialog-actions button, ${ROOT} .ts-dialog-actions .pt-ib`,
    6,
  );
  const label = await probe(page, `${ROOT} .ts-dialog-field-label`);
  const input = await probe(
    page,
    `${ROOT} input:not([type="checkbox"]):not([type="radio"]), ${ROOT} select`,
  );
  ...
  const facts = await page.evaluate((s) => {
    ...
    return {
      ...
      controls: [...root.querySelectorAll('[data-control]')]
        ...
        .slice(0, 50),
      scrollRegions: [...root.querySelectorAll('*')]
        .filter(
          (e) =>
            e.scrollHeight > e.clientHeight + 2 &&
            /auto|scroll/.test(getComputedStyle(e).overflowY),
        )
        .map((e) => `${e.className.toString().split(' ')[0]} ${e.clientHeight}/${e.scrollHeight}`),
      ...
      tipOnOpen: tipShown
        ? {
            name: tip.querySelector('.pt-tip-name')?.textContent,
            overDialog: tipBox.bottom > rootBox.top && tipBox.top < rootBox.bottom,
          }
        : null,
  ...
  return {
    dialog: d
      ...
      : null,
    title: title
      ? { text: title.text, font: title.font, letterSpacing: title.letterSpacing }
      : null,
    lead: lead
      ? {
          text: lead.text.slice(0, 120),
          font: lead.font,
          color: lead.colorHex,
          contrast: lead.textContrast,
        }
      : null,
    x: x
      ? {
    ...
    label: label
      ? { text: label.text, font: label.font, color: label.colorHex, contrast: label.textContrast }
      : null,
    input: input
      ? {
          h: input.height,
    ...
    check: check
      ? {
          size: `${check.box.w}x${check.box.h}`,
          border: check.borderHex,
          borderContrast: check.borderContrast,
        }
      : null,
    ...facts,
    escapeCloses,
  ...
  for (const run of [
    ...
    await step(table, 'keys', `${tag} the ? key and Cmd+/`, async () => {
      ...
      const after = await page.evaluate(() => ({
        dialog:
          document.querySelector('.ts-dialog-scrim [role="dialog"] .ts-dialog-title')
            ?.textContent ?? null,
        help: document.querySelector('.pt-help-card') !== null,
        snackbar:
          document.querySelector('.ts-snackbar.is-on, .pt-toast.is-on')?.textContent?.trim() ??
          null,
      }));
      ...
      await press(page, 'Escape', 2);
      const snackLife = await (async () => {
        const t0 = Date.now();
        while (Date.now() - t0 < 12_000) {
          const on = await page.evaluate(
            () => document.querySelector('.ts-snackbar.is-on') !== null,
          );
    ...
    if (textId) {
      await step(table, 'tails', `${tag} Bold pressed state and the colour plate`, async () => {
        ...
        const strong = await page.evaluate(
          () =>
            document.querySelector(
              '.ts-stagewrap.ts-editor .pt-slide [data-block] b, .ts-stagewrap.ts-editor .pt-slide [data-block] strong, .ts-stagewrap.ts-editor .pt-slide [data-block] [data-mark]',
            ) !== null,
        );
        ...
        const strong2 = await page.evaluate(
          () =>
            document.querySelector(
              '.ts-stagewrap.ts-editor .pt-slide [data-block] b, .ts-stagewrap.ts-editor .pt-slide [data-block] strong',
            ) !== null,
        );
        // the colour plate
        ...
        const plate = await page.evaluate(() => {
          const fixed = [...document.querySelectorAll('body *')].filter((e) => {
            ...
            return (
              ...
              e.closest('.ts-toolbar') === null &&
              e.closest('.pt-stagewrap') === null &&
              !e.closest('.pt-sb')
            );
          });
          const p =
            fixed.find((e) => e.querySelector('button, [role="option"], [data-control]')) ??
            fixed[0];
          if (!p) return null;
          const r = p.getBoundingClientRect();
          const tiles = [...p.querySelectorAll('button')].filter(
            (b) => b.getBoundingClientRect().width < 48,
          );
          ...
          return {
            ...
            tiles: tiles.length,
            tile: tiles[0]
              ? `${Math.round(tiles[0].getBoundingClientRect().width)}x${Math.round(tiles[0].getBoundingClientRect().height)}`
              : null,
            tileLabel:
              tiles[0]?.getAttribute('aria-label') ??
              tiles[0]?.getAttribute('title') ??
              tiles[0]?.getAttribute('data-tip') ??
              null,
        ...
        if (plate)
          await S('toolbar-color-plate', {
            x: Math.max(0, tc.x - 60),
            y: 72,
            width: 420,
            height: Math.min(V.h - 72, plate.h + 80),
          });
        await press(page, 'Escape', 3);
        return {
          beforePressed: before?.pressed,
          afterCmdB: {
            pressed: on?.pressed,
            color: on?.colorHex,
            border: on?.borderHex,
          ...
          afterClick: { pressed: afterClick?.pressed, strong: strong2 },
          plate,
        };
      });
    }
