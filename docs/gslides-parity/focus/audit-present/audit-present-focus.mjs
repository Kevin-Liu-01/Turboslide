...
const invoke = (page, action, input = {}) => page.evaluate(([id, v]) => window.turboslide.studio.invoke(id, v), [action, input]);
...
const runs = (page) => page.evaluate(() => [...document.querySelectorAll('.ts-stagewrap.ts-editor .pt-slide:not(.is-leaving) [data-run]')].map((el) => el.getAttribute('data-run')));
...
const snackbar = (page) =>
  page.evaluate(() => [...document.querySelectorAll('.ts-snackbar, .pt-toast, [role="status"]')].map((el) => el.textContent?.trim() ?? '').filter(Boolean).join(' | ') || null);
...
const page = A.page;
const shot = async (p, name) => p.screenshot({ path: path.join(SHOTS, `${name}.png`) }).catch(() => undefined);
const since = (n, test) =>
  responses
    .slice(n)
    .filter(test)
    .map((r) => `${r.tag} ${r.url} ${r.status}${r.body ? ` ${JSON.stringify(r.body).slice(0, 200)}` : ''}`);
const others = [];
...
try {
  ...
  headRun = await headRunOf(page);
  {
    ...
    for (let i = 0; i < 2; i += 1) {
      ...
      await pollUntil(async () => (await slideOrder(page)).length, (n) => n > before, 6000);
      await settled(page);
    }
    await clearAll(page);
    record('Scratch deck', 'Open /new, type a title, add two slides', 'works', `${deckId}; title "${await headingText(page)}"; slides ${(await slideOrder(page)).length}; revision ${(await state(page)).revision}`);
  }
  ...
  const versionFacts = () =>
    page.evaluate(() => ({
      windows: [...document.querySelectorAll('[data-control^="versionHistory.window."]')].map((el) => `${el.getAttribute('data-control')}[${el.getAttribute('aria-expanded') ?? '?'}]`),
      picks: [...document.querySelectorAll('[data-control^="versionHistory."][data-control$=".pick"]')].map((el) => el.getAttribute('data-control')),
      restores: [...document.querySelectorAll('[data-control^="versionHistory."][data-control$=".restore"]')].map((el) => el.getAttribute('data-control')),
      selected: [...document.querySelectorAll('[data-control="panel.versionHistory"] [aria-selected="true"], [data-control="panel.versionHistory"] .is-picked, [data-control="panel.versionHistory"] .is-selected')].map((el) => el.getAttribute('data-control') ?? el.className),
      changes: document.querySelectorAll('[data-control="versions.change"], [data-control="versions.changeRun"]').length,
      text: document.querySelector('[data-control="panel.versionHistory"]')?.textContent?.trim().slice(0, 300) ?? '',
    }));
  const expandAll = async () => {
    for (let round = 0; round < 3; round += 1) {
      ...
      for (let i = 0; i < n; i += 1) await clickLocator(page, windows.nth(i)).catch(() => undefined);
      await sleep(500);
    }
  };
  ...
  await attempt('Version history', 'Open Version history and expand the day window', async () => {
    ...
    return { ok: f.picks.length >= 1 && f.restores.length >= 1, evidence: `opened via ${via}; windows ${f.windows.join(', ')}; picks ${f.picks.length}; restores ${f.restores.length}; text "${f.text.slice(0, 160)}"` };
  });
  await attempt('Version history', 'Pick a version and read Show changes', async () => {
    ...
    if (showChanges === false && (await has(page, '[data-control="versionHistory.showChanges"]'))) {
      await clickControl(page, 'versionHistory.showChanges.row').catch(() => clickControl(page, 'versionHistory.showChanges'));
      await sleep(1200);
      marks = (await versionFacts()).changes;
    }
    await shot(page, '23-version-picked');
    return { ok: f1.selected.length >= 1, evidence: `picked ${f0.picks[f0.picks.length - 1]}; selected ${JSON.stringify(f1.selected)}; Show changes was ${showChanges}; change marks after turning it on ${marks}` };
  });
  await attempt('Version history', 'Name current version', async () => {
    ...
    await press(page, 'Enter');
    const toast = await pollUntil(() => snackbar(page), (x) => Boolean(x), 6000);
    await sleep(800);
    const f = await pollUntil(versionFacts, (x) => /Before the call/.test(x.text), 10_000);
    await shot(page, '24-version-named');
    return { ok: /Before the call/.test(f.text), evidence: `snackbar ${JSON.stringify(toast)}; panel text has "Before the call" ${/Before the call/.test(f.text)}; windows ${f.windows.join(', ')}` };
  });
  const restoreRow = (interaction, which, shotName) =>
    attempt('Version history', interaction, async () => {
      ...
      await expandAll();
      const f = await versionFacts();
      if (f.restores.length === 0) return { ok: false, evidence: `no Restore control; windows ${f.windows.join(', ')}` };
      const control = which === 'oldest' ? f.restores[f.restores.length - 1] : f.restores[Math.min(1, f.restores.length - 1)];
      ...
      return { ok: after.rev > before.rev && changed, evidence: `restore control ${control} of ${f.restores.length}; title "${before.title}" -> "${after.title}"; slides ${before.slides} -> ${after.slides}; revision ${before.rev} -> ${after.rev}; snackbar ${JSON.stringify(toast)}; answers ${JSON.stringify(since(n0, (r) => r.status >= 400))}` };
    });
  const restored = await restoreRow('Restore the previous version (owner only history)', 'previous', '25-restore-owner-only');
  ...
  } else record('Version history', 'Cmd+Z undoes the restore', 'not driven', 'the restore did not succeed, so there was nothing to undo');
  ...
  const cardFacts = (p) =>
    p.evaluate(() => ({
      ...
      submitDisabled: document.querySelector('[data-control="comment.card.new.submit"]')?.disabled ?? null,
  ...
  const addComment = async (open) => {
    ...
    await open();
    let f = await pollUntil(() => cardFacts(page), (x) => x.newField, 5000);
    if (!f.newField) return { ok: false, evidence: `chip "${chip}"; no comment card; snackbar ${JSON.stringify(await snackbar(page))}` };
    ...
    const toast = await pollUntil(() => snackbar(page), (x) => Boolean(x), 6000);
    const after = await pollUntil(() => cardFacts(page), (x) => x.markers >= 1, 30_000);
    await settled(page);
    ...
    const answers = since(n0, (r) => r.status >= 400 || /comment/.test(r.url) || /_server|actions|ops/.test(r.url));
    return {
  ...
  await attempt('Comments', 'Select the title block, Cmd+Option+M, type, Comment', () => addComment(() => press(page, 'Meta+Alt+m')));
  if (!commented) await attempt('Comments', 'Select the title block, toolbar Insert comment, type, Comment', () => addComment(() => clickControl(page, 'toolbar.insertComment')));
  if (!commented)
  ...
  await attempt('Comments', 'Resolve the thread', async () => {
    ...
    await closePanel();
    return { ok: label !== null && after.markers === 0, evidence: `resolve button "${label}"; markers after ${after.markers}; panel threads ${panel.threads}, list "${panel.text.slice(0, 120)}"` };
  });
  ...
  await editorReady(C.page);
  {
    ...
    for (let i = 1; i <= 3; i += 1) {
      ...
      const t0 = Date.now();
      const ownC = await pollUntil(async () => (await slideOrder(C.page)).length, (n) => n > beforeC, 6000);
      await answerNamePrompt(C.page, 'Editor tab');
      const after = await pollUntil(async () => (await slideOrder(page)).length, (n) => n > before, 45_000);
      const ms = Date.now() - t0;
      lat.push({ i, ownC: `${beforeC} -> ${ownC}`, owner: `${before} -> ${after}`, ms: after > before ? ms : `not within ${ms}` });
      await sleep(1500);
    }
    const okAll = lat.every((l) => typeof l.ms === 'number' && l.ms <= 10_000);
    const anyLate = lat.some((l) => typeof l.ms !== 'number' || l.ms > 10_000);
    record('Collaboration', 'Three slides added in the second browser (Ctrl+M), each timed until the owner filmstrip shows it', okAll ? 'works' : anyLate && lat.some((l) => typeof l.ms === 'number') ? 'flaky' : 'broken', JSON.stringify(lat));
  }
  await attempt('Collaboration', 'The second browser types into the title; timed until the owner sees it', async (t) => {
    ...
    const t0 = Date.now();
    const seen = await pollUntil(() => headingText(page), (x) => (x ?? '').includes(`+C${t}`), 45_000);
    return { ok: (seen ?? '').includes(`+C${t}`), evidence: `owner title "${seen}" after ${Date.now() - t0} ms` };
  });
  await restoreRow('Restore the oldest version after the second browser edited', 'oldest', '29-restore-after-collab');
  await restoreRow('Restore the previous version after the second browser edited', 'previous', '29b-restore-previous-after-collab');
} catch (error) {
  record('The focused run ran to completion', 'no exception outside a step', 'broken', error instanceof Error ? (error.stack ?? error.message) : String(error));
} finally {
  ...
  if (deckId) {
    ...
    record('Teardown', `GET /edit/${deckId} after the delete`, status === 404 ? 'works' : 'broken', `status ${status}`);
  }
  await browser.close().catch(() => undefined);
  const summary = { base: BASE, deckId, startedAt: new Date(startedAt).toISOString(), ms: Date.now() - startedAt, rows, consoleErrors, responses: responses.filter((r) => r.status >= 400 || /comment|version/.test(r.url)).slice(0, 80) };
