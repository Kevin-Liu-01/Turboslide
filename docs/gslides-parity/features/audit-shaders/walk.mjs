...
const step = async (name, expected, fn) => {
  try {
    ...
    record(name, expected, `error: ${error instanceof Error ? error.message : String(error)}`, false);
...
const openMenu = async (page, id) => {
  ...
  await page.locator(`${menuRoot(id)} [data-control^="menu."]`).first().waitFor({ timeout: 4000 }).catch(() => undefined);
...
const menuRows = (page, id) =>
  page.evaluate(
    (root) => [...document.querySelectorAll(`${root} [data-control^="menu."]`)].map((el) => el.getAttribute('data-control').replace(/^menu\./, '')),
    menuRoot(id),
  );
...
const setAdvanced = async (page, on) => {
  ...
  await pollUntil(() => advancedOn(page), (x) => x === on, 8000);
...
const canvasFacts = (page) =>
  page.evaluate(() => {
    ...
    return all.map((c) => {
      ...
      const inFilm = Boolean(c.closest('.ts-filmstrip, [class*="filmstrip"], [class*="Filmstrip"]'));
      return { w: c.width, h: c.height, cssW: Math.round(r.width), cssH: Math.round(r.height), inStage, inFilm, parent: c.parentElement?.className ?? '' };
    });
  });
      ...
      new Promise((resolve) => {
        ...
        const tick = () => {
          ...
          if (performance.now() - start < dur) requestAnimationFrame(tick);
            ...
            observer?.disconnect();
            resolve({ fps: Math.round((frames * 1000) / dur), longTasks, longMs: Math.round(longMs), observed: observer !== null });
          }
        };
...
try {
  ...
  await step('Tools > Advanced tools on, then Insert menu', 'a Material row at the foot of Insert', async () => {
    ...
    return { ok: on && ids.includes('insert.material'), observed: `advanced ${on}; row present ${ids.includes('insert.material')}; row text "${label.trim()}"; ${ids.length} rows` };
  });

  let materialBlockId = null;
  await step('Insert > Material opens a dialog of names', 'the Insert material dialog: a list of catalog rows with preset counts and no thumbnails', async () => {
    ...
    return { ok: facts.rows.length > 0, observed: `title "${facts.title}"; lead "${facts.lead}"; ${facts.rows.length} rows; ${facts.images} pictures or canvases; ${facts.search} search fields; rows: ${facts.rows.join(' | ')}` };
  });

  await step('pick Liquid metal', 'a material block on the slide, its recipe with the first preset', async () => {
    ...
    const objs = await pollUntil(() => objectsOf(page, titleSlide), (o) => o.some((x) => !before.includes(x.id) && x.type === 'material'), 20_000);
    const block = objs.find((x) => !before.includes(x.id) && x.type === 'material') ?? null;
    const others = objs.filter((x) => !before.includes(x.id) && x.type !== 'material').map((x) => `${x.id} ${x.type}`);
    materialBlockId = block?.id ?? null;
    const tBlock = Date.now() - t0;
    await page.locator('.ts-stagewrap.ts-editor [data-recipe] canvas').first().waitFor({ timeout: 30_000 }).catch(() => undefined);
    const tCanvas = Date.now() - t0;
    await settled(page);
    await sleep(1200);
    return { ok: block !== null, observed: block ? `${block.id} ${block.type} pos ${JSON.stringify(block.pos)} recipe ${JSON.stringify({ materialId: block.block.materialId, preset: block.block.preset, uniforms: block.block.uniforms, anchor: block.block.anchor, asset: block.block.asset })}; the block in the document after ${tBlock} ms, its canvas on the stage after ${tCanvas} ms; other objects that gained a pos: ${others.join(', ') || 'none'}` : 'no new material object' };
  });
  ...
  await step('material.list on production', 'the catalog entries with availability', async () => {
    ...
    const uniforms = Object.fromEntries(list.filter((e) => e.available).map((e) => [e.id, e.uniforms.length]));
  ...
  await step('the material block on the sheet', 'the frozen frame; today the plate ground with a titanium label and a live canvas over it', async () => {
    const facts = await page.evaluate(() => {
      ...
      return { label, canvas: canvas ? { w: canvas.width, h: canvas.height } : null, img: Boolean(img), live: fig.getAttribute('data-live') };
    });
    await shot('s04-material-block-inserted');
    const card = page.locator(`[data-control="filmstrip.slide.${titleSlide}"]`).first();
    await card.screenshot({ path: path.join(OUT, 's04c-filmstrip-card.png') }).catch(() => undefined);
    const cardFacts = await card.evaluate((el) => ({ canvases: el.querySelectorAll('canvas').length, labels: [...el.querySelectorAll('.material-label')].map((l) => l.textContent) })).catch(() => null);
    facts.card = cardFacts;
    return { ok: facts !== null, observed: facts ? `label "${facts.label}"; canvas ${JSON.stringify(facts.canvas)}; img ${facts.img}; data-live ${facts.live}; filmstrip card ${JSON.stringify(facts.card)}` : 'no [data-recipe] root in the stage' };
  });

  await step('the live canvas animates in the editor', 'two screenshots of the block 900 ms apart differ', async () => {
    ...
    return { ok: !a.equals(b), observed: `frames differ ${!a.equals(b)}; block box ${Math.round(r.w)} by ${Math.round(r.h)} css px` };
  });

  await step('frame cadence with one shader mounted', 'rAF near 60 per second, few long tasks', async () => {
    const c = await cadence(page, 3000);
    const canvases = await canvasFacts(page);
    return { ok: c.fps >= 50, observed: `${c.fps} rAF per second over 3 s; long tasks ${c.longTasks} (${c.longMs} ms)${c.observed ? '' : ', longtask observer unavailable'}; canvases on the page ${canvases.length}: ${canvases.map((k) => `${k.w}x${k.h}${k.inStage ? ' stage' : ''}${k.inFilm ? ' filmstrip' : ''}`).join(', ')}` };
  });

  await step('select the material block and read the toolbar', 'the block selected by one click; the toolbar tail for a material', async () => {
    ...
    const tail = await page.evaluate(() => [...document.querySelectorAll('[data-control^="toolbar."]')].map((el) => el.getAttribute('data-control')).filter((id) => !id.startsWith('toolbar.font') && !id.startsWith('toolbar.textColor')).join(', '));
    const handles = await page.evaluate(() => [...document.querySelectorAll('[aria-label*=": Resize "]')].map((el) => el.getAttribute('aria-label')));
    await shot('s05-material-selected');
    return { ok: chip !== null, observed: `chip "${chip}"; handles ${handles.length}: ${handles.slice(0, 2).join(', ')}; toolbar controls: ${tail.slice(0, 600)}` };
  });

  await step('drag the material block by 120 by 80 sheet px', 'pos.x and pos.y move by about 120 and 80', async () => {
    ...
    return { ok: Math.abs(dx - 120) <= 6 && Math.abs(dy - 80) <= 6, observed: `moved ${Math.round(dx)} by ${Math.round(dy)}; before ${JSON.stringify(before.pos)} after ${JSON.stringify(after.pos)}` };
  });

  await step('resize the material block by its south east handle', 'the box grows and the canvas follows', async () => {
    const before = (await objectsOf(page, titleSlide)).find((o) => o.id === materialBlockId);
    ...
    const canvas = await page.evaluate(() => {
      ...
      return c ? { w: c.width, h: c.height, cssW: Math.round(c.getBoundingClientRect().width) } : null;
    });
    await shot('s05b-material-resized');
    return { ok: after.pos.w > before.pos.w, observed: `before ${before.pos.w}x${before.pos.h} after ${after.pos.w}x${after.pos.h}; canvas ${JSON.stringify(canvas)}` };
  });

  // ---- 5. Format options, the Material section
  let materialControls = [];
  await step('Format > Format options with the material selected', 'the panel with a Material section: material, preset, the uniform rows, anchor, two-tone, plate, Capture frame', async () => {
    await openMenu(page, 'format');
    await clickControl(page, 'menu.format.formatOptions');
    await sleep(800);
    const base = `block.${materialBlockId}`;
    await page.locator(`[data-control="${base}.materialId"]`).waitFor({ timeout: 8000 }).catch(() => undefined);
    const facts = await page.evaluate((b) => {
      const els = [...document.querySelectorAll(`[data-control^="${b}."]`)];
      const rows = els.map((el) => ({ id: el.getAttribute('data-control').slice(b.length + 1), tag: el.tagName.toLowerCase(), type: el.getAttribute('type') ?? '', value: el.value ?? el.textContent?.trim().slice(0, 30) ?? '' }));
      ...
      const headings = [...document.querySelectorAll('[data-control^="formatOptions."], .ts-insp-section-title, button[aria-expanded]')].map((el) => el.textContent?.trim().slice(0, 40)).filter(Boolean);
      return { rows, sectionHeight: r ? Math.round(r.height) : null, sectionTop: r ? Math.round(r.top) : null, panelHeight: pr ? Math.round(pr.height) : null, sliders, numbers, texts, colorInputs, selects, headings: headings.slice(0, 30) };
    }, base);
    ...
    if (await sec.isVisible().catch(() => false)) {
      ...
      await sleep(300);
      await sec.screenshot({ path: path.join(OUT, 's06b-material-section-1440.png') }).catch(() => undefined);
    }
    return {
  ...
  await step('type a uniform value and read the canvas', 'Repetition 3 to 6 writes /uniforms and the live shader changes', async () => {
    const base = `block.${materialBlockId}`;
    const field = ctl(page, `${base}.uniforms.u_repetition`).first();
    if (!(await field.isVisible().catch(() => false))) return { ok: null, observed: 'not driven: no u_repetition field' };
    const r = await rectOf(page, '.ts-stagewrap.ts-editor [data-recipe] .material');
    const clip = { x: Math.max(0, r.x), y: Math.max(0, r.y), width: Math.max(1, r.w), height: Math.max(1, r.h) };
    const before = (await objectsOf(page, titleSlide)).find((o) => o.id === materialBlockId);
    ...
    return { ok: after.block.uniforms?.u_repetition === 6, observed: `uniforms before ${JSON.stringify(before.block.uniforms)} after ${JSON.stringify(after.block.uniforms)}; revision ${(await state(page)).revision}` };
  });

  await step('change the preset from the Preset select', 'Chrome (Paper default) writes /preset and drops nothing else', async () => {
    const base = `block.${materialBlockId}`;
    const sel = ctl(page, `${base}.preset`).first();
    if (!(await sel.isVisible().catch(() => false))) return { ok: null, observed: 'not driven: no preset select' };
    const options = await sel.evaluate((el) => [...el.options].map((o) => `${o.value}:${o.textContent}`));
    await sel.selectOption('chrome');
    await settled(page);
    await sleep(1000);
    const after = (await objectsOf(page, titleSlide)).find((o) => o.id === materialBlockId);
    await shot('s06d-after-preset-chrome');
    return { ok: after.block.preset === 'chrome', observed: `options ${options.join(' | ')}; preset after ${after.block.preset}; uniforms ${JSON.stringify(after.block.uniforms)}` };
  });

  await step('the Material section at 1280 by 800', 'the section fits the panel without clipping', async () => {
    await page.setViewportSize({ width: 1280, height: 800 });
    await sleep(900);
    const facts = await page.evaluate(() => {
      const section = document.querySelector('.ts-material');
      const r = section?.getBoundingClientRect();
      const fields = [...document.querySelectorAll('.ts-material input, .ts-material select')].map((el) => {
        const b = el.getBoundingClientRect();
        return { right: Math.round(b.right), w: Math.round(b.width) };
      });
      return { height: r ? Math.round(r.height) : null, right: r ? Math.round(r.right) : null, overflow: fields.filter((f) => f.right > window.innerWidth).length, fieldWidths: [...new Set(fields.map((f) => f.w))] };
    });
    await shot('s07-format-options-material-1280');
    await page.setViewportSize({ width: 1440, height: 900 });
    await sleep(600);
    return { ok: facts.overflow === 0, observed: `section ${facts.height} px tall, right edge ${facts.right}; ${facts.overflow} fields past the viewport; field widths ${facts.fieldWidths.join(', ')}` };
  });

  // ---- 6. Capture frame through the hosted job
  await step('Capture frame runs the hosted material.capture', 'a frozen frame asset set as the block frame within 90 s', async () => {
    const base = `block.${materialBlockId}`;
    const btn = ctl(page, `${base}.capture`).first();
    if (!(await btn.isVisible().catch(() => false))) return { ok: null, observed: 'not driven: no Capture frame button' };
    const t0 = Date.now();
    ...
    const result = await pollUntil(
      async () => {
        ...
        const err = await page.evaluate(() => document.querySelector('.ts-material-note[role="alert"]')?.textContent ?? null);
    ...
    const facts = await page.evaluate(() => {
      const fig = document.querySelector('.ts-stagewrap.ts-editor [data-recipe]');
      const img = fig?.querySelector('img');
      return { img: img ? { src: (img.currentSrc || img.src).slice(0, 120), w: img.naturalWidth, h: img.naturalHeight } : null, label: fig?.querySelector('.material-label')?.textContent ?? null, canvas: Boolean(fig?.querySelector('canvas')) };
    });
    await shot('s08-after-capture');
    return { ok: result.asset !== null, observed: `${Date.now() - t0} ms; asset ${result.asset}; button "${result.label}"; error ${result.err}; frame ${JSON.stringify(facts)}` };
  });

  // ---- 7. Slide > Change background, the Material row
  await step('Slide > Change background, the Material row', 'a Choose list of the catalog and a Place button; Dither toggle beside', async () => {
    await press(page, 'Escape', 2);
    await openMenu(page, 'slide');
    await clickControl(page, 'menu.slide.changeBackground');
    await page.locator('[data-control="dialog.background.material.choose"]').waitFor({ timeout: 8000 });
    await sleep(400);
    await shot('s09-background-dialog');
    await clickControl(page, 'dialog.background.material.choose');
    await sleep(500);
    const facts = await page.evaluate(() => {
      const rows = [...document.querySelectorAll('[data-control^="dialog.background.material."]')].map((el) => `${el.getAttribute('data-control').replace('dialog.background.material.', '')}="${el.textContent?.trim().replace(/\s+/g, ' ').slice(0, 40)}"`);
      const pics = document.querySelectorAll('[data-control^="dialog.background.material."] img, [data-control^="dialog.background.material."] canvas').length;
      return { rows, pics };
    });
    await shot('s09b-background-material-list');
    return { ok: facts.rows.length > 2, observed: `${facts.rows.length} material controls, ${facts.pics} previews; ${facts.rows.slice(0, 40).join(', ')}` };
  });

  await step('Place a gem smoke background', 'slide.setBackgroundMaterial captures a frame server side and places a covering picture at the back', async () => {
    const pick = page.locator('[data-control="dialog.background.material.paper:gem-smoke.brand-blue"]').first();
    if (!(await pick.isVisible().catch(() => false))) return { ok: null, observed: 'not driven: no gem smoke row' };
    ...
    await sleep(600);
    const chooseText = await ctl(page, 'dialog.background.material.choose').first().textContent().catch(() => null);
    const t0 = Date.now();
    ...
    if (!plr) {
      const controls = await page.evaluate(() => [...document.querySelectorAll('[data-control^="dialog.background."]')].map((el) => el.getAttribute('data-control')).filter((id) => !id.includes('material.paper')).join(', '));
      return { ok: false, observed: `no visible Place button after choosing; Choose reads "${chooseText}"; dialog controls: ${controls}` };
    }
    if (placeDisabled) return { ok: false, observed: `Place is disabled after choosing; Choose reads "${chooseText}"` };
    const before = (await objectsOf(page, titleSlide)).map((o) => o.id);
    await clickAt(page, plr.x + plr.width / 2, plr.y + plr.height / 2);
    const objs = await pollUntil(() => objectsOf(page, titleSlide), (o) => o.some((x) => !before.includes(x.id)), 90_000, 1000);
    const placed = objs.find((x) => !before.includes(x.id)) ?? null;
    await sleep(1500);
    const err = await page.evaluate(() => document.querySelector('[role="alert"]')?.textContent ?? null);
    await shot('s10-background-material-placed');
    return { ok: placed !== null, observed: `${Date.now() - t0} ms; placed ${placed ? `${placed.id} ${placed.type} pos ${JSON.stringify(placed.pos)} asset ${placed.block.asset} dither ${JSON.stringify(placed.block.dither)}` : 'nothing'}; alert ${err}` };
  });

  await step('close the dialog and read the stage', 'the background frame under the material block; canvases on the page', async () => {
    if (await has(page, '[data-control="dialog.background.done"]')) await clickControl(page, 'dialog.background.done');
    else await press(page, 'Escape');
    await sleep(800);
    const canvases = await canvasFacts(page);
    const recipes = await page.evaluate(() => [...document.querySelectorAll('.ts-stagewrap.ts-editor [data-recipe]')].map((el) => `${el.tagName.toLowerCase()} ${el.getAttribute('data-block') ?? ''} ${el.getAttribute('data-recipe').slice(0, 60)}`));
    await shot('s11-stage-two-materials');
    return { ok: true, observed: `${recipes.length} recipe roots: ${recipes.join(' | ')}; canvases ${canvases.length}: ${canvases.map((k) => `${k.w}x${k.h}${k.inStage ? ' stage' : ''}`).join(', ')}` };
  });

  // ---- 8. off screen: a new slide, then back
  let secondSlide = null;
  await step('New slide, then the canvases left behind', 'the shader of the title slide is disposed when the stage shows another slide', async () => {
    await openMenu(page, 'slide');
    ...
    return { ok: secondSlide !== titleSlide, observed: `active ${secondSlide}; canvases ${canvases.length}: ${canvases.map((k) => `${k.w}x${k.h}${k.inStage ? ' stage' : ''}${k.inFilm ? ' film' : ''}`).join(', ')}; ${c.fps} rAF per second with no shader in view` };
  });

  // ---- 9. the slideshow
  await step('the slideshow on the title slide', 'the show; whether a shader plays or the frozen frame stands', async () => {
    await invoke(page, 'view.goto', { slideId: titleSlide }).catch(() => undefined);
    await sleep(800);
    await clickControl(page, 'present.open');
    await sleep(2500);
    const facts = await page.evaluate(() => {
      const present = document.querySelector('.ts-present, [data-present], .ts-slideshow, [class*="present"]');
      ...
      return { present: Boolean(present), url: location.pathname, canvases, recipes, labels, fullscreen: Boolean(document.fullscreenElement) };
    });
    const r = await rectOf(page, '[data-recipe] .material, [data-recipe]');
    let differ = null;
    if (r && r.w > 10) {
      const clip = { x: Math.max(0, r.x), y: Math.max(0, r.y), width: Math.max(1, Math.min(r.w, 1440 - r.x)), height: Math.max(1, Math.min(r.h, 900 - r.y)) };
      const a = await page.screenshot({ clip, type: 'png' });
      await sleep(900);
      const b = await page.screenshot({ clip, type: 'png' });
      differ = !a.equals(b);
    ...
    await press(page, 'Escape');
    await sleep(800);
    return { ok: facts.recipes > 0, observed: `path ${facts.url}; present root ${facts.present}; ${facts.recipes} recipe roots; canvases ${facts.canvases.join(', ') || 'none'}; labels ${facts.labels.join(' | ')}; frames differ ${differ}` };
  });

  // ---- 10. the exports through the window API
  const downloadUrls = [];
  page.on('download', (d) => downloadUrls.push(d.url()));
  for (const [format, extra] of [['pdf', {}], ['pptx', { mode: 'flatten' }]]) {
    await step(`export.run ${format} with a material block and a material background`, 'a report; what the material became', async () => {
      downloadUrls.length = 0;
      const t0 = Date.now();
      const result = await Promise.race([
        page.evaluate(async ([f, e]) => {
          const report = await window.turboslide.studio.invoke('export.run', { format: f, ...e });
          return JSON.stringify(report ?? null).slice(0, 1500);
        }, [format, extra]),
        sleep(150_000).then(() => 'timeout after 150 s'),
      ]);
      await pollUntil(() => Promise.resolve(downloadUrls.length), (n) => n > 0, 20_000, 200);
      ...
      if (await has(page, '[data-control="export.report.close"]')) await clickControl(page, 'export.report.close');
      writeFileSync(path.join(OUT, `export-${format}-report.json`), String(result));
      return { ok: !String(result).startsWith('timeout'), observed: `${Date.now() - t0} ms; download ${saved}; ${String(result).slice(0, 600)}` };
    });
  }

  // ---- 11. reduced motion in a second context
  await step('the editor under prefers-reduced-motion: reduce', 'whether the material canvas still animates', async () => {
    ...
    try {
      ...
      await p2.locator('.ts-stagewrap.ts-editor [data-recipe] canvas').first().waitFor({ timeout: 20_000 }).catch(() => undefined);
      await sleep(2500);
      ...
      const reduced = await p2.evaluate(() => matchMedia('(prefers-reduced-motion: reduce)').matches);
      ...
      return { ok: true, observed: `reduced motion ${reduced}; canvases ${canvases}; frames differ ${!a.equals(b)} (the shader ${!a.equals(b) ? 'still plays' : 'does not play'})` };
    } finally {
      await ctx2.close();
    }
  });
  ...
  await step('the shared viewer /deck/<id>', 'the frozen frames; no live canvas', async () => {
    ...
    try {
      ...
      const facts = await p3.evaluate(() => ({
        ...
        imgs: [...document.querySelectorAll('[data-recipe] img, .material img')].map((i) => `${i.naturalWidth}x${i.naturalHeight}`),
      }));
  ...
  record('console errors during the walk', 'none', `${consoleErrors.length}${consoleErrors.length ? `: ${[...new Set(consoleErrors)].slice(0, 4).join(' || ')}` : ''}`, consoleErrors.length === 0 ? true : null);
} catch (error) {
  record('the walk ran to completion', 'no exception outside a step', error instanceof Error ? (error.stack ?? error.message) : String(error), false);
} finally {
  if (deckId) {
    let trashed = false;
    try {
      ...
      record('the product trash path', 'File > Move to trash then Delete forever', `failed: ${error instanceof Error ? error.message : String(error)}; falling back to the actions API`, false);
    }
    if (!trashed) {
      try {
    ...
    for (const route of ['deck', 'edit']) {
      ...
      record(`the scratch deck answers 404 on /${route}`, `GET /${route}/${deckId} is 404 within 25 s`, `status ${status}`, status === 404);
    }
  }
  writeFileSync(path.join(OUT, 'walk.json'), JSON.stringify({ base: BASE, deckId, startedAt: new Date(started).toISOString(), ms: Date.now() - started, rows, consoleErrors: [...new Set(consoleErrors)].slice(0, 20) }, null, 2));
  await browser.close();
