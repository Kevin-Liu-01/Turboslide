...
import { FONT_CATEGORY_LABELS, FONT_PICKER, filterRows } from '../font-picker-model';
...
/**
 * More fonts (gslides-parity SPEC-5-amendments A5 item 4; SPEC-5 14.1 title; B7): Google's dialog
 * form. The left column searches the catalog and filters it by category, every face drawn in
 * itself with its licence line under the name; a click picks it. The right column, "In this
 * presentation", lists the families the presentation uses and the pick. OK applies the pick to the
 * selected block, Cancel closes. The rows are the `font.list` answer the plate loaded; a face's
 * stylesheet links when its row shows (FontRowLabel), never before.
 */
            ...
            {shown.map((row) => {
              ...
              return (
                <div
                  ...
                  data-control={`dialog.moreFonts.pick.${row.id}`}
                  data-font={row.id}
                ...
                >
                  ...
                  <span className="ts-more-fonts-text">
                    ...
                    <span className="ts-more-fonts-meta">
                      {FONT_CATEGORY_LABELS[row.category]} · {words.licenceLine(row.licence)}
                    </span>
                  </span>
                </div>
              );
