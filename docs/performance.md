...
| The filmstrip shows an edit at the next frame                                          | today               | SPEC-4 0.30, 3.2 (B4, landed at merge 2): the card is the renderer's HTML (`Thumb` `capture: 'never'`) and no capture request leaves the editor for the filmstrip; the verifier's post round runs read the current card's clone carrying the typed text 0 ms after the local commit on the node-server build, `vite preview` and the preview deployment (VERIFICATION-4 section 5.3)                                                                                                                                                                                                                                                                                                                                                                                      |
| Static contracts, served from the bundle                                               | today               | SPEC-4 3.11 (B4, landed at merge 2): `apps/studio/src/server/contracts.ts` imports the generated files; production answered `/openapi.json` with the 2,072,821 byte document and `/llms.txt` with 6,417 bytes on 2026-09-14 (the round three deploy answered a 236 byte placeholder, R05 8.4)                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| Every route ships only its own code                                                    | after               | SPEC-4 0.44, 3.12: the editor split, the shape table as one string, the lazy dialogs, the material mount and CodeMirror on first use landed at merge 2; production loads 946 to 2,205 KB of decoded JavaScript per route against 3,049 to 3,187 before the round (VERIFICATION-4 section 5), and the entry chunk was 914,233 bytes against the 600,000 ceiling on that deploy because the `vendor` chunk group is never applied inside the Vite 8.2.2 build (`build-4/integrator.md` section 17). The fixer round cut the schema package and the page copy out of the root graph, so the entry chunk is 577,949 bytes on the tree that ships (check step 6, 2026-09-14; `build-4/integrator.md` 22.1); the row flips when step 31's `js decoded` rows meet their ceilings |
...
| Preloading: a hover starts the next route's data                                       | today               | `defaultPreload: 'intent'`, the title row's mark as a `Link`, the first twelve cards on `preload="viewport"` and `defaultPreloadStaleTime` at 30 s (`apps/studio/src/router.tsx`, SPEC-4 0.39, B3 at merge 2): `decks->edit` 168 ms in page and `edit->decks` 16 ms in page on production, where day 0 read 598 ms and a 1,114 ms document load (VERIFICATION-4 section 5.2, 2026-09-14)                                                                                                                                                                                                                                                                                                                                                                                  |
...
| A fast home page                                                                       | after               | SPEC-4 3.1: the `blob` tier's list from manifests (B4) and the shell streamed before the list with the Recent row from `localStorage` (B3) landed at merge 2; production's `/decks` first byte fell from 5.3 to 6.3 s to 409 ms cold and 235 ms warm at the median, with a worst cold sample of 3,782 ms (VERIFICATION-4 section 5.1), so the slow mode is rarer and shorter, not gone; the row flips when the cold first byte meets its 400 ms ceiling on every sample                                                                                                                                                                                                                                                                                                   |
...
(SPEC-4 0.26): the verifier's post round run on production measured the click and studio ready
transition from `/decks` at 168 ms for the 85 slide GT deck and the title row mark back to the
list at 16 ms in page, warm (`VERIFICATION-4.md` section 5.2, 2026-09-14; the day 0 baseline read
598 ms and a 1,114 ms document load, R04 had 399 and 723 ms on 2026-09-13). The closing numbers of
the speed band come from the same run (`verification-4/perf-budget-production-2026-09-14.json`):
`/decks` first byte 409 ms cold at the median with a 3,782 ms worst sample, `/deck/gt-brand` ready
1,067 ms cold, `/new` ready 570 ms cold, and 946 to 2,205 KB of decoded JavaScript per route. The
day 0 numbers stay in section 7 as the baseline every gain is read against.
