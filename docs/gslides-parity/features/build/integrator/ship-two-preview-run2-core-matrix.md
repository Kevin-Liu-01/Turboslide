...
Base https://turboslide-2lbzoh8o7-kl01s-projects.vercel.app, started 2026-09-25T00:52:45.217Z, 8857 s. 814 rows judged: 758 passed, 36 failed, 20 not driven (1 of them manual, the checklist's: text.clipboard.paste-without-formatting), 0 no step. Measurement rows (PRODUCT.md 8.2, recorded and never holding the ship; the cost rows of SYNC.md 6.1 among them, which hold it over their ceiling on the preview): export.download.large-deck-pdf passed (PDF: GT brand deck.pdf, 85 slides in 16.0 s, 0.19 s per slide; PDF: GT brand deck.pdf, 85 slides in 16.0 s, 0.19 s per slide); export.download.large-deck-pptx passed (Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 214.2 s, 2.52 s per slide; Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 214.2 s, 2.52 s per slide); cost.editor-idle.calls failed (function requests 9.66 a minute (ceiling 12; 29 in 3 min); the store settled 3.24 s after the state was ready (1 sync.status read(s) before the window); store simple 43 a minute (ceiling 40; 3 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced 13 a minute (ceiling 11); instances answering sync.status 1); cost.editor-hidden.calls not driven (function requests 9.65 a minute and 9 session poll(s) were read from the tab that stayed visible); cost.editor-editing.calls passed (function requests 72.61 a minute (ceiling 75; 218 in 3 min); the store settled 2.46 s after the state was ready (1 sync.status read(s) before the window); store simple 123 a minute (ceiling 140; 3 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced 72 a minute (ceiling 85); instances answering sync.status 1); cost.two-tabs-idle.calls failed (the store settled 2.21 s after the state was ready (1 sync.status read(s) before the window); store advanced 19 a minute (ceiling 30); store list 1 (ceiling 0); instances answering sync.status 1); cost.show.calls passed (function requests 0 in the window (ceiling 0); the store settled 115.33 s after the state was ready (12 sync.status read(s) before the window; the first read: head 13, put 0, list 0, del 0); store calls at the first sample 1 (head 1, get 0, put 0, list 0, del 0), of which 1 the probe's own sync.status read(s) (one deck.json head each, b3.md R7 b); the page's 0 (ceiling 0); function requests between the load and the window 0 (the settle, 115.33 s); instances answering sync.status 2); shaders.perf.editor-frame passed (longest animation frame 60.6 ms over 5 s with one shader on the stage (467 animation frames; long-animation-frame; 1 canvas; renderer ANGLE (Google, Vulkan 1.3.0 (SwiftShader Device (LLVM 10.0.0) (0x0000C0DE)), SwiftShader driver)); longest animation frame 60.6 ms over 5 s with one shader on the stage (467 animation frames; long-animation-frame; 1 canvas; renderer ANGLE (Google, Vulkan 1.3.0 (SwiftShader Device (LLVM 10.0.0) (0x0000C0DE)), SwiftShader driver))). Cost rows over their ceiling in this run: cost.editor-idle.calls, cost.two-tabs-idle.calls. Verdict failed; retries 0 configured, 0 test(s) retried; exit 1. A not driven row is never counted as passed. Features a ship on this run would park (rule 4 of section 1; RETURN.md rule 2): inbox, brand, fonts, templates, assist, shaders; rows whose own controls a ship would keep parked: view.live-pointers.second-browser (view.livePointers.mine, view.livePointers.collaborators); versions.show-changes-marks (file.versionHistory.showChanges); export.png.current-slide (file.download.png); templates.gallery.page (home.gallery, file.new.templateGallery); templates.card.rename-and-delete (templates.card.menu); templates.default.use-for-new (panel.brand.template.useForNew); tables.seam.row-drag (handle.table.row); tables.edge.add-row-column (handle.table.add.column, handle.table.add.row); tables.heads.select-row-column (handle.table.head.column, handle.table.head.row); tables.bar.row-column-buttons (bar.table); arrange.group.tail-text-controls (toolbar.group.text); wordart.tail.fill-outline (toolbar.wordart.outline); logos.index.refresh-fixture (insert.logo); logos.index.cached-offline (insert.logo); logos.picker.variants (dialog.logo.kind.wordmark, dialog.logo.tone.mono); logos.kit.find-a-logo (panel.brand.logo.find); shaders.insert.selected-free-rectangle (insert.shader); shaders.insert.words (insert.shader); shaders.panel.slider-live-undo (formatOptions.shader.strength); shaders.panel.preset-tiles (formatOptions.shader.preset); shaders.panel.control-sentences (formatOptions.shader); shaders.panel.kit-colours (formatOptions.shader.color); shaders.panel.one-home (formatOptions.shader); shaders.frame.auto-capture (insert.shader); shaders.frame.box-aspect (insert.shader); shaders.frame.reuse-and-prune (insert.shader); shaders.frame.one-capturer (insert.shader); shaders.perf.one-context (insert.shader); shaders.background.add-to-theme (dialog.background.shader.addToTheme); shaders.library.glyph-engines-render (dialog.shader.engine.glyph); shaders.frame.scrubber-capture (formatOptions.shader.frame.scrubber, formatOptions.shader.frame.capture); shaders.show.plays-when-on (view.playShaders, formatOptions.shader.play); shaders.show.frame-when-off (view.playShaders); rows of an unparkable feature blocking the ship: decks.card.rename-enter, decks.card.make-a-copy, images.insert.upload-while-pending, versions.restore, share.dialog.you-label, share.dialog.co-edit-from-copied-link, sync.title.concurrent-both-kept, sync.block.offline-replay-converges, sync.resend.idempotent, shaders.export.pdf-frame.
...
| `decks.card.rename-enter` | decks | core/decks.spec.ts | broken | failed | Error: [2mexpect([22m[31mlocator[39m[2m).[22mtoHaveText[2m([22m[32mexpected[39m[2m)[22m failed |
...
| `decks.card.make-a-copy` | decks | core/decks.spec.ts | flaky | failed | TimeoutError: page.waitForFunction: Timeout 90000ms exceeded. |
...
| `versions.restore` | versions | core/share.spec.ts | flaky | failed | Error: the restore lands within 35 s: the text did not land within 35 s (panel notice Restored the version of Sep 24, 07:43 PM) |
...
| `versions.show-changes-marks` | versions | probe --core | not driven | failed | a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.889.pick (named row versionHistory.889.pick); marks with Show changes on 0 (); off 0 |
...
| `export.png.current-slide` | export | core/export.spec.ts | broken | failed | Error: a png file or a tab within 30 s |
...
| `share.dialog.you-label` | share | core/share.spec.ts | broken | failed | Error: the second browser's name reaches the owner's roster |
...
| `cost.editor-idle.calls` | cost | cost-probe | broken | failed | store simple 43 a minute over 40; store advanced 13 a minute over 11 |
...
| `shaders.perf.one-context` | shaders | probe --core | not driven | failed | a second shader block placed through the window API; select each in turn and count the canvases: with nothing selected 1 canvas on the stage (2 on the page); first selected: 2 on the stage, 2 on the page; second selected: 1 on the stage after 406 ms, the second's root 1 canvas, the first's root 0 ca |
...
- `decks.card.rename-enter`: Error: [2mexpect([22m[31mlocator[39m[2m).[22mtoHaveText[2m([22m[32mexpected[39m[2m)[22m failed
- `decks.card.make-a-copy`: TimeoutError: page.waitForFunction: Timeout 90000ms exceeded.
...
- `versions.restore`: Error: the restore lands within 35 s: the text did not land within 35 s (panel notice Restored the version of Sep 24, 07:43 PM)
- `view.live-pointers.second-browser`: Error: the second browser's pointer is drawn on the first within 10 s
- `versions.show-changes-marks`: a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.889.pick (named row versionHistory.889.pick); marks with Show changes on 0 (); off 0
- `export.png.current-slide`: Error: a png file or a tab within 30 s
...
- `share.dialog.you-label`: Error: the second browser's name reaches the owner's roster
...
- `cost.editor-idle.calls`: store simple 43 a minute over 40; store advanced 13 a minute over 11
...
- `export.download.large-deck-pdf`: passed; recorded PDF: GT brand deck.pdf, 85 slides in 16.0 s, 0.19 s per slide; PDF: GT brand deck.pdf, 85 slides in 16.0 s, 0.19 s per slide
- `export.download.large-deck-pptx`: passed; recorded Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 214.2 s, 2.52 s per slide; Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 214.2 s, 2.52 s per slide
- `cost.editor-idle.calls`: failed (store simple 43 a minute over 40; store advanced 13 a minute over 11); recorded function requests 9.66 a minute (ceiling 12; 29 in 3 min); the store settled 3.24 s after the state was ready (1 sync.status read(s) before the window); store simple 43 a minute (ceiling 40; 3 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced 13 a (a cost row: over its ceiling on the preview it holds the ship)
...
- `cost.editor-editing.calls`: passed; recorded function requests 72.61 a minute (ceiling 75; 218 in 3 min); the store settled 2.46 s after the state was ready (1 sync.status read(s) before the window); store simple 123 a minute (ceiling 140; 3 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced  (a cost row: over its ceiling on the preview it holds the ship)
- `cost.two-tabs-idle.calls`: failed (store list 1, ceiling none); recorded the store settled 2.21 s after the state was ready (1 sync.status read(s) before the window); store advanced 19 a minute (ceiling 30); store list 1 (ceiling 0); instances answering sync.status 1 (a cost row: over its ceiling on the preview it holds the ship)
- `cost.show.calls`: passed; recorded function requests 0 in the window (ceiling 0); the store settled 115.33 s after the state was ready (12 sync.status read(s) before the window; the first read: head 13, put 0, list 0, del 0); store calls at the first sample 1 (head 1, get 0, put 0, list 0, del 0), of which 1 the probe's own sync.stat (a cost row: over its ceiling on the preview it holds the ship)
- `shaders.perf.editor-frame`: passed; recorded longest animation frame 60.6 ms over 5 s with one shader on the stage (467 animation frames; long-animation-frame; 1 canvas; renderer ANGLE (Google, Vulkan 1.3.0 (SwiftShader Device (LLVM 10.0.0) (0x0000C0DE)), SwiftShader driver)); longest animation frame 60.6 ms over 5 s with one shader on the sta
