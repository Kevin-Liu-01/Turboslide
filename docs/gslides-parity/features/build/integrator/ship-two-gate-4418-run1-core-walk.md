...
Base http://localhost:4418, started 2026-09-24T19:52:30.158Z, 3159 s, deck untitled-20260924-3j41. 586 probe rows: 552 passed, 16 failed, 18 not driven, 0 no step. Verdict failed; exit 1. A row passes only when every tagged step of it passed; a not driven row is never counted as passed.
...
| `versions.show-changes-marks` | versions | not driven | failed | a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.447.pick (named row versionHistory.447.pick); marks with Show changes on 0 (); off 0 | 488 |
...
| `inbox.settings-persist` | inbox | broken | passed |  | 602 |
...
| `brand.colors.primary-live` | brand | not driven | passed |  | 531 |
...
| `assist.agent.propose-accept` | assist | not driven | failed | assist.propose over HTTP, assist.accept with the card, then the card with one byte changed: assist.propose answered 500 {"error":{"name":"ModelCallError","status":500,"message":"The assistant’s model answered 401","action":"assist.propose"}} | 600 |
...
| `logos.picker.search` | logos | not driven | failed | Insert > Logo; type figma at human speed; Enter: 0 result tiles 3 ms after the last key (first none; foot "Logos from thesvg.org. Brand marks belong to their owners; use them to name the brand, not to imply endorsement"); Enter inserted nothing; dialog closed false | 561 |
| `logos.picker.paper-and-ink` | logos | not driven | failed | search vercel; read the tile's paper and ink halves; Slide > Change theme light; insert; read the mark: no Vercel tile among 1 tiles | 562 |
| `logos.picker.your-brand` | logos | not driven | passed |  | 563 |
| `logos.picker.empty-state` | logos | not driven | passed |  | 564 |
| `logos.picker.licence-words` | logos | not driven | failed | read the foot; search figma and read the licence row and its tooltip; the dialog's text; search aws; Include cloud service icons: foot "Logos from thesvg.org. Brand marks belong to their owners; use them to name the brand, not to imply endorsement" (at the open "Logos from thesvg.org. Brand marks belong to their owners; use them to name the brand, not to imply endorsement"); Figma row "none" with  | 565 |
| `logos.insert.one-click-asset` | logos | not driven | failed | search figma; one click on the tile; read the asset record, the img and the source document: error: locator.boundingBox: Timeout 30000ms exceeded. | 566 |
| `logos.insert.logo-size` | logos | not driven | failed | read the inserted Figma mark against the body slot, its chip and the text runs: error: locator.boundingBox: Timeout 30000ms exceeded. | 567 |
| `logos.insert.mono-tint` | logos | not driven | failed | on the dark deck insert the Stripe mark's mono (asked for through the window API); read its variant and the fills of its source file; then AWS through the dialog with the cloud switch on: the Stripe mono was not inserted within 20 s (page.evaluate: Error: No logo named stripe on thesvg.org) | 568 |
| `logos.insert.every-slide` | logos | not driven | failed | read the foot check and its sentence; check it; insert Figma; read the title slide and a new slide; the snackbar; Cmd+Z; open the dialog again: error: locator.boundingBox: Timeout 30000ms exceeded. | 569 |
| `logos.tailor.find-customer-logo` | logos | not driven | not driven | not on this build: dialog.tailor.logo.find (docs/PRODUCT.md 7.1, B1); the Tailor dialog draws no Find the Figma logo button once To names a brand (FEATURES.md 4.5) | 570 |
| `logos.replace-image.row` | logos | not driven | failed | a picture placed through the window API; right click it; Replace image > Logo; Figma: error: locator.boundingBox: Timeout 30000ms exceeded. | 571 |
...
| `shaders.insert.selected-free-rectangle` | shaders | broken | failed | Insert > Shader; click Liquid metal on the Title slide: Liquid metal inserted material at 646,129 480x272 (paper:liquid-metal, preset diamond); move handle true; resize handles 8; rotate true; ring true; chip "Shader"; session false; caret false; overlaps no text box of 3; kind title -> content | 578 |
| `shaders.insert.words` | shaders | broken | failed | read the Insert row, the chip, the section title and the alt; the sheet, the viewer and the show for a paper: id or "not captured": Insert row "Shader"; chip "Shader"; section absent; alt "The liquid metal shader"; sheet clean (0 label elements); show clean; viewer unread: Error: page.evaluate: Target page, context or browser has been closed | 579 |
| `shaders.panel.slider-live-undo` | shaders | not driven | not driven | not on this build: formatOptions.shader (docs/PRODUCT.md 7.1, B5 (inspector/shader.tsx)); no Shader section in Format options (FEATURES.md 5.3) | 580 |
| `shaders.panel.preset-tiles` | shaders | broken | not driven | not on this build: formatOptions.shader (docs/PRODUCT.md 7.1, B5 (inspector/shader.tsx)); no Shader section in Format options (FEATURES.md 5.3) | 581 |
| `shaders.panel.control-sentences` | shaders | not driven | not driven | not on this build: formatOptions.shader (docs/PRODUCT.md 7.1, B5 (inspector/shader.tsx)); no Shader section in Format options (FEATURES.md 5.3) | 582 |
| `shaders.panel.kit-colours` | shaders | not driven | not driven | not on this build: formatOptions.shader (docs/PRODUCT.md 7.1, B5 (inspector/shader.tsx)); no Shader section in Format options (FEATURES.md 5.3) | 583 |
| `shaders.panel.one-home` | shaders | broken | failed | Tools > Advanced tools; Tools > Pictures and materials with the shader selected; the Background dialog's options link: Pictures and materials sections Pictures and materials, Pictures0 (with the switch on); the Background dialog's link dialog.background.shader.options: Format options true, Shader section absent, Pictures and materials false, dialog false | 585 |
| `shaders.background.place-answers` | shaders | broken | passed |  | 584 |
| `shaders.perf.one-context` | shaders | not driven | failed | a second shader block placed through the window API; select each in turn and count the canvases: with nothing selected 1 canvas on the stage (1 on the page); first selected: 1 on the stage, 1 on the page; second selected: 2 on the stage after 2675 ms, the second's root 1 canvas, the first's root 1 canvas, frame img none | 586 |
