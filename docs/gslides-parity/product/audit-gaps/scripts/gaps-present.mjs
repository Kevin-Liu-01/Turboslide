...
try {
  ...
  await t.step(
    'Type a speaker note; New slide from the toolbar and a title on it',
    'the note is stored; two slides',
    async () => {
      ...
      if (runs[0]) {
        await t.openRun(runs[0]);
        await t.typeHuman('Pipeline by region');
        await t.press('Escape', 2);
        await t.settled();
      }
      return {
        ok: Boolean(notes) && order.length === 2,
        observed: `notes "${notes}"; slides ${order.join(',')}; second ${second}`,
        extra: { second },
      };
    },
  );
  ...
  let presenter = null;
  await t.step(
    'Slideshow arrow > Presenter view: the second window',
    'a window with the notes, the next slide and a timer; the show in the first window',
    async () => {
      ...
      const rows = await page.evaluate(() =>
        [...document.querySelectorAll('[data-control^="menu.title.slideshow."]')]
      ...
      const [popup] = await Promise.all([
        ...
        t.clickRow('title.slideshow.presenterView'),
      ]);
      ...
      const facts = await popup.evaluate(() => ({
        ...
        notes: document
          .querySelector('[data-control="presenter.notesText"]')
          ?.textContent?.trim()
          ?.slice(0, 120),
        ...
        frames: document.querySelectorAll(
          '.ts-presenter iframe, .ts-presenter .pt-slide, .ts-presenter [data-slide]',
        ).length,
        controls: [...document.querySelectorAll('[data-control^="presenter."]')]
      ...
      const audience = await page.evaluate(() => ({
        show:
          document.querySelector('.ts-slideshow') !== null ||
          document.querySelector('[data-control="present.toolbar"]') !== null,
      ...
      await popup.evaluate(() =>
        document.querySelector('[data-control="presenter.next"]')?.click(),
      );
      await sleep(1200);
      const after = await t.state();
      return {
        ok: Boolean(facts.notes) && /renewal/.test(facts.notes),
        observed: `rows ${rows.join(',')}; presenter ${JSON.stringify(facts)}; audience ${JSON.stringify(audience)}; after Next in the console: editor slide ${after.slideId}; ${shot0}; ${path.basename(f1)}; ${shot}`,
      };
    },
  );
  ...
  await t.step(
    'Slideshow: L toggles the laser; the Options menu lists the pen',
    'the laser dot draws; the pen row is a stub',
    async () => {
      await t.openShow();
      ...
      if (opts) {
        ...
        rows = await page.evaluate(() =>
          [...document.querySelectorAll('[data-control^="menu.present.options"]')]
            .filter((el) => el.getClientRects().length > 0)
            .map((el) => ({
      ...
      return {
        ok: laser,
        observed: `laser dot ${laser}; options ${JSON.stringify(rows)}; show still open after Escape on the menu ${stillShow}; ${shot}; ${shot2}`,
      };
    },
  );
  ...
  await t.step(
    'Offline in the show: ArrowRight, ArrowLeft, a digit and Enter with the network off',
    'the show keeps working after load without the network',
    async () => {
      if (
        !(await t.has(
          '.ts-slideshow, [data-control="present.toolbar"], [data-control="present.blank"]',
        ))
      )
        await t.openShow();
      ...
      const words = await page.evaluate(
        () =>
          document.body.textContent
            ?.match(/offline|Offline|reconnect|Reconnecting|connection/g)
            ?.slice(0, 5) ?? null,
      );
      await context.setOffline(false);
      await sleep(1500);
      return {
        ok: s1.slideId !== s0.slideId && s2.slideId === s0.slideId,
        observed: `slide ${s0.slideId} -> ${s1.slideId} -> ${s2.slideId}; snackbar "${snack}"; words on the page ${JSON.stringify(words)}; ${shot}`,
      };
    },
  );
  await t.step(
    'Reload the editor with the network off',
    'what the page shows without the network',
    async () => {
      await t.press('Escape');
      await sleep(500);
      await context.setOffline(true);
      ...
      const body = await page
        ...
        .catch((e) => `unreadable: ${String(e).split('\n')[0]}`);
      ...
      return {
        ok: true,
        observed: `reload ${res && res.error ? res.error : `status ${typeof res?.status === 'function' ? res.status() : 'n/a'}`}; body "${body}"; ${shot}`,
      };
    },
  );
  ...
  await t.step(
    'Ctrl+M adds a slide; Cmd+D duplicates the selected card',
    'both chords work as Google binds them',
    async () => {
      ...
      return {
        ok: afterM === before + 1 && afterD === afterM + 1,
        observed: `slides ${before} -> ${afterM} after Ctrl+M -> ${afterD} after Cmd+D`,
      };
    },
  );
  await t.step(
    'Cmd+Shift+H, Cmd+/, Alt+/, Cmd+Option+Shift+S, Cmd+Option+M',
    'Find and replace, the shortcuts dialog, Search the menus, the notes focus, the comment card',
    async () => {
      const out = {};
      ...
      await t.press('Meta+Slash');
      await sleep(600);
      out.shortcuts = await t.visible('[data-control="dialog.keyboardShortcuts"]');
      await t.closeDialogs();
      await t.press('Alt+Slash');
      await sleep(600);
      out.searchMenus = await page.evaluate(() => {
        const el = document.querySelector(
          '[data-control="finder"], [data-control="palette"], [data-control^="finder."], .ts-finder, .ts-palette, [role="combobox"][aria-label*="menus" i]',
        );
        return el ? (el.getAttribute('data-control') ?? el.className) : null;
      });
      await t.closeDialogs();
      await t.press('Meta+Alt+Shift+s');
      await sleep(500);
      ...
      await sleep(800);
      out.commentCard = await page.evaluate(() =>
        [...document.querySelectorAll('[data-control$=".field"]')].some(
      ...
      await t.closeDialogs();
      return {
        ok: out.findReplace && out.shortcuts && out.commentCard,
        observed: `${JSON.stringify(out)}; ${shot}`,
      };
    },
  );
  ...
  const smallScreen = async (label, width, height, mobile) => {
    ...
    const readFacts = () =>
      p2.evaluate(() => {
        ...
        return {
          ...
          viewerControls: [
            ...document.querySelectorAll('[data-control^="viewer."], [data-control^="present."]'),
          ]
            .map((el) => el.getAttribute('data-control'))
            .filter((c, i, a) => a.indexOf(c) === i)
            .slice(0, 12)
    ...
    if (mobile) {
      const s = await p2.evaluate(() => {
        const el = document.querySelector('.pt-slide');
        if (!el) return null;
        const r = el.getBoundingClientRect();
        return { x: r.x + r.width / 2, y: r.y + r.height / 2 };
      });
      if (s) {
        await p2.touchscreen.tap(s.x, s.y);
        await sleep(900);
      }
      out.viewerAfterTap = await p2.evaluate(() => ({
        counter:
          document
            .querySelector('[data-control="present.counter"], .counter')
    ...
    await p2
      .waitForFunction(() => Boolean(window.turboslide?.studio), null, { timeout: 60_000 })
      .catch(() => undefined);
    await p2
    ...
    if (!mobile && width === 1280) {
      ...
      const head = await p2.evaluate(() => {
        const el = document.querySelector('.ts-stagewrap.ts-editor .pt-slide [data-run]');
        if (!el) return null;
        const r = el.getBoundingClientRect();
        return { x: r.x + r.width / 2, y: r.y + r.height / 2 };
      });
      if (head) {
        await p2.mouse.click(head.x, head.y);
        await sleep(500);
        await p2.keyboard.press('Escape');
        await sleep(300);
      }
      out.tailAt1280 = await p2.evaluate(() => ({
        visible: [...document.querySelectorAll('[data-control^="toolbar."]')].filter(
          (el) => el.getClientRects().length > 0,
        ).length,
    ...
    if (mobile) {
      ...
      if (head) {
        await p2.touchscreen.tap(head.x, head.y);
        await sleep(600);
      }
      out.editorAfterTap = await p2.evaluate(() => ({
        chip: document.querySelector('.ts-overlay .ts-select-chip')?.textContent ?? null,
        editing: document.querySelector('.ts-stagewrap.ts-editor[data-editing]') !== null,
        handles: document.querySelectorAll('.ts-overlay [data-control^="handle."]').length,
      }));
  ...
  await t.step(
    'The viewer and the editor at 375 by 812 (a phone)',
    'no horizontal overflow; the viewer pages by a tap; the editor is usable or says it is for larger screens',
    async () => {
      const { out, shots } = await smallScreen('phone-375', 375, 812, true);
      return {
        ok: !out.viewer.horizontalOverflow && !out.editor.horizontalOverflow,
        observed: `${JSON.stringify(out)}; ${shots.join('; ')}`,
      };
    },
  );
  await t.step(
    'The viewer and the editor at 768 by 1024 (a tablet)',
    'no horizontal overflow; the editor draws its rows',
    async () => {
  ...
  await t.step(
    'The editor at 1280 by 800 (a laptop)',
    'the toolbar fits or folds into More; no horizontal overflow',
    async () => {
      const { out, shots } = await smallScreen('laptop-1280', 1280, 800, false);
      return {
        ok: !out.editor.horizontalOverflow,
        observed: `${JSON.stringify(out)}; ${shots.join('; ')}`,
      };
    },
  );
  ...
  writeFileSync(
    path.join(EVIDENCE, 'present-console.json'),
