...
Base https://turboslide-bxjtrzqfy-kl01s-projects.vercel.app, started 2026-09-25T20:06:58.406Z, 8463 s. 814 rows judged: 751 passed, 28 failed, 35 not driven (1 of them manual, the checklist's: text.clipboard.paste-without-formatting), 0 no step. Measurement rows (PRODUCT.md 8.2, recorded and never holding the ship; the cost rows of SYNC.md 6.1 among them, which hold it over their ceiling on the preview): export.download.large-deck-pdf passed (PDF: GT brand deck.pdf, 85 slides in 14.3 s, 0.17 s per slide; PDF: GT brand deck.pdf, 85 slides in 14.3 s, 0.17 s per slide); export.download.large-deck-pptx passed (Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 212.4 s, 2.50 s per slide; Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 212.4 s, 2.50 s per slide); cost.editor-idle.calls failed (function requests 9.99 a minute (ceiling 12; 30 in 3 min); the store settled 2.32 s after the state was ready (1 sync.status read(s) before the window); store simple 48 a minute (ceiling 40; 3 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced 10 a minute (ceiling 11); instances answering sync.status 2); cost.editor-hidden.calls not driven (function requests 9.66 a minute and 9 session poll(s) were read from the tab that stayed visible); cost.editor-editing.calls passed (function requests 59.28 a minute (ceiling 75; 178 in 3 min); the store settled 63.64 s after the state was ready (7 sync.status read(s) before the window; the first read: head 20, put 3, list 1, del 0); store simple 121 a minute (ceiling 140; 3 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced 75 a minute (ceiling 85); instances answering sync.status 1); cost.two-tabs-idle.calls failed (the store settled 2.18 s after the state was ready (1 sync.status read(s) before the window); store advanced 19 a minute (ceiling 30); store list 1 (ceiling 0); instances answering sync.status 2); cost.show.calls passed (function requests 0 in the window (ceiling 0); the store settled 94.69 s after the state was ready (10 sync.status read(s) before the window; the first read: head 16, put 17, list 0, del 0); store calls at the first sample 1 (head 1, get 0, put 0, list 0, del 0), of which 1 the probe's own sync.status read(s) (one deck.json head each, b3.md R7 b); the page's 0 (ceiling 0); function requests between the load and the window 0 (the settle, 94.69 s); instances answering sync.status 1); shaders.perf.editor-frame passed (longest animation frame 125.0 ms over 5 s with one shader on the stage (581 animation frames; rAF gaps; 1 canvas; renderer ANGLE (Google, Vulkan 1.3.0 (SwiftShader Device (LLVM 10.0.0) (0x0000C0DE)), SwiftShader driver)); longest animation frame 125.0 ms over 5 s with one shader on the stage (581 animation frames; rAF gaps; 1 canvas; renderer ANGLE (Google, Vulkan 1.3.0 (SwiftShader Device (LLVM 10.0.0) (0x0000C0DE)), SwiftShader driver))). Cost rows over their ceiling in this run: cost.editor-idle.calls, cost.two-tabs-idle.calls. Verdict failed; retries 0 configured, 0 test(s) retried; exit 1. A not driven row is never counted as passed. Features a ship on this run would park (rule 4 of section 1; RETURN.md rule 2): inbox, brand, fonts, templates, assist; rows whose own controls a ship would keep parked: view.live-pointers.second-browser (view.livePointers.mine, view.livePointers.collaborators); decks.file.open-list-search (file.open); versions.show-changes-marks (file.versionHistory.showChanges); templates.gallery.page (home.gallery, file.new.templateGallery); templates.save.as-template (file.saveAsTemplate); templates.save.same-name-replaces (file.saveAsTemplate); templates.card.rename-and-delete (templates.card.menu); templates.default.use-for-new (panel.brand.template.useForNew); brand.logo.use-on-every-slide (format.image.useOnEverySlide); tables.seam.row-drag (handle.table.row); tables.edge.add-row-column (handle.table.add.column, handle.table.add.row); tables.heads.select-row-column (handle.table.head.column, handle.table.head.row); tables.bar.row-column-buttons (bar.table); arrange.group.tail-text-controls (toolbar.group.text); wordart.tail.fill-outline (toolbar.wordart.outline); logos.index.refresh-fixture (insert.logo); logos.index.cached-offline (insert.logo); logos.picker.variants (dialog.logo.kind.wordmark, dialog.logo.tone.mono); logos.kit.find-a-logo (panel.brand.logo.find); shaders.frame.auto-capture (insert.shader); shaders.background.place-answers (dialog.background.shader); shaders.background.add-to-theme (dialog.background.shader.addToTheme); shaders.library.glyph-engines-render (dialog.shader.engine.glyph); shaders.frame.scrubber-capture (formatOptions.shader.frame.scrubber, formatOptions.shader.frame.capture); shaders.show.plays-when-on (view.playShaders, formatOptions.shader.play); shaders.show.frame-when-off (view.playShaders); rows of an unparkable feature blocking the ship: decks.home.new-presentation, decks.file.make-a-copy, images.insert.first-on-new-deck, collab.presence-chips, comments.on-object, comments.reply, comments.resolve, share.dialog.you-label, sync.title.concurrent-both-kept, sync.resend.idempotent.

| Row | Feature | Driver | Today | Result | Reason |
| --- | --- | --- | --- | --- | --- |
| `decks.home.new-presentation` | decks | core/decks.spec.ts | works | failed | TimeoutError: page.waitForURL: Timeout 20000ms exceeded. |
...
| `decks.file.make-a-copy` | decks | core/decks.spec.ts | not driven | failed | Error: the copy opens in the new tab; the dialog says "decks/northwind-renewal-v4-without-notes exists already; pick another name" and the tab is closed |
...
| `collab.presence-chips` | share | core/share.spec.ts | broken | failed | Error: apiRequestContext._wrapApiCall: Target page, context or browser has been closed |
...
| `comments.on-object` | comments | core/present.spec.ts | works | failed | Error: page.evaluate: ConflictError: baseRevision 12 is stale; the document is at revision 13 |
| `comments.reply` | comments | core/present.spec.ts | works | failed | Error: a thread on this slide draws its marker after the reload |
| `comments.resolve` | comments | core/present.spec.ts | flaky | failed | Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoBeGreaterThan[2m([22m[32mexpected[39m[2m)[22m |
...
| `decks.file.open-list-search` | decks | probe --core | works | failed | File > Open; read the list; type in the search; clear it: 68 decks listed after 16225 ms, first untitled-20260925-43je; filtered by nonsense 0; cleared 68 |
...
| `versions.show-changes-marks` | versions | probe --core | not driven | failed | a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.884.pick (named row versionHistory.884.pick); marks with Show changes on 0 (); off 0 |
...
| `inbox.settings-persist` | inbox | probe --core | broken | failed | Tools > Notification settings, None, Save; reopen; reload: level forYou -> picked none; Save closed true; reopened forYou; after a reload none |
| `inbox.notification-arrives` | inbox | core/share.spec.ts | not driven | failed | Error: the panel lists the comment as a mention |
...
| `share.dialog.you-label` | share | core/share.spec.ts | broken | failed | Error: page.evaluate: Target page, context or browser has been closed |
...
| `templates.save.as-template` | templates | core/brand.spec.ts | not driven | failed | Error: the gallery lists Core spec template muhg1mdn under Your organisation |
...
| `templates.card.rename-and-delete` | templates | core/brand.spec.ts | not driven | failed | TimeoutError: page.waitForFunction: Timeout 90000ms exceeded. |
| `templates.default.use-for-new` | templates | core/brand.spec.ts | not driven | failed | Error: a template of this file with a kit |
...
| `brand.logo.replace-every-slide` | brand | core/brand.spec.ts | not driven | failed | TimeoutError: page.waitForFunction: Timeout 90000ms exceeded. |
| `brand.logo.use-on-every-slide` | brand | probe --core | not driven | not driven | setup failed: a picture for the logo rows |
| `brand.logo.remove` | brand | probe --core | not driven | not driven | setup failed: a picture for the logo rows |
| `brand.colors.primary-live` | brand | probe --core | not driven | not driven | setup failed: a picture for the logo rows |
| `brand.colors.palette-row` | brand | probe --core | broken | not driven | setup failed: a picture for the logo rows |
| `brand.colors.control-ids-unique` | brand | probe --core | broken | not driven | setup failed: a picture for the logo rows |
| `brand.colors.collab-rerender` | brand | core/share.spec.ts | not driven | passed |  |
| `brand.colors.version-history-entry` | brand | probe --core | not driven | not driven | setup failed: a picture for the logo rows |
| `brand.colors.role-tooltips` | brand | probe --core | not driven | not driven | setup failed: a picture for the logo rows |
| `brand.background.enter-keeps-open` | brand | probe --core | broken | not driven | setup failed: a picture for the logo rows |
| `brand.fonts.roles` | brand | probe --core | not driven | not driven | setup failed: a picture for the logo rows |
| `brand.footer.text` | brand | core/export.spec.ts | not driven | passed |  |
| `brand.counter.format` | brand | probe --core | not driven | not driven | setup failed: a picture for the logo rows |
...
| `brand.reset.default-kit` | brand | probe --core | not driven | not driven | setup failed: a picture for the logo rows |
| `brand.export.pdf-logo` | brand | core/export.spec.ts | not driven | failed | Error: page.evaluate: Target page, context or browser has been closed |
| `brand.surfaces.viewer-and-show` | brand | core/share.spec.ts | not driven | passed |  |
| `brand.layout.tiles-in-kit` | brand | probe --core | broken | not driven | setup failed: a picture for the logo rows |
| `brand.agent.set-get` | brand | probe --core | not driven | not driven | setup failed: a picture for the logo rows |
...
| `assist.viewer.disabled` | assist | core/share.spec.ts | not driven | failed | Error: every scratch deck is trashed, deleted forever and answers 404 |
...
| `cost.editor-idle.calls` | cost | cost-probe | broken | failed | store simple 48 a minute over 40 |
...
| `brand.objects.kit-colours-first` | brand | probe --core | not driven | not driven | setup failed: a picture for the logo rows |
...
| `shaders.frame.auto-capture` | shaders | core/shaders.spec.ts | broken | failed | Error: locator.count: Target page, context or browser has been closed |
...
| `shaders.background.place-answers` | shaders | probe --core | broken | failed | Slide > Change background on the body slide; Shader; a tile; Place; read the ground, the button and the dialog: chose Liquid metal; Place dialog.background.shader.place; the ground changed after 15352 ms (covering picture background, asset liquid-metal); the button read "Placing, 14 s" with the seco |
...
- `brand.logo.use-on-every-slide`: setup failed: a picture for the logo rows
- `brand.logo.remove`: setup failed: a picture for the logo rows
- `brand.colors.primary-live`: setup failed: a picture for the logo rows
- `brand.colors.palette-row`: setup failed: a picture for the logo rows
- `brand.colors.control-ids-unique`: setup failed: a picture for the logo rows
- `brand.colors.version-history-entry`: setup failed: a picture for the logo rows
- `brand.colors.role-tooltips`: setup failed: a picture for the logo rows
- `brand.background.enter-keeps-open`: setup failed: a picture for the logo rows
- `brand.fonts.roles`: setup failed: a picture for the logo rows
- `brand.counter.format`: setup failed: a picture for the logo rows
- `brand.reset.default-kit`: setup failed: a picture for the logo rows
- `brand.layout.tiles-in-kit`: setup failed: a picture for the logo rows
- `brand.agent.set-get`: setup failed: a picture for the logo rows
...
- `brand.objects.kit-colours-first`: setup failed: a picture for the logo rows
...
- `decks.home.new-presentation`: TimeoutError: page.waitForURL: Timeout 20000ms exceeded.
- `decks.file.make-a-copy`: Error: the copy opens in the new tab; the dialog says "decks/northwind-renewal-v4-without-notes exists already; pick another name" and the tab is closed
- `images.insert.first-on-new-deck`: Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoBeGreaterThan[2m([22m[32mexpected[39m[2m)[22m
- `collab.presence-chips`: Error: apiRequestContext._wrapApiCall: Target page, context or browser has been closed
- `comments.on-object`: Error: page.evaluate: ConflictError: baseRevision 12 is stale; the document is at revision 13
- `comments.reply`: Error: a thread on this slide draws its marker after the reload
- `comments.resolve`: Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoBeGreaterThan[2m([22m[32mexpected[39m[2m)[22m
- `view.live-pointers.second-browser`: Error: the second browser's pointer is drawn on the first within 10 s
- `decks.file.open-list-search`: File > Open; read the list; type in the search; clear it: 68 decks listed after 16225 ms, first untitled-20260925-43je; filtered by nonsense 0; cleared 68
- `versions.show-changes-marks`: a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.884.pick (named row versionHistory.884.pick); marks with Show changes on 0 (); off 0
...
- `inbox.settings-persist`: Tools > Notification settings, None, Save; reopen; reload: level forYou -> picked none; Save closed true; reopened forYou; after a reload none
- `inbox.notification-arrives`: Error: the panel lists the comment as a mention
- `share.dialog.you-label`: Error: page.evaluate: Target page, context or browser has been closed
- `templates.gallery.page`: TimeoutError: locator.waitFor: Timeout 6000ms exceeded.
- `templates.save.as-template`: Error: the gallery lists Core spec template muhg1mdn under Your organisation
...
- `templates.card.rename-and-delete`: TimeoutError: page.waitForFunction: Timeout 90000ms exceeded.
- `templates.default.use-for-new`: Error: a template of this file with a kit
- `brand.logo.replace-every-slide`: TimeoutError: page.waitForFunction: Timeout 90000ms exceeded.
- `brand.export.pdf-logo`: Error: page.evaluate: Target page, context or browser has been closed
- `assist.viewer.disabled`: Error: every scratch deck is trashed, deleted forever and answers 404
- `sync.title.concurrent-both-kept`: Error: both words in both browsers in every round of three
- `cost.editor-idle.calls`: store simple 48 a minute over 40
...
- `shaders.frame.auto-capture`: Error: locator.count: Target page, context or browser has been closed
- `shaders.background.place-answers`: Slide > Change background on the body slide; Shader; a tile; Place; read the ground, the button and the dialog: chose Liquid metal; Place dialog.background.shader.place; the ground changed after 15352 ms (covering picture background, asset liquid-metal); the button read "Placing, 14 s" with the seco

## Measurement rows, by id (PRODUCT.md 8.2; the cost rows of SYNC.md 6.1)

- `export.download.large-deck-pdf`: passed; recorded PDF: GT brand deck.pdf, 85 slides in 14.3 s, 0.17 s per slide; PDF: GT brand deck.pdf, 85 slides in 14.3 s, 0.17 s per slide
- `export.download.large-deck-pptx`: passed; recorded Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 212.4 s, 2.50 s per slide; Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 212.4 s, 2.50 s per slide
- `cost.editor-idle.calls`: failed (store simple 48 a minute over 40); recorded function requests 9.99 a minute (ceiling 12; 30 in 3 min); the store settled 2.32 s after the state was ready (1 sync.status read(s) before the window); store simple 48 a minute (ceiling 40; 3 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced 10 a (a cost row: over its ceiling on the preview it holds the ship)
...
- `cost.editor-editing.calls`: passed; recorded function requests 59.28 a minute (ceiling 75; 178 in 3 min); the store settled 63.64 s after the state was ready (7 sync.status read(s) before the window; the first read: head 20, put 3, list 1, del 0); store simple 121 a minute (ceiling 140; 3 of the heads the probe's own sync.status reads, which t (a cost row: over its ceiling on the preview it holds the ship)
- `cost.two-tabs-idle.calls`: failed (store list 1, ceiling none); recorded the store settled 2.18 s after the state was ready (1 sync.status read(s) before the window); store advanced 19 a minute (ceiling 30); store list 1 (ceiling 0); instances answering sync.status 2 (a cost row: over its ceiling on the preview it holds the ship)
- `cost.show.calls`: passed; recorded function requests 0 in the window (ceiling 0); the store settled 94.69 s after the state was ready (10 sync.status read(s) before the window; the first read: head 16, put 17, list 0, del 0); store calls at the first sample 1 (head 1, get 0, put 0, list 0, del 0), of which 1 the probe's own sync.stat (a cost row: over its ceiling on the preview it holds the ship)
- `shaders.perf.editor-frame`: passed; recorded longest animation frame 125.0 ms over 5 s with one shader on the stage (581 animation frames; rAF gaps; 1 canvas; renderer ANGLE (Google, Vulkan 1.3.0 (SwiftShader Device (LLVM 10.0.0) (0x0000C0DE)), SwiftShader driver)); longest animation frame 125.0 ms over 5 s with one shader on the stage (581 an
