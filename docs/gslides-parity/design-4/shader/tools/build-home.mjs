...
const icon = (name) => {
  const o = icons[name];
  if (!o) throw new Error(`no icon ${name}`);
  return `<svg class="ts-ic" viewBox="${o.vb}" aria-hidden="true">${o.body}</svg>`;
};
const mark = (S, cls = '') => {
  const g = geometry(S);
  return `<svg viewBox="0 0 ${S} ${S}" width="${S}" height="${S}" shape-rendering="crispEdges" fill="currentColor" aria-hidden="true"${cls ? ` class="${cls}"` : ''}>${markShapes(g, 1)}</svg>`;
};
function fnv(s) {
  let h = 0x811c9dc5;
  for (const c of s) {
...
function avatar(id) {
  ...
  let d = '';
  for (let y = 0; y < 8; y += 1)
    for (let x = 0; x < 8; x += 1) if (bayer8(y, x) < tier) d += `M${x * 3} ${y * 3}h3v3h-3z`;
...
html = html
  .replace(/\{\{mark32\}\}/g, mark(32))
  .replace(/\{\{mark80\}\}/g, mark(80))
  .replace(/\{\{mark16\}\}/g, mark(16));
html = html.replace(
  '{{avatars}}',
  ['agent:run-7f3a', 'studio', 'agent:run-c21d'].map(avatar).join(''),
);
