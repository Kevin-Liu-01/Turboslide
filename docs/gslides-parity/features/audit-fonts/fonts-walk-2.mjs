...
const record = (step, expected, observed, ok) => {
  ...
  console.log(
    `${ok === true ? 'ok  ' : ok === false ? 'FAIL' : 'n/d '} ${rows.length} ${step}\n       expected: ${expected}\n       observed: ${String(observed).slice(0, 1200)}`,
  );
};
...
const clickControl = async (page, c) => {
  const el = ctl(page, c).first();
...
const invoke = (page, a, i = {}) =>
  page.evaluate(([id, v]) => window.turboslide.studio.invoke(id, v), [a, i]);
...
const runBox = async (page, re) => {
  const all = page.locator('.ts-stagewrap.ts-editor .pt-slide [data-run]');
  const n = await all.count();
  for (let i = 0; i < n; i += 1) {
    const name = await all.nth(i).getAttribute('data-run');
    if (re.test(name ?? '')) return { box: await all.nth(i).boundingBox(), run: name };
  }
  return { box: null, run: null };
};
const shot = (page, name, clip) =>
  page.screenshot({ path: path.join(OUT, `${name}.png`), ...(clip ? { clip } : {}) });
...
try {
  ...
  await step(
    'a digits column in the body run',
    'proportional digits: the rows 1111 and 0000 end at different x',
    async () => {
      ...
      const cs = await page.evaluate(() => {
        ...
        ].find((e) => /lead|body|sub/.test(e.getAttribute('data-run') ?? ''));
      ...
      await shot(
        page,
        'body-digits-proportional-2x',
        nb
          ? {
              x: Math.max(0, nb.x - 8),
              y: Math.max(0, nb.y - 8),
              width: Math.min(1440 - nb.x + 8, Math.max(420, nb.width + 16)),
              height: Math.min(900 - nb.y + 8, nb.height + 16),
            }
          : undefined,
      );
      return {
        ok: cs.fontVariantNumeric === 'normal' && cs.fontFeatureSettings === 'normal',
        observed: JSON.stringify(cs),
      };
    },
  );
  ...
  await step('the disabled Font control at 2x', 'a zoom of the text tail around Font', async () => {
    ...
    await shot(page, 'toolbar-font-disabled-2x', {
      x: Math.max(0, b.x - 120),
      y: b.y - 20,
      width: 420,
      height: 72,
    });
  ...
  await step(
    'Format > Text submenu rows',
    'the rows of the Text submenu on production, and whether a Font row exists',
    async () => {
      ...
      const items = await page.evaluate(() =>
        [...document.querySelectorAll('[data-control^="menu.format.text"]')]
      ...
      return {
        ok: items.length > 0,
        observed: JSON.stringify({ items, fontRow: items.some((i) => /font/i.test(i)) }),
      };
    },
  );
} finally {
    ...
    await step('Delete forever, then GET /edit and /deck', 'both 404', async () => {
      ...
      for (;;) {
        for (const p of ['edit', 'deck']) {
          status[p] = (await context.request.get(`${BASE}/${p}/${deckId}`)).status();
        }
