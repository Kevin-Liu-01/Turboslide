...
- `invoke('view.goto', { slideId })`, `view.mode`, `view.theme`, `view.present` and `view.zoom` (a factor from 0.25 to 4, or `'fit'`) for the view; `invoke('render.slide', ...)` for a render through the studio; every editor action of the table (`slide.update`, `block.set`, `lint.run`, `fix.run`, `version.save`) runs through the same dispatcher a click uses.
