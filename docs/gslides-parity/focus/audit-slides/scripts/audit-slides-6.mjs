...
const OUT =
  process.argv[2] ??
  '/private/tmp/claude-501/-Users-kevinliu-gt-gt-cloud/293a64b7-8ef6-4b00-b382-682288c84431/scratchpad/focus/slides/run-6.json';
const SHOT_START = Number(process.argv[3] ?? 170);
...
try {
  ...
  for (let trial = 1; trial <= 3; trial += 1) {
    await row('Delete slide', `Trial ${trial}: in the grid, Shift click two tiles, press Delete, click Undo on the snackbar; then reload and compare the server's order`, async () => {
      ...
      if (!ra || !rb) { await clickControl(page, 'view.filmstripView'); return { ok: false, evidence: `tiles missing: ${a} ${Boolean(ra)}, ${b} ${Boolean(rb)}; grid ${JSON.stringify(await gridFacts(page))}` }; }
      ...
      return {
        ok: g.selectedTiles === 2 && mid.length === before.length - 2 && after.join(',') === before.join(',') && server.join(',') === before.join(',') && ref.dialogs.length === 0,
        evidence: `selected tiles ${g.selectedTiles}; ${before.length} -> Delete ${mid.length} (snackbar ${JSON.stringify(sb)}; sync ${JSON.stringify(syncMid)}) -> Undo by ${undoBy} ${after.length} (restored in the tab ${after.join(',') === before.join(',')}; sync ${JSON.stringify(syncAfter)}; save "${ref.save}"; dialogs ${ref.dialogs.length ? ref.dialogs.join(' || ') : 'none'}); after reload the server holds ${server.length}: ${server.join(',')} (matches ${server.join(',') === before.join(',')})${dismissed ? `; dismissed ${dismissed}` : ''}`,
  ...
  for (let trial = 1; trial <= 3; trial += 1) {
    await row('Delete slide', `Trial ${trial}: in the filmstrip, Shift click two cards, press Delete, click Undo on the snackbar; then reload and compare the server's order`, async () => {
      const before = await topUp();
      const a = before[before.length - 2];
      const b = before[before.length - 1];
      ...
      const syncAfter = await syncFacts();
      const ref = await refusals(page);
      const sh = ref.dialogs.length ? await shot(`film-two-undo-${trial}`) : null;
      const dismissed = await dismissRefusals();
      await settled(page, 15_000);
      const server = await serverOrder();
      return {
        ok: cs.filter((c) => c.selected).length === 2 && mid.length === before.length - 2 && after.join(',') === before.join(',') && server.join(',') === before.join(',') && ref.dialogs.length === 0,
        evidence: `selected ${cs.filter((c) => c.selected).map((c) => c.id).join(',')}; ${before.length} -> Delete ${mid.length} (snackbar ${JSON.stringify(sb)}; sync ${JSON.stringify(syncMid)}) -> Undo by ${undoBy} ${after.length} (restored in the tab ${after.join(',') === before.join(',')}; sync ${JSON.stringify(syncAfter)}; save "${ref.save}"; dialogs ${ref.dialogs.length ? ref.dialogs.join(' || ') : 'none'}); server after reload ${server.length}: ${server.join(',')} (matches ${server.join(',') === before.join(',')})${dismissed ? `; dismissed ${dismissed}` : ''}`,
        shot: sh,
      };
    }, { tries: 1 });
  }
  ...
  for (let trial = 1; trial <= 3; trial += 1) {
    await row('Delete slide', `Trial ${trial}: two cards selected, Slide menu > Delete slide, then Cmd+Z; then reload and compare the server's order`, async () => {
      const before = await topUp();
      const a = before[before.length - 2];
      const b = before[before.length - 1];
      const cs = await selectTwoCards(a, b);
      ...
      const syncAfter = await syncFacts();
      const ref = await refusals(page);
      const sh = ref.dialogs.length ? await shot(`menu-two-undo-${trial}`) : null;
      const dismissed = await dismissRefusals();
      await settled(page, 15_000);
      const server = await serverOrder();
      return {
        ok: cs.filter((c) => c.selected).length === 2 && mid.length === before.length - 2 && after.join(',') === before.join(',') && server.join(',') === before.join(',') && ref.dialogs.length === 0,
        evidence: `selected ${cs.filter((c) => c.selected).map((c) => c.id).join(',')}; ${before.length} -> Delete ${mid.length} (snackbar ${JSON.stringify(sb)}; sync ${JSON.stringify(syncMid)}) -> Cmd+Z ${after.length} (restored in the tab ${after.join(',') === before.join(',')}; sync ${JSON.stringify(syncAfter)}; save "${ref.save}"; dialogs ${ref.dialogs.length ? ref.dialogs.join(' || ') : 'none'}); server after reload ${server.length}: ${server.join(',')} (matches ${server.join(',') === before.join(',')})${dismissed ? `; dismissed ${dismissed}` : ''}`,
        shot: sh,
      };
    }, { tries: 1 });
  }
  ...
  for (let trial = 1; trial <= 3; trial += 1) {
    await row('Delete slide', `Trial ${trial}: in the grid, Shift click two tiles and press Delete, no undo; reload and compare the server's order`, async () => {
      const before = await topUp();
      const a = before[before.length - 2];
      const b = before[before.length - 1];
      await clearAll(page);
      ...
      const ra = await rectOf(page, `.pt-grid .pt-thumb[data-id="${a}"]`);
      const rb = await rectOf(page, `.pt-grid .pt-thumb[data-id="${b}"]`);
      if (!ra || !rb) { await clickControl(page, 'view.filmstripView'); return { ok: false, evidence: `tiles missing` }; }
      await clickAt(page, ra.x + ra.w / 2, ra.y + ra.h / 2);
      await sleep(300);
      await clickAt(page, rb.x + rb.w / 2, rb.y + rb.h / 2, { modifiers: ['Shift'] });
      await sleep(400);
      ...
      if (await has(page, '[data-control="view.filmstripView"]')) await clickControl(page, 'view.filmstripView').catch(() => undefined);
      const server = await serverOrder();
      const want = before.filter((id) => id !== a && id !== b);
      return {
        ok: mid.length === before.length - 2 && server.join(',') === want.join(',') && ref.dialogs.length === 0,
        evidence: `${before.length} -> Delete ${mid.length} (snackbar ${JSON.stringify(sb)}; sync ${JSON.stringify(sync)}; save "${ref.save}"; dialogs ${ref.dialogs.length ? ref.dialogs.join(' || ') : 'none'}); server after reload ${server.length}: ${server.join(',')} (the two gone ${server.join(',') === want.join(',')})${dismissed ? `; dismissed ${dismissed}` : ''}`,
      };
    }, { tries: 1 });
  }
} catch (error) {
