...
/**
 * GET /fonts/* (gslides-parity SPEC-5-amendments A5 items 3 and 4; B7): the catalog's faces for
 * the editor, the viewer and the picker. Two forms, both under the catalog's version so a refetch
 * at another commit is another URL and the answers cache as immutable:
 *
 ...
 * glob of vite.deploy.config.ts names the `fonts/assets/<id>/<file>` files, the integrator's R7).
 * Same origin CSS and fonts need no CORS header; `Cross-Origin-Resource-Policy: same-site` keeps
 * the files ours.
 */
