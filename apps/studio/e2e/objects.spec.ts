...
test('a text box drawn on the Title slide converts it; move, rotate, flip, group, ungroup, regroup, delete, each one write undone back to the Title slide', async ({
  ...
  /* with the switch on the Shapes row draws the gallery plate (docs/FOCUS.md section 4, the
     `altEffect`), so the rectangle is the plate's tile, the id the parity walk clicks (b1 R31) */
  await page.locator('[data-control="insert.shape.shapes.pick.rect"]').click();
