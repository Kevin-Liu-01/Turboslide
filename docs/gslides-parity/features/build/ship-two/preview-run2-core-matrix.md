...
Base https://turboslide-bxjtrzqfy-kl01s-projects.vercel.app, started 2026-09-25T22:28:03.645Z, 9503 s. 814 rows judged: 743 passed, 32 failed, 39 not driven (1 of them manual, the checklist's: text.clipboard.paste-without-formatting), 0 no step. Measurement rows (PRODUCT.md 8.2, recorded and never holding the ship; the cost rows of SYNC.md 6.1 among them, which hold it over their ceiling on the preview): export.download.large-deck-pdf not driven; export.download.large-deck-pptx not driven; cost.editor-idle.calls failed (function requests 9.65 a minute (ceiling 12; 29 in 3 min); the store settled 95.09 s after the state was ready (10 sync.status read(s) before the window; the first read: head 26, put 8, list 1, del 0); store simple 26 a minute (ceiling 40; 3 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced 12 a minute (ceiling 11); instances answering sync.status 1); cost.editor-hidden.calls not driven (function requests 9.66 a minute and 9 session poll(s) were read from the tab that stayed visible); cost.editor-editing.calls passed (function requests 71.25 a minute (ceiling 75; 214 in 3 min); the store settled 2.85 s after the state was ready (1 sync.status read(s) before the window); store simple 127 a minute (ceiling 140; 3 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced 75 a minute (ceiling 85); instances answering sync.status 1); cost.two-tabs-idle.calls failed (the store settled 2.60 s after the state was ready (1 sync.status read(s) before the window); store advanced 13 a minute (ceiling 30); store list 1 (ceiling 0); instances answering sync.status 1); cost.show.calls passed (function requests 0 in the window (ceiling 0); the store settled 115.22 s after the state was ready (12 sync.status read(s) before the window; the first read: head 18, put 17, list 0, del 0); store calls at the first sample 1 (head 1, get 0, put 0, list 0, del 0), of which 1 the probe's own sync.status read(s) (one deck.json head each, b3.md R7 b); the page's 0 (ceiling 0); function requests between the load and the window 0 (the settle, 115.22 s); instances answering sync.status 1); shaders.perf.editor-frame not driven. Cost rows over their ceiling in this run: cost.editor-idle.calls, cost.two-tabs-idle.calls. Verdict failed; retries 0 configured, 0 test(s) retried; exit 1. A not driven row is never counted as passed. Features a ship on this run would park (rule 4 of section 1; RETURN.md rule 2): diagrams, formatting, inbox, brand, fonts, templates, assist, shaders; rows whose own controls a ship would keep parked: view.live-pointers.second-browser (view.livePointers.mine, view.livePointers.collaborators); versions.show-changes-marks (file.versionHistory.showChanges); templates.gallery.page (home.gallery, file.new.templateGallery); templates.card.rename-and-delete (templates.card.menu); templates.default.use-for-new (panel.brand.template.useForNew); tables.seam.row-drag (handle.table.row); tables.edge.add-row-column (handle.table.add.column, handle.table.add.row); tables.heads.select-row-column (handle.table.head.column, handle.table.head.row); tables.bar.row-column-buttons (bar.table); arrange.group.tail-text-controls (toolbar.group.text); wordart.tail.fill-outline (toolbar.wordart.outline); logos.picker.search (insert.logo); logos.index.cached-offline (insert.logo); logos.picker.variants (dialog.logo.kind.wordmark, dialog.logo.tone.mono); logos.kit.find-a-logo (panel.brand.logo.find); shaders.panel.slider-live-undo (formatOptions.shader.strength); shaders.panel.preset-tiles (formatOptions.shader.preset); shaders.panel.kit-colours (formatOptions.shader.color); shaders.frame.auto-capture (insert.shader); shaders.frame.box-aspect (insert.shader); shaders.frame.reuse-and-prune (insert.shader); shaders.frame.one-capturer (insert.shader); shaders.background.place-answers (dialog.background.shader); shaders.perf.one-context (insert.shader); shaders.perf.hidden-pauses (insert.shader); shaders.background.add-to-theme (dialog.background.shader.addToTheme); shaders.library.glyph-engines-render (dialog.shader.engine.glyph); shaders.frame.scrubber-capture (formatOptions.shader.frame.scrubber, formatOptions.shader.frame.capture); shaders.show.plays-when-on (view.playShaders, formatOptions.shader.play); shaders.show.frame-when-off (view.playShaders); rows of an unparkable feature blocking the ship: decks.file.make-a-copy, images.insert.upload-while-pending, comments.resolve, versions.restore, export.download.options-dialog, export.download.progress-per-slide, export.download.mode-sentence, sync.title.concurrent-both-kept, sync.block.concurrent-same-offset-order, sync.resend.idempotent, logos.export.pdf-pptx-crisp, shaders.export.pdf-frame, shaders.export.pptx-frame, shaders.export.html-frame, shaders.export.missing-frame-row.
...
| `comments.resolve` | comments | core/present.spec.ts | flaky | failed | Error: page.waitForTimeout: Target page, context or browser has been closed |
...
| `versions.restore` | versions | core/share.spec.ts | flaky | failed | Error: locator.count: Target page, context or browser has been closed |
...
| `formatting.persistence` | formatting | probe --core | works | failed | a superscript word, UPPERCASE, a highlighted word and a grouped pair; reload; the show; the viewer: set {"sup":true,"highlight":true,"upper":true,"group":true}; after the reload {"sup":true,"highlight":true,"upper":true,"group":true}; the show {"sup":true,"highlight":true,"upper":true}; the viewer { |
...
| `versions.show-changes-marks` | versions | probe --core | not driven | failed | a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.959.pick (named row versionHistory.959.pick); marks with Show changes on 0 (); off 0 |
...
| `export.download.options-dialog` | export | core/export.spec.ts | not driven | failed | Error: page.goto: net::ERR_ABORTED at https://turboslide-bxjtrzqfy-kl01s-projects.vercel.app/edit/untitled-20260926-4anv |
| `export.download.progress-per-slide` | export | core/export.spec.ts | broken | failed | Error: page.waitForTimeout: Target page, context or browser has been closed |
...
| `brand.logo.replace-every-slide` | brand | core/brand.spec.ts | not driven | failed | Error: every slide's footer draws the picture within 5 s |
...
| `brand.background.enter-keeps-open` | brand | probe --core | broken | failed | Slide > Change background, type #0b3d91, Enter, Done; then Add to theme: Done: locator.boundingBox: Timeout 30000ms exceeded. |
...
| `brand.frame.toggles` | brand | core/present.spec.ts | not driven | failed | Error: locator.click: Target page, context or browser has been closed |
...
| `brand.surfaces.viewer-and-show` | brand | core/share.spec.ts | not driven | failed | Error: page.waitForTimeout: Target page, context or browser has been closed |
...
| `sync.title.concurrent-both-kept` | sync | core/sync.spec.ts | broken | failed | Error: page.waitForTimeout: Target page, context or browser has been closed |
...
| `cost.editor-idle.calls` | cost | cost-probe | broken | failed | store advanced 12 a minute over 11 |
...
| `logos.picker.search` | logos | probe --core | not driven | failed | Insert > Logo; type figma at human speed; Enter: 1 result tiles 328 ms after the last key (first figma active true; foot "Logos from thesvg.org, updated 25 September 2026. Brand marks belong to their owners; use them to name the brand, not to imply endorsement"); Enter inserted shot logo 467,415 108 |
...
| `shaders.panel.kit-colours` | shaders | probe --core | not driven | failed | read the Colors row's swatches; brand.set /colors/<appearance>/primary '#0b3d91'; read the shader and its frame: the Primary swatch clicked (revision 1054 -> 1055); swatches text, background, caption, hint, primary, accent, custom (6 of the six roles); the sheet renders dark (deck.info and the chrom |
| `shaders.panel.one-home` | shaders | probe --core | broken | passed |  |
| `shaders.frame.auto-capture` | shaders | core/shaders.spec.ts | broken | failed | Error: the frame arrives within 10 s of the change (800 ms plus the capture) |
| `shaders.frame.box-aspect` | shaders | core/shaders.spec.ts | not driven | failed | Error: browserContext._wrapApiCall: Target page, context or browser has been closed |
| `shaders.frame.reuse-and-prune` | shaders | core/shaders.spec.ts | not driven | not driven | skipped |
| `shaders.frame.one-capturer` | shaders | core/shaders.spec.ts | not driven | not driven | skipped |
| `shaders.background.place-answers` | shaders | probe --core | broken | failed | Slide > Change background on the body slide; Shader; a tile; Place; read the ground, the button and the dialog: chose Liquid metal; Place dialog.background.shader.place; the ground changed after 13506 ms (covering picture background, asset liquid-metal); the button read "Placing, 11 s" with the seco |
| `shaders.export.pdf-frame` | export | core/export.spec.ts | broken | not driven | skipped |
| `shaders.export.pptx-frame` | export | core/export.spec.ts | broken | not driven | skipped |
| `shaders.export.html-frame` | export | core/export.spec.ts | broken | not driven | skipped |
| `shaders.export.missing-frame-row` | export | core/export.spec.ts | not driven | not driven | skipped |
| `shaders.perf.one-context` | shaders | probe --core | not driven | failed | a second shader block placed through the window API; select each in turn and count the canvases: with nothing selected 1 canvas on the stage (1 on the page); first selected: 1 on the stage, 1 on the page; second selected: 1 on the stage after 730 ms, the second's root 1 canvas, the first's root 0 ca |
| `shaders.perf.hidden-pauses` | shaders | core/shaders.spec.ts | not driven | not driven | skipped |
| `shaders.perf.editor-frame` | shaders | core/shaders.spec.ts | not driven | not driven | skipped |
| `shaders.agent.list-insert-set-render` | shaders | core/shaders.spec.ts | not driven | not driven | skipped |
...
| `shaders.library.glyph-engines-render` | shaders | core/shaders.spec.ts | not driven | not driven | skipped |
...
| `shaders.show.plays-when-on` | shaders | core/shaders.spec.ts | broken | not driven | skipped |
| `shaders.show.frame-when-off` | shaders | core/shaders.spec.ts | not driven | not driven | skipped |
...
- `shaders.frame.reuse-and-prune`: skipped
- `shaders.frame.one-capturer`: skipped
- `shaders.export.pdf-frame`: skipped
- `shaders.export.pptx-frame`: skipped
- `shaders.export.html-frame`: skipped
- `shaders.export.missing-frame-row`: skipped
- `shaders.perf.hidden-pauses`: skipped
- `shaders.perf.editor-frame`: skipped
- `shaders.agent.list-insert-set-render`: skipped
...
- `shaders.library.glyph-engines-render`: skipped
...
- `shaders.show.plays-when-on`: skipped
- `shaders.show.frame-when-off`: skipped
...
- `comments.resolve`: Error: page.waitForTimeout: Target page, context or browser has been closed
- `versions.restore`: Error: locator.count: Target page, context or browser has been closed
- `formatting.persistence`: a superscript word, UPPERCASE, a highlighted word and a grouped pair; reload; the show; the viewer: set {"sup":true,"highlight":true,"upper":true,"group":true}; after the reload {"sup":true,"highlight":true,"upper":true,"group":true}; the show {"sup":true,"highlight":true,"upper":true}; the viewer {
- `view.live-pointers.second-browser`: Error: the second browser's pointer is drawn on the first within 10 s
- `versions.show-changes-marks`: a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.959.pick (named row versionHistory.959.pick); marks with Show changes on 0 (); off 0
...
- `export.download.options-dialog`: Error: page.goto: net::ERR_ABORTED at https://turboslide-bxjtrzqfy-kl01s-projects.vercel.app/edit/untitled-20260926-4anv
- `export.download.progress-per-slide`: Error: page.waitForTimeout: Target page, context or browser has been closed
- `brand.logo.replace-every-slide`: Error: every slide's footer draws the picture within 5 s
- `brand.background.enter-keeps-open`: Slide > Change background, type #0b3d91, Enter, Done; then Add to theme: Done: locator.boundingBox: Timeout 30000ms exceeded.
- `brand.frame.toggles`: Error: locator.click: Target page, context or browser has been closed
- `brand.surfaces.viewer-and-show`: Error: page.waitForTimeout: Target page, context or browser has been closed
- `sync.title.concurrent-both-kept`: Error: page.waitForTimeout: Target page, context or browser has been closed
...
- `cost.editor-idle.calls`: store advanced 12 a minute over 11
...
- `shaders.frame.auto-capture`: Error: the frame arrives within 10 s of the change (800 ms plus the capture)
- `shaders.frame.box-aspect`: Error: browserContext._wrapApiCall: Target page, context or browser has been closed
...
- `cost.editor-idle.calls`: failed (store advanced 12 a minute over 11); recorded function requests 9.65 a minute (ceiling 12; 29 in 3 min); the store settled 95.09 s after the state was ready (10 sync.status read(s) before the window; the first read: head 26, put 8, list 1, del 0); store simple 26 a minute (ceiling 40; 3 of the heads the probe's own sync.status reads, which the  (a cost row: over its ceiling on the preview it holds the ship)
...
- `cost.editor-editing.calls`: passed; recorded function requests 71.25 a minute (ceiling 75; 214 in 3 min); the store settled 2.85 s after the state was ready (1 sync.status read(s) before the window); store simple 127 a minute (ceiling 140; 3 of the heads the probe's own sync.status reads, which the ceiling carries, b3.md R7 b); store advanced  (a cost row: over its ceiling on the preview it holds the ship)
- `cost.two-tabs-idle.calls`: failed (store list 1, ceiling none); recorded the store settled 2.60 s after the state was ready (1 sync.status read(s) before the window); store advanced 13 a minute (ceiling 30); store list 1 (ceiling 0); instances answering sync.status 1 (a cost row: over its ceiling on the preview it holds the ship)
- `cost.show.calls`: passed; recorded function requests 0 in the window (ceiling 0); the store settled 115.22 s after the state was ready (12 sync.status read(s) before the window; the first read: head 18, put 17, list 0, del 0); store calls at the first sample 1 (head 1, get 0, put 0, list 0, del 0), of which 1 the probe's own sync.sta (a cost row: over its ceiling on the preview it holds the ship)
- `shaders.perf.editor-frame`: not driven (skipped)
