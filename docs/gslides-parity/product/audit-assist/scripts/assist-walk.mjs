...
const has = (page, selector) =>
  page.locator(selector).first().isVisible({ timeout: 300 }).catch(() => false);
    ...
    (p) =>
      [...document.querySelectorAll(`[data-control^="${p}"]`)]
        ...
        .map((el) => ({
          ...
          label: (el.getAttribute('aria-label') ?? el.getAttribute('data-tip') ?? el.textContent ?? '').trim().slice(0, 80),
        })),
    prefix,
  );
...
const runs = (page) =>
  page.evaluate(() =>
    [...document.querySelectorAll('.ts-stagewrap.ts-editor .pt-slide [data-run]')].map((el) => {
      ...
      return { run: el.getAttribute('data-run'), text: el.textContent?.trim() ?? '', rect: { x: r.x, y: r.y, w: r.width, h: r.height } };
    }),
  );
const stageText = (page) =>
  page.evaluate(() => document.querySelector('.ts-stagewrap.ts-editor .pt-slide')?.textContent?.trim() ?? '');
const snackbar = (page) =>
  page.evaluate(
    () =>
...
const banner = (page) =>
  page.evaluate(() => document.querySelector('.ts-banner[data-state="external"]')?.textContent?.trim() ?? null);
...
const paletteRows = (page) =>
  page.evaluate(() =>
    [...document.querySelectorAll('[data-control^="palette."]')]
      .filter((el) => el.getClientRects().length > 0 && el.getAttribute('data-control') !== 'palette.query')
      .map((el) => ({ id: el.getAttribute('data-control'), text: el.textContent?.trim().replace(/\s+/g, ' ').slice(0, 120) ?? '' })),
  );
const paletteCount = (page) =>
  page.evaluate(() => {
    ...
    const count = card ? (card.textContent?.match(/(\d+ results?|Nothing matches)/)?.[0] ?? null) : null;
    return { count, empty };
  });
...
const BULLETS = ['Pricing per seat, billed yearly', 'Onboarding in two weeks', 'Support in twelve languages'];
const NOTE = 'Ask about their renewal date before the pricing slide.';

try {
  // ---- 1. a scratch deck from /new with its title
  await step('open /new and type the title', 'the editor is ready; the title is written; the address is /edit/<id>', async () => {
    ...
    if (await has(page, '[data-control="dialog.namePrompt"]')) await clickControl(page, 'dialog.namePrompt.close').catch(() => undefined);
    const list = await runs(page);
    ...
    return { ok: rev >= 1 && /\/edit\//.test(page.url()) && text.includes('Acme'), observed: `deck ${deckId}, revision ${rev}, ${page.url().replace(BASE, '')}, stage reads "${text.slice(0, 60)}"` };
  });

  // ---- 2. where an assistant would sit: the title row and the toolbar
  await step('read the title row and the toolbar for an assistant entry point', 'no control named Ask, Assist, Gemini or AI', async () => {
    ...
    const hits = [...title, ...toolbar, ...menubar].filter((c) => words.test(c.label) || words.test(c.id));
    ...
    return { ok: hits.length === 0, observed: `title row ${title.length} controls (${title.map((c) => c.id.replace('title.', '')).join(', ')}); toolbar ${toolbar.length}; menubar ${menubar.map((c) => c.id.replace('menubar.', '')).join(' ')}; assistant words: ${hits.length === 0 ? 'none' : hits.map((h) => h.id).join(', ')}` };
  });

  // ---- 3. a second slide with three lines in the body, and a speaker note
  await step('New slide from the toolbar, three lines typed in the body', 'a second slide whose body holds three paragraphs', async () => {
    const before = (await invoke(page, 'slide.list')).length ?? null;
    await clickControl(page, 'toolbar.newSlide');
    await pollUntil(() => state(page), (s) => s.slideId !== titleSlide, 10_000);
    await sleep(600);
    const s = await state(page);
    ...
    if (!body) return { ok: false, observed: `slide ${secondSlide}: runs ${list.map((x) => x.run).join(', ')}; no body run` };
    ...
    facts.secondSlide = { id: secondSlide, kind: slide.kind, layout: slide.layout, blocks: JSON.stringify(slide).length };
    await shot(page, '02-second-slide-three-lines.png');
    return { ok: paragraphs === 3, observed: `slide ${secondSlide} (${slide.kind}, layout ${JSON.stringify(slide.layout).slice(0, 40)}): ${paragraphs} of 3 lines found in the document; slides ${before} before` };
  });

  await step('type a speaker note under the slide', 'slide.notes carries the sentence', async () => {
    ...
    const got = await pollUntil(
      () => invoke(page, 'slide.get', { slideId: secondSlide }).then((g) => (g.slide ?? g).notes ?? null),
      (n) => n !== null && n.includes('renewal'),
      8000,
    );
    return { ok: typeof got === 'string' && got.includes('renewal'), observed: `notes: ${JSON.stringify(got)}` };
  });
  ...
  for (const [phrase, expect] of PHRASES) {
    await step(`Search the menus: "${phrase}"`, `rows for ${expect}`, async () => {
      ...
      let file = null;
      if (SHOT_PHRASES.has(phrase)) file = await shot(page, `03-search-${phrase.replace(/\s+/g, '-')}.png`);
      await press(page, 'Escape');
      await page.locator('[data-control="palette.query"]').waitFor({ state: 'detached', timeout: 4000 }).catch(() => undefined);
      const rowsText = list.map((r) => r.text).join(' || ');
      facts.search.push({ phrase, expect, rows: list.length, ids: list.map((r) => r.id), texts: list.map((r) => r.text), count, file });
      return { ok: null, observed: `${count.count ?? (count.empty ?? `${list.length} rows`)}: ${rowsText.slice(0, 400) || 'no rows'}`, extra: { rows: list.length } };
    });
  }

  // ---- 5. Check slides, the one suggestion surface
  await step('Tools > Check slides on slide two', 'the panel lists suggestions with Fix where one exists', async () => {
    ...
    const head = await page.evaluate(() => document.querySelector('[data-control="panel.checkSlides"] .ts-panel-head')?.textContent?.trim() ?? '');
    const empty = await page.evaluate(() => document.querySelector('[data-control="panel.checkSlides"] .ts-lint-empty')?.textContent?.trim() ?? null);
    ...
    return { ok: true, observed: `panel "${head}"; ${suggestions.length} suggestions (${suggestions.filter((s) => s.fix).length} with Fix): ${suggestions.map((s) => s.text).join(' || ').slice(0, 400)}${empty ? `; empty: ${empty}` : ''}` };
  });

  // ---- 6. the raw agent surface: Extensions > Agent access and Run an action, behind the switch
  await step('Extensions with the switch off, then Tools > Advanced tools on', 'Extensions is not drawn; after the switch it is', async () => {
    const before = (await controlsUnder(page, 'menubar.')).map((c) => c.id.replace('menubar.', ''));
    await openMenu(page, 'tools');
    const checkedBefore = await ctl(page, 'menu.tools.advancedTools').first().getAttribute('aria-checked');
    await clickRow(page, 'tools.advancedTools');
    await sleep(500);
    await press(page, 'Escape');
    const after = (await controlsUnder(page, 'menubar.')).map((c) => c.id.replace('menubar.', ''));
    return { ok: !before.includes('extensions') && after.includes('extensions'), observed: `switch was ${checkedBefore}; menubar before: ${before.join(' ')}; after: ${after.join(' ')}` };
  });

  await step('Extensions > Agent access', 'the dialog with the MCP and API addresses and the token sentence', async () => {
    ...
    await sleep(400);
    const text = await page.evaluate(() => document.querySelector('[role="dialog"]')?.textContent?.trim().replace(/\s+/g, ' ') ?? '');
    facts.agentAccess = { rows: extRows.map((r) => r.label), text };
    await shot(page, '05-agent-access-dialog.png');
    await press(page, 'Escape');
    await sleep(300);
    return { ok: /token/i.test(text), observed: `Extensions rows: ${extRows.map((r) => r.label).join(', ')}; dialog: ${text.slice(0, 500)}` };
  });

  await step('Tools > Advanced > Run an action…', 'the full palette with every action by name', async () => {
    await openMenu(page, 'tools');
    ...
    await page.locator('[data-control="palette.query"]').waitFor({ timeout: 8000 });
    await sleep(400);
    const placeholder = await page.evaluate(() => document.querySelector('[data-control="palette.query"]')?.getAttribute('placeholder') ?? null);
    ...
    await press(page, 'Escape');
    await page.locator('[data-control="palette.query"]').waitFor({ state: 'detached', timeout: 4000 }).catch(() => undefined);
    return { ok: list.length > 0, observed: `placeholder "${placeholder}"; open: ${count0.count}; ">replace": ${count1.count}: ${list.map((r) => r.text).join(' || ').slice(0, 400)}` };
  });

  await step('Tools > Advanced tools off again', 'the switch reads false and Extensions leaves the bar', async () => {
    await openMenu(page, 'tools');
    await clickRow(page, 'tools.advancedTools');
    await sleep(400);
    await press(page, 'Escape');
  ...
  await step('go back to the title slide and read the revision', 'the title slide is current', async () => {
    const card = page.locator(`[data-control="filmstrip.slide.${titleSlide}"]`).first();
    const r = await card.boundingBox();
    if (r) await clickAt(page, r.x + r.width / 2, r.y + r.height / 2);
    await pollUntil(() => state(page), (s) => s.slideId === titleSlide, 8000);
    await settled(page);
    const s = await state(page);
    facts.beforeAgentWrite = { revision: s.revision, text: await stageText(page) };
    return { ok: s.slideId === titleSlide, observed: `slide ${s.slideId}, revision ${s.revision}, stage "${facts.beforeAgentWrite.text.slice(0, 60)}"` };
  });

  await step('POST /api/actions/text.replaceAll as agent:assist-probe (Acme to Globex) while the editor holds the slide', 'a 409 with the lease holder, or the write lands', async () => {
    if (!token) return { ok: null, observed: 'not driven: no bearer for production in hosts.json' };
    const info = await invoke(page, 'deck.info');
    const post = async (force) => {
      const res = await fetch(`${BASE}/api/actions/text.replaceAll?deck=${encodeURIComponent(deckId)}${force ? '&force=1' : ''}`, {
        method: 'POST',
        headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json', 'x-turboslide-author': 'agent:assist-probe' },
    ...
    const describe = (r) => {
      ...
      if (r.body.error) return `${r.status} ${r.body.error.name}: ${String(r.body.error.message).slice(0, 160)}${r.body.error.holder ? `; holder ${JSON.stringify(r.body.error.holder).slice(0, 120)}` : ''}`;
    ...
    return { ok: landed !== null, observed: `first: ${describe(first)}${facts.agentWrite.forced ? `; with force=1: ${facts.agentWrite.forced.summary}` : ''}` };
  });

  await step('read the editor after the outside write', 'the stage reads Globex within 10 s; the banner or the snackbar names the author', async () => {
    const text = await pollUntil(() => stageText(page), (t) => t.includes('Globex'), 12_000, 300);
    ...
    return { ok: text.includes('Globex'), observed: `stage "${text.slice(0, 60)}"; revision ${s.revision}; banner: ${b ?? 'none'}; snackbar: ${sb ?? 'none'}` };
  });

  await step('Cmd+Z after the outside write', 'whether the seller can undo the assistant write with one undo', async () => {
    await clickAt(page, 1400, 500); // the workspace beside the sheet, so the stage has the focus
    await sleep(200);
    const canUndo = await page.evaluate(() => window.turboslide?.studio?.describe?.().state?.history?.canUndo ?? null);
    ...
    return { ok: null, observed: `canUndo before ${canUndo}; after Cmd+Z the stage reads "${text.slice(0, 60)}" (revision ${s.revision}); snackbar: ${sb ?? 'none'}` };
  });

  await step('File > Version history > See version history', 'the agent write is a row with the Agent mark', async () => {
    ...
    const entries = await page.evaluate(() =>
      [...document.querySelectorAll('[data-control="panel.versionHistory"] [data-author]')].map((el) => ({
        author: el.getAttribute('data-author'),
        text: el.textContent?.trim().replace(/\s+/g, ' ').slice(0, 160) ?? '',
        agentMark: el.querySelector('[data-trust="agent"], .is-agent, [class*="agent"]') !== null,
        label: el.querySelector('[aria-label]')?.getAttribute('aria-label') ?? null,
      })),
    );
    const text = await page.evaluate(() => document.querySelector('[data-control="panel.versionHistory"]')?.textContent?.trim().replace(/\s+/g, ' ').slice(0, 600) ?? '');
    ...
    const r = await x.boundingBox();
    if (r) await clickAt(page, r.x + r.width / 2, r.y + r.height / 2);
    const agentRows = entries.filter((e) => /agent/i.test(e.text) || /agent/i.test(e.label ?? '') || e.agentMark);
    return { ok: entries.length > 0, observed: `${entries.length} rows; agent rows ${agentRows.length}: ${agentRows.map((e) => `${e.label ?? ''} ${e.text}`).join(' || ').slice(0, 300)}; panel: ${text.slice(0, 300)}` };
  });
  ...
  await step('the New slide arrow: the layout picker', 'the layouts by name', async () => {
    ...
    const tiles = await page.evaluate(() =>
      [...document.querySelectorAll('[data-control^="layout."]')]
        .filter((el) => el.getClientRects().length > 0 && el.getAttribute('data-control') !== 'layout')
        .map((el) => ({ id: el.getAttribute('data-control').replace('layout.', ''), label: (el.getAttribute('aria-label') ?? el.textContent ?? '').trim().slice(0, 60) })),
    );
    facts.layouts = tiles;
    await shot(page, '10-layout-picker.png');
    await press(page, 'Escape');
    await sleep(300);
    return { ok: tiles.length > 0, observed: `${tiles.length} layouts: ${tiles.map((t) => t.label || t.id).join(', ')}` };
  });

  // ---- 9. the reads an assist would send as the deck's context
  await step('the context reads: deck.info, export.text, slide.get', 'sizes of what a server would send to a model', async () => {
    const info = await invoke(page, 'deck.info');
    ...
    return { ok: typeof text.text === 'string' && text.text.includes('renewal'), observed: `deck.info ${facts.context.deckInfoChars} chars; export.text ${text.bytes} bytes over ${text.slides} slides (notes included: ${text.text.includes('renewal')}); slide.get ${facts.context.slideGetChars} chars; slide.list ${facts.context.slideListChars} chars` };
  });
} catch (error) {
  record('the walk', 'no exception outside a step', error instanceof Error ? (error.stack ?? error.message) : String(error), false);
} finally {
  ...
  if (deckId) {
    ...
    try {
      ...
      record('the scratch deck answers 404', `GET /edit/${deckId} is 404 within 20 s`, `status ${status}`, status === 404);
      const deck = await page.request.get(`${BASE}/deck/${deckId}`, { maxRedirects: 0 });
      record('the scratch deck answers 404 on /deck', `GET /deck/${deckId} is 404`, `status ${deck.status()}`, deck.status() === 404);
    } catch (error) {
      record('the scratch deck answers 404', `GET /edit/${deckId} is 404`, `probe failed: ${error instanceof Error ? error.message : String(error)}`, false);
    }
  }
