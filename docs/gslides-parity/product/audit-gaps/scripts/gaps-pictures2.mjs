...
try {
  ...
  let pic = null;
  await t.step(
    'Insert > Image > Upload from computer with a PNG: the block that lands',
    'a shot block with the asset, 480 by 300 at the content origin',
    async () => {
      ...
      return {
        ok: Boolean(obj),
        observed: `${obj ? brief(obj.block) : 'no shot'} in ${Date.now() - t0} ms; objects now ${all.join(',')}; snackbar "${await t.snackbar()}"; ${shot}`,
      };
    },
  );
  ...
  await t.step(
    ...
    async () => {
      ...
      if (!uploadRow) {
        ...
        return {
          ok: false,
          observed: `no Upload row after the click; rows ${rows.join(',')}; ${shot0}`,
        };
      }
      const t0 = Date.now();
      ...
      return {
        ...
        observed: `rows ${rows.join(',')}; asset ${before?.asset} -> ${after?.asset} in ${Date.now() - t0} ms; pos ${JSON.stringify(before?.pos)} -> ${JSON.stringify(after?.pos)}; snackbar "${await t.snackbar()}"; ${shot0}; ${shot}`,
      };
    },
  );

  await t.step(
    'Crop: double click the picture, drag the east crop edge 100 sheet px in, Enter',
    'crop mode, the frame narrows, the trim is written and the box shrinks',
    async () => {
      if (!pic) return { ok: false, observed: 'no picture' };
      ...
      if (!east) {
        ...
        return { ok: false, observed: `no crop handles; ${handles.join(',')}; chip "${chip}"` };
      }
      ...
      return {
        ...
        observed: `chip "${chip}"; ${brief(block)}; ${shot}`,
      };
    },
  );

  await t.step(
    'Undo the crop from the toolbar',
    'the trim leaves and the box returns',
    async () => {
      if (!pic) return { ok: false, observed: 'no picture' };
  ...
  await t.step(
    'Mask image with the switch on: the Crop image arrow, Ellipse',
    'the picture takes an ellipse mask and draws inside it',
    async () => {
      if (!pic) return { ok: false, observed: 'no picture' };
      ...
      const rows = await page.evaluate(() =>
        [
          ...document.querySelectorAll(
            '[data-control^="menu.format.image.maskImage"], [data-control^="format.image.maskImage"]',
          ),
        ]
          .filter((el) => el.getClientRects().length > 0)
          .map((el) => el.getAttribute('data-control')),
      ...
      if (!ellipse) {
        await t.closeMenus();
        return { ok: false, observed: `no ellipse cell; rows ${rows.slice(0, 12).join(',')}` };
      }
      ...
      const drawn = await page.evaluate((id) => {
        ...
        const fc = getComputedStyle(free);
        return {
          clipPath: cs.clipPath,
          mask: cs.maskImage || cs.webkitMaskImage,
          freeClip: fc.clipPath,
          svgClip: inner.querySelector('clipPath, mask') !== null,
        };
      }, pic.id);
      const shot2 = await t.shot('mask-applied');
      await t.closeMenus();
      return {
        ok: Boolean(block?.mask),
        observed: `cells ${rows.length}; ${brief(block)}; drawn ${JSON.stringify(drawn)}; ${shot}; ${shot2}`,
      };
    },
  );

  await t.step(
    'Image options: the sections; drag Transparency to about 40 percent',
    'adjust.transparency about 0.4 and the picture at 0.6 opacity',
    async () => {
      if (!pic) return { ok: false, observed: 'no picture' };
      await t.selectObject(pic.id);
      ...
      return {
        ok: typeof block?.adjust?.transparency === 'number',
        observed: `sections ${sections.join(', ')}; track ${JSON.stringify(track)}; ${brief(block)}; img ${JSON.stringify(box?.img)}; recolor ${await t.has('[data-control="formatOptions.adjustments.recolor"]')}; ${shot}`,
      };
    },
  );
  ...
  await t.step(
    ...
    'a frame colour is written and drawn around the picture',
    async () => {
      if (!pic) return { ok: false, observed: 'no picture' };
      await t.selectObject(pic.id);
      ...
      const cells = await page.evaluate(() =>
        [
          ...document.querySelectorAll(
            '.ts-ctl-swatches button, [data-control^="toolbar.borderColor."]',
          ),
        ]
          .filter((el) => el.getClientRects().length > 0)
          .map((el) => el.getAttribute('data-control') ?? el.getAttribute('aria-label')),
      );
      const shot = await t.shot('border-picker');
      const cell =
        cells.find((c) => /\.ink$|\bink\b/.test(c ?? '')) ??
        cells.find((c) => c && !/none|hex|custom/i.test(c));
      if (cell) {
        const loc = cell.startsWith('toolbar.')
          ? page.locator(`[data-control="${cell}"]`)
          : page.locator(`.ts-ctl-swatches button[aria-label="${cell}"]`);
      ...
      const drawn = await page.evaluate((id) => {
        ...
        const free = inner.closest('.free') ?? inner;
        const img = inner.querySelector('img') ?? inner;
        const a = getComputedStyle(free);
        const c = getComputedStyle(img);
        return {
          free: `${a.borderWidth} ${a.borderStyle} ${a.borderColor}; outline ${a.outlineWidth} ${a.outlineColor}; shadow ${a.boxShadow}`,
          img: `${c.borderWidth} ${c.borderStyle} ${c.borderColor}; outline ${c.outlineWidth}; shadow ${c.boxShadow}`,
          frameEl: inner.querySelector('.ts-frame, [data-frame], rect') !== null,
        };
      }, pic.id);
      ...
      return {
        ...
        observed: `cells ${cells.join(',')}; picked ${cell}; ${brief(block)}; drawn ${JSON.stringify(drawn)}; ${shot}; ${shot2}`,
      };
    },
  );

  await t.step(
    'Alt text on the picture (Format options > Alt text)',
    ...
    async () => {
      ...
      const before = await page.evaluate(
        () =>
          document.querySelector('[data-control="formatOptions.altText.description"]')?.value ??
          null,
      );
      ...
      const asset = await page
        .evaluate(
          (id) => window.turboslide.studio.describe().state?.document?.deck?.assets?.[id] ?? null,
          block?.asset,
        )
        .catch(() => null);
      const shot = await t.shot('alt-text');
      return {
        ok: /Acme/.test(JSON.stringify(block) + JSON.stringify(asset)),
        observed: `field before "${before}"; ${brief(block)}; asset record alt "${asset?.alt}"; ${shot}`,
      };
    },
  );
  await t.press('Escape', 2);
  ...
  const dropFile = async (name, sx, sy) => {
    ...
    const bytes =
      name === 'b'
        ? Buffer.concat([b.subarray(0, 40), Buffer.from([0x01]), b.subarray(41)])
        : pngFixture();
  ...
  await t.step(
    ...
    'the picture is replaced and keeps its box; no new object',
    async () => {
      if (!pic) return { ok: false, observed: 'no picture' };
      const before = await shotOf(pic.id);
      ...
      await dropFile(
        'a',
        (box.inner.x + box.inner.w / 2 - sheet.x) / kk,
        (box.inner.y + box.inner.h / 2 - sheet.y) / kk,
      );
      const after = await t.pollUntil(
        () => shotOf(pic.id),
        (bl) => bl?.asset !== before?.asset,
      ...
      return {
        ok: after?.asset !== before?.asset && countAfter === count,
        observed: `asset ${before?.asset} -> ${after?.asset}; objects ${count} -> ${countAfter}; pos ${JSON.stringify(before?.pos)} -> ${JSON.stringify(after?.pos)}; ${shot}`,
      };
    },
  );

  await t.step(
    'Reload; the pictures survive; the show draws them',
    'the shot count holds after the reload and the show draws the picture',
    async () => {
        ...
        () =>
          [...document.querySelectorAll('img')].filter(
            (el) =>
              ...
              el.closest('.ts-filmstrip, [data-control="filmstrip"]') === null,
          ).length,
      );
      ...
      return {
        ok: after === n && n > 0 && show,
        observed: `shots ${n} -> ${after}; show ${show}; visible img elements ${imgs}; ${shot}`,
      };
    },
  );
  ...
  writeFileSync(
    path.join(EVIDENCE, 'pictures2-console.json'),
