#!/usr/bin/env node
// The hotfix ship step's commit path list (VERIFICATION-3 section 17; the orchestrator's rule:
// only this hotfix's source, tests, docs, scripts and evidence). Reads `git status --porcelain
// -uall`, drops what never ships (the other workflow's `.github/`, `research-4/`, `design-4/`,
// `verification-4/`, `SPEC-4.md`, `MILESTONES-4.md`; the scratch decks a spec or a probe left
// under decks/; `scripts/__pycache__`; the earlier ship step's driver flag files), asserts no
// forbidden path is left (`.turboslide`, `.vercel`, `dist`, `target`, `node_modules`, `.log`,
// `.env`, `.sqlite`, `.tsbuildinfo`, `.pyc`, `.DS_Store`) and prints one path per line.
//
//   node commit-paths.mjs > /tmp/paths.txt
...
const SCRATCH_DECK = /^decks\/(e2e-|untitled-|verifier-|ship-|probe-)/;
// the chain drivers' done flags: scratch, never evidence
const FLAG_FILE = /\.flag$/;
const FORBIDDEN = [
  /(^|\/)\.turboslide(\/|$)/,
  /(^|\/)\.vercel(\/|$)/,
  /(^|\/)dist(\/|$)/,
