...
import { helpTitle } from '../apps/studio/src/components/help/help-title.ts';
...
/**
 * The titles of the two help pages as a generated module (gslides-parity SPEC-5 7.6; VERIFICATION-5
 * finding 10, the round five fix round): a route's `head` names its page title, and reading it
 * from the markdown at runtime held the whole document in the entry chunk on every route (the
 * route file is entry code, its component is split). The generator reads each document's first
 * heading with the reader's own rule (`helpTitle`, pinned against `markdownTitle` in
 * help-title.test.ts) and `--check` fails when a document's heading moved; `--help-titles` writes
 * the module alone, without rebuilding the pictures.
 */
const HELP_TITLES_MODULE_PATH = 'apps/studio/src/components/help/help-titles.ts';
const HELP_DOCS = {
  training: { path: 'docs/training.md', fallback: 'Training' },
  updates: { path: 'docs/updates.md', fallback: 'Updates' },
} as const;
...
function helpTitlesData(): Record<keyof typeof HELP_DOCS, string> {
  const out = {} as Record<keyof typeof HELP_DOCS, string>;
  for (const [page, doc] of Object.entries(HELP_DOCS) as [
    keyof typeof HELP_DOCS,
    { path: string; fallback: string },
  ][]) {
    const file = resolve(ROOT, doc.path);
    if (!existsSync(file)) fail(`${doc.path} is missing`);
    out[page] = helpTitle(readFileSync(file, 'utf8'), doc.fallback);
  }
  return out;
}

function helpTitlesModuleSource(data: Record<string, string>): string {
  return [
    ...
    `export const HELP_TITLES = ${JSON.stringify(data, null, 2)} as const;`,
    '',
  ].join('\n');
}

async function checkHelpTitles(): Promise<void> {
  const file = resolve(ROOT, HELP_TITLES_MODULE_PATH);
  if (!existsSync(file)) fail(`${HELP_TITLES_MODULE_PATH} is missing; run --help-titles`);
  const loaded = (await import(pathToFileURL(file).href)) as {
    HELP_TITLES?: Record<string, string>;
  };
  const wanted = helpTitlesData();
  if (JSON.stringify(loaded.HELP_TITLES) !== JSON.stringify(wanted))
    fail(`${HELP_TITLES_MODULE_PATH} does not match the documents' headings; run --help-titles`);
}

/** The generated modules in the repository's own style, so the format gate reads them as any other file. */
function formatGenerated(paths: readonly string[]): void {
  const prettier = spawnSync(
    resolve(ROOT, 'node_modules/.bin/prettier'),
    ['--write', '--log-level', 'warn', ...paths],
    { cwd: ROOT, stdio: 'inherit' },
  );
  if (prettier.status !== 0) fail(`prettier --write exited ${prettier.status}`);
}

function writeHelpTitles(): void {
  writeFileSync(resolve(ROOT, HELP_TITLES_MODULE_PATH), helpTitlesModuleSource(helpTitlesData()));
}

async function check(): Promise<void> {
  await checkHelpTitles();
  ...
  console.log(
    `build-home-assets --check: ${files} files of ${manifest.shots.length} pictures match ${MANIFEST_PATH}, ${MODULE_PATH} and their sources; ${FACTS_MODULE_PATH} matches ${FACTS_PATH}; ${HELP_TITLES_MODULE_PATH} matches the help documents`,
  );
}
...
async function main(): Promise<void> {
  ...
  if (argv.includes('--help-titles')) {
    writeHelpTitles();
    formatGenerated([HELP_TITLES_MODULE_PATH]);
    console.log(`build-home-assets --help-titles: wrote ${HELP_TITLES_MODULE_PATH}`);
    return;
  }
  ...
  writeHelpTitles();
  /* the generated modules in the repository's own style, so the format gate reads them as any other file */
  formatGenerated([MODULE_PATH, FACTS_MODULE_PATH, HELP_TITLES_MODULE_PATH]);
  ...
  console.log(
    `${outputs.length} files, ${total} bytes under ${PUBLIC_DIR}; ${MANIFEST_PATH}, ${MODULE_PATH}, ${FACTS_MODULE_PATH} and ${HELP_TITLES_MODULE_PATH} written`,
  );
}
