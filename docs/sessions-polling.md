...
3. The unheld poll. `POLL_MS` is 0 on the client and `POLL_MAX_MS` 0 on the server: the registry's `poll()` answers an unknown id at once and an empty answer at once, and the hook pauses `EMPTY_ANSWER_PAUSE_MS` (20,000 ms) after an empty answer, so a held poll that cost about 25 s of a 2 GB function per cycle costs about 50 ms. For `HOT_WINDOW_MS` (30,000 ms) after a command the pace is `HOT_PAUSE_MS` (1,000 ms), so a second command lands within a second. `DEFAULT_COMMAND_TIMEOUT_MS` is 30 s (it was 15 s), so a command issued right after a poll survives the pause; a page attached but not answering waits up to 30 s before its 504.
