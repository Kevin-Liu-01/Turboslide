...
describe('the Vercel client under the store deadline', () => {
  ...
  it('the SDK alone keeps retrying a failed request past any deadline the caller kept (the mechanism of C2-F24)', async () => {
    hits = 0;
  ...
  it('the bounded store client ends the call underneath at the deadline: no attempt follows it', async () => {
    hits = 0;
  ...
  it('a put, a list and a del carry the signal the same way (a get of a public store reads the blob URL, not the API, so it is not driven here)', async () => {
    hits = 0;
    const client = boundedBlobClient(vercelBlobClient({ BLOB_READ_WRITE_TOKEN: TOKEN }), {
      readMs: 200,
      documentWriteMs: 200,
