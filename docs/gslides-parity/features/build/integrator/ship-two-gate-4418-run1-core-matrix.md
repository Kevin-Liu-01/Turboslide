...
Base http://localhost:4418, started 2026-09-24T19:52:30.002Z, 6601 s. 814 rows judged: 752 passed, 34 failed, 28 not driven (1 of them manual, the checklist's: text.clipboard.paste-without-formatting), 0 no step. Measurement rows (PRODUCT.md 8.2, recorded and never holding the ship; the cost rows of SYNC.md 6.1 among them, which hold it over their ceiling on the preview): export.download.large-deck-pdf passed (PDF: GT brand deck.pdf, 85 slides in 3.7 s, 0.04 s per slide; PDF: GT brand deck.pdf, 85 slides in 3.7 s, 0.04 s per slide); export.download.large-deck-pptx passed (Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 35.2 s, 0.41 s per slide; Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 35.2 s, 0.41 s per slide); cost.editor-idle.calls passed (function requests 10 a minute (ceiling 12; 30 in 3 min); the store half reads zero on this base (the file store); the function requests alone are asserted); cost.editor-hidden.calls not driven (function requests 10 a minute and 9 session poll(s) were read from the tab that stayed visible); cost.editor-editing.calls passed (function requests 74.63 a minute (ceiling 75; 224 in 3 min); the store half reads zero on this base (the file store); the function requests alone are asserted); cost.two-tabs-idle.calls passed (the store half reads zero on this base (the file store); the function requests alone are asserted); cost.show.calls passed (function requests 0 in the window (ceiling 0); the store half reads zero on this base (the file store); the function requests alone are asserted); shaders.perf.editor-frame failed (longest animation frame 158.4 ms over 5 s with one shader on the stage (583 animation frames; rAF gaps; 1 canvas; renderer ANGLE (Google, Vulkan 1.3.0 (SwiftShader Device (LLVM 10.0.0) (0x0000C0DE)), SwiftShader driver)); longest animation frame 158.4 ms over 5 s with one shader on the stage (583 animation frames; rAF gaps; 1 canvas; renderer ANGLE (Google, Vulkan 1.3.0 (SwiftShader Device (LLVM 10.0.0) (0x0000C0DE)), SwiftShader driver))). Cost rows over their ceiling in this run: none. Verdict failed; retries 0 configured, 0 test(s) retried; exit 1. A not driven row is never counted as passed. Features a ship on this run would park (rule 4 of section 1; RETURN.md rule 2): inbox, brand, fonts, templates, assist, logos; rows whose own controls a ship would keep parked: view.live-pointers.second-browser (view.livePointers.mine, view.livePointers.collaborators); versions.show-changes-marks (file.versionHistory.showChanges); templates.gallery.page (home.gallery, file.new.templateGallery); assist.rewrite.card-accept-undo (panel.assist.starter.shorter); assist.notes.draft (panel.assist.starter.notes); assist.free-ask.fallback-sentence (panel.assist.prompt); tables.seam.row-drag (handle.table.row); tables.edge.add-row-column (handle.table.add.column, handle.table.add.row); tables.heads.select-row-column (handle.table.head.column, handle.table.head.row); tables.bar.row-column-buttons (bar.table); arrange.group.tail-text-controls (toolbar.group.text); wordart.tail.fill-outline (toolbar.wordart.outline); logos.picker.search (insert.logo); logos.picker.paper-and-ink (insert.logo); logos.picker.recents (dialog.logo.group.recent); logos.picker.licence-words (dialog.logo.source); logos.picker.chrome-1280 (insert.logo); logos.insert.one-click-asset (insert.logo); logos.insert.logo-size (insert.logo); logos.insert.mono-tint (insert.logo); logos.insert.every-slide (dialog.logo.everySlide); logos.tailor.find-customer-logo (dialog.tailor.logo.find); logos.replace-image.row (format.image.replaceImage.logo); logos.route.mark-headers (insert.logo); logos.index.refresh-fixture (insert.logo); logos.index.cached-offline (insert.logo); logos.cache.open-licence-only (insert.logo); logos.picker.variants (dialog.logo.kind.wordmark, dialog.logo.tone.mono); logos.kit.find-a-logo (panel.brand.logo.find); shaders.insert.selected-free-rectangle (insert.shader); shaders.insert.words (insert.shader); shaders.panel.slider-live-undo (formatOptions.shader.strength); shaders.panel.preset-tiles (formatOptions.shader.preset); shaders.panel.control-sentences (formatOptions.shader); shaders.panel.kit-colours (formatOptions.shader.color); shaders.panel.one-home (formatOptions.shader); shaders.frame.auto-capture (insert.shader); shaders.frame.box-aspect (insert.shader); shaders.frame.reuse-and-prune (insert.shader); shaders.frame.one-capturer (insert.shader); shaders.perf.one-context (insert.shader); shaders.background.add-to-theme (dialog.background.shader.addToTheme); shaders.library.glyph-engines-render (dialog.shader.engine.glyph); shaders.frame.scrubber-capture (formatOptions.shader.frame.scrubber, formatOptions.shader.frame.capture); shaders.view.play-setting (view.playShaders); shaders.show.plays-when-on (view.playShaders, formatOptions.shader.play); shaders.show.frame-when-off (view.playShaders); rows of an unparkable feature blocking the ship: sync.resend.idempotent, logos.export.pdf-pptx-crisp, shaders.export.missing-frame-row.
...
| `versions.show-changes-marks` | versions | probe --core | not driven | failed | a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.447.pick (named row versionHistory.447.pick); marks with Show changes on 0 (); off 0 |
...
| `assist.agent.propose-accept` | assist | probe --core | not driven | failed | assist.propose over HTTP, assist.accept with the card, then the card with one byte changed: assist.propose answered 500 {"error":{"name":"ModelCallError","status":500,"message":"The assistant’s model answered 401","action":"assist.propose"}} |
...
| `logos.picker.search` | logos | probe --core | not driven | failed | Insert > Logo; type figma at human speed; Enter: 0 result tiles 3 ms after the last key (first none; foot "Logos from thesvg.org. Brand marks belong to their owners; use them to name the brand, not to imply endorsement"); Enter inserted nothing; dialog closed false |
| `logos.picker.paper-and-ink` | logos | probe --core | not driven | failed | search vercel; read the tile's paper and ink halves; Slide > Change theme light; insert; read the mark: no Vercel tile among 1 tiles |
...
| `logos.picker.licence-words` | logos | probe --core | not driven | failed | read the foot; search figma and read the licence row and its tooltip; the dialog's text; search aws; Include cloud service icons: foot "Logos from thesvg.org. Brand marks belong to their owners; use them to name the brand, not to imply endorsement" (at the open "Logos from thesvg.org. Brand marks be |
| `logos.picker.chrome-1280` | logos | core/chrome.spec.ts | not driven | failed | Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoBeGreaterThanOrEqual[2m([22m[32mexpected[39m[2m)[22m |
| `logos.insert.one-click-asset` | logos | probe --core | not driven | failed | search figma; one click on the tile; read the asset record, the img and the source document: error: locator.boundingBox: Timeout 30000ms exceeded. |
| `logos.insert.logo-size` | logos | probe --core | not driven | failed | read the inserted Figma mark against the body slot, its chip and the text runs: error: locator.boundingBox: Timeout 30000ms exceeded. |
| `logos.insert.mono-tint` | logos | probe --core | not driven | failed | on the dark deck insert the Stripe mark's mono (asked for through the window API); read its variant and the fills of its source file; then AWS through the dialog with the cloud switch on: the Stripe mono was not inserted within 20 s (page.evaluate: Error: No logo named stripe on thesvg.org) |
| `logos.insert.every-slide` | logos | probe --core | not driven | failed | read the foot check and its sentence; check it; insert Figma; read the title slide and a new slide; the snackbar; Cmd+Z; open the dialog again: error: locator.boundingBox: Timeout 30000ms exceeded. |
| `logos.tailor.find-customer-logo` | logos | probe --core | not driven | not driven | not on this build: dialog.tailor.logo.find (docs/PRODUCT.md 7.1, B1); the Tailor dialog draws no Find the Figma logo button once To names a brand (FEATURES.md 4.5) |
| `logos.replace-image.row` | logos | probe --core | not driven | failed | a picture placed through the window API; right click it; Replace image > Logo; Figma: error: locator.boundingBox: Timeout 30000ms exceeded. |
| `logos.route.mark-headers` | logos | core/logos.spec.ts | not driven | not driven | not driven: the index holds no figma on this base ({"error":{"name":"RangeError","status":404,"message":"No logo named figma on thesvg.org"}}); the unit tests of docs/FEATURES.md 7.3 cover the sanitizer (apps/studio/src/server/logos.test.ts) |
...
| `logos.cache.open-licence-only` | logos | core/logos.spec.ts | not driven | failed | Error: the Figma insert |
| `logos.export.pdf-pptx-crisp` | export | core/export.spec.ts | not driven | not driven | not driven: the logo index answered "page.evaluate: Error: No logo named figma on thesvg.org     at Object.deserialize (http://localhost:4418/@fs/Users/kevin" on this base |
| `logos.agent.search-insert` | logos | core/logos.spec.ts | not driven | failed | Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoMatch[2m([22m[32mexpected[39m[2m)[22m |
...
| `shaders.insert.words` | shaders | probe --core | broken | failed | read the Insert row, the chip, the section title and the alt; the sheet, the viewer and the show for a paper: id or "not captured": Insert row "Shader"; chip "Shader"; section absent; alt "The liquid metal shader"; sheet clean (0 label elements); show clean; viewer unread: Error: page.evaluate: Targ |
| `shaders.panel.section-groups` | shaders | core/chrome.spec.ts | broken | passed |  |
| `shaders.panel.slider-live-undo` | shaders | probe --core | not driven | not driven | not on this build: formatOptions.shader (docs/PRODUCT.md 7.1, B5 (inspector/shader.tsx)); no Shader section in Format options (FEATURES.md 5.3) |
| `shaders.panel.preset-tiles` | shaders | probe --core | broken | not driven | not on this build: formatOptions.shader (docs/PRODUCT.md 7.1, B5 (inspector/shader.tsx)); no Shader section in Format options (FEATURES.md 5.3) |
| `shaders.panel.control-sentences` | shaders | probe --core | not driven | not driven | not on this build: formatOptions.shader (docs/PRODUCT.md 7.1, B5 (inspector/shader.tsx)); no Shader section in Format options (FEATURES.md 5.3) |
| `shaders.panel.kit-colours` | shaders | probe --core | not driven | not driven | not on this build: formatOptions.shader (docs/PRODUCT.md 7.1, B5 (inspector/shader.tsx)); no Shader section in Format options (FEATURES.md 5.3) |
| `shaders.panel.one-home` | shaders | probe --core | broken | failed | Tools > Advanced tools; Tools > Pictures and materials with the shader selected; the Background dialog's options link: Pictures and materials sections Pictures and materials, Pictures0 (with the switch on); the Background dialog's link dialog.background.shader.options: Format options true, Shader se |
...
| `shaders.perf.one-context` | shaders | probe --core | not driven | failed | a second shader block placed through the window API; select each in turn and count the canvases: with nothing selected 1 canvas on the stage (1 on the page); first selected: 1 on the stage, 1 on the page; second selected: 2 on the stage after 2675 ms, the second's root 1 canvas, the first's root 1 c |
...
| `shaders.view.play-setting` | view | probe --core | not driven | failed | View > Play shaders; read the three rows; pick Off; reload; the show Options menu; the default back: rows On, In the show only, Off (checked In the show only); describe().state.settings.playShaders "show" -> Off "off" -> after a reload "off"; the show's Options menu lists no Play shaders row; the de |
...
- `logos.tailor.find-customer-logo`: not on this build: dialog.tailor.logo.find (docs/PRODUCT.md 7.1, B1); the Tailor dialog draws no Find the Figma logo button once To names a brand (FEATURES.md 4.5)
- `logos.route.mark-headers`: not driven: the index holds no figma on this base ({"error":{"name":"RangeError","status":404,"message":"No logo named figma on thesvg.org"}}); the unit tests of docs/FEATURES.md 7.3 cover the sanitizer (apps/studio/src/server/logos.test.ts)
...
- `logos.export.pdf-pptx-crisp`: not driven: the logo index answered "page.evaluate: Error: No logo named figma on thesvg.org     at Object.deserialize (http://localhost:4418/@fs/Users/kevin" on this base
...
- `shaders.panel.slider-live-undo`: not on this build: formatOptions.shader (docs/PRODUCT.md 7.1, B5 (inspector/shader.tsx)); no Shader section in Format options (FEATURES.md 5.3)
- `shaders.panel.preset-tiles`: not on this build: formatOptions.shader (docs/PRODUCT.md 7.1, B5 (inspector/shader.tsx)); no Shader section in Format options (FEATURES.md 5.3)
- `shaders.panel.control-sentences`: not on this build: formatOptions.shader (docs/PRODUCT.md 7.1, B5 (inspector/shader.tsx)); no Shader section in Format options (FEATURES.md 5.3)
- `shaders.panel.kit-colours`: not on this build: formatOptions.shader (docs/PRODUCT.md 7.1, B5 (inspector/shader.tsx)); no Shader section in Format options (FEATURES.md 5.3)
...
- `versions.show-changes-marks`: a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.447.pick (named row versionHistory.447.pick); marks with Show changes on 0 (); off 0
...
- `assist.agent.propose-accept`: assist.propose over HTTP, assist.accept with the card, then the card with one byte changed: assist.propose answered 500 {"error":{"name":"ModelCallError","status":500,"message":"The assistant’s model answered 401","action":"assist.propose"}}
- `logos.picker.search`: Insert > Logo; type figma at human speed; Enter: 0 result tiles 3 ms after the last key (first none; foot "Logos from thesvg.org. Brand marks belong to their owners; use them to name the brand, not to imply endorsement"); Enter inserted nothing; dialog closed false
- `logos.picker.paper-and-ink`: search vercel; read the tile's paper and ink halves; Slide > Change theme light; insert; read the mark: no Vercel tile among 1 tiles
- `logos.picker.recents`: Error: figma is a result
- `logos.picker.licence-words`: read the foot; search figma and read the licence row and its tooltip; the dialog's text; search aws; Include cloud service icons: foot "Logos from thesvg.org. Brand marks belong to their owners; use them to name the brand, not to imply endorsement" (at the open "Logos from thesvg.org. Brand marks be
- `logos.picker.chrome-1280`: Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoBeGreaterThanOrEqual[2m([22m[32mexpected[39m[2m)[22m
- `logos.insert.one-click-asset`: search figma; one click on the tile; read the asset record, the img and the source document: error: locator.boundingBox: Timeout 30000ms exceeded.
- `logos.insert.logo-size`: read the inserted Figma mark against the body slot, its chip and the text runs: error: locator.boundingBox: Timeout 30000ms exceeded.
- `logos.insert.mono-tint`: on the dark deck insert the Stripe mark's mono (asked for through the window API); read its variant and the fills of its source file; then AWS through the dialog with the cloud switch on: the Stripe mono was not inserted within 20 s (page.evaluate: Error: No logo named stripe on thesvg.org)
- `logos.insert.every-slide`: read the foot check and its sentence; check it; insert Figma; read the title slide and a new slide; the snackbar; Cmd+Z; open the dialog again: error: locator.boundingBox: Timeout 30000ms exceeded.
- `logos.replace-image.row`: a picture placed through the window API; right click it; Replace image > Logo; Figma: error: locator.boundingBox: Timeout 30000ms exceeded.
- `logos.cache.open-licence-only`: Error: the Figma insert
- `logos.agent.search-insert`: Error: [2mexpect([22m[31mreceived[39m[2m).[22mtoMatch[2m([22m[32mexpected[39m[2m)[22m
...
- `shaders.insert.words`: read the Insert row, the chip, the section title and the alt; the sheet, the viewer and the show for a paper: id or "not captured": Insert row "Shader"; chip "Shader"; section absent; alt "The liquid metal shader"; sheet clean (0 label elements); show clean; viewer unread: Error: page.evaluate: Targ
- `shaders.panel.one-home`: Tools > Advanced tools; Tools > Pictures and materials with the shader selected; the Background dialog's options link: Pictures and materials sections Pictures and materials, Pictures0 (with the switch on); the Background dialog's link dialog.background.shader.options: Format options true, Shader se
...
- `shaders.view.play-setting`: View > Play shaders; read the three rows; pick Off; reload; the show Options menu; the default back: rows On, In the show only, Off (checked In the show only); describe().state.settings.playShaders "show" -> Off "off" -> after a reload "off"; the show's Options menu lists no Play shaders row; the de
...
- `export.download.large-deck-pdf`: passed; recorded PDF: GT brand deck.pdf, 85 slides in 3.7 s, 0.04 s per slide; PDF: GT brand deck.pdf, 85 slides in 3.7 s, 0.04 s per slide
- `export.download.large-deck-pptx`: passed; recorded Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 35.2 s, 0.41 s per slide; Editable text PowerPoint with Embed fonts: GT brand deck (editable).pptx, 85 slides in 35.2 s, 0.41 s per slide
...
- `cost.editor-editing.calls`: passed; recorded function requests 74.63 a minute (ceiling 75; 224 in 3 min); the store half reads zero on this base (the file store); the function requests alone are asserted (a cost row: over its ceiling on the preview it holds the ship)
...
- `shaders.perf.editor-frame`: failed (Error: under 150 ms); recorded longest animation frame 158.4 ms over 5 s with one shader on the stage (583 animation frames; rAF gaps; 1 canvas; renderer ANGLE (Google, Vulkan 1.3.0 (SwiftShader Device (LLVM 10.0.0) (0x0000C0DE)), SwiftShader driver)); longest animation frame 158.4 ms over 5 s with one shader on the stage (583 an
