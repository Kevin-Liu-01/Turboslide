...
| `shapes.geometry.shapes.multipath-can`       | shapes  | Can by one click: the block's svg holds three `<path>` elements, the lid's path with `stroke="none"` where the definition says `stroke false`, and the fill of the block on the `norm` paths                                                                                                                                                                                  | probe --core        | broken (severity 3) | audit-objects rows 26 to 49; `primitives.ts` 492 to 503 writes one path                          | `insert.shape.gallery`                                                       |
...
| `shapes.geometry.arrows.curved-right`        | shapes  | Curved right arrow by a drag to 240 by 240: three paths, arcs in each, the shade path (`darken`) drawn as the ink overlay of 2.3 at opacity 0.3                                                                                                                                                                                                                               | probe --core        | broken (severity 3) | audit-objects rows 26 to 49                                                                      | `insert.shape.arrows`                                                        |
...
| `shapes.geometry.callouts.cloud`             | shapes  | Cloud callout by one click: five paths, the two small ellipses of the tail below and left of the cloud, cubic curves in the cloud's path                                                                                                                                                                                                                                      | probe --core        | broken (severity 3) | audit-objects rows 26 to 49                                                                      | `insert.shape.callouts`                                                      |
...
| `shapes.geometry.pinned-three`               | shapes  | Rectangle, Rounded rectangle and Ellipse by one click each: their paths equal ship one's byte for byte at 240 by 160 (the box; `M0,26.5 A26.5,26.5 0 0 1 26.5,0 …` with four arcs; four arcs of 120 by 80), and the rounded rectangle's Corner field at 50 gives a radius of 80                                                                                               | probe --core        | works               | `shapes.test.ts` 148 to 186; `shapes.insert.rounded-click` works on production                   |                                                                              |
...
| `shapes.geometry.resize-keeps-adjust`        | shapes  | The star of `star5-adjust` resized by its se handle to 600 by 600: `adjust` still reads [30000, 105146, 110557] and the inner radius is 30 percent of the outer at the new size; Cmd+Z restores the box                                                                                                                                                                       | probe --core        | not driven          | `Editor.tsx` 2500 to 2536 writes `pos` alone (the invariant this row pins)                       | `insert.shape.gallery`                                                       |
...
- `packages/schema/src/shapes/geometry.test.ts` (B2): each of the seventeen operations with a value;
  ...
  the box within 1 px at the defaults; `rect`'s path is the box; `roundRect`'s and `ellipse`'s
