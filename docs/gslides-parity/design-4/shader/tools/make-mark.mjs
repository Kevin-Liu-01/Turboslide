...
const OUT =
  process.argv[2] ?? '/Users/kevinliu/repos/Turboslide/docs/gslides-parity/design-4/shader';
const INK = '#070707',
  PAPER = '#ffffff',
  PAPER_INK = '#f2f2f0';
...
export function geometry(S) {
  ...
  if (cell) {
    ...
    const dl = Math.abs(bar + lo + stroke - target),
      dh = Math.abs(bar + hi + stroke - target);
    inner = dh <= dl ? hi : lo;
  }
...
export function panelCells(g) {
  ...
  const out = [];
  for (let i = 0; i < m; i += 1)
    for (let j = 0; j < n; j += 1) if (rampInk(j + 1, i, n + 1)) out.push([j, i]);
  return out;
}
...
export function markShapes(g, k = 1) {
  ...
  if (cells.length) {
    const px = g.stemX + g.bar,
      ...
      c = g.cell / k;
    const d = cells
      .map(([j, i]) => `M${(px + j * g.cell) / k} ${(py + i * g.cell) / k}h${c}v${c}h${-c}z`)
      .join('');
    parts.push(`<path d="${d}"/>`);
  }
  return parts.join('\n    ');
...
export function markSvg(
  S,
  { plate = null, ink = 'currentColor', fit = 1, id = null, viewBoxUnits = null } = {},
) {
...
for (const S of SIZES) {
  await writePng(`${OUT}/previews/mark-${S}-paper.png`, markSvg(S, { plate: PAPER, ink: INK }), {
    palette: true,
    colours: 2,
  });
  await writePng(`${OUT}/previews/mark-${S}-ink.png`, markSvg(S, { plate: INK, ink: PAPER_INK }), {
    palette: true,
    colours: 2,
  });
}
...
for (const S of [16, 32, 48])
  await writePng(`${OUT}/icons/favicon-${S}.png`, markSvg(S, { plate: PAPER, ink: INK }));
await writePng(
  `${OUT}/icons/apple-touch-icon.png`,
  markSvg(180, { plate: PAPER, ink: INK, fit: 0.8 }),
);
await writePng(`${OUT}/icons/icon-192.png`, markSvg(192, { plate: PAPER, ink: INK }), {
  palette: true,
  colours: 2,
});
await writePng(`${OUT}/icons/icon-512.png`, markSvg(512, { plate: PAPER, ink: INK }), {
  palette: true,
  colours: 2,
});
await writePng(
  `${OUT}/icons/icon-mask-192.png`,
  markSvg(192, { plate: PAPER, ink: INK, fit: 0.75 }),
  { palette: true, colours: 2 },
);
await writePng(
  `${OUT}/icons/icon-mask-512.png`,
  markSvg(512, { plate: PAPER, ink: INK, fit: 0.75 }),
  { palette: true, colours: 2 },
);
await writePng(`${OUT}/icons/icon-mono-512.png`, markSvg(512, { plate: null, ink: '#000000' }));
await writePng(`${OUT}/icons/icon-dark-192.png`, markSvg(192, { plate: INK, ink: PAPER_INK }), {
  palette: true,
  colours: 2,
});
await writePng(`${OUT}/icons/icon-dark-512.png`, markSvg(512, { plate: INK, ink: PAPER_INK }), {
  palette: true,
  colours: 2,
});
const webmanifest = {
...
manifest.push({
  path: 'icons/manifest.webmanifest',
  bytes: Buffer.byteLength(wm),
  sha256: sha(wm),
});
...
// 4. The hinting sheet: every size at 1:1 on paper and ink, and 16, 32, 48 enlarged 8x nearest.
{
  ...
  const row = async (S, plate, ink, top) => {
    const buf = await sharp(Buffer.from(markSvg(S, { plate, ink })))
      .png()
      .toBuffer();
  ...
  let left = 24;
  for (const S of SIZES) {
    for (const [plate, ink, top] of [
      [PAPER, INK, 24],
      [INK, PAPER_INK, 24 + 540],
    ]) {
      const buf = await sharp(Buffer.from(markSvg(S, { plate, ink })))
        .png()
        .toBuffer();
  ...
  const enl = [];
  for (const S of [16, 32, 48]) {
    const buf = await sharp(Buffer.from(markSvg(S, { plate: PAPER, ink: INK })))
      .png()
      .toBuffer();
    const big = await sharp(buf)
      .resize(S * 6, S * 6, { kernel: 'nearest' })
      .png()
      .toBuffer();
    enl.push(big);
  }
  const sheet = sharp({
    create: { width: 1560, height: 1120, channels: 3, background: '#8a8f98' },
  });
  ...
  const label = Buffer.from(
    `<svg width="1560" height="40"><text x="24" y="28" font-family="Helvetica" font-size="16" fill="#fff">Turboslide mark at 512, 180, 64, 32 and 16 px on paper (top) and ink (bottom), each drawn on its own pixel grid; right: 16, 32 and 48 px at 6x nearest</text></svg>`,
  );
...
// 5. The confusion sheet: the mark at 32 px beside generic silhouettes at 32 px, all in one grey.
{
  ...
  const sil = [
    ...
    [
      'triangle',
      `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"><rect width="32" height="32" fill="${PAPER}"/><path d="M16 4 L29 28 L3 28Z" fill="${G}"/></svg>`,
    ],
    [
      'cube',
      `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"><rect width="32" height="32" fill="${PAPER}"/><path d="M16 3 L28 9.5 L28 22.5 L16 29 L4 22.5 L4 9.5Z M16 3 L16 16 M4 9.5 L16 16 L28 9.5" fill="none" stroke="${G}" stroke-width="2"/></svg>`,
    ],
    [
      'Z',
      `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"><rect width="32" height="32" fill="${PAPER}"/><path d="M5 5 H27 L5 27 H27" fill="none" stroke="${G}" stroke-width="4" stroke-linejoin="miter"/></svg>`,
    ],
    [
      'rect in rect',
      `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"><rect width="32" height="32" fill="${PAPER}"/><rect x="4" y="3" width="24" height="26" rx="3" fill="${G}"/><rect x="9" y="12" width="14" height="9" fill="${PAPER}"/></svg>`,
    ],
    [
      'chevrons',
      `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"><rect width="32" height="32" fill="${PAPER}"/><path d="M6 6 L14 16 L6 26 M16 6 L24 16 L16 26" fill="none" stroke="${G}" stroke-width="3"/></svg>`,
    ],
  ];
  const comps = [];
  let left = 24;
  for (const [name, svg] of sil) {
    ...
    comps.push({
      input: Buffer.from(
        `<svg width="140" height="24"><text x="0" y="16" font-family="Helvetica" font-size="12" fill="#070707">${name}</text></svg>`,
      ),
      left,
      top: 216,
    });
  ...
  await sharp({ create: { width: left + 24, height: 260, channels: 3, background: PAPER } })
    .composite(comps)
    .png()
    .toFile(`${OUT}/previews/confusion-sheet.png`);
}
...
for (const S of [16, 32, 48, 64, 180, 192, 512]) {
  ...
  for (let i = 0; i < buf.data.length; i += buf.info.channels)
    seen.add(`${buf.data[i]},${buf.data[i + 1]},${buf.data[i + 2]}`);
  check[S] = {
    colours: seen.size,
    geometry: g,
    cells: panelCells(g).length,
    cellGrid: g.cell ? `${g.interior / g.cell} by ${g.inner / g.cell}` : 'none',
  };
}
    ...
    Object.fromEntries(
      Object.entries(check).map(([k, v]) => [
        k,
        {
          ...
          W: v.geometry.W,
          H: v.geometry.H,
          stroke: v.geometry.stroke,
          bar: v.geometry.bar,
          cell: v.geometry.cell,
          grid: v.cellGrid,
          lit: v.cells,
        },
      ]),
