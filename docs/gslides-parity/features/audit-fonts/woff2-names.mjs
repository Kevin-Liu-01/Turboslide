...
export function readWoff2(path) {
  ...
  let off = 0;
  for (const t of dir) {
    tables.set(t.tag, data.subarray(off, off + t.transformLength));
...
function fvar(tbl) {
  ...
  for (let i = 0; i < axisCount; i += 1) {
    ...
    axes.push({
      tag: tbl.toString('latin1', a, a + 4),
      min: tbl.readInt32BE(a + 4) / 65536,
      def: tbl.readInt32BE(a + 8) / 65536,
      max: tbl.readInt32BE(a + 12) / 65536,
    });
  }
    ...
    JSON.stringify(
      {
        path,
        bytes,
        tables: dir.map((d) => `${d.tag}:${d.origLength}`),
        family: n.get(1),
        subfamily: n.get(2),
        fullName: n.get(4),
        version: n.get(5),
        ...
        designer: n.get(9),
        copyright: n.get(0),
        license: n.get(13)?.slice(0, 120),
        licenseUrl: n.get(14),
        headRevision: rev.toFixed(3),
        unitsPerEm: upem,
        created: new Date((created + epoch) * 1000).toISOString(),
        fsType,
        fvar: fvar(tables.get('fvar')),
        gsubFeatures: gsub.filter((t) => !/^(cv|ss)\d\d$/.test(t)),
        cv,
        ss,
      },
      null,
