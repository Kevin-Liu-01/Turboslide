...
| `formatting.persistence` | formatting | works | failed | a superscript word, UPPERCASE, a highlighted word and a grouped pair; reload; the show; the viewer: set {"sup":true,"highlight":true,"upper":true,"group":true}; after the reload {"sup":true,"highlight":true,"upper":true,"group":true}; the show {"sup":true,"highlight":true,"upper":true}; the viewer {"found":false,"sup":false,"highlight":false,"upper":false} | 224 |
...
| `versions.show-changes-marks` | versions | not driven | failed | a heading edit and an added box after the named version; pick the older version with Show changes on; then off: picked versionHistory.959.pick (named row versionHistory.959.pick); marks with Show changes on 0 (); off 0 | 488 |
...
| `brand.background.enter-keeps-open` | brand | broken | failed | Slide > Change background, type #0b3d91, Enter, Done; then Add to theme: Done: locator.boundingBox: Timeout 30000ms exceeded. | 536 |
...
| `logos.picker.search` | logos | not driven | failed | Insert > Logo; type figma at human speed; Enter: 1 result tiles 328 ms after the last key (first figma active true; foot "Logos from thesvg.org, updated 25 September 2026. Brand marks belong to their owners; use them to name the brand, not to imply endorsement"); Enter inserted shot logo 467,415 108x160; dialog closed true | 561 |
...
| `shaders.panel.slider-live-undo` | shaders | not driven | failed | select the shader; drag Amplitude with the mount at rest (Strength maps to no uniform of liquid metal, build/integrator.md); release; Cmd+Z: mount at rest 0.00 percent moving; during the drag the value read 5.6 with 3.0 percent of pixels changed and revision 1050 (1050 before); release: revision 1050, value 3.2 -> 5.6 (block controls {"amplitude":5.6}); no Cmd+Z (nothing was written): value 5.6, 3 | 580 |
| `shaders.panel.preset-tiles` | shaders | broken | failed | read the Preset row's tiles; click a tile that is not pressed: 10 tiles ("Paper on ink", "Ink on paper", "Primary", "Accent", "Captions", "Hints", "Diamond" pressed, "Sphere", "Chrome", "Noir") against 10 presets of paper:liquid-metal; labels not in sentence case: none; click on formatOptions.shader.preset.chrome: preset diamond -> chrome (revision 1053), canvas mean luminance moved 151.8, 95 perc | 581 |
| `shaders.panel.control-sentences` | shaders | not driven | passed |  | 582 |
| `shaders.panel.kit-colours` | shaders | not driven | failed | read the Colors row's swatches; brand.set /colors/<appearance>/primary '#0b3d91'; read the shader and its frame: the Primary swatch clicked (revision 1054 -> 1055); swatches text, background, caption, hint, primary, accent, custom (6 of the six roles); the sheet renders dark (deck.info and the chrome read dark); brand.set /colors/dark/primary ok; frame frame-f91c12dd0d05c000 (key sha256:f91c1) ->  | 583 |
| `shaders.panel.one-home` | shaders | broken | passed |  | 585 |
| `shaders.background.place-answers` | shaders | broken | failed | Slide > Change background on the body slide; Shader; a tile; Place; read the ground, the button and the dialog: chose Liquid metal; Place dialog.background.shader.place; the ground changed after 13506 ms (covering picture background, asset liquid-metal); the button read "Placing, 11 s" with the seconds | 584 |
| `shaders.perf.one-context` | shaders | not driven | failed | a second shader block placed through the window API; select each in turn and count the canvases: with nothing selected 1 canvas on the stage (1 on the page); first selected: 1 on the stage, 1 on the page; second selected: 1 on the stage after 730 ms, the second's root 1 canvas, the first's root 0 canvas, frame img decoded | 586 |
