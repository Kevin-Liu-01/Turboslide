...
describe('the sprite', () => {
  it('has 63 Heroicons plus gt-mark, in the order of the schema’s icon list', () => {
    ...
    expect(symbols.filter((symbol) => symbol.viewBox === '0 0 20 20')).toHaveLength(63);
  ...
  it('writes the deck’s icon, mark and GT word markup', () => {
    ...
    expect(spriteMarkup().match(/<symbol /g)).toHaveLength(64);
  });
