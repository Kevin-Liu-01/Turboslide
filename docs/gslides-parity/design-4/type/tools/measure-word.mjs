...
const b64 = readFileSync(
  '/Users/kevinliu/repos/Turboslide/packages/fonts/assets/InterVariable.woff2',
).toString('base64');
...
const result = await page.evaluate(() => {
  ...
  for (const id of ['a', 'b']) {
    ...
    out[id] = {
      ...
      height: base.height,
      chars,
      fontsLoaded: document.fonts.check('500 1000px Inter'),
    };
  }
  return out;
});
...
console.log(
  JSON.stringify(result.a.chars.map((c) => [c.ch, +c.x.toFixed(1), +c.w.toFixed(1)])),
  'total',
  result.a.width.toFixed(1),
  'loaded',
  result.a.fontsLoaded,
);
console.log(
  'no tracking',
  JSON.stringify(result.b.chars.map((c) => [c.ch, +c.x.toFixed(1)])),
  'total',
  result.b.width.toFixed(1),
);
await browser.close();
